...
export async function POST(req: NextRequest) {
  ...
  if (!vivaConfigured) return NextResponse.json({ error: 'viva_not_configured' }, { status: 501 });
  ...
  try {
    ...
    return NextResponse.json({ order_code: orderCode, checkout_url: checkoutUrl, amount: total });
  } catch (e) {
    return NextResponse.json({ error: 'viva_error', detail: (e as Error).message }, { status: 502 });
  }
}
