      ...
      <div style={{ background: 'rgba(250,250,247,.70)', backdropFilter: 'blur(18px) saturate(150%)', WebkitBackdropFilter: 'blur(18px) saturate(150%)', borderBottom: '1px solid rgba(255,255,255,.6)', boxShadow: '0 1px 0 rgba(231,227,218,.7), 0 10px 30px -24px rgba(40,50,25,.5)' }}>
        ...
        <div className="hfm-navrow" style={{ position: 'relative', borderTop: '1px solid #E7E3DA', background: 'rgba(255,255,255,.45)' }}>
          <div style={{ maxWidth: '1340px', margin: '0 auto', padding: '0 28px', height: '48px', display: 'flex', alignItems: 'center', gap: '2px' }}>
            {menu.map((cat) => {
              const open = openCat === cat.id_category;
              const subs = cat.children || [];
              const cols = subs.length > 16 ? 5 : subs.length > 9 ? 4 : subs.length > 4 ? 3 : 2;
              return (
                <div
                  key={cat.id_category}
              ...
              return (
                <div key={cat.id_category} style={{ borderRadius: '14px', background: '#fff', border: '1px solid #ECEAE3', overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    {subs.length ? (
                      <button type="button" onClick={() => setOpenCat(open ? null : cat.id_category)} aria-label={cat.name} style={{ flex: 'none', width: '48px', height: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer', color: '#9A9A9A' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ transition: 'transform .2s ease', transform: open ? 'rotate(90deg)' : 'none' }}><path d="M9 6l6 6-6 6" /></svg>
                      </button>
                    ) : (
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#C9C2AF" strokeWidth="2" strokeLinecap="round" style={{ flex: 'none', marginRight: '15px' }}><path d="M9 6l6 6-6 6" /></svg>
                    )}
                  </div>
                  {open && subs.length ? (
