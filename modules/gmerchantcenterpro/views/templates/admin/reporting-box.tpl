...
{else}
	<div id="bt_facebook-reporting gmc" class="col-xs-12" style="height:860px !important; min-width: 100% !important;" >

	<div class="row">
		<div class="col-xs-12">
			<h3 class="subtitle">
				{l s='Feed data diagnostic tool' mod='gmerchantcenterpro'}&nbsp;:&nbsp;
				...
				{if isset($iProductCount)}
					{if $iProductCount > 0}
						<p class="col-xs-3 badge badge-success pull-right"> {l s='Total of products exported :' mod='gmerchantcenterpro'} <strong>{$iProductCount|intval}</strong></p>
					{else}
						<p class="col-xs-3 badge badge-danger pull-right"> {l s='Total of products exported :' mod='gmerchantcenterpro'} <strong>{$iProductCount|intval}</strong></p>
					{/if}
				{/if}
			</h3>
		</div>
	...
	<div class="col-xs-12">
		<div class="alert alert-success">
			<p>{l s='Data feed is exported.' mod='gmerchantcenterpro'}</p>
		</div>
	</div>
	...
	<div class="alert alert-info">
		...
		{l s='Please read the following FAQ to learn how the diagnostic tool works :' mod='gmerchantcenterpro'}</strong>
		...
		<br />
		<p>{l s='This tool allows you to display the feed quality diagnostic. The diagnostic catches and shows you any more or less important information about your data, so that you can take them into account and make some corrections to your products or your feed configuration before submitting (again) your feed to Google.' mod='gmerchantcenterpro'}</p>
		<p>{l s='For each type of notice, we will provide you the ability to see which products are affected as well as how to fix the potential issue.' mod='gmerchantcenterpro'}</p><br/>
</div>
			...
			{if !empty($aReport) && !empty($aReport.error)}
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					<div class="box box-reporting box-reporting-error" >
						...
						<div class="info"  style="min-height: 120px;">
							...
							<p class="center" ">{l s='By clicking on \"view details\", you will see products that have generated errors' mod='gmerchantcenterpro'}</p>
							<p class="center""><strong>{l s='All of these products will NOT be exported in the data feed' mod='gmerchantcenterpro'}</strong></p>
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					...
					<div class="info bt_box_success"  style="min-height: 120px;">
							<p></p>
							<p class="center"><strong>{l s='Good job ! There isn\'t any error' mod='gmerchantcenterpro'}</strong></p>
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					<div class="box  box-reporting box-reporting-warning">
						...
						<div class="info" style="min-height: 120px;">
							...
							<p class="center" >{l s='By clicking on \"view details\", you will see products that require your attention' mod='gmerchantcenterpro'}</p>
							<p class="center""><strong>{l s='However, these products will be exported in your data feed' mod='gmerchantcenterpro'}</strong></p>
							<div class="clr_30"></div>
							<div class="center">
								<a id="btn-reporting-warning" class="btn btn-lg btn-lg-custom btn-warning">{l s='View details' mod='gmerchantcenterpro'}</a>
							</div>
						</div>
					</div>
				</div>
			{else}
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					<div class="box  box-reporting box-reporting-success">
						...
						<div class="info bt_box_success" style="min-height: 120px;">
							<p></p>
							<p class="center"><strong>{l s='Good job ! There isn\'t any warning' mod='gmerchantcenterpro'}</strong></p>
							<p class="center bt_success_text"><strong><i class="icon icon-thumbs-up icon-3x"></i> </strong></p>
						</div>
					</div>
				</div>
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					<div class="box  box-reporting box-reporting-info">
						...
						<div class="info" style="min-height: 120px;">
							...
							<p class="center">{l s='By clicking on \"view details\", you will see the products YOU have decided not to export (according to your module configuration).' mod='gmerchantcenterpro'}</p>
							<p class="center"><strong>{l s=' As you have decided, all of these products will not be exported in the data feed' mod='gmerchantcenterpro'}</strong></p>
			...
			{else}
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
					<div class="box box-reporting box-reporting-success">
						...
						<div class="info bt_box_success" style="min-height: 120px;">
							<p class="center"><strong>{l s='No information. In your configuration, you have' mod='gmerchantcenterpro'}<br />
							{l s='not chosen to exclude any product' mod='gmerchantcenterpro'}</strong></p>
							<p class="center  bt_success_text"><strong><i class="icon icon-thumbs-up icon-3x"></i> </strong></p>
						</div>
					</div>
				</div>
	...
	<div id="reporting" class="bt-reporting-details">
		{if !empty($aReport) && !empty($aReport.notice)}
			<div id="btn-reporting-info-box" style="display: none;">
				...
				<table class="table table-striped">
					...
					<th class="bt_notice" > {l s='Affected product(s)' mod='gmerchantcenterpro'}</th>
					<th class="bt_notice" > {l s='Solution' mod='gmerchantcenterpro'}</th>
					</thead>
					<tbody>
					{foreach from=$aReport.notice item=aTag key=tagName name=report}
						<tr class="bt_reporting_line">
							<td>
								<span class="bt_report_notification">{$aTag.label|escape:'htmlall':'UTF-8'}</span><span class="badge badge-info pulse pulse2 pull-right">{$aTag.count|intval}</strong>&nbsp;{l s='notice(s)' mod='gmerchantcenterpro'}{if $aTag.count > 1}{/if}</span>
							</td>
							<td>
								<p class="summary bt_report_notification"">{$aTag.msg}</p>
							</td>
							<td>
								<span class="bt_report_notification"><a href="#" class="btn btn-sm btn-warning" onclick="$('#tagReport{$tagName|escape:'htmlall':'UTF-8'}').toggle(); return false;"><i class="icon-eye-open"></i> &nbsp;{l s='View affected product(s)' mod='gmerchantcenterpro'}</a></span>
							</td>
							<td>
								<span class="bt_report_notification"><a class="btn btn-sm btn-info" href="{$sFaqURL|escape:'htmlall':'UTF-8'}{$sFaqLang|escape:'htmlall':'UTF-8'}/faq/{$aTag.faq_id|intval}{if !empty($aTag.anchor)}#{$aTag.anchor|escape:'htmlall':'UTF-8'}{/if}" target="_blank"><i class="icon-question-sign"></i>&nbsp; {l s='Learn how to fix this problem' mod='gmerchantcenterpro'}</a></span>
							</td>
						</tr>
						<tr>
									...
									{foreach from=$aTag.data item=aProduct key=key}
										<tr>
											...
											<td class="bt_reporting_products-lines"><a class="btn btn-sm btn-success" href="{$aProduct.productUrl|escape:'htmlall':'UTF-8'}" target="_blank"><i class="icon icon-eye-open"></i> {l s='View product' mod='gmerchantcenterpro'}</a>
												<a class="btn btn-sm btn-success" href="{$sProductLinkController|escape:'htmlall':'UTF-8'}&id_product={$aProduct.productId|intval}{$sProductAction|escape:'htmlall':'UTF-8'}&token={$sToken|escape:'htmlall':'UTF-8'}" target="_blank"><i class="icon icon-edit"></i> {l s='Edit' mod='gmerchantcenterpro'}</a>
											</td>
										</tr>
									{/foreach}
									</tbody>
		...
		{if !empty($aReport) && !empty($aReport.error)}
			<div id="btn-reporting-error-box" style="display: none;">
				...
				<table class="table table-striped">
					...
					<th class="bt_error" > {l s='Affected product(s)' mod='gmerchantcenterpro'}</th>
					<th class="bt_error" > {l s='Solution' mod='gmerchantcenterpro'}</th>
					</thead>
					<tbody>
					{foreach from=$aReport.error item=aTag key=tagName name=report}
						<tr class="bt_reporting_line">
							<td>
								<span class="bt_report_notification">{$aTag.label|escape:'htmlall':'UTF-8'}</span><span class="badge badge-danger pull-right">{$aTag.count|intval}</strong>&nbsp;{l s='notice(s)' mod='gmerchantcenterpro'}{if $aTag.count > 1}{/if}</span>
							</td>
							<td>
								<p class="summary bt_report_notification"">{$aTag.msg}</p>
							</td>
		...
		{if !empty($aReport) && !empty($aReport.warning)}
			<div id="btn-reporting-warning-box" style="display: none;">
				...
				<table class="table table-striped">
					<thead>
					<th class="bt_warning col-xs-3">{l s='Type' mod='gmerchantcenterpro'}</th>
					<th class="bt_warning ">{l s='Description' mod='gmerchantcenterpro'}</th>
					<th class="bt_warning" > {l s='Affected product(s)' mod='gmerchantcenterpro'}</th>
					<th class="bt_warning" > {l s='Solution' mod='gmerchantcenterpro'}</th>
					</thead>
					<tbody>
					{foreach from=$aReport.warning item=aTag key=tagName name=report}
						<tr class="bt_reporting_line">
							<td>
							<span class="bt_report_notification">{$aTag.label|escape:'htmlall':'UTF-8'}</span><span class="badge badge-warning pull-right">{$aTag.count|intval}</strong>&nbsp;{l s='notice(s)' mod='gmerchantcenterpro'}{if $aTag.count > 1}{/if}</span>
							</td>
							<td>
								<p class="summary bt_report_notification"">{$aTag.msg}</p>
							</td>
	...
	<div class="clr_20"></div>
		...
		<p style="text-align: center !important;">
			<button class="btn btn-danger btn-lg" value="{l s='Close' mod='gmerchantcenterpro'}"  onclick="$.fancybox.close();return false;">{l s='Close' mod='gmerchantcenterpro'}</button>
		</p>
	</div>
{/if}
</div>
