		...
		<tr>
			...
			{if !empty($aTaxonomy.updated)}
				<td id="gcupd_{$sCode|escape:'htmlall':'UTF-8'}" class="center">
					<a id="handleGoogle" class="fancybox.ajax btn btn-sm btn-default" href="{$sURI|escape:'htmlall':'UTF-8'}&{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.googleCat.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.googleCat.type|escape:'htmlall':'UTF-8'}&iLangId={$aTaxonomy.id_lang|intval}&sLangIso={$sCode|escape:'htmlall':'UTF-8'}"><i class="icon-pencil"></i></a>
				</td>
			{else}
			...
			{/if}
			<td class="center">
				<a class="btn btn-sm btn-default" id="updateGoogleCategories" href="#" onclick="$('#loadingGoogleCatListDiv').show();oGmcPro.hide('bt_google-cat-list');oGmcPro.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.googleCatSync.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.googleCatSync.type|escape:'htmlall':'UTF-8'}&iLangId={$aTaxonomy.id_lang|intval}&sLangIso={$sCode|escape:'htmlall':'UTF-8'}', 'bt_google-cat-list', 'bt_google-cat-list', null, null, 'loadingGoogleCatListDiv');"><i class="icon-refresh"></i></a>
