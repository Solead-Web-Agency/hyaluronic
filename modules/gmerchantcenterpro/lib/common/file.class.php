...
class BT_File
{
    ...
    /**
     * @static array $AVAILABLE_CONF : all available params
     */
    static private $AVAILABLE_CONF = array(
    ...
    {
        // use case - set buffer
        if (self::$AVAILABLE_CONF['buffer'] === true) {
            if ($bError === true) {
    ...
    public function write($sFileName, $sContent, $bVerbose = false, $bAdd = false, $bStripTag = false)
    {
        ...
        if (is_writable($dirName)) {
            ...
            if ($bStripTag === true || self::$AVAILABLE_CONF['strip_tags']['value'] === true) {
                $sContent = strip_tags(str_replace(array('<br/>', '<BR/>', '<br>', '<BR>'), "\n", $sContent));
            }

            // use case - add content mode
            if ($bAdd === true || self::$AVAILABLE_CONF['add']['value'] === true) {
                $sMode = 'a';
            } else {
                $sMode = 'wb';
            }
            ...
            if ($rHandle === false) {
                ...
                // if lock exists
                if (self::$AVAILABLE_CONF['lock']['value'] !== null) {
                    if (flock($rHandle, $this->_sLock)) {
                        ...
                        // throw exception
                        throw new Exception(
                            $this->sDate . " $dirName : " . self::ERR_LOCK_FILE_M,
                ...
                if ($nDataLength != $nWrittenData) {
                    throw new Exception(
                        $this->sDate . " $sFileName : " . self::ERR_LENGTH_DATA_WRITE_M,
                        self::ERR_LENGTH_DATA_WRITE_N
                    );
                } elseif ($bVerbose === true || self::$AVAILABLE_CONF['verbose']['value'] === true) {
    ...
    public function copy($sSource, $sDest, $bDelete = false, $bRename = false, $bVerbose = false)
    {
        ...
        if (is_writable($sDirDest)) {
            ...
            if ($bRename === true || self::$AVAILABLE_CONF['rename']['value'] === true) {
                if (file_exists($sDest) === false) {
                    if (!copy($sSource, $sDest)) {
                        // throw exception
                        throw new Exception(
                            $this->sDate . $sSource . ' in to ' . $sDest . ' : ' . self::ERR_COPY_FILE_M,
                            self::ERR_COPY_FILE_N
                        );
                    } elseif ($bVerbose === true || self::$AVAILABLE_CONF['verbose']['value'] === true) {
                        self::verbose(
                            ' --- File ' . $sSource . ' has been copied ' . $sDest . ' in to directory ' . $sDirDest,
                            'WRITING'
                        );
                    }
                    ...
                    // throw exception
                    throw new Exception(
                        $this->sDate . $sSource . ' in to ' . $sDest . ' : ' . self::ERR_RENAME_FILE_M,
                        self::ERR_RENAME_FILE_N
                    );
                } elseif ($bVerbose === true || self::$AVAILABLE_CONF['verbose']['value'] === true) {
                    self::verbose(
                        ' --- File ' . $sSource . ' has been renamed  as ' . $sDest . ' in to directory ' . $sDirDest,
                        'WRITING'
                    );
                }
            ...
            elseif (!file_exists($sDest)) {
                if (!copy($sSource, $sDest)) {
                    ...
                    self::verbose(
                        ' --- File ' . $sSource . ' has been copied as ' . $sDest . ' in to directory ' . $sDirDest,
                        'WRITING'
                    );
                }
            ...
            elseif ($bVerbose === true || self::$AVAILABLE_CONF['verbose']['value'] === true) {
                self::verbose(
                    ' --- File ' . $sSource . ' already exists as ' . $sDest . ' in to directory ' . $sDirDest,
                    'WRITING'
                );
            }
            // test if have to delete source file
            if ($bDelete === true || self::$AVAILABLE_CONF['delete']['value'] === true) {
                unlink($sSource);
                if ($bVerbose === true || self::$AVAILABLE_CONF['verbose']['value'] === true) {
    ...
    /**
     * set options, display / add and rename file + add buffering
    ...
    public static function init(array $aOptions)
    {
        ...
        foreach ($aOptions as $sKey => $mValue) {
            if (array_key_exists($sKey, self::$AVAILABLE_CONF)) {
                if (is_array(self::$AVAILABLE_CONF[$sKey]['condition'])) {
                    if (in_array($mValue, self::$AVAILABLE_CONF[$sKey]['condition'])) {
                        self::$AVAILABLE_CONF[$sKey]['value'] = $mValue;
                    } else {
                        ...
                        throw new Exception('Specified value for ' . $sKey . ' is not avalaible value : ' . implode(
                            ' , ',
                            self::$AVAILABLE_CONF[$sKey]['condition']
                        ));
                    }
                } elseif (self::$AVAILABLE_CONF[$sKey]['condition']($mValue)) {
                    self::$AVAILABLE_CONF[$sKey]['value'] = $mValue;
                } else {
                    // throw exception if specified directory do not exist
                    throw new Exception('Specified value for ' . $sKey . ' is not avalaible value : ' . self::$AVAILABLE_CONF[$sKey]['condition']);
                }
            }
        }
    }
