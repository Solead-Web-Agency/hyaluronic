...
class BT_GmcProModuleTools
{
    ...
    /**
     * Generates a public module URL based on the PS version
     *
     * @return string
     */
    public static function generatePublicModuleUrl()
    {
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')) {
            return self::getBaseLink() . basename(_PS_ADMIN_DIR_) . '/' . Context::getContext()->link->getAdminLink('AdminModules') . '&configure=' . GMerchantCenterPro::$oModule->name  . '&tab=gsa';
        }

        return Context::getContext()->link->getAdminLink('AdminModules') . '&configure=' . GMerchantCenterPro::$oModule->name . '&tab=gsa';
    }

    /**
     * Check if the Oauth with the GSA api is done
     *
     * @return bool
     */
    public static function isOauthed()
    {
        ...
        if ($bOauth === false) {
            $bOauth = !empty(json_decode(GMerchantCenterPro::$conf['GMCP_OAUTH'])->accessToken) ? true : false;
        }
