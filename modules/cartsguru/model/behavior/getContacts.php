 ...
 */
class GetContacts implements CartsGuru\CartsGuru\Shared\BehaviorInterface
{
    public function execute($context)
    {
        Module::getInstanceByName('cartsguru')->log('GetContacts');
        ...
        $contact = new CGContact();
        $data = $contact->getImportContacts($since, $limit, $context['page']);

        header('HTTP/1.1 200 OK');
        header('Content-Type: application/json');

        if ($contact->existsNextPage($since, $limit, $context['page'])) {
            ++$context['page'];
