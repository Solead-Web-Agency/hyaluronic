 ...
 */
class AggregateContact implements CartsGuru\CartsGuru\Shared\BehaviorInterface
{
    private $helper;
    ...
    public function execute($jsonContact)
    {
        Module::getInstanceByName('cartsguru')->log('AggregateContact');

        $idShopGroup = (int) Shop::getContextShopGroupID();
        $idShop = (int) Shop::getContextShopID();

        $url = Configuration::get('CARTS_GURU_HOOKS_CONTACT_URL', null, $idShopGroup, $idShop);
        ...
        $this->helper->postAsync($url, $jsonContact);
    }
}
