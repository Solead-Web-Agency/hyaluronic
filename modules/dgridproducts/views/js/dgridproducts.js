...
$('.dropdown-menu').live('click', function (e) {
    $(this).parent('div').find('.dropdown-toggle').trigger( "click" );
});
...
$(document).ready(function () {
    $('.id_brand, .id_condition, .id_data_add, .location, .visibility').change(function(){
    ...
    $('.form_edit_field [data-event-save=1]').live('save_field', function () {
        ...
        var test = $(self).data('field-name')
        if (test == 'stock_location') {
            test = 'location';
        }
        value = $(self).val();
        $.ajax({
...
function updateFormField(event) {
    if(event.keyCode === undefined || event.keyCode == 13)
    {
        if(event.keyCode == 13 && $('table.product div:visible').find('> textarea, > input[type=text]').length) {
            $('table.product div:visible').find('> textarea, > input[type=text]').val($('table.product div:visible').find('> textarea, > input[type=text]').val().replace(/\n/g, ''));
