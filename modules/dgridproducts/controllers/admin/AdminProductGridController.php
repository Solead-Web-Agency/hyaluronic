...
if (!class_exists('PackRowSeoSa')) {
    require_once(_PS_MODULE_DIR_ . 'dgridproducts/classes/More/PackRowSeoSa.php');
}
...
class AdminProductGridController extends ModuleAdminController
{
    ...
    protected $price_to;
    protected $default_pagination = 50;
    protected $search_attributes;
    protected $low = 0;
    protected $weight;
    protected $custom_feature;
    protected $yes_discount;
    protected $no_discount;
    protected $percent_discount;
    protected $value_discount;
    ...
    public function __construct()
    {
        ...
        LEFT JOIN `' . _DB_PREFIX_ . 'product_shop` sa ON (
        ...
        ' . (version_compare(_PS_VERSION_, '1.6.1.0', '>=') ? 'image_shop.`id_image`' :
                ...
                ' sa.wholesale_price,' : '') . '
         cl.`name` AS `name_category`, sa.`price`, 0 AS `price_final`, pd.`nb_downloadable`, 
         sav.`quantity` AS `sav_quantity`, sa.`active`,sa.`available_for_order`,
         ' . (version_compare(_PS_VERSION_, '1.7.7.1', '>=') ? ' sa.`wholesale_price`,' : '').'
        ...
        if (Tools::getValue('yes_discount') == 1 && Tools::getValue('no_discount') == 0) {
            $value_discount = Tools::getValue('value_discount');
            $percent_discount = Tools::getValue('percent_discount');
            if ($percent_discount > 0 ||  $value_discount > 0) {
                $this->_join .= ' LEFT JOIN `' . _DB_PREFIX_ . 'specific_price` sp ON sp.`id_product` = a.`id_product`';
                if ($percent_discount > 0 &&  $value_discount > 0) {
                    $this->_join .= ' AND (sp.`reduction` = ' . $percent_discount / 100 . ' AND sp.reduction_type = "percentage") 
                                 OR  (sp.`reduction` = ' . $value_discount . ' AND sp.reduction_type = "amount")  ';
                } elseif ($percent_discount > 0 &&  $value_discount == 0) {
                    $this->_join .= '  AND sp.`reduction` = ' . $percent_discount / 100 . ' AND sp.reduction_type = "percentage"';
                } elseif ( $value_discount > 0 && $percent_discount == 0) {
                    $this->_join .= '  AND  sp.`reduction` = ' .  $value_discount . ' AND sp.reduction_type = "amount"';
                } else {
                    $this->_join .= 'LEFT JOIN ' . _DB_PREFIX_ . 'specific_price sp ON sp.`id_product` = a.`id_product`';
                }
                $this->_where .= '  AND sp.`id_product` = a.`id_product`';
        ...
        } elseif (Tools::getValue('no_discount') == 1 && Tools::getValue('yes_discount') == 0) {
            $this->_join .= 'LEFT JOIN `' . _DB_PREFIX_ . 'specific_price` sp ON sp.`id_product` = a.`id_product`';
            $this->_where .= ' AND NOT EXISTS (SELECT * FROM  `' . _DB_PREFIX_ . 'specific_price` where sp.`id_product` = a.`id_product`)';
            $this->_group = ' GROUP BY a.`id_product`';
        }
        ...
        if (Tools::getValue('visibility') && Tools::getValue('visibility') != '0') {
            $this->_where .= ' AND a.`visibility` = "' . Tools::getValue('visibility') .'"';
        }

        if (Tools::getValue('carriers')) {
            $this->search_carriers = Tools::getValue('carriers');
            if ($this->search_carriers == -1) {
                $this->_where .= ' AND pc.`id_carrier_reference` IS NULL';
            } else {
                $count_carriers = count(explode(',', $this->search_carriers));
                if (Tools::strrpos($this->search_carriers, ',', 1)) {
                    if (Tools::getValue('carrier_and') == 0) {
                        $this->_where .= ' AND pc.`id_carrier_reference` IN (' . pSQL($this->search_carriers) . ')';
                        $this->_group = 'GROUP BY sa.`id_product`';
                        if (Tools::getValue('carrier_pre') == 1) {
                            $this->_where .= ' AND (select count(*) from ' . _DB_PREFIX_ .
                                'product_carrier cv where cv.`id_product` = a.`id_product`) = 1';
                        }
                    } else {
                        $this->_where .= ' AND pc.`id_carrier_reference` IN(' . pSQL($this->search_carriers) . ') 
                        group by sa.`id_product` having count(distinct pc.`id_carrier_reference`) = ' . $count_carriers;
                        if (Tools::getValue('carrier_pre') == 1) {
                            $this->_where .= ' AND (select count(*) from ' . _DB_PREFIX_ . 'product_carrier cv 
                            where cv.`id_product` = a.`id_product`) = ' . $count_carriers;
                        }
                    }
                } else {
        ...
        if (Tools::getValue('with_images') == 1 && Tools::getValue('no_images') == 0) {
            $this->_where .= ' AND i.`id_product` = a.id_product';
        }
        if (Tools::getValue('with_images') == 0 && Tools::getValue('no_images') == 1) {
            $this->_where .= '  AND i.`id_product` IS NULL';
        }
        ...
        if ($order_by && $order_way) {
            switch ($order_by) {
                ...
                case 'tax_rules':
                    $this->_join .= '
                    LEFT JOIN ' . _DB_PREFIX_ . 'tax tax ON (
                    a.id_tax_rules_group = tax.id_tax)';
                    $this->_orderBy = 'tax.rate';
                    break;
                case 'brand':
                    $this->_join .= '
...
// полученный референс вставляем в сетку
            ...
            $log_date =  Db::getInstance()->executeS(
                'SELECT `date_upd`, `message`, `id_employee`
                     ...
                     WHERE object_id = ' . (int)$item['id_product'] . ' AND object_type = \'Product\'');
            ...
            if ($count_last < 0) {
                ...
                $item['date_upd'] = $log_date[$count_last]['date_upd'];
                $item['message'] = $log_date[$count_last]['message'];
                ...
                $item['employee'] = $name_employee['lastname'] . ' ' . $name_employee['firstname'];
            }
        }
    }
...
//            'tagify', // not work for 1.5
        ...
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproducts/views/css/jquery-confirm.css');
        ...
        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproducts/views/js/compatibility.js');
        }
        ...
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproducts/views/js/vendor/jquery.select2.js');
        if (version_compare(_PS_VERSION_, '1.6', '<')) {
        ...
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproducts/views/js/jquery-confirm.js');
        ...
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproducts/views/js/pack.js');
    ...
    public function ajaxProcessSaveField()
    {
        ...
        if (!empty($id)) {
            PrestaShopLogger::addLog(
                sprintf('%s modification', 'Module dgridproduct '),
                1,
                null,
                "Product",
                (int)$id,
        ...
        if ($name == 'visibility' && $criterion == 'id_product') {
            Db::getInstance()->execute('UPDATE '
                . _DB_PREFIX_ . 'product_shop SET `visibility` = "' . pSQL($value) . '" WHERE id_product = ' . $id);
        }
...
WHERE `id_shop` = ' . $idShop . ' and `id_lang` = ' . $this->context->language->id);
    ...
    public function ajaxProcessSaveMore()
    {
        ...
        foreach ($this->getLanguages() as $l) {
            ...
            $link_rewrite[$l['id_lang']] = Tools::getValue('friend_url_' . $l['id_lang']);
...
//        }
    ...
    public function getAttributesParams($id_product, $id_product_attribute)
    {
        $result = Db::getInstance()->executeS('
            ...
            WHERE pa.`id_product` = ' . (int)$id_product . '
                ...
                AND agl.`id_lang` = ' . (int)$this->context->language->id . ' ORDER BY agl.`name`');
    ...
    public function updateObjectField($class_name, $field, $id, $value, $id_shop = null)
    {
        ...
        if (!$lang && $shop) {
            ...
            if ($field == 'active') {
                $sql_prod = 'UPDATE ' . _DB_PREFIX_ . $definition['table'];
                $sql_prod .= ' SET `' . pSQL($field) . '` = "'
                    . ObjectModel::formatValue($value, $definition_field['type']) . '"';
                $sql_prod .= ' WHERE `' . $definition['primary'] . '` = ' . (int)$id;
                Db::getInstance()->execute($sql_prod);
            }
    ...
    public function ajaxProcessGetCustomizationBlock()
    {
        ...
        die(Tools::jsonEncode(array(
            ...
            'html' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproducts/views/templates/admin/ajax/customization.tpl'
            )
        )));
    }
    ...
    public function getLinkProductForList($echo)
    {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'dgridproducts/views/templates/admin/product_link.tpl');
    }
 ...
 <br />' . Tools::displayError('You cannot delete all of the items.');
                    ...
                    if (is_array($products) && ($count = count($products))) {
                        // Deleting products can be quite long on a cheap server.
                        // Let's say 1.5 seconds by product (I've seen it!).
    ...
    public function ajaxProcessGetlocation()
    {
	   ...
	   Group BY pac.`id_product_attribute`
        ...
        $content = $this->context->smarty->fetch(
            _PS_MODULE_DIR_ . 'dgridproducts/views/templates/admin/ajax/combination_locale.tpl'
        );
