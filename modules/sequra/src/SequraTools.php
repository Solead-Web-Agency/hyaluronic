...
class SequraTools
{
    public const ISO8601_PATTERN = '^((\d{4})-([0-1]\d)-([0-3]\d))+$|P(\d+Y)?(\d+M)?(\d+W)?(\d+D)?(T(\d+H)?(\d+M)?(\d+S)?)?$';
    ...
    {
        return base64_encode(openssl_digest($value . \Configuration::get('SEQURA_PASS'), 'sha256', true));
    }
    ...
    public static function totals($cart)
    {
        ...
        return ['with_tax' => $total_with_tax];
    }
    ...
    {
        if (method_exists(\Order::class, 'getIdByCartId')) { // PS >= 1.7.1
    ...
    {
        if (method_exists(\Order::class, 'getByCartId')) { // PS >= 1.7.1
