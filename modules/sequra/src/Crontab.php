...
class Crontab
{
    ...
    {
        return strtotime('tomorrow') + 3600 * (int) \Configuration::get('SEQURA_AUTOCRON_H') + 60 * (int) \Configuration::get('SEQURA_AUTOCRON_M');
    }
    ...
    public static function poolCron()
    {
        if (\Configuration::get('SEQURA_AUTOCRON')
            ...
            if (!$client->succeeded()) {
                ...
                $logger->addLog(
                    'seQura Delivery Report could not be sent: (' . $client->getStatus() . ') ' .
                    $client->getRawResult()
                );
            }
        }
