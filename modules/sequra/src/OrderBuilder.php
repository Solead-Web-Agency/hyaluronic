...
class OrderBuilder
{
    ...
    protected function fixRoundingProblems($order)
    {
        ...
        if ($diff_with_tax == 0 || count($order['cart']['items']) < abs($diff_with_tax)) {
            $logger = new Logger();
            $prices = [];
            foreach ($order['cart']['items'] as $item) {
                $prices[] = $item['total_with_tax'] ?? 0;
            }
            $logger->logError(
                'Error in order totals: ' . $order['cart']['order_total_with_tax'] .
                '( ' . implode('+', $prices) . ' ) ' .
                ' != ' . $totals['with_tax'] . ' diff: ' . $diff_with_tax,
                __FUNCTION__,
                __CLASS__
            );
    ...
    public function previousOrders()
    {
        ...
        if ($orders = \Order::getCustomerOrders($this->cart->id_customer)) {
            foreach ($orders as $order) {
                ...
                if ($orderstate) {
                    $name = is_array($orderstate->name) ? $orderstate->name[$this->getLangId()] : $orderstate->name;
    ...
    private function getLangId()
    {
        ...
        return $customer->id_lang ? (int) $customer->id_lang : (int) $id_lang;
    }
