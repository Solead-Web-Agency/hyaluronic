...
class OrderConfirmer
{
    ...
    public function initFromIpn()
    {
        ...
        if (SequraTools::sign($cart_id) != \Tools::getValue('signed')) {
            $this->logger->logError('The cart signature does not match', __FUNCTION__, __CLASS__);
    ...
    public function run()
    {
        ...
        $this->module->currentOrderReference = $this->order->reference;
    ...
    {
        $is_active = $this->module->active;
        if (!$is_active) {
            $this->logger->logError('Module is not active', __FUNCTION__, __CLASS__);
        }
    ...
    public function orderIsCompleted()
    {
        ...
        $this->logger->logError('Order is not completed', __FUNCTION__, __CLASS__);
    ...
    {
        $is_created = (bool) SequraTools::getOrderIdByCartId($this->cart->id);
        if (!$is_created) {
            $this->logger->logError('Order is not created', __FUNCTION__, __CLASS__);
        }
    ...
    {
        $cart = $this->cart;
        $is_cart_abandoned = $cart->id_customer == 0 || $cart->id_address_delivery == 0 || $cart->id_address_invoice == 0;
        if ($is_cart_abandoned) {
            $this->logger->logError('Cart is abandoned', __FUNCTION__, __CLASS__);
        }

        return $is_cart_abandoned;
    }
    ...
    public function customerHasDisappeared()
    {
        ...
        $is_customer_loaded = !\Validate::isLoadedObject($this->customer);
        if ($is_customer_loaded) {
            $this->logger->logError('Customer has disappeared', __FUNCTION__, __CLASS__);
        }

        return $is_customer_loaded;
    }
    ...
    public function approvedBySequra()
    {
        ...
        if ($this->client->cartHasChanged()) {
            $this->logger->logError('Order not approved by seQura due to cart has changed', __FUNCTION__, __CLASS__);
            if ($this->ipn) {
            ...
            $this->logger->logError('Order not approved by seQura', __FUNCTION__, __CLASS__);
    ...
    protected function registerCartAsOrder()
    {
        ...
        try {
            ...
            $this->logger->logError('Exception at order validation: ' . $e->getMessage(), __FUNCTION__, __CLASS__);
    ...
    public function sendOrderRefToSequra()
    {
        ...
        if (!$this->client->succeeded()) {
            $this->logger->logError('Error sending order ref to seQura', __FUNCTION__, __CLASS__);
        }
    }
