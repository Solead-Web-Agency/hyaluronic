...
class ReportBuilder
{
    ...
    public static function getOrderVouchers($id_customer, $id_order)
    {
        ...
        foreach ($rules as $rule) {
            // Ends with needle
            if (strpos($rule['name'], $needle, strlen($rule['name']) - strlen($needle)) !== false) {
                $vouchers[] = $rule;
            }
        }
    ...
    public function itemsFromOrder($order)
    {
        ...
        foreach ($order->getProducts() as $item) {
            ...
            if ($remaining_quantity > 0) {
                ...
                $item['total_with_tax'] = $item['price_with_tax'] * $quantity;
            }
    ...
    private function getLangId($order)
    {
        ...
        return $customer->id_lang ? (int) \Configuration::get('PS_LANG_DEFAULT') : (int) $customer->id_lang;
    }
