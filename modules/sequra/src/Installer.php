...
class Installer
{
    ...
    public function install()
    {
        ...
        /* Just for the language string generator */
        $this->module->l('seQura: Reviewing');
        $this->module->l('seQura: Approved');
        $this->module->l('seQura: Canceled');
        /* end */
        $this->addOrderState('SEQURA_OS_NEEDS_REVIEW', 'seQura: Reviewing', 'Orange');
        $this->addOrderState('SEQURA_OS_APPROVED', 'seQura: Approved', '#009b5a', false, true, false, true, false, 'payment', true);
        $this->addOrderState('SEQURA_OS_CANCELED', 'seQura: Canceled', 'Crimson', false, false, false, true, false, 'order_canceled');
    ...
    {
        $lang = \Language::getIsoById(
            (int) \Language::getIdByIso($country)
        );
        if (!$lang) {
            return;
        }
    ...
    public function addOrderState(
        ...
        $orderState->name = [];
        foreach ($this->module->getAllowedLanguages() as $language) {
            $lang_id = \Language::getIdByIso($language);
            if (!$lang_id) {
                continue;
            }
            $orderState->name[$lang_id] = $this->module->l($title, 'installer', $language);
            $orderState->template[$lang_id] = $template;
        }
    ...
    public function ensureSequraOrdersTableExists()
    {
        $results = $this->db->executeS(
            'SHOW TABLES LIKE "' . _DB_PREFIX_ . 'sequra_order"'
        );
        if (!$results) {
