...
class Sequra extends PaymentModule
{
     ...
     **/
    public const CSS_FILE = 'custom.css';
    ...
    protected $builder;
    protected $valid_api_credentials;
    ...
    public function __construct()
    {
        ...
        $this->limited_countries = ['es', 'fr', 'it', 'pt'];
    ...
    public function ensureSequraOrdersTableExists()
    {
        $installer = new Installer($this);
        $installer->ensureSequraOrdersTableExists();
    }
    ...
    public function disable($force_all = false)
    {
        ...
        array_walk(
            $countries,
            function ($country) {
                Configuration::updateValue('SEQURA_ACTIVE_METHOD_' . $country, json_encode([]));
            }
        );
    ...
    public function addCSS($uri)
    {
        ...
        return $this->context->controller->registerStylesheet(
            $this->version,
            $uri,
            ['media' => 'all', 'priority' => 80, 'server' => $server]
        );
    }
    ...
    public function addJS($uri, $priority = 80)
    {
        ...
        return $this->context->controller->registerJavascript(
            $this->version . $uri,
            $uri,
            ['position' => 'bottom', 'priority' => $priority, 'server' => $server]
        );
    }
    ...
    {
        $this->logger->logInfo('Hook executed', __FUNCTION__, __CLASS__);
        Crontab::poolCron();
        $scriptBaseUri = self::getScriptBaseUri();
        $this->addCSS($this->_path . 'views/css/' . self::CSS_FILE);
        ...
        $this->context->smarty->assign(
            [
                ...
                'sequra_products' => json_decode(Configuration::get('SEQURA_ACTIVE_METHODS_' . $inferred_country)),
    ...
    public function hookDisplayHome($params)
    {
        ...
        return $this->maybeDisplayBanner('displayHome');
    }
    ...
    public function hookDisplayBanner($params)
    {
        ...
        return $this->maybeDisplayBanner('displayBanner');
    }
    ...
    public function hookDisplayTop($params)
    {
        ...
        return $this->maybeDisplayBanner('displayTop');
    }

    /**
     * Get banner for a hook and the current country if it is available.
     *
     * @param string $hook Supported hooks: 'displayHome', 'displayBanner', 'displayTop'
     *
     * @return array|false
     */
    private function maybeGetBanner($hook)
    {
        $country = self::inferCountry();
        if (!in_array($country, $this->getCountries())) {
            $this->logger->logInfo("Country $country not allowed", __FUNCTION__, __CLASS__);
        ...
        if (empty($banners)) {
            $this->logger->logInfo('No banners configured', __FUNCTION__, __CLASS__);
        ...
        try {
            ...
            if (!is_array($banners)) {
                throw new Exception('$banners is not an array');
            }
        } catch (Throwable $e) {
            $this->logger->logError('Error decoding banners: ' . $e->getMessage(), __FUNCTION__, __CLASS__);
        ...
        foreach ($banners as $b) {
            if (!isset($b['hook']) || !isset($b['country']) || empty($b['image'])) {
                continue;
            }

            if ($b['hook'] === $hook && strtoupper($b['country']) === strtoupper($country)) {
                $banner = $b;
                break;
            }
        }
        ...
        if (!$banner) {
            $this->logger->logInfo("No banner found for hook $hook and country $country", __FUNCTION__, __CLASS__);
    ...
    /**
     * Display a banner for a hook and the current country if it is available.
     *
     * @param string $hook Supported hooks: 'displayHome', 'displayBanner', 'displayTop'
     *
     * @return mixed
     */
    private function maybeDisplayBanner($hook)
    {
        $banner = $this->maybeGetBanner($hook);
        if (!$banner) {
            return;
        }
    ...
    public function hookModuleRoutes($params)
    {
        ...
        return [
            'sequra-getidentificationform' => [
                'controller' => 'getidentificationform',
                'rule' => 'sequra/form/{product}',
                'keywords' => [
                    'product' => ['regexp' => '[_a-zA-Z0-9_-]+', 'param' => 'module'],
                ],
                'params' => [
                    'fc' => 'module',
                    'module' => $this->name,
        ...
        if ($this->isProductPage() && PreQualifier::canDisplayWidgetInProductPage($id_product)) {
            ...
            */
            ...
            $ret = array_map(
                ...
                array_filter(
                    PaymentMethodsSettings::getMerchantPaymentMethods(false, $country_code),
                    function ($method) use ($price) {
                        return
                            PaymentMethodsSettings::getFamilyFor($method) != 'CARD'
                            && PreQualifier::isDateInRange($method)
                            && PreQualifier::isPriceWithinMethodRange(
                                $method,
                                $price,
                                PaymentMethodsSettings::getFamilyFor($method) != 'PARTPAYMENT'
                            );
                    }
                )
            );
    ...
    public function getClient()
    {
        ...
        $this->client = new Sequra\PhpClient\Client(
            ...
            Configuration::get('SEQURA_MODE') != 'live',
            _PS_MODULE_DIR_ . implode(DIRECTORY_SEPARATOR, ['sequra', 'sequra.log'])
        );

        return $this->client;
    }

    public function getConfigJson($encode = true)
    {
        $settings = new Settings($this);
        $config = [];
        ...
        $config['SEQURA_PASS'] = '********';
        // Remove / from start and end of the regex so it can be used in JS
        $config['IP_REGEX'] = substr(Settings::IP_REGEX, 1, -1);
        $config['ISO8061_REGEX'] = substr(Settings::ISO8061_REGEX, 1, -1);
        $config['category_options'] = $settings->getCategoryOptions();

        return $encode ? json_encode($config) : $config;
    }
    ...
    public function getContent()
    {
        ...
        $this->ensureSequraOrdersTableExists();
