...
{
    const AMAZON_DOCUMENTATION_URL = 'https://documentation.common-services.com/amazonv5';
    ...
    const TUTORIAL_CRON_PARAMS = 7555;
    ...
    const TUTORIAL_BUSINESS = 7167;
    const TUTORIAL_AMAZON_BE = 7306;
    ...
    {
        $amazon = new Amazon();
        $lang = Amazon::availableLang(Language::getIsoById($amazon->id_lang));
        ...
        if ($alternate_string == null && $lang) {
            $alternate_string = $amazon->l('Amazon Marketplace for Prestashop Online Documentation', basename(__FILE__, '.php'));
        }
    ...
    public static function message($msg, $id = null)
    {
        if ($id) {
            ...
            $support_found = $amazon->l('An online support has been found on this topic', basename(__FILE__, '.php'));
            $click = $amazon->l('Click on this link to obtain support', basename(__FILE__, '.php'));

            $link = sprintf('&lt;a href="%s" title="%s" target="_blank"&gt;%s&lt;/a&gt;', $url, $amazon->l('Amazon Marketplace for Prestashop Online Documentation', basename(__FILE__, '.php')), $url);
