...
class AmazonAddress extends Address
{
    ...
    const KEY_COLISSIMO_RELAY_ID = 'COLISSIMO_RELAY_ID_';

    const INVALID_PATTERN_NAME = '/[0-9!<>,;?=+()@#"°*`{}_^$%:¤\[\]|\.。]|(?!^)[。\.](?:\s|$)/u';
     ...
     * @return mixed|null
     */
    public static function addressExistsByAlias($alias, $id_customer = null, $vatNumber = null)
    {
        $selectVat = '';
        $verifyVat = false;
     ...
     * @return string|void
     */
    public function validateIsoCode($countryCode) {
        if ($countryCode) {
            switch ($countryCode){
                case 'UK':
                    return 'GB';
    ...
    /**
     * @param $id_lang
     * @param AmazonSPDefShippingAddress $amazonAddress
     * @param AmazonSPDefBuyerTaxInformation $buyerTaxInfo
     * @return false|int|mixed|null
     * @throws PrestaShopException
     */
    public function lookupOrCreateAmazonAddress($id_lang, $amazonAddress, $buyerTaxInfo)
    {
        $alias = $this->hash($amazonAddress);
        $id_address = self::addressExistsByAlias($alias, null, $buyerTaxInfo->BuyerTaxRegistrationId);
        ...
        if (!$id_address) {
            ...
            $amzCountryCode = $this->validateIsoCode($amazonAddress->CountryCode);
            $this->id_country = Country::getByIso($amzCountryCode);

            if (!$this->id_country) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s::%s - line #%d'."\n", basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__));
                    CommonTools::p("Unable to load country: ". $amzCountryCode);
                }
                return (false);
            }
            ...
            $this->validatePhoneNumber($amazonAddress);
            ...
            if (property_exists($this, 'vat_number') && $buyerTaxInfo->BuyerTaxRegistrationId) {
                $this->vat_number = $buyerTaxInfo->BuyerTaxRegistrationId;
            }
     ...
     * @return void
     */
    private function validatePhoneNumber($amazonAddress)
    {
        if ($amazonAddress->Phone) {
            $this->phone = preg_replace('/,/', '-', $amazonAddress->Phone);
            $this->phone = preg_replace('/[^+0-9. ()-]/', '', $this->phone);

            if ((bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_FORMAT_PHONE_FT_ZERO)) {
                // Check phone number not start with 0 and +.
                $firstPhoneChar = $this->phone;
                $firstPhoneChar = mb_substr($firstPhoneChar, 0, 1);
                if ($firstPhoneChar != '0' && $firstPhoneChar != '+') {
                    $this->phone = '0' . $this->phone;
                }
            }

            if ((bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_FORMAT_PHONE_REMOVE_SPACING)) {
                $this->phone = str_replace(' ', '', $this->phone);
            }

            // Add phone mobile
            if (!empty($this->phone) && empty($this->phone_mobile)) {
                $this->phone_mobile = $this->phone;
            }
        }
    }
     ...
     * @return bool
     */
    public function updateOtherToAppendRelayId($storeChainStoreId)
    {
        if ($this->other) {
            $this->other .= ' - ';
        }
        $this->other .= self::KEY_COLISSIMO_RELAY_ID . $storeChainStoreId;
    ...
    public static function getAmazonName($fullname, $id_lang = null)
    {
        ...
        $fullname = str_replace('C\0', 'c/o', (string)$fullname);
        $fullname = str_replace('c\o', 'c/o', preg_replace(self::INVALID_PATTERN_NAME, '', $fullname));
        $fullname = str_replace(' / ', ' ', $fullname); // Fixed issue: invalid address. Example: "08521 / Novachem / Aereo"
        ...
        if ($has_co) {
            ...
            // For special case has space between c - o (e.g: xxx c   / o yyyy)
            $has_co = preg_match('/(.+)[\s,-]+[C\/O;:|\-,]+[\s,-]+(.+)/i', $var, $name_company1_1);
            if ($has_co) {
                $var = trim($name_company1_1[1]);
            }
        ...
        // fix only empty lastname
        if (empty(trim($result['lastname']))) {
            $parse_first_name = explode(' ', $result['firstname']);
            if (is_array($parse_first_name) && count($parse_first_name) > 1) {
                $lastname = end($parse_first_name);
                $result['lastname'] = $lastname;
                $result['firstname'] = trim(str_replace($lastname, '', $result['firstname']));
            }
        }
