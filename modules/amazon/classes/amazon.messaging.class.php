...
class AmazonMessaging extends Amazon
{
    ...
    public function sendReviewIncentive($id_order)
    {
        ...
        if (!isset($mail_review['template']) || empty($mail_review['template'])) {
            echo $message = $this->l('You must select an email template');
            if ($this->_debug) {
                CommonTools::p(sprintf('%s:#%d %s', basename(__FILE__), __LINE__, $message));
            }
        ...
        if ($account_type != Amazon::ACCOUNT_TYPE_INDIVIDUAL) {
            echo $message = 'This feature can work only with global account configuration';
            if ($this->_debug) {
                CommonTools::p(sprintf('%s:#%d %s', basename(__FILE__), __LINE__, $message));
            }
        ...
        if (!isset($order->marketPlaceOrderId) || empty($order->marketPlaceOrderId)) {
            echo $message = sprintf($this->l('Invalid Order (%d) - marketPlaceOrderId is wrong or missing'), $order->marketPlaceOrderId);
            if ($this->_debug) {
                CommonTools::p(sprintf('%s:#%d Invalid Order (%d) - marketPlaceOrderId is wrong or missing: %s', basename(__FILE__), __LINE__, $message));
            }
        ...
        if ((int)$mail_review['delay'] && AmazonTools::getWorkingDays($dateOrdered, $dateCurrent) >= (int)$mail_review['delay']) {
            echo $message = $this->l(sprintf('Out of delay: created on %s - sent on %s', $dateOrdered, $dateCurrent));
        ...
        if (!file_exists($template_file)) {
            $message = sprintf('%s:#%d Template file doesn\'t exists for this lang: %s(%d)', basename(__FILE__), __LINE__, $lang, $id_lang);
            if ($this->_debug) {
