...
class AmazonProduct extends Product
{
     ...
     */
    public static function checkProduct($SKU, $id_shop = null, $id_lang = null, $feature = '')
    {
        $count = 0;
        $parentSku = '';
        /// Temporary workaround
        if (version_compare(_PS_VERSION_, '1.5', '>=') && empty($id_shop)) {
        ...
        // new logic for import the order
        if ((bool)AmazonConfiguration::get(AmazonConstant::CONFIG_IMPORT_BY_ID) && $count == 0 && strpos($SKU,
                '_') !== false && $feature == 'import') {
            $split_combination = explode('_', $SKU);
            if (count($split_combination) == 2) {
                $id_product = (int)$split_combination[0];
                $id_combination = (int)$split_combination[1];

                $sql_product = ' (SELECT `disable` FROM `' . _DB_PREFIX_ . AmazonConstant::TABLE_MARKETPLACE_PRODUCT_OPTION . '` po
            WHERE po.`id_product` = p.`id_product`  '
                    . $sql_lang . self::shopContextSQL()
                    . ' AND `disable` = 1 '
                    . ' )';

                $sql = 'SELECT count(p.`id_product` ) as count, p.`reference`, ';
                $sql .= $sql_product . ' as disabled ';
                $sql .= ' FROM `' . _DB_PREFIX_ . 'product` p ';
                $sql .= $id_shop ? ' JOIN `' . _DB_PREFIX_ . 'product_shop` ps on (ps.`id_shop` = ' . (int)$id_shop . ' AND ps.`id_product` = p.`id_product`) ' : null;
                $sql .= 'JOIN `' . _DB_PREFIX_ . 'product_attribute` pa ON (pa.`id_product` = p.`id_product`)';
                $sql .= 'WHERE p.`id_product` = ' . pSQL(trim($id_product)) . ' AND pa.`id_product_attribute` = ' . pSQL(trim($id_combination));
                ...
                if (isset($result['reference'])) {
                    $parentSku = $result['reference'];
                }
            }
        } elseif ((bool)AmazonConfiguration::get(AmazonConstant::CONFIG_IMPORT_BY_ID) && $feature == 'import') { // Ticket: https://common-services-force.monday.com/boards/1971464818/views/42129803/pulses/5320507986
            $id_product = $SKU;
            ...
            $sql .= $id_shop ? ' JOIN `' . _DB_PREFIX_ . 'product_shop` ps on (ps.`id_shop` = ' . (int)$id_shop . ' AND ps.`id_product` = p.`id_product`) ' : null;
            $sql .= 'JOIN `' . _DB_PREFIX_ . 'product_attribute` pa ON (pa.`id_product` = p.`id_product`)';
            $sql .= 'WHERE p.`id_product` = "' . pSQL(trim($id_product)). '"';
        ...
        return array('count' => $count, 'disabled' => $disabled, 'parentSku' => $parentSku);
    }
     ...
     * Replace the original function `marketplaceGetCategory` which loads all the categories of all products --> does not work with huge catelog
     * @param $id_product
     * @return array
     * @throws PrestaShopDatabaseException
     */
    public static function marketplaceGetCategories($id_product)
    {
        ...
        if (is_array($results) && count($results)) {
            return array_column($results, 'id_category');
        }
    ...
    public static function marketplaceActionAcknowledge($action, $id_lang, $productList, $dateAcknowledgde)
    {
        ...
        if (in_array($action, array(Amazon::ADD, Amazon::REMOVE, Amazon::UPDATE))) {
            $sql = 'DELETE FROM `'.$table.'` WHERE `date_upd` IS NOT NULL AND `action`="'.pSQL($action).'"'.self::shopContextSQL();
            Db::getInstance()->execute($sql);
        }
    }
