...
class AmazonRepricing extends Amazon
{
    ...
    public static function setQueueName()
    {
        ...
        if ($queue_prefix === null) {
            $queue_prefix = Tools::ucfirst(Tools::substr(AmazonTools::toKey(Configuration::get('PS_SHOP_NAME')), 0, 15));
    ...
    {
        // check by listSubscriptions
        $subscription_result = $amazonApi->listSubscriptions();

        // have no Subscriptions
        if (!($subscription_result instanceof SimpleXMLElement) || !isset($subscription_result->SubscriptionList)
            || empty($subscription_result->SubscriptionList)) {
            return (false) ;
        } else {
            $registered = array();

            // have the Subscription List
            if ($subscription_result instanceof SimpleXMLElement) {
                $subscription_result->registerXPathNamespace('xmlns', 'http://mws.amazonservices.com/schema/Subscriptions/2013-07-01');
                $xpath_result = $subscription_result->xpath('//xmlns:SubscriptionList/xmlns:member/xmlns:Destination/xmlns:AttributeList/xmlns:member');

                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s(#%d): AmazonRepricing::checkSubscription - Queues Found: %s', basename(__FILE__), __LINE__, nl2br(print_r($xpath_result, true))));
                }
                ...
                if (is_array($xpath_result) && count($xpath_result)) {
                    foreach ($xpath_result as $queue) {
                        ...
                        $queue_name = trim(basename((string)$queue->Value));                      
                        if ($queue_name == trim(basename((string)$target_queue))) {
                            return (true) ;
                        } else {
                            $registered[$queue_name] = (string)$queue->Value;
                        }
            ...
            // Duplicate check by target_queue URL
            if (in_array($target_queue, $registered)) {
                return (true) ;
            } else {
                return (false) ;
            }
