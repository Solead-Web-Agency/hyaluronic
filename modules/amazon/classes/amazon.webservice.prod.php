...
class AmazonWebService
{
    ...
    const WSTR_FEEDS = 'Feeds';
    const WSTR_REPORTS = 'Reports';
    const WSTR_MERCHANT_FULFILLMENT = 'MerchantFulfillment';
    const WSTR_ORDERS = 'Orders';
    const WSTR_CANCEL_FEED_SUBMISSIONS = 'CancelFeedSubmissions';
    const WSTR_FBA_CREATE_ORDER = 'FulfillmentOutboundShipment';
    ...
    private static $mwsops_version = 'v101';
    
    /** @var object Parsed error object */
    protected $responseError;

    /** @var string Feed content of last feed request */
    protected $lastFeedContent;

    // The module version to send to proxy, replaced AMAZON_MARKETPLACE_VERSION
    private static $marketplace_version;
     ...
     * @param <bool> $debug
     */
    public function __construct($auth, $from, $marketPlaces = null, $debug = false, $cr = "", $mkpVersion = '')
    {
        if (! $cr) {
            $cr = nl2br(Amazon::LF);
        }
        ...
        // Get the module version, this should be passed from the callers who initialize this class.
        // But there are too many, migrating slowly.
        if ($mkpVersion) {
            self::$marketplace_version = $mkpVersion;
        } else {
            $module = new Amazon();
            self::$marketplace_version = $module->version;
        }
    }
    ...
    private function setAuth($auth)
    {
        ...
        if (isset($auth['AWSAccessKeyID']) && $auth['AWSAccessKeyID'] != null) {
            $this->awsak = $auth['AWSAccessKeyID'];
        }
        // if (isset($auth['SecretKey']) && $auth['SecretKey'] != null) {
    ...
    private function retRegion($countryFrom)
    {
        ...
        switch ($countryFrom) {
            ...
            case 'ae': // United Arab Emirates
                return 'ae';
            case 'sg': // Singapore (SG)
                return 'sg';
            case 'nl': // Netherlands (NL)
                return 'nl';
            case 'sa': // Saudi Arabia (SA)
                return 'sa';
            case 'se': // Sweden (SE)
                return 'se';
            case 'pl': // Poland
            ...
            case AmazonConstant::PLATFORM_EGYPT:
                return AmazonConstant::PLATFORM_EGYPT;
        }
        if ($this->_debug) {
            printf('%sgenerateFeedServiceUrl() function. %sError. Incorrect Country Code "%s" . Please verify and try again.', $this->_cr, $this->_att, $countryFrom);
        }
    ...
    private function setCurrency($curr)
    {
        ...
        if (preg_match('/USD|GBP|EUR|JPY|CAD|INR|MXN|AUD|CNY|BRL|TRY|AED|SGD|SAR|SEK|PLN|EGP/i', $curr)) {
    ...
    {
        return $this->MarketPlaces ?: array();
    }
    ...
    public function simpleCallWS($params, $API, $returnXML = true, $feedContent = null)
    {
        ...
        switch ($API) {
            case 'Feeds':
                ...
                $uri = '/';
                if ($feedContent) {
                    $md5 = base64_encode(md5($feedContent, true));  // TODO: Validation: Need to encode Amazon feed
                    $header += array(
                        'Expect: ',
                        'Accept: ',
                    ...
                    // 2020-02-04: For upload VAT invoice, set content-type pdf
                    $contentTypeHeader = 'Content-Type: application/x-www-form-urlencoded; charset=utf-8';
                    if (isset($params['FeedType']) && self::FEED_TYPE_UPLOAD_VAT_INVOICE == $params['FeedType']) {
                        $contentTypeHeader = 'Content-Type: application/pdf; charset=utf-8';
                    }
        ...
        $curlOptions = array(
            CURLOPT_POST => true,
            CURLOPT_USERAGENT => self::buildCurlOptionUserAgent(),
        ...
        if (self::$mwsops_active) {
            $curlOptions[CURLOPT_URL] = sprintf('%s/post%s?uri=%s', self::$mwsops, self::$mwsops_version, base64_encode($final_url));   // TODO: Validation: Need to encode Amazon request
        }

        $disable_ssl_check = (bool)Configuration::get('AMAZON_DISABLE_SSL_CHECK');

        if (!$disable_ssl_check) {
            $cert = AmazonCertificates::getCertificate();   // Validation: Yes, it exist in `certificates.class.php`
        ...
        $this->lastCurlData = $result;
        $this->lastCurlErrorNumber = curl_errno($curlHandle);
        $this->lastCurlError = curl_error($curlHandle);
    ...
    public static function getServiceStatus($debug)
    {
        ...
        if (self::$mwsops_active) {
            $curlOptions[CURLOPT_URL] = sprintf('%s/post%s?uri=%s', self::$mwsops, self::$mwsops_version, base64_encode($url)); // TODO: Validation: Need to encode Amazon request
        }
        $curlOptions[CURLOPT_POSTFIELDS] = $canonicalized_query.'&Signature='.$signature;
    ...
    private function _endPointURL()
    {
        switch (Tools::strtolower(trim($this->region))) {
            ...
            case 'ae': // United Arab Emirates
                return ('mws-eu.amazonservices.com');
            case 'sg': // Singapore
                return ('mws-fe.amazonservices.com');
            case 'nl': // Netherlands
                return ('mws-eu.amazonservices.com');
            default:
                if ($this->_debug) {
    ...
    public function listSubscriptions($returnXML = true)
    {
        ...
        if (isset($xml->ListSubscriptionsResult)) {
            if ($returnXML) {
                return ($xml->ListSubscriptionsResult);
            } else {
                return (true);
            }
        }
    ...
    private function processFeed($feedType, $feedContent)
    {
        ...
        // Amazon Europe
        //
        if ($feedType != '_POST_ORDER_ACKNOWLEDGEMENT_DATA_') {
        ...
        $response = $this->simpleCallWS($parameters, 'Feeds', true, $feedContent);  // Process feed
    ...
    {
        if ($this->_debug) {
            printf("$this->_cr _callWSs() function starts here $this->_cr");
        }
        ...
        switch ($WSTR) {
            ...
            case 'MerchantFulfillment':
                $params['Version'] = '2015-06-01';
                $uri = '/MerchantFulfillment';
        ...
        $curl_info = curl_getinfo($this->curlHandle);
        $curl_errno = '';
        $curl_error = '';
        ...
        if ($this->_debug) {
            CommonTools::p(sprintf("%s::%d _callWSs", basename(__FILE__), __LINE__));
            ...
            CommonTools::p($curl_info);
            CommonTools::p('cURL error number:'.$curl_errno);
            CommonTools::p('cURL error:'.$curl_error);
        }
        ...
        // todo: This is not a reliable check, AddressLine can contain the string 'error' (Via vittime del terrorismo)
        if ($result === false || strpos($result, 'error')) {
            // Already print debug
            return false;
        }
        ...
        try {
            ...
            if ($this->_debug) {
                ...
                CommonTools::p(sprintf('Returned result: %s'."\n", print_r($result, true)));
            }
            return false;
        }
...
//        $Submissions[3] = $this->overrideShipping($products);
    ...
    public function partiallyUpdateProducts($products, $updatingType = null, $ch = true)
    {
        ...
        for ($i = 0; $i < $count_products; $i++) {
            ...
            if ($products[$i] != null) {
                ...
                $Messages[$m] = $temp = $this->createProductMessage(
                    $Document,
                    $products[$i]['SKU'],
    ...
    private function createProductMessage(DOMDocument $Document, $SKU, $ProductIDType, $ProductIDCode, $ConditionType, $ConditionNote, $ProductData, $ProductDescription, $MessageID, $updatingType = null)
    {
        ...
        if (is_array($ProductDescription)) {
            ...
            }else if($is_parent && is_array($ProductData)){
                //remove fields that would be included and may be required
                foreach($ProductData as $key => $value){
                    if(isset(AmazonXSD::$descriptionDataAdditionalFields[$key])){
                        if(isset(AmazonXSD::$descriptionDataAdditionalFields[$key]['includeInParent'])
                            && AmazonXSD::$descriptionDataAdditionalFields[$key]['includeInParent']
                        ){
                            continue;
                        }
            ...
            if(!$is_parent){
                ...
                if (isset($ProductDescription['TargetAudience']) && !empty($ProductDescription['TargetAudience'])) {
                    ...
                    $TargetAudienceText = $Document->createTextNode($ProductDescription['TargetAudience']);
        ...
        foreach(AmazonXSD::$productXsdGenericFields as $field=>$fieldData){
        ...
        if (is_array($arrayOfData)) {
            ...
            */
            ...
            foreach ($arrayOfData as $key => $value) {
                ...
                $isReallyRequired = in_array($key,array_keys( $arrayOfData[$param]['ReallyRequiredFields'] ));
                ...
                if($key == 'VariationTheme'
                   ...
                   && isset( AmazonXSD::$uppercaseVariationThemes[$arrayOfData['Definition']] )
                   && isset($arrayOfData['ProductType']) 
                   && !in_array($arrayOfData['ProductType'], AmazonXSD::$uppercaseVariationThemes[$arrayOfData['Definition']])
                ){
    ...
    private function createDescriptionDataNode(DOMDocument $Document, &$arrayOfData, &$referenceToClean, $parentTagName = 'DescriptionData')
    {
        ...
        $param = 'Parameters';
        $attributes = 'Attributes';
        $excluded = array('VariationData', 'ClassificationData', 'ProductType', 'ProductSubtype', 'ClothingType', 'ShoeSizeComplianceData');
        $universe = null;
        ...
        if (is_array($arrayOfData)) {
            ...
            */
            ...
            foreach($productTypeArr as $field){
                if(isset(AmazonXSD::$excludedFromDescriptionData[$field])){
                    $validateExclusion = true;
                    $exclusionRef = AmazonXSD::$excludedFromDescriptionData[$field];
                }
            }
    ...
    */
     ...
     * @param $Price
     * @param $Sales
     * @param AmazonBusinessPriceMessage $Business
     * @param $MessageID
     *
     * @return DOMElement
     */
    private function createPriceMessage(DOMDocument $Document, $SKU, $Price, $Sales, $Business, $MessageID)
    {
        ...
        $businessPriceTags = $Business->resolveXmlTags($Document, $Price);
        if (count($businessPriceTags)) {
            foreach ($businessPriceTags as $businessPriceTag) {
                $PriceX->appendChild($businessPriceTag);
            }
        }
    ...
    private function getOrderItemsXml($AmazonOrderId)
    {
        ...
        $this->addSimpleLog("getOrderItemsXml() function. WebService response is: " . $d);
    ...
    public function getOrders($amazonOrderIds)
    {
        $data = null;
        $params = array(
            // MarketplaceId takes no effect on this endpoint
            'Action' => 'GetOrder',
            'SellerId' => $this->mid,
            'SignatureVersion' => '2',
            'SignatureMethod' => 'HmacSHA256',
    ...
    /**
     * todo: Use _getOrdersByNextToken() instead
     * @param $token
     * @param $Orders
     *
     * @return bool|null
    ...
    public function GetOrdersByNextToken($token, &$Orders)
    {
        ...
        if ($this->_debug) {
            if ($data instanceof SimpleXMLElement) {
                CommonTools::p(htmlentities($data->asXML()));
            } else {
                print_r($data);
            }
        }
     ...
     * @return string
     */
    private function _getOrdersByNextToken($token, &$Orders)
    {
        $this->pdd(sprintf('GetOrdersByNextToken function. Token: %s', $token));
        $params = array(
            'Action' => 'ListOrdersByNextToken',
            'NextToken' => $token,
            'MarketplaceId.Id.1' => $this->mpid,
        );
        
        $data = $this->_simpleCallWs(self::WSTR_ORDERS, $params);
        
        if ($data) {
            $nextToken = isset($data->ListOrdersByNextTokenResult->NextToken) ?
                $data->ListOrdersByNextTokenResult->NextToken : '';
            $countOrdersBefore = count($Orders);

            if (isset($data->ListOrdersByNextTokenResult, $data->ListOrdersByNextTokenResult->Orders, $data->ListOrdersByNextTokenResult->Orders->Order)) {
                $orderList = $data->ListOrdersByNextTokenResult->Orders->Order;
                $index = 0;
                while ($orderList[$index]) {
                    $Orders[] = new PlacedOrder($orderList[$index], null, $this->_debug);
                    $index++;
                }
            }
            $countOrdersAfter = count($Orders);

            return ($nextToken && $countOrdersAfter > $countOrdersBefore) ? $nextToken : '';       
        }
        
        return '';
    }

    private function pdd()
    {
        if ($this->_debug) {
            $this->_backtrace(func_get_args());
        }
    }

    private function _backtrace($args)
    {
        ...
        foreach ($args as $arg) {
            AmazonTools::pre(array($callerStackStr, $arg), true) . $this->_cr;
        }
    }
    ...
    public function createFulfillmentOrder($order, $returnXML = false)
    {
        ...
        $params = array(
            ...
            'SignatureMethod' => 'HmacSHA256',
            'Version' => '2010-10-01',
            'SellerFulfillmentOrderId' => mb_substr($order['SellerFulfillmentOrderId'], 0, 40),
            'DisplayableOrderId' => trim(mb_substr($order['DisplayableOrderId'], 0, 40)),
            'DisplayableOrderDateTime' => $order['DisplayableOrderDateTime'],
            'DisplayableOrderComment' => mb_substr($order['DisplayableOrderComment'], 0, 1000),
        ...
        */
        ...
        $data = $this->_simpleCallWs(self::WSTR_FBA_CREATE_ORDER, $params);
     ...
     * @return array|bool
     */
    public function GetUnshippedOrdersListv4($createdAfterDate, $createdBeforeDate = null, $Status = 'All', $channel = '', $returnXML = false, $cronMode = false)
    {
        $data = null;
        if ($this->_debug == true) {
        ...
        // 2021-11-03: Tran removed date param by $FBA
        if (!$cronMode) {
            $params['CreatedAfter'] = $createdAfterDate;
            $params['CreatedBefore'] = $createdBeforeDate;
        } else {
        ...
        if (!empty($channel) && is_string($channel)) {
            $params['FulfillmentChannel.Channel.1'] = $channel;
        }
        ...
        $order_statuses = explode(',', $Status);
        // get all orders by skip this step if included status ALL
        if (!in_array(AmazonConstant::OI_ORDER_STATUS_ALL, $order_statuses)) {
            for ($i = 0; $i < count($order_statuses); $i++) {
                $params['OrderStatus.Status.' . ($i + 1)] = $order_statuses[$i];
            }
        }
        ...
        if (is_null($data)) {
            $data = $this->_simpleCallWs(self::WSTR_ORDERS, $params, $returnXML);
        }
        ...
        while ($nextToken) {
            $nextToken = $this->_getOrdersByNextToken($nextToken, $Orders);
        }
     ...
     * @return bool|SimpleXMLElement
     */
    public function confirmMultipleOrders($fulfillmentMessages)
    {
        if ($this->_debug == true) {
        ...
        $m = 0;
        if (is_array($fulfillmentMessages) && count($fulfillmentMessages)) {
            foreach ($fulfillmentMessages as $amazonShipping) {
                if (!$amazonShipping->hasCarrier()) {
                    if ($this->_debug == true) {
                ...
                // buildXML() has been deprecated
                $Messages[$m] = $amazonShipping->buildXML($Document, $m + 1);
                $m++;
            }
        }
        if (!$m) {
     ...
     * @return bool|string
     */
    public function acknowledgeOrders($orders, $demo = false)
    {
        $Messages = array();
        ...
        $data = !$demo ? $this->processFeed('_POST_ORDER_ACKNOWLEDGEMENT_DATA_', $feed) :
            new SimpleXMLElement('<?xml version="1.0"?><SubmitFeedResponse xmlns="http://mws.amazonaws.com/doc/2009-01-01/"><SubmitFeedResult><FeedSubmissionInfo><FeedSubmissionId>' . rand(100000000000, 999999999999) . '</FeedSubmissionId><FeedType>_POST_ORDER_ACKNOWLEDGEMENT_DATA_</FeedType><SubmittedDate>2020-02-24T06:21:10+00:00</SubmittedDate><FeedProcessingStatus>_SUBMITTED_DEMO_</FeedProcessingStatus></FeedSubmissionInfo></SubmitFeedResult><ResponseMetadata><RequestId>8ff121e9-ee21-demo-a0ef-81e4b7d61ef0</RequestId></ResponseMetadata></SubmitFeedResponse>');
        $this->setLastFeedContent($feed, $data ? $data->saveXML() : '');
     ...
     * @return bool|SimpleXMLElement
     */
    public function uploadVATInvoice($invoice, $feedOptions)
    {
        $nl = $this->_cr;
        $feedType = self::FEED_TYPE_UPLOAD_VAT_INVOICE;

        if ($this->_debug) {
            print("$nl processFeed() function started. $nl Now we send following request to the WebService: $nl \$feedType : $feedType $nl \$feedOptions: $nl $feedOptions $nl");
        }

        // todo: Remove redundant param
        $parameters = array(
            'Action' => 'SubmitFeed',
            'MarketplaceIdList.Id.1' => $this->mpid,
            'Merchant' => $this->mid,
        ...
        $response = $this->_simpleCallWs(self::WSTR_FEEDS, $parameters, true, $invoice);  // Process feed

        if (false === $response) {
            if ($this->_debug) {
                printf("Response is false: %s $nl", $this->_caller());
            }
        ...
        if ($this->_debug) {
            printf("Response is true: %s $nl", nl2br(print_r($response, true)));
        }
    ...
    public function cancelOrders($orders)
    {
        ...
        foreach ($orders as $order) {
            ...
            $Messages[] = $this->createEnhancedOrderAcknowledgementMessage($Document, $order, $i + 1, 'Failure');
        }
    ...
    */
    ...
    {
        $parameters = array(
            'Action' => self::WSTR_CANCEL_FEED_SUBMISSIONS,
            'FeedSubmissionIdList.Id.1' => $feedSubmissionId,
        );
        return $this->_simpleCallWs(self::WSTR_CANCEL_FEED_SUBMISSIONS, $parameters);
    }

    /**
     * 2020-02-13: Tran add: Simple call WS refactor
     * @param $WSTR
     * @param $params
     * @param bool $returnXml
     * @param null $feedContent
     * @return bool|SimpleXMLElement
     */
    public function _simpleCallWs($WSTR, $params, $returnXml = true, $feedContent = null)
    {
        $host = $this->_endPointURL();
        $method = 'POST';
        $uri = $this->getUriOfAction($WSTR);
        $url = 'https://' . $host . $uri;

        // 1. Build params
        $params = $this->buildParams($WSTR, $params);
        ksort($params);

        // 2. Curl init
        $signatureQuery = $this->buildSignatureQuery($params, $host, $method, $uri);
        $curlOptions = $this->buildCurlOptions($url, $signatureQuery, $params, $feedContent);
        $curlHandle = curl_init();
        curl_setopt_array($curlHandle, $curlOptions);
        ...
        if ($curlOptions[CURLOPT_VERBOSE]) {
            rewind($curlOptions[CURLOPT_STDERR]);
            $verboseLog = stream_get_contents($curlOptions[CURLOPT_STDERR]);
            echo "Verbose information:\n<pre>", htmlspecialchars($verboseLog), "</pre>\n";
        }
        if ($this->_debug) {
            CommonTools::p(sprintf("%s::%d _callWSs", basename(__FILE__), __LINE__));
            ...
            if (isset($curlOptions[CURLOPT_HTTPHEADER])) {
                CommonTools::p("Headers:" . print_r($curlOptions[CURLOPT_HTTPHEADER], true));
            }
        ...
        // Close curl handle and feed content handle if any
        curl_close($curlHandle);
        if (isset($curlHandle[CURLOPT_INFILE])) {
            fclose($curlHandle[CURLOPT_INFILE]);
        }

        // 4. Return data
        return $this->returnData($result, $returnXml);
    }

    protected function getUriOfAction($WSTR)
    {
        switch ($WSTR) {
            case self::WSTR_REPORTS:
                return '/Reports';
            case self::WSTR_ORDERS:
                return '/Orders';
            case self::WSTR_MERCHANT_FULFILLMENT:
                return '/MerchantFulfillment';
            case self::WSTR_FBA_CREATE_ORDER:
    ...
    protected function getDefaultParamsOfAction($WSTR)
    {
        switch ($WSTR) {
            case self::WSTR_ORDERS:
                return array('Version' => '2013-09-01');
            case self::WSTR_FEEDS:
            case self::WSTR_REPORTS:
                return array('Version' => '2009-01-01');
            case self::WSTR_MERCHANT_FULFILLMENT:
                return array('Version' => '2015-06-01');
            case self::WSTR_FBA_CREATE_ORDER:
                return array('Version' => '2010-10-01');
            default:
                return array();
        }
    }

    public function buildParams($WSTR, $params)
    {
        $defaultParams = array(
        ...
        if ($this->tk) {
            $defaultParams['MWSAuthToken'] = $this->tk;
        }

        $defaultParamsOfAction = $this->getDefaultParamsOfAction($WSTR);

        return array_merge($defaultParams, $params, $defaultParamsOfAction);
    }

    protected function buildSignatureQuery($params, $host, $method, $uri)
    {
        $canonicalized_query = array();
        foreach ($params as $param => $value) {
            $param = str_replace('%7E', '~', rawurlencode($param));
        ...
        $string_to_sign = $method . Amazon::LF . $host . Amazon::LF . $uri . Amazon::LF . $canonicalized_query;
        // calculate HMAC with SHA256 and base64-encoding
        //TODO: Validation: API Requirement
    ...
    protected function buildCurlOptions($url, $signatureQuery, $params, $feedContent)
    {
        $curlOptions = array(
            CURLOPT_POST => true,
            CURLOPT_USERAGENT => self::buildCurlOptionUserAgent(),
        ...
        // Upload feed need additional headers & infile
        $headers = array();
        if ($feedContent) {
            $md5 = base64_encode(md5($feedContent, true));  // TODO: Validation: Need to encode Amazon request
            $headers = array('Expect: ', 'Accept: ', 'Transfer-Encoding: chunked', 'Content-MD5: ' . $md5);
            // 2020-02-04: For upload VAT invoice, set content-type pdf
            $contentTypeHeader = 'Content-Type: application/x-www-form-urlencoded; charset=utf-8';
            if (isset($params['FeedType']) && self::FEED_TYPE_UPLOAD_VAT_INVOICE == $params['FeedType']) {
                $contentTypeHeader = 'Content-Type: application/pdf; charset=utf-8';
            ...
            $feedContentHandle = @fopen('php://temp', 'rw+');
            fwrite($feedContentHandle, $feedContent);
            rewind($feedContentHandle);

            $finalUrl = $url . '?' . $signatureQuery;
            $curlOptions[CURLOPT_INFILE] = $feedContentHandle;
        } else {
            $finalUrl = $url;
            $curlOptions[CURLOPT_POSTFIELDS] = $signatureQuery;
        }
        ...
        // Override URL
        if (self::$mwsops_active) {
            $curlUrl = sprintf('%s/post%s?uri=%s', self::$mwsops, self::$mwsops_version, base64_encode($finalUrl)); // TODO: Validation: Need to encode Amazon request
        } else {
            $curlUrl = $finalUrl;
        }
        $curlOptions[CURLOPT_URL] = $curlUrl;
        ...
        if ($this->_debug) {
            $curlOptions[CURLOPT_VERBOSE] = true;
            $curlOptions[CURLOPT_STDERR] = fopen('php://temp', 'w+');
        }
     ...
     * @return bool|SimpleXMLElement
     */
    protected function returnData($result, $returnXml)
    {
        // Request false
        if (false === $result) {
            return false;
        ...
        // Return raw result if demand
        if (!$returnXml) {
            return $result;
        }

        // Return XML or false if error
        try {
        ...
        // No need to handle case: false === $responseXml
        if ($responseXml->getName() === 'ErrorResponse') {
    ...
    protected function initResponseError()
    {
        $responseError = new stdClass();
        $responseError->HasError = false;
        $responseError->Type = null;
    ...
    /**
     * @param $responseXml string raw XML
     */
    protected function handleResponseError($responseXml)
    {
        try {
            $responseParsed = new SimpleXMLElement($responseXml);
            if (isset($responseParsed->Error)) {
                $error = $responseParsed->Error;
                $this->responseError->HasError = true;
                $this->responseError->Type = isset($error->Type) ? (string)$error->Type : null;
                $this->responseError->Code = isset($error->Code) ? (string)$error->Code : null;
                $this->responseError->Message = isset($error->Message) ? (string)$error->Message : null;
            }
        } catch (Exception $exception) {
            CommonTools::p('Exception Caught: Cannot parse error response. ' . $exception->getMessage());
        }
    }

    protected function setLastFeedContent($feedContent, $feedSubmission)
    {
        $this->lastFeedContent = new stdClass();
        $this->lastFeedContent->feed = $feedContent;
        $this->lastFeedContent->sumbission = $feedSubmission;
    }

    public function getLastFeedContent()
    {
    ...
    public function getLastRequestInfo()
    {
        return array(
            ...
            'curl_data' => $this->lastCurlData,
            'curl_error_number' => $this->lastCurlErrorNumber,
    ...
    private $simpleLog;
    public function getSimpleLog()
    {
        return $this->simpleLog;
    }

    protected function addSimpleLog($error)
    {
        $this->simpleLog = $error;
    }
}
...
class PlacedOrder
{
    ...
    public function __construct(SimpleXMLElement $order, $Items, $debug = false)
    {
        ...
        $this->infoTaxIdentity = new InfoTaxIdentity($order);
        $this->Address = new AmazonWsAddress($order->ShippingAddress, $this->infoTaxIdentity);

        $this->isReplacementOrder = isset($order->IsReplacementOrder) && (string)$order->IsReplacementOrder == 'true' ? true : false;
        $this->replacedOrderId = isset($order->ReplacedOrderId) ? (string)$order->ReplacedOrderId : null;

        if ((string)$order->BuyerName != (string)$order->BillingAddress->Name) {
            $this->BillingAddress = new AmazonWsAddress($order->ShippingAddress, $this->infoTaxIdentity);
...
class AmazonWsAddress
{
     ...
     */
    public function __construct(SimpleXMLElement $Address, InfoTaxIdentity $InfoTaxIdentity=null)
    {
        ...
        if($InfoTaxIdentity != null){
            $this->InfoTaxIdentity = $InfoTaxIdentity;
        }
    }
    ...
    public function printing()
    {
        ...
        if($this->InfoTaxIdentity != null){
            CommonTools::p(
                sprintf(
                    "taxRegistrationId: %s, taxRegistrationAuthorityCountry: %s, taxRegistrationType: %s",
                    $this->InfoTaxIdentity->taxRegistrationId,
                    $this->InfoTaxIdentity->taxRegistrationAuthorityCountry,
                    $this->InfoTaxIdentity->taxRegistrationType
                )
            );
        }
    }
...
class OrderedItem
{
    ...
    /** @var array|null Raw data from Amazon, parse to array if possible. */
    public $Customization;
    
    // New on July 1st, 2021
    public $DeemedResellerCategory; // IOSS | OSS
    public $IossNumber; // eg. IM4420001201
    
    protected static $definition = array(
        'DeemedResellerCategory' => array('type' => 'string', 'required' => false),
        'IossNumber' => array('type' => 'string', 'required' => false),
    );
    ...
    public function __construct(SimpleXMLElement $Item)
    {
        ...
        foreach (self::$definition as $prop => $definition) {
            if (isset($Item->$prop)) {
                $this->$prop = (string)$Item->$prop;
            }
        }
    }
    ...
    public function getCustomizedData($customizedUrl)
    {
        ...
        if (@copy($customizedUrl, $temp_file)) {
            ...
            // Dirtily close the zip to gid rid of: Invalid or uninitialized Zip object in
            @$zip->close();
        }
 ...
 * VAT information
 */
class InfoTaxIdentity{
    /**
     * ID number. e.g. ESB12345678
     *
     * @var string
     */
    ...
    /**
     * Country code for which tax ID is registered. e.g. ES
     *
     * @var string
     */
    public $taxRegistrationAuthorityCountry;

    /**
     * Registration type. e.g. VAT
     *
     * @var string
     */
    public $taxRegistrationType;

    /**
     * InfoTaxIdentity Constructor
     *
     * @param SimpleXMLElement $order
     */
    public function __construct($order){
        ...
        $this->taxRegistrationAuthorityCountry = null;
        $this->taxRegistrationType = null;

        if(isset($order->TaxRegistrationDetails)
           && isset($order->TaxRegistrationDetails->member)
        ){
            $member = $order->TaxRegistrationDetails->member;

            if(isset($member->taxRegistrationId) 
              && Tools::strlen((string)$member->taxRegistrationId)){
                $this->taxRegistrationId = (string)$member->taxRegistrationId;
            }

            if(isset($member->taxRegistrationAuthority)
               && isset($member->taxRegistrationAuthority->country)
               && Tools::strlen((string)$member->taxRegistrationAuthority->country)
            ){
                $this->taxRegistrationAuthorityCountry = (string)$member->taxRegistrationAuthority->country;
            }

            if(isset($member->taxRegistrationType) 
              && Tools::strlen((string)$member->taxRegistrationType)){
                $this->taxRegistrationType = (string)$member->taxRegistrationType;
            }
