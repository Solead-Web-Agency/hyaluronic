...
class AmazonAdminOrder extends Amazon
{
    ...
    public function marketplaceOrderDisplay($params)
    {
        ...
        $_tokens = AmazonConfiguration::get('CRON_TOKEN');
        ...
        $amzMkpId = $amazonOrder->amazon_order_info->marketplace_id;
        // The order is eligible for MCF order
        if (!$amzMkpId) {
            $lang2Mkp = AmazonConfiguration::get(AmazonConstant::CONFIG_PS_LANG_TO_AMZ_MKP_ID);
            if (isset($lang2Mkp[$this->_order_id_lang])) {
                $amzMkpId = $lang2Mkp[$this->_order_id_lang];
            }
        }
        ...
        $view_params = array(
            ...
            'ps_version_is_17' => version_compare(_PS_VERSION_, '1.7', '>='),
            'ps_version_is_16' => version_compare(_PS_VERSION_, '1.6', '>=') && version_compare(_PS_VERSION_, '1.7', '<'),
            'ps_version_is_15' => version_compare(_PS_VERSION_, '1.5', '>=') && version_compare(_PS_VERSION_, '1.6', '<'),
            'css_url' => $this->url.'views/css/admin_order.css?v=' . $this->version,

            'amazon_token' => $token,
            'id_order' => $amazonOrder->id,
            'id_lang' => $this->_order_id_lang,
            'marketplace_region' => AmazonTools::idToDomain($this->_order_id_lang),
            'marketplace_id' => $amzMkpId,
            'marketplace_flag' => $amzMkpId ? $this->images . "geo_flags_web2/{$amzMkpId}_32px.png" : '',
            'marketplace_order_id' => $amazonOrder->marketPlaceOrderId ?: null,
            'marketplace_channel' => $this->resolveMkpChannel($amazonOrder),
            'glossary' => AmazonSettings::getGlossary(Language::getIsoById($this->id_lang), 'admin_order_details'),
            'context_key' => isset($this->context) && $this->context instanceof Context ?
                AmazonContext::getKey($this->context->shop) : null,
        );
        ...
        if ($amazonOrder->amazon_order_info instanceof AmazonOrderInfo && $amazonOrder->amazon_order_info->is_extended_feature_available && $amazonOrder->amazon_order_info->getOrderInfo()) {
            ...
            if ($enabled_shipping_label && $amazonOrder->amazon_order_info->shipping_services && !empty($amazonOrder->amazon_order_info->shipping_services)) {
                ...
                if (isset($shipping_services->errors) && !empty($shipping_services->errors)) {
                    $shipping_service_errors = AmazonDataAdjustment::unserialize($shipping_services->errors) ;
                ...
                if (isset($shipping_services->shipping_services) && !empty($shipping_services->shipping_services)) {
                    ...
                    $shipping_services = AmazonDataAdjustment::unserialize($shipping_services->shipping_services);
            ...
            // todo: Use AmzOrder instead
            $view_params['amazon_order_prime_info'] = $this->buildSpPrimeParams($id_order);
     ...
     * @return string
     */
    private function resolveMkpChannel($amazonOrder)
    {
        if (isset($amazonOrder->marketPlaceChannel) && $amazonOrder->marketPlaceChannel) {
            switch ($amazonOrder->marketPlaceChannel) {
                case AmazonMultiChannel::AMAZON_FBA_AMAZON:
                    return $this->l('Fulfilled By Amazon');
                case AmazonMultiChannel::AMAZON_FBA_MULTICHANNEL:
                    return $this->l('Multi-Channel Order / Fulfilled By Amazon');
            }
        }
    ...
    private function marketplaceOrderDisplayFbaEligibleToMultichannel(&$view_params, &$order, &$params)
    {
        ...
        $view_params['fbaorder_url'] = $this->url . 'functions/fbaorder.php';
        $this->_marketplaceOrderDetail($view_params, $params);

        $this->ed('Amazon, Debug Mode', $view_params);
    ...
    private function marketplaceOrderDisplayFbaMultichannel(&$view_params, &$order, &$params)
    {
        ...
        switch (Tools::strtolower($multiChannelOrder->marketPlaceChannelStatus)) {
            case AmazonMultiChannel::AMAZON_FBA_STATUS_SUBMITTED:
    ...
    protected function buildVIDRParams($view_params)
    {
        ...
        if ($vcsEnable && isset($view_params['marketplace_order_id'], $view_params['marketplace_id'])) {
            $mpOrderId = $view_params['marketplace_order_id'];
            $marketplaceId = $view_params['marketplace_id'];
            $marketplace = AmazonVIDRShipment::getMarketplaceIso($marketplaceId, false);
