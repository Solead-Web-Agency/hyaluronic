...
class AmazonOrderInfo
{
    ...
    public $shipping_services = null;
    public $shipping_method;
    public $fulfillment_center_id = null;
            ...
            if ($this->getOrderInfo()) {
                ...
                foreach ($to_insert as $field => $value) {
                    if($value instanceof stdClass){
                        //prevents including "shipping_services" property
                        continue;
                    }
    ...
    public function getOrderInfo()
    {
        ...
        // get Amazon Shipping Services
        // todo: Prime - Only fetch when need
        if (AmazonTools::tableExists(_DB_PREFIX_.Amazon::TABLE_MARKETPLACE_SHIPPING_SERVICE)) {
            $sql = 'SELECT * FROM `'._DB_PREFIX_.Amazon::TABLE_MARKETPLACE_SHIPPING_SERVICE.'` WHERE `id_order` = '.(int)$this->id_order;

            if ($result = Db::getInstance()->getRow($sql)) {
                $this->shipping_services = new stdClass();
                ...
                foreach ($result as $field => $value) {
                    if ($field == 'mp_order_id') {
                        continue;
                    }
                    $this->shipping_services->{$field} = $value;
                }
            }
        }
