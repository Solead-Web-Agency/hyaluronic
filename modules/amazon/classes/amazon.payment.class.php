...
class AmazonPaymentModule extends PaymentModule
{
    ...
    public function validateMarketplaceOrder($id_cart, $id_order_state, $paymentMethod = 'Unknown', $mpOrderId = null, $mpOrderStatus = null, $amazonCart = null, $useTaxes = false, $date_add = false)
    {
        ...
        $order->total_shipping = (float)($useTaxes ? abs($amazonCart->amzShippingPriceTaxIncl) : abs($amazonCart->amzShippingPriceTaxExcl));
        ...
        if (version_compare(_PS_VERSION_, '1.5', '>=')) {
            ...
            $order->total_shipping_tax_excl = abs((float)$amazonCart->amzShippingPriceTaxExcl);
            $order->total_shipping_tax_incl = abs((float)$amazonCart->amzShippingPriceTaxIncl);
        ...
        if (Validate::isLoadedObject($order)) {
            $referenceFormat = AmazonConfiguration::get(AmazonConstant::CONFIG_OI_REFERENCE_FORMAT);
            $order->reference = $referenceFormat == 'ORDER_ID' ? $order->id : self::generateReference($referenceFormat);
            $order->save();
            ...
            if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                ...
                if ($order->id_carrier) {
                    ...
                    $order_carrier->shipping_cost_tax_excl = abs($order->total_shipping_tax_excl);
                    $order_carrier->shipping_cost_tax_incl = abs($order->total_shipping_tax_incl);
     ...
     * @return string
     */
    public static function generateReference($referenceFormat) {
        $type = (!empty($referenceFormat) && $referenceFormat != '') ? $referenceFormat : 'NO_NUMERIC';

        if ($type == 'PS_DEFAULT') { // Use the prestashop default reference - for thirst party custom purposes
            return Order::generateReference();
        }

        return strtoupper(Tools::passwdGen(9, $type));
    }
}
