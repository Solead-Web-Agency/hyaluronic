...
class AmazonBulletPoint extends Amazon
{
    ...
    public function bulletPointEncode()
    {
        ...
        $json_object = json_encode($json_raw);

        if (!$json_object instanceof stdClass) {
            return (false);
        }

        return AmazonTools::encode(AmazonDataAdjustment::serialize($json_object));
    }
    ...
    public function bulletPointDecode($encoded_bullet_point)
    {
        ...
        $json_object = AmazonDataAdjustment::unSerialize(AmazonTools::decode($encoded_bullet_point));
