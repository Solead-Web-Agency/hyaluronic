...
class AmazonSpecificField extends Amazon
{
    ...
    const WEIGHT = 8;
    const RETAIL_PRICE = 9; //Retail price
    const RETAIL_PRICE_WITH_TAX = 10; //Retail price tax
    ...
    public static function displayFields($id_lang, $profile_name, &$extraFieldsArray, $target_attribute = null, $json_output = false)
    {
        ...
        self::addRecommendedFieldsToSavedProfile($extraFieldsArray, $id_lang);
        ...
        foreach ($extraFieldsArray['field'] as $field => $value) {
            ...
            if (isset($extraFieldsArray['recommended'][$field]) && $extraFieldsArray['recommended'][$field]) {
                $recommended[$field] = $value;
                continue;
            }
            
            if (isset($extraFieldsArray['featured'][$field]) && $extraFieldsArray['featured'][$field]) {                
            ...
            if (isset($extraFieldsArray['choice_allowed_values'][$field])) {
                $options = array();
                $choice_data = '';
                $choice_options = AmazonDataAdjustment::unSerialize(AmazonTools::decode($extraFieldsArray['choice_allowed_values'][$field]));
        ...
        foreach ($fields as $field => $value) {
            ...
            if (array_key_exists($field, $fields_settings) && isset($fields_settings[$field]) && Tools::strlen($fields_settings[$field]['sample'])) {
                $sample = sprintf('(%s: %s)', $parent_instance->l('Eg'), $fields_settings[$field]['sample']);
            } else {
                $sample = null;
            }

            if (array_key_exists($field, $fields_settings) && isset($fields_settings[$field]) && Tools::strlen($fields_settings[$field]['description'])) {
            ...
            foreach (self::$prestashop_fields as $id_prestashop_field) {
                switch ($id_prestashop_field) {
                    ...
                    case self::WEIGHT:
                        ...
                        break;
                    case self::RETAIL_PRICE:
                        $name = $parent_instance->l('Retail price');
                        break;
                    case self::RETAIL_PRICE_WITH_TAX:
                        $name = $parent_instance->l('Retail price with tax');
                        break;
                    default:
                        $name = null;
                        break;
            ...
            if (isset($extraFieldsArray['allowed_values'][$field])) {
                $allowedValues = AmazonDataAdjustment::unSerialize(AmazonTools::decode($extraFieldsArray['allowed_values'][$field]));
                ...
                if (isset($value['choice_allowed_values'])) {
                    $specific_field_view['fields']['choice_allowed_values'][] = array(
                        ...
                        'value' =>  AmazonTools::encode(AmazonDataAdjustment::serialize($value['choice_allowed_values'])),
                    );
                }
            } elseif (!isset($extraFieldsArray['has_valid_values'][$field])) {
                $specific_field_view['recommended'] = $is_recommended;
            ...
            if (isset($extraFieldsArray['attributes'][$field])
                ...
                if (isset($extraFieldsArray['values'][$field]) && $kind) {
                    ...
                    $allowedValues = AmazonDataAdjustment::unSerialize(AmazonTools::decode($extraFieldsArray['values'][$field]));
            ...
            if ($is_required) {
                ...
                $specific_field_view['fields']['recommended'][] = array(
                    'comment' => sprintf('%d: specific field/anyfield/hidden input (recommended) - %s: %s', __LINE__, $profile_name, $field),
                    'name' => 'profiles[extra]['.$profile_key.']['.$id_lang.'][recommended]['.$field.']',
                    'value' => 1,
                );
            }elseif($is_featured) {
                $specific_field_view['fields']['featured'][] = array(
                    'comment' => sprintf('%d: specific field/anyfield/hidden input (featured) - %s: %s', __LINE__, $profile_name, $field),
                    'name' => 'profiles[extra]['.$profile_key.']['.$id_lang.'][featured]['.$field.']',
                    'value' => 1,
                );
            }
    ...
    public static function getFields($id_lang, $profile_name, $extraFieldsArray)
    {
        static $parent_instance = null;
        static $context = null;
        static $regions = null;
        ...
        $json = json_encode(array('specific_fields' => AmazonTools::fixEncoding($fields)));
        //$json = json_encode(array('specific_fields' => $fields));
        ...
        if (is_array($actives)) {
            ...
            foreach (AmazonTools::languages() as $language) {
                ...
                if (!isset($actives[$id_lang]) || !$actives[$id_lang] ||
                    !isset($regions[$id_lang]) || empty($regions[$id_lang])) {
                    continue;
                }
                ...
                if ($europe && AmazonTools::isEuropeMarketplaceId($marketPlaceId) && $id_lang != $master_id_lang) {
                    ...
                    // 2020-09-14: en-us: ISO = en, but region = us. Cast to region.
                ...
                $marketplaces[$id_lang] = array(
                    'default' => $default,
                    'name' => sprintf('www.amazon.%s', AmazonTools::idToDomain($id_lang)),
                    'region' => $regions[$id_lang],
                    'id_lang' => $id_lang,
                    'europe' => $europe,
                    'iso_code' => $language['iso_code'],
                    'lang_code' => $langCode ? $langCode : $language['iso_code'],
                    'active' => $language['active'],
                    'image' => $images.'geo_flags_web2/'.$flag,
    ...
    /**
     * Add fields generic to profile once it's saved
     *
     * @return void
     */
    public static function addRecommendedFieldsToSavedProfile(&$profile, $id_lang){
        $marketPlaceRegions = AmazonConfiguration::get('REGION');
        $region = Tools::strtoupper($marketPlaceRegions[$id_lang]);
        if(!isset($profile['recommended'])){
            $profile['recommended'] = array();
        }

        foreach(AmazonXSD::$descriptionDataAdditionalFields as $fieldName=>$fieldData){
            if(!array_key_exists($fieldName, $profile['field'])){
                continue;
            }

            $profile['recommended'][$fieldName] = 
            isset($fieldData['recommendedFor']) 
            && 
            (
                in_array($region, $fieldData['recommendedFor'])
                ||
                in_array('*', $fieldData['recommendedFor'])
            );

            $profile['required'][$fieldName] = 
            isset($fieldData['requiredFor'])  
            && 
            (
                in_array($region, $fieldData['requiredFor'])
                ||
                in_array('*', $fieldData['requiredFor'])
            );
        }

        foreach(AmazonXSD::$productXsdGenericFields as $fieldName=>$data){
            $profile['field'][$fieldName] = isset($profile['field'][$fieldName]) ? $profile['field'][$fieldName] : null;
            $profile['recommended'][$fieldName] = 
