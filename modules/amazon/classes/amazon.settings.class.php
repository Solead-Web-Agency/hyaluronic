...
class AmazonSettings extends Amazon
{
    ...
    public static function getSettings($path)
    {
        ...
        foreach ($data as $line) {
            ...
            $result[] = Tools::stripslashes($line);
        }

        return array_values(array_unique($result));
    }

    public static function getCustomDefinitions($type, $force = false)
    {
        if (empty($type)) {
            return (false);
        }

        $datadir = dirname(dirname(__FILE__)). DIRECTORY_SEPARATOR . AmazonXSD::getCustomSizeDir();

        $filename = AmazonXSD::getCustomSizeFileName();
        $file = $datadir.$filename;

        $remote_dir = 'settings'. DIRECTORY_SEPARATOR .'json'. DIRECTORY_SEPARATOR ;
        ...
        if (file_exists($file) && is_readable($file)) {
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf("AmazonSettings::getCustomDefinitions: %s", $file));
            }
        }
        return (true);
    }
    ...
    public static function getGlossary($lang, $section)
    {
        ...
        $sections = array();
        $dataDir = implode(DIRECTORY_SEPARATOR, array(dirname(dirname(__FILE__)), 'settings', 'glossary', $section)) . DIRECTORY_SEPARATOR;
    ...
    {
        $datadir = implode(DIRECTORY_SEPARATOR, array(dirname(dirname(__FILE__)), 'settings', 'features')) . DIRECTORY_SEPARATOR;
        ...
        if ($files && is_array($files)) {
            foreach ($files as $file) {
                ...
                if (is_array($ini) && isset($ini['weight'])) {
                    $weight = $ini['weight'];
                    // 2021-12-14: Tran removed ENABLE_EXPERIMENTAL_FEATURES && amazonlite
                    $features[$weight] = $ini;
                }
            }
        }
