...
class AmazonProductTab extends Amazon
{
    ...
    protected $cacheConfigShippingGroups = null;
    ...
    public function marketplaceProductTabContent($params)
    {
        ...
        if ($active) {
            ...
            //$view_params['repricing'] = (bool)$this->amazon_features['repricing']; TODO: Bring back the Repricing feature

            $view_params['product'] = array(
                'name' => $product_name,
                'complex_id' => sprintf('%d_0', $product->id),
        ...
        if (is_array($actives)) {
            foreach (AmazonTools::languages() as $language) {
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf("%s(%d): id_lang - '%s'\n", basename(__FILE__), __LINE__, (string) $id_lang));
                }
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf("%s(%d): product_id %s product_options - '%s'\n", basename(__FILE__), __LINE__, $product->id, json_encode($product_options)));
                }
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf("%s(%d): product_id %s product_actions - '%s'\n", basename(__FILE__), __LINE__, $product->id, print_r($view_params['action'][$id_lang], true)));
                }
    ...
    public function productOptionsDetails($product, $id_lang, $defaults)
    {
        ...
        $tax_code_list = AmazonTaxes::getPtcList($region);
        ...
        $view_params['transparencycode'] = $this->ptTransparencyCode($id_lang, $defaults);
        ...
        //Brand override
        $view_params['brand'] = array(
            'id_lang' => $id_lang,
            'default' => isset($defaults['brand']) ? $defaults['brand'] : null
        );
        
        //Manufacturer override
        $view_params['manufacturer'] = array(
            'id_lang' => $id_lang,
            'default' => isset($defaults['manufacturer']) ? $defaults['manufacturer'] : null
        );
        ...
        // TODO: Bring back the Repricing feature
        ...
        $view_params['product_tax_override'] = array(
            'id_lang' => $id_lang,
            'product_tax_list' => $tax_code_list,
            'default' => isset($defaults['product_tax_override']) ? $defaults['product_tax_override'] : null
        );
    ...
    public function countrySelector()
    {
        ...
        $marketplaces = array();
        $master_mkp_idLang = AmazonConfiguration::get(AmazonConstant::CONFIG_MASTER_MARKETPLACE) ? Language::getIdByIso(AmazonConfiguration::get(AmazonConstant::CONFIG_MASTER_MARKETPLACE)) : '';
        ...
        if (is_array($actives)) {
            ...
            foreach (AmazonTools::languages() as $language) {
                ...
                $marketplaces[$id_lang]['default'] = $master_mkp_idLang ? ($master_mkp_idLang == $id_lang) : $default;
    ...
    private function ptShippingGroup(&$product, $id_lang, &$defaults = null)
    {
        ...
        $configured_group_names = $this->getConfigShippingGroups();
        $group_names = array();
     ...
     */
    protected function getConfigShippingGroups()
    {
        if (is_null($this->cacheConfigShippingGroups)) {
            $config = AmazonConfiguration::get('shipping_groups');
            $this->cacheConfigShippingGroups = $config;
        }
        
        return $this->cacheConfigShippingGroups;
    }
     ...
     * @return array
     */
    private function ptTransparencyCode($id_lang, $defaults = null)
    {
        $view_params = array(
            'default' => isset($defaults['transparencycode']) ? $defaults['transparencycode'] : null,
            'id_lang' => $id_lang
        );
