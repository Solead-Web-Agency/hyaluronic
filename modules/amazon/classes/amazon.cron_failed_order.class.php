...
class AmazonCronFailedOrder
{
    ...
    {
        $previousFailedOrders = AmazonConfiguration::get(AmazonConstant::IMPORT_ORDERS_CRON_FAILED_LIST);
        ...
        foreach ($previousFailedOrders as $orderId => $order) {
            if (is_array($mkpIds) && in_array($order["marketplace"], $mkpIds)) {
    ...
    {
        return AmazonConfiguration::updateValue(AmazonConstant::IMPORT_ORDERS_CRON_FAILED_LIST, json_encode(array()));
    }
