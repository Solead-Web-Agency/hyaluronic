...
class AmazonXSDTools
{
            ...
            if (is_array($refElements) && count($refElements)) {
                ...
                if ($targetElement != $product_instance && is_array($targetElement)) {
                    foreach ($targetElement as $variationThemeItem) {
                        $variationThemeItemRef = self::getCombinationFieldName($variationThemeItem);
                        ...
                        foreach ($variation[$variationThemeItem]['fields'] as $variationField) {
                            $reference = $variationField;
                            if (Tools::strtoupper($variationField) == $variationField
                                || Tools::strtolower($variationField) == $variationField) {  
                                    $variationField = AmazonTools::ucfirst(Tools::strtolower($variationField));
                            }
                                                          
                            $variationField = AmazonXSDTools::getVariationFieldNameWithDimension($variationField);
                            ...
                            if ($variationData instanceof stdClass && property_exists($variationData, $variationField) && property_exists($variationData->{$variationField}, 'attr')) {
                                if (property_exists($variationData->{$variationField}->attr, 'unitOfMeasure') && property_exists($variationData->{$variationField}->attr->unitOfMeasure, 'allowedValues')) {
                                    $variation[$variationThemeItem]['attributes'][$variationField] = AmazonTools::encode(AmazonDataAdjustment::serialize($variationData->{$variationField}->attr->unitOfMeasure->allowedValues));
                            ...
                            if(!$attributesFound && $product_instance instanceof stdClass && isset($product_instance->ProductData->{$universe})) {
                                ...
                                if (property_exists($productAttrs, $variationField) && property_exists($productAttrs->{$variationField}, 'attr') && property_exists($productAttrs->{$variationField}->attr, 'unitOfMeasure') && property_exists($productAttrs->{$variationField}->attr->unitOfMeasure, 'allowedValues')) {
                                    $variation[$variationThemeItem]['attributes'][$variationField] = AmazonTools::encode(AmazonDataAdjustment::serialize($productAttrs->{$variationField}->attr->unitOfMeasure->allowedValues));
                                    $attributesFound = true;
                                }
                            }
                            ...
                            if(!$attributesFound && $product_instance instanceof stdClass && isset($product_instance->DescriptionData)) {
                                $productAttrs = $product_instance->DescriptionData;

                                if (property_exists($productAttrs, $variationField) && property_exists($productAttrs->{$variationField}, 'attr') && property_exists($productAttrs->{$variationField}->attr, 'unitOfMeasure') && property_exists($productAttrs->{$variationField}->attr->unitOfMeasure, 'allowedValues')) {
                                    $variation[$variationThemeItem]['attributes'][$reference] = AmazonTools::encode(AmazonDataAdjustment::serialize($productAttrs->{$variationField}->attr->unitOfMeasure->allowedValues));
                                    $attributesFound = true;
                                }
                            }
                        }
    ...
    public static function getVariationFieldNameWithDimension($variationField){
        
        //consider variationTheme: "itemweight" which is defined as "ItemWeight"
        foreach(array("Weight","Length", "Height", "Width") as $dim){
            $variationField = str_replace(
                    Tools::strtolower($dim), 
                    $dim, 
                    $variationField);
        }
    ...
    public static function getCombinationFieldName($variationThemeFieldName){
        
        $variationThemeItem = $variationThemeFieldName;

        // Size/Color exception
        //keep lower/upper case of variation theme value
        if(strcasecmp($variationThemeItem, 'SizeColor') == 0){
            $variationThemeItem = Tools::substr($variationThemeItem,0,4) . '-' . Tools::substr($variationThemeItem,4);
        }elseif(strcasecmp($variationThemeItem, 'ColorSize') == 0){
            $variationThemeItem = Tools::substr($variationThemeItem,0,5) . '-' . Tools::substr($variationThemeItem,5);
        }

        $variationThemeItemRef = str_ireplace(
            array(
                'Size_name', 
                'Sizename',
                'Color_name',
        ...
        if (Tools::strtoupper($variationThemeItemRef) == $variationThemeItemRef) {
            $variationThemeItemRef = ucwords(Tools::strtolower($variationThemeItemRef));
        } elseif (Tools::strtolower($variationThemeItemRef) == $variationThemeItemRef) {
            $variationThemeItemRef = ucwords(Tools::strtolower($variationThemeItemRef));
        }

        return $variationThemeItemRef;
    }
}
