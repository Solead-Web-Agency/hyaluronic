...
class AmazonXSD
{
     ...
     *
     */
    const MAIN_URL = 'https://images-na.ssl-images-amazon.com/images/G/01/rainier/help/xsd/release_1_9/Product.xsd';
    /**
     *
     */
    const BASE_XSD = 'https://images-na.ssl-images-amazon.com/images/G/01/rainier/help/xsd/release_1_9/amzn-base.xsd';
    /**
     *
     */
     ...
     * @var array
     */
    public static $rewriteFieldsUniverseProductType = array(
        'Baby' => array('BabyProducts','InfantToddlerCarSeat',
                    'Stroller','StrollerProtectionShield',
                    'BabyJumperWalker', 'ChangingPad',
                    'HangingMobile', 'BottleNipple'
                ),
    );
    ...
    public static $descriptionDataAdditionalFields = array(
        ...
        'MSRP' => array('recommendedFor'=>array('*')),
        'MSRPWithTax' => array('recommendedFor'=>array('*')),
        ...
        'DepartmentName'=> array('recommendedFor'=>array('FR', 'ES', 'IT', 'DE', 'UK', 'AU', 'BE')),
        'ProductExpirationType'=>array('recommendedFor'=>array('FR', 'ES', 'IT', 'DE', 'UK', 'AU')),
        'SizeName'=> array('recommendedFor'=>array('FR', 'ES', 'IT', 'DE', 'UK', 'AU')),
        'CountryOfOrigin' => array('requiredFor'=>array('FR', 'ES', 'IT', 'DE', 'UK', 'NL', 'BE'),
                                   'recommendedFor'=>array('AU'),
                                   'includeInParent'=>true
                                ),
        'ItemTypeName' => array('recommendedFor'=>array('FR', 'ES', 'IT', 'DE', 'UK', 'NL', 'BE', 'AU','US')),
    ...
    /**
     * Includes universe and fields which will be excluded from DescriptionData, to be included in ProductData
     *
     * @var array
     */
    ...
    public static $productXsdGenericFields = array(
        ...
        'UVPListPrice' => array('recommendedFor' => array('*')),
     ...
     * @var array
     */
    private static $exceptions = array(); //array('LargeAppliances.xsd' => 'http://g-ecx.images-amazon.com/images/G/01/rainier/help/xsd/release_4_1/LargeAppliances.xsd');
    ...
    /**
     * Keeps in memory data loaded from SHOE_SIZE_COMPLIANCE_FILE
     * @var type 
     */
    private static $sizeComplianceData = array();


    public static $uppercaseVariationThemes = array(  
        'Baby'=> array(
                "BabyProducts", "InfantToddlerCarSeat", "Stroller", "BabyCarrier"),
        'Home'=> array(
                "Home", "BedAndBath", "FurnitureAndDecor", "Kitchen","OutdoorLiving", "SeedsAndPlants", 
                "Art", "Fabric", "VacuumCleaner", "Mattress", "Bed", "Headboard", "Dresser", "Cabinet", 
                "Chair", "Table", "Bench", "Sofa", "Desk", "FloorCover", "Bakeware", "Cookware", "Cutlery", 
                "Dinnerware", "Serveware", "KitchenTools", "SmallHomeAppliances", "BedLinen", 
    ...
    public function __construct($product)
    {
        ...
        $this->productInstance = $this->getElements($content, 'Product');
     ...
     * @return stdClass
     */
    private function getElements(DOMNodeList &$elements, $parentNodeName=null)
    {
        ...
        foreach ($elements as $element) {
            ...
            if(!$this->isDirectChildNode($element, $parentNodeName)){
                continue;
            }
    ...
    /**
     * Verifies if node has the closer parent xsd:element with name = $parentName
     *
     * @return void
     */
    private function isDirectChildNode(DOMElement $node, $parentName){
        $ok = false;

        if($node == null || !($node instanceof DOMElement) || $node->parentNode == null){
            return $ok;
        }
        ...
        $exceptions = array('SizeMap', 'StyleName');

        //Some fields may be duplicated in DescriptionData and ProductData, this exception
        //gives priority to include the field in ProductData
        if($parentName == 'DescriptionData' 
            && isset(self::$excludedFromDescriptionData[$this->productName])
            && isset(self::$excludedFromDescriptionData[$this->productName][$nodeName])
            ){
            $ok = false;
        }else if($parentName == 'DescriptionData' &&  in_array ($nodeName, $exceptions)){
            //specific exception for sizeMap as this tab is included inside "ProductData" and inside "Product/ClassificationData"
            $ok = false;
        }else if(AmazonNode::isElement($parentNodeName)){
            $curName = $parent->getAttribute('name');
            $ok = ($curName == $parentName);
        } else{
            $ok = $this->isDirectChildNode($parent, $parentName);
        }
            ...
            if (!AmazonNode::hasNoData($r->nodeName)) {
                ...
                switch ($r->nodeName) {
                    ...
                    case 'xsd:enumeration':
                        if ($attrValue != null  && !empty($attrValue)) {
            ...
            if (!AmazonNode::hasNoData($node->nodeName)) {
                switch ($node->nodeName) {
                    case 'xsd:union':
                        ...
                        //Exception for Computers/VariationData
                        if(count($arrTypes) == 1 
                            && empty($arrTypes[0]) 
                            ...
                            $arrTypes =array();
                            $obj = null;
                            $obj = $this->getSimpleType($name, $node->childNodes->item(1));
                            return $obj;
                        }
    ...
    private function getComplexType($name, DOMElement &$element, $parentNode)
    {
        ...
        if ($content) {
            $complexType = $this->getElements($content, $name);
        }
            ...
            if (!AmazonNode::hasNoData($node->nodeName)) {
                switch ($node->nodeName) {
                    case 'xsd:extension':
                        ...
                        if(XmlDataType::isXmlDataType($baseType)){
                            $object->type = XmlDataType::getHtmlEquivalent($baseType);
                        }else{
                            foreach ($arrTags as $tag) {
                        ...
                        foreach ($node->childNodes as $attr) {
                            if ($attr->nodeName == 'xsd:attribute') {
                                ...
                                if ($attrName != null) {
                                    ...
                                    //second attempt for mising attribute values
                                    if(!$attrFound){
                                        foreach ($arrTags as $tag) {
                                            $querySimple = $tag;
                                            foreach ($arrDom as $dom) {
    ...
    public static function getPathToElement(&$productInstance, $searchedElement, $refElement = null, $recursiveSearch = true, $parentPath = null, $casesensitive = true)
    {
        ...
        $cache_key = self::toKey(sprintf('%s/%s/%s', md5(AmazonDataAdjustment::serialize($productInstance)), $refElement, $searchedElement));
    ...
    /**
     * Returns the compliance restrictions, based on a language for the marketplace region associated profile->lang->marketplaceRegion
     * @param string $name
     * @return array of custom attributes for
     */
    public static function getCustomSizeConfiguration($universe, $name, $region) {
        $empty = array();
        $dataJsonArray = array();

        if (
                self::$sizeComplianceData 
                && is_array(self::$sizeComplianceData) 
                && count(self::$sizeComplianceData)
        ) {
            $dataJsonArray = self::$sizeComplianceData;
        }else{
            $jsonfile = dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . self::getCustomSizeFile();

            if (!file_exists($jsonfile)) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s:#%d JSON file doesn\'t exist: %s', basename(__FILE__), __LINE__, $jsonfile));
                }

                return ($empty);
            }

            //loads data from json file, based on markets specific attributes        
            $data = Tools::file_get_contents($jsonfile);

            $dataJsonArray = self::$sizeComplianceData = json_decode($data, true);
        }

        $fields = array();
        if(isset($dataJsonArray['AdditionalSizeData'])
            && 
           isset($dataJsonArray["AdditionalSizeData"][$universe])
        ){
            $root = $dataJsonArray["AdditionalSizeData"][$universe];
            if(isset($root['AllowedRegions'])
            &&
            is_array($root['AllowedRegions'])
            &&
            (in_array($region, $root['AllowedRegions']) || in_array('*', $root['AllowedRegions']))
            ){
                $fields = $root['Fields'];
            }else{
                $fields = array();
    ...
    /**
     * Filename used to load size customization by region
     * @return string
     */
    protected static function getCustomSizeFile()
    {
        return  self::getCustomSizeDir() . self::getCustomSizeFileName();
    }

    public static function getCustomSizeDir(){
        return "settings" . DIRECTORY_SEPARATOR . "json" . DIRECTORY_SEPARATOR;
    }


    public static function getCustomSizeFileName(){
        return "CustomSizeAttributes.json";
    }


    public static function isCustomField($universe, $name, $region){        
        $region = Tools::strtoupper($region);
        $config = self::getCustomSizeConfiguration($universe, $name, $region);
        $isCustomField = is_array($config) && count($config);
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s:#%d isCustomField (%s-%s-%s): %s', basename(__FILE__), __LINE__, $universe, $name, $region, json_encode($isCustomField)));
        }
    ...
    /**
     * Delete XSD file by Universe
     * @return string
     */
    public function clearXSD(){
        $dir = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'xsd';
        $filename = $dir . DIRECTORY_SEPARATOR . basename($this->productName) . '.gz';
        ...
        if (file_exists($filename)) {
            if (is_writable($filename)) {
                if (unlink($filename)) {
                    ...
                    return "An error occurred while attempting to delete the file!";
                }
            } else {
