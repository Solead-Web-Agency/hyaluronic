...
if (!isset($start_time)) {  //TODO: Yes, it is defined, we need it to calculate allowed execution_time
...
class AmazonRepricingAutomaton extends Amazon
{
    ...
    public function pushPendingProducts()
    {
        ...
        if (is_array($my_prices) && count($my_prices)) {
            ...
            AmazonProduct::marketplaceActionAcknowledge(Amazon::REPRICE, $this->amazon_id_lang, $skus_to_acknowledge, date('Y-m-d H:i:s'));
        }
    }
    ...
    public function repricing(&$notification)
    {
        ...
        if ($reprice) {
            ...
            if ($product_idenfier instanceof stdClass) {
                ...
                $category_set = AmazonProduct::marketplaceGetCategories($id_product);
                ...
                switch ($strategy['base']) {
                    case AmazonRepricing::REPRICING_WHOLESALE_PRICE:
                        if (!($tax = Tax::getProductTaxRate($id_product, $this->id_address))) {
                            ...
                            break;  // 7.3 will emit a warning: continue targeting switch
                        }
                ...
                if ($this->verbose) {
                    ...
                    self::p(array(
                        "Action:".$action."\n",
                        "Final Price:".AmazonTools::displayPrice($smart_price, $this->toCurrency, false, $this->context)."\n",
                        "Repriced:".AmazonTools::displayPrice($reprice, $this->toCurrency, false, $this->context)."\n",
                        "Calculated:".AmazonTools::displayPrice($calculated, $this->toCurrency, false, $this->context)."\n",
                        "Base Price:".AmazonTools::displayPrice($current_price, $this->toCurrency, false, $this->context)."\n",
                        "Range(min):".AmazonTools::displayPrice($price_min, $this->toCurrency, false, $this->context).$text_delta_min."\n",
                        "Range(max):".AmazonTools::displayPrice($price_max, $this->toCurrency, false, $this->context).$text_delta_max."\n",
                        "Base:".AmazonTools::displayPrice($base_price, $this->toCurrency, false, $this->context)."\n",
                        "Limit:".AmazonTools::displayPrice($base_price_limit, $this->toCurrency, false, $this->context).($limit > 0 ? sprintf(' (+%02d&#37;)', $limit) : sprintf(' (%02d&#37;)', $limit))."\n",
                        $currency_verbose,
