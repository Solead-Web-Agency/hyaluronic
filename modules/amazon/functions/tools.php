...
class AmazonToolFunction extends Amazon
{
    ...
    public function dispatch()
    {
        ...
        switch (Tools::getValue('action')) {
            ...
            case 'install-valid-values':
                ...
                break;
            case 'install_amazon_states':
                $this->installAmazonStates();
                break;
            ...
            case 'switch-address-invoice-name':
                ...
                break;
            case 'update-customs':
                $this->customDefinitions('update-customs');
                break;
            case 'delete-logs':
                echo json_encode($this->spDeleteLogs());
                break;
            default:
                break;
        }
    ...
    {
        $errors = array();
        $migration = $this->migrationManager->installAmazonStates();
        switch ($migration['code']) {
            case -1:
                $errors[] = $this->l('Unable to download Amazon states');
                break;
            case -2:
                $errors[] = $this->l('Unable to import Amazon states');
                break;
            default:
                break;
        }
    ...
    /**
     * Update JSON file: validate/json/CustomSizeAttributes.json
     */
    public function customDefinitions($type)
    {
        require_once(dirname(__FILE__).'/../classes/amazon.settings.class.php');
        ...
        $pass = AmazonSettings::getCustomDefinitions($type, true);

        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf("AmazonSettings::getCustomDefinitions: %s - %s", $type, $pass ? 'Ok' : 'Failed'));
        }

        if ($pass) {
            $success[] = $this->l('Amazon custom definitions successfully updated');
        } else {
            $error[] = $this->l('Unable to update Amazon custom definitions');
        }
     ...
     * @return void
     */
    private function spDeleteLogs()
    {
        require_once(dirname(__FILE__).'/../classes/amazon.logger.class.php');
        $logger = new AmazonLogger('');
        $logger->clearOldLogs(true, 0);

        $callback = Tools::getValue('callback');
        $json = json_encode(array(
            'success' => array($this->l('Delete log files status : Ok')),
            'pass' => true,
            'continue' => false
        ));
