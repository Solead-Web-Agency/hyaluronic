...
/**
 * @deprecated Only used in PS1.4- via ProductsAmazon
 */
...
class AmazonAutomaton extends Amazon
{
    ...
    public function __construct()
    {
        ...
        $amazon = AmazonTools::selectPlatform($this->amazon_id_lang, 0);
    ...
    */
    ...
    public function matchProducts()
    {
        ...
        if (($type = Tools::getValue('type')) == 'ean13') {
            ...
            self::$errors[] = sprintf('%s - %s(#%d): %s - %s', date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $this->l('Failed'));
            self::$products = null;
        ...
        if ($productCount) {
            ...
            if (!$xml instanceof SimpleXMLElement or isset($xml->Error) or !isset($xml->GetMatchingProductForIdResult)) {
                ...
                self::$errors[] = sprintf('%s - %s(#%d): %s - %s (%s)', date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $this->l('Failed'), $error_msg);
                self::$products = null;
