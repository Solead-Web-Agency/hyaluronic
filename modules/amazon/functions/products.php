 ...
 */
class AmazonExportProducts extends AmazonFunctionWithFeeds
{
    const NOTICE = 1;
    const WARNING = 1;
    const ERROR = 2;
    ...
    private $custom_features_values = array();

    protected $logChannel = AmazonLogger::CHANNEL_PRODUCT_EXPORT;
    ...
    public function doIt()
    {
        ...
        if (Tools::getValue('cron')) {
            ...
            $id_lang = (int)AmazonTools::getValue('amazon_lang');
            if (!$id_lang) {
        ...
        $params->extendedDatas = (bool)((int)AmazonTools::getValue('extended-datas'));
        $params->extendedDatasVariant = (int)AmazonTools::getValue('extended-datas-variant'); //TODO change with select extendedDatas
        $params->noPriceUpdate = (bool)((int)AmazonTools::getValue('no-price'));
        $params->relationShipsOnly = (bool)((int)AmazonTools::getValue('relations-only'));
        $params->xmlOnly = (bool)((int)AmazonTools::getValue('xml-only'));
        $params->includeMsrp = (bool)((int)AmazonTools::getValue('include-msrp'));
        $params->action = AmazonTools::getValue('action');
        ...
        $params->php_limits = $params->max_execution_time || $params->memory_limit;
        ...
        parent::loadFeatures(false, !$params->mappingFeatureCustom);
        ...
        // Init
        //
        $params->toCurrency = $spConnector->getCurrency();
        ...
        $params->marketplaceID = $spConnector->getMarketplaceId();
        $params->merchantID = $spConnector->getSellerId();
        $params->fullfillmentCenterId = AmazonTools::marketplaceIdToFullfillmentCenterId($spConnector->getMarketplaceId());

        $this->setApiOperationMode(AmazonWebService::OPERATIONS_UPDATE);
        ...
        $status_filter = AmazonConfiguration::get(AmazonConstant::FILTER_STATUS);
        if (!is_array($status_filter) || !isset($status_filter['prestashop_inactive']) || !$status_filter['prestashop_inactive']) {
            $params->sendActives = false; // Status filter Prestashop inactive!
        }
        ...
        switch ($params->action) {
            ...
            case 'delete-verify':
                ...
                $params->deleteConfirmed = (bool)AmazonTools::getValue('delete-confirm');
                $params->sendToAmazon = false;
                if (AmazonTools::getValue('delete-overrides')) {
                    $params->deleteShippingOverrides = true;
                }
                break;
            case 'delete-export':
                $params->deleteProducts = true;
                if (AmazonTools::getValue('delete-confirm')) {
        ...
        if ($params->create) {
            ...
            $this->setApiOperationMode(AmazonWebService::OPERATIONS_CREATE);
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p('Product query: ' . AmazonProduct::getLastQuery());
        ...
        if ($p) {
            foreach ($p as $key => $val) {
                ...
                if (AmazonConfiguration::shopIsFeatureActive() && $params->id_shop) {
                    $details = new Product($id_product, false, $params->extendDataIDLang, $params->id_shop);
                } else {
                    $details = new Product($id_product, false, $params->extendDataIDLang);
                }
                ...
                //Prevent future validations on disabled/inactive products
                if ($action != Amazon::REMOVE
                    && $this->checkSkipInactiveOrDisabledProduct($details, $product_options, $val)
                ) {
                    $skipped++;
                    continue;
                }
                ...
                $category_set = $this->getProductCategorySet($id_product, $details->id_category_default, $id_category_default_has_profile);
                ...
                if (!$skip && $profile_name && is_numeric($profile_id)) {
                    ...
                    /* START: Profile - Supplier Price Rule */
                    $supplierPriceRules = new AmazonSupplierPriceRule($profile_id, $id_lang);
                    $has_supplier_price_rule = $supplierPriceRules->hasSupplierPriceRule($details->id_supplier);
                    $p_supplier_price_rules = $has_supplier_price_rule ? $supplierPriceRules->getPriceRule() : null;
                    /* END: Profile - Supplier Price Rule */
                ...
                if ($params->extendedDatas && !Tools::strlen($manufacturer)) {
                    $message = AmazonSupport::message(sprintf($this->l('Skipping : %d - Field Manufacturer is mandatory'), $details->id), AmazonSupport::FUNCTION_EXPORT_NO_BRAND);
                    $this->errorOutput($message, self::ERROR);
                    $skipped++;
                    continue;
                }
                ...
                foreach ($combinations as $combination) {
                    $this->elc('Combination', $combination);
                    ...
                    if (isset($combination['attribute_name']) && !empty($combination['attribute_name'])) {
                        ...
                        if($params->attributeNameInTitle){
                            $group_details[$id_product_attribute]['attribute_name'] .= sprintf('%s %s, ', $combination['group_name'], $combination['attribute_name']);
                        }else{
                ...
                foreach ($group_details as $id_product_attribute => $combination) {
                    ...
                    if ($details->name) {
                        ...
                        $this->elc(
                            sprintf('Name: %s. ', $details->name),
                            sprintf('Product ID: %d/%d. ', $id_product, $id_product_attribute),
                            sprintf('Reference: %s. ', $combination['reference']),
                            sprintf('Using: %s/%s. ', $az_code, $ps_code),
                            sprintf('Code: %s', isset($combination[$ps_code]) && !empty($combination[$ps_code]) ? $combination[$ps_code] : '#?#')
                        );
                    }

                    //Flag to indicate if product is excluded
                    $disabled = (bool)$options['disable'];
                    
                    
                    //Prevent future validations on disabled products
                    if ($disabled && $action != Amazon::REMOVE) {                        
                        printf($this->l('Skipped,disabled product'). '. ID: %s/%s Reference: %s' .$cr, 
                              $id_product, $id_product_attribute, $combination['reference']);
                        $productIndex--;
                        $skipped++;
                        continue;
                    }
                        ...
                        if (is_array($product_features) && count($product_features)) {
                            ...
                            foreach ($product_features as $feature) {
                                ...
                                if (isset($feature['custom']) && (bool)$feature['custom'] && !isset($features_values[$id_feature][$id_feature_value])) {
                                    $custom_features_values = $this->getCustomFeaturesValues($params->id_lang, $id_feature);
                    ...
                    if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                        $quantity = Product::getRealQuantity($details->id, $id_product_attribute ?: null, $params->id_warehouse);
                    } else {
                        $quantity = Product::getQuantity($details->id, $id_product_attribute ?: null);
                    }
                    ...
                    if (Validate::isLoadedObject($details)) {
                        ...
                        // Use product country tax
                        $useProductCountryTax = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_PRODUCT_COUNTRY_VAT);
                        if ($useProductCountryTax) {
                            $stdPrice = $details->getPrice(false, ($id_product_attribute ? $id_product_attribute : null), 6, null, false, !$details->on_sale && $params->specials);
                            $stdPrice = AmazonTools::getProductStdPriFtCountryTax($stdPrice, $params->id_lang);
                        }
                        ...
                        if ($params->priceRulesEnabled) {   
                            //expert config to apply price_rules after conversion to target currency is done                         
                            if(!$params->apply_price_rules_after_conversion){
                                $newPrice = $this->getPriceWithRule($p_price_rules, $params->priceRules, $stdPrice, $p_supplier_price_rules);
                            } else {
                                $newPrice = $stdPrice;
                            }
                        } else {
                        ...
                        if ($params->specials) {
                            ...
                            if ($details->on_sale && $specificPrice && isset($specificPrice['reduction_type']) && isset($specificPrice['from']) && isset($specificPrice['to']) && (int)$specificPrice['from'] && (int)$specificPrice['to']) {
                                ...
                                printf('%s : %s - %s : %s %s %s %s'.$cr, $this->l('Apply Sale Price'), $dateStart, $dateEnd, $this->l('Old Price'), AmazonTools::displayPrice($stdPrice, $params->toCurrency), $this->l('Sale Price'), $salePrice);
                                ...
                                if ($params->priceRulesEnabled) {                                    
                                    ...
                                    if($params->apply_price_rules_after_conversion){
                                        $salePrice = $this->getPriceWithRule($p_price_rules, $params->priceRules, $salePrice);     
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('Price after rules applied: %s', $salePrice));
                                        }                                
                                    }
                                }
                        ...
                        if (Amazon::$debug_mode) {
                            ...
                            CommonTools::p(sprintf('Final Price - price : %s was : %s', AmazonTools::displayPrice($newPrice, $params->toCurrency), AmazonTools::displayPrice($stdPrice, $params->toCurrency)));
                        }
                    } else {
                        $stdPrice = $newPrice = null;
                    }

                    $statusFilter = AmazonConfiguration::get(AmazonConstant::FILTER_STATUS);
                    $isZeroPrice = (is_array($statusFilter) && isset($statusFilter['zero_price']) && $statusFilter['zero_price']);
                    
                    if (!$isZeroPrice && (empty($stdPrice) || !is_numeric($stdPrice)) ) {
                        ...
                        $retail_price = $details->getPrice(false, ($id_product_attribute ? $id_product_attribute : null), 6, null, false, !$details->on_sale && $params->specials);
                        $combination['retail_price'] = $retail_price ;
                        
                        $retail_price_tax = $details->getPrice(true, ($id_product_attribute ? $id_product_attribute : null), 6, null, false, !$details->on_sale && $params->specials);
                        $combination['retail_price_with_tax'] = $retail_price_tax ;
                    ...
                    $nopexport = $options['nopexport'] || $params->noPriceUpdate || $params->stockOnly;
                    $noqexport = $options['noqexport'] ?: $params->pricesOnly;
                    $manufacturer = isset($options['manufacturer']) && !empty($options['manufacturer']) ? $options['manufacturer'] : $manufacturer;
                    $brand = isset($options['brand']) && !empty($options['brand']) ? $options['brand'] : $manufacturer;
                    ...
                    $this->elc(
                        'Stock Issues:',
                        "Stock Management: $params->stock_management",
                        "Out Of Stock param: $params->outOfStock",
                        "Quantity: $quantity",
                        "Force: " . ($force ? 'On' : 'Off'),
                        "Disabled: ".($disabled ? 'On' : 'Off'),
                        "Active: ".($details->active ? 'Yes' : 'No'),
                        "To Remove: ".($action == Amazon::REMOVE ? 'Yes' : 'No'),
                        "Preorder: ".($params->preorder ? 'Yes' : 'No'),
                        "Remote Cart Quantity: ".$remote_cart_quantity,
                        "Date Restock: ".(isset($combination['available_date']) ? $combination['available_date'] : null)
                    );
                    ...
                    if (isset($combination['available_date']) && Validate::isDate($combination['available_date'])) {
                        ...
                        if (!$params->preorder && $psDateRestock && $psDateRestock > $dateNow) {
                            //
                            $quantity = 0;
                        } elseif ($params->preorder && $psDateRestock && $psDateRestock + 86400 > $dateNow) {
                    ...
                    if ($generate_product_feed) {
                        ...
                        if ($params->extendedDatas) {
                            ...
                            switch ($p_universe) {
                                ...
                                case 'LargeAppliances':
                                    ...
                                    //unset to prevent coincidence as ProductDescription element
                                    unset($product_data['ProductName']);
                                    unset($product_data['Manufacturer']);
                                    $product_data['ProductType'] = $p_product_type;
                                    $product_data['Definition'] = $p_universe;
                                    break;

                                case 'Toys':
                                    $product_data['Parameters']['ColorMap'] = array($p_universe, 'ColorSpecification', 'ColorMap');
                                    $product_data['Parameters']['Color'] = array($p_universe, 'ColorSpecification', 'Color');
                                    $removeTagForParent[] = 'Color';

                                default:
                                    // to solve the problem when do update offers and selected the option Extended Data, becuase the xml missing ProductType tag
                                    //if ($params->create) {
                                        $product_data['ProductType'] = $p_product_type;
                                        $product_data['Definition'] = $p_universe;
                                    //}
                        ...
                        if ($params->create || $params->extendedDatas) {
                            $variant = ($params->create || $params->update) && isset($p_parameters['Parentage']);
                            if (Amazon::$debug_mode) {
                            ...
                            //
                            if ($p_extra) {
                                $product_data['Parameters']['ReallyRequiredFields'] = isset($p_extra['required']) ? $p_extra['required'] : array();
                                ...
                                if (in_array($p_universe, array('Jewelry')) && isset($p_extra['field']['MetalType']) && !empty($p_extra['field']['MetalType']) && !isset($product_data['Parentage'])) {
                                    $product_data['Parentage'] = 'child';
                                }else if(in_array($p_universe, array('Health')) 
                                    && !isset($product_data['Parentage'])
                                    && isset($product_data['VariationTheme'])
                                ){
                                    //Basecamp ticket https://3.basecamp.com/3914949/buckets/6077817/todos/2933188113
                                    //Parentage fields is not included as part of XML request. But on XSD it's defined as <xsd:element name="Parentage">, which means it's required
                                    //2022-11-14 added condition to include parentage only when VariationTheme is set
                                    $product_data['Parentage'] = 'child';
                                }
                                
                                if (isset($p_extra['multiple'])) {
                                    $pass = true;
                                    ...
                                    foreach (array_keys($p_extra['multiple']) as $field) {
                                        ...
                                        $perProductType = isset(AmazonXSD::$rewriteFieldsUniverseProductType[$p_universe]);
                                        $rewriteProductType = $perProductType && in_array($p_product_type, AmazonXSD::$rewriteFieldsUniverseProductType[$p_universe]);

                                        // Rewrite particular cases
                                        //Rewrite in Baby applies only to some of its product types
                                        if($perProductType
                                            && $rewriteProductType
                                            && isset(AmazonXSD::$rewriteFieldsUniverse[$p_universe]) 
                                            && isset(AmazonXSD::$rewriteFieldsUniverse[$p_universe][$field])){
                                        ...
                                        if( $p_universe == "CE" ) {
                                            $allowed_field_maps = array("ColorMap");
                                        } else {
                                            $allowed_field_maps = array("ColorMap", "SizeMap");
                                        }
                                        // TEST OLIVIER 2020/09
                                        $allowed_field_maps = array("ColorMap", "SizeMap");
                                        ...
                                        if ($mapping['is_variant_field'] && in_array($field, array('Color', 'ColorName', 'Size', 'SizeName'))) {
                                            ...
                                            if ($mapping['has_mapping'] && Tools::strlen($mapping['value']) && in_array($field_map, $allowed_field_maps) ) {
                                            ...
                                            if ( $mapping['has_mapping'] && Tools::strlen($mapping['value']) && in_array($field_map, $allowed_field_maps) ) {
                                                ...
                                                //This line was causing ColorMap to be included as part of Product, when it should be part of ClassificationData (on Shoes Universe)
                                        ...
                                        //Product has attribute, but it was not set because field has no value
                                        if ($has_amazon_attribute && !isset($product_data['Attributes'][$field]) ) {
                                            $amazon_attribute_key = key($p_extra['attributes'][$field]);
                                            $amazon_attribute_value = reset($p_extra['attributes'][$field]);
                                        ...
                                        if ($has_choice_required) {
                                            if (isset($p_extra['choices'][$field]) && isset($field_value) && Tools::strlen($field_value)) {
                                                ...
                                                if (isset($p_extra['choice_allowed_values'][$field])) {
                                                    $choice_allowed_values =  AmazonDataAdjustment::unSerialize(AmazonTools::decode($p_extra['choice_allowed_values'][$field]));   // TODO: Validation: Need to get the Amazon data
                            ...
                            switch ($p_code_exemption) {
                                case Amazon::EXEMPTION_MODEL_NUMBER:
                                    // Sports > ProductType is simpleType, it cannot have children.
                                    // https://3.basecamp.com/3914949/buckets/6077817/todos/1551222247
                                    // todo: Fix for all others simpleType (ToysBaby, Luggage, Miscellaneous)
                                    if ('Sports' != $p_universe) {
                                        $product_data['Parameters']['ModelNumber'] = CodeExemption::modelNumber($p_universe, $productTag);
                                        $product_data['ModelNumber'] = $combination['reference'];
                                    }
                                    break;
                                case Amazon::EXEMPTION_MODEL_NAME:
                                    if ('Sports' != $p_universe) {
                                        $product_data['Parameters']['ModelName'] = CodeExemption::modelName($p_universe, $productTag);
                                        $product_data['ModelName'] = $combination['reference'];
                                    }
                                    break;
                                case Amazon::EXEMPTION_MFR_PART_NUMBER:
                                    $product_data['MfrPartNumber'] = CodeExemption::mfrPartNumber($p_sku_as_sup_ref_unconditionnaly, $supplier_reference, $combination['reference']);
                                    break;
                                ...
                                case Amazon::EXEMPTION_STYLE_NUMBER:
                                    if ('Sports' != $p_universe) {
                                        $product_data['Parameters']['StyleNumber'] = CodeExemption::styleNumber($p_universe, $productTag);
                            ...
                            // Product Tax Code
                            //
                            if(isset($options['product_tax_override']) && Tools::strlen($options['product_tax_override'])){
                                $product_data['ProductTaxCode'] = $options['product_tax_override'];
                                $removeTagForParent[] = 'ProductTaxCode';
                            }else if (Tools::strlen($p_ptc)) {
                            ...
                            } elseif (!isset($product_data['UnitCount']) && !isset($product_data['ItemPackageQuantity']) && in_array($p_universe, array('Beauty'))) {
                            ...
                            }elseif (!isset($product_data['UnitCount']) && !isset($product_data['ItemPackageQuantity']) && in_array($p_universe, array('Health'))) {
                                // 2020-07-27 - Separate from Beauty condition as "count" is included inside "VariationData" for Health
                            ...
                            //if (isset($product_data['UnitCount']) && !isset($product_data['Attributes']['UnitCount']['unitOfMeasure'])) {
                                // https://sellercentral.amazon.com/forums/t/update-to-the-listing-requirements-for-consumable-product-types-in-the-us/457839
                            //    $product_data['Attributes']['UnitCount']['unitOfMeasure'] = 'Count';
                            //}
                            ...
                            if ($params->create && in_array($p_universe, array('Luggage'))) {
                                ...
                                if (isset($product_data['ItemDimensions']) && isset($product_data['Weight'])) {
                    ...
                    if ($params->extendedDatas) {
                        ...
                        if (AmazonTools::isEuropeMarketplaceId($params->marketplaceID) && in_array($p_universe, array('ProductClothing', 'Shoes'))) {
                            ...
                            foreach (array(
                                ...
                                if (Tools::strlen(trim($bullet_point)) > 4) {
                                    $index++;
                    ...
                    if ($params->priceRulesEnabled 
                        && $params->apply_price_rules_after_conversion) {   
                        //expert config to apply price_rules after conversion to target currency is done                         
                        $newPrice = $this->getPriceWithRule($p_price_rules, $params->priceRules, $newPrice, $p_supplier_price_rules);
                    } 
                    ...
                    if (!$params->create && $params->authorizeToDelete && $params->deleteProducts && ($quantity == 0 && ($combination[$ps_code] || $params->asinOverride)) || $delete) {
                        ...
                        if (empty($productsDelete[$d]['SKU'])) {
                            $this->elc(sprintf('Skipping : %d - Has No SKU !', $details->id_product));
                            $skipped++;
                            continue;
                        }
                        ...
                        if (AmazonTools::strlen($details->reference) && count($overall_quantities) > 1 && !array_filter($overall_quantities) && isset($group_details[count($group_details)-1]['reference'])) {
                        ...
                        if (!$params->conditionIgnore) {
                            $productCondition = $this->getProductCondition($params, $details);
                        ...
                        // Check no update price here
                        $productsUpdate[$u]['NoPriceExport'] = (bool)$nopexport;
                        $productsUpdate[$u]['NoQtyExport'] = (bool)$noqexport;
                        $productsUpdate[$u]['Sales'] = $sales;
                        $productsUpdate[$u]['Business'] = $this->resolveBusinessPrice(
                            // Use price after rule & converter
                            $newPrice, $id_product, $id_product_attribute,
                            $params->id_shop, $params->fromCurrency->id, $params->id_country_default,
                            $params->id_business_group, $params->specials,
                            $params->priceRulesEnabled, $p_price_rules, $params->priceRules
                        );
                        ...
                        //
                        if($params->includeMsrp && $generate_product_feed){ 
                                
                            $listPrice = $stdPrice;

                            if ($params->fromCurrency->iso_code != $params->toCurrency->iso_code) {
                                $listPrice = Tools::convertPrice($listPrice, $params->toCurrency);
                            }

                            if ($params->rounding && in_array($params->rounding, array(Amazon::ROUNDING_ONE_DIGIT, Amazon::ROUNDING_TWO_DIGITS))) {
                                $listPrice = Tools::ps_round($listPrice, $params->rounding);
                            } elseif ($params->rounding && $params->rounding == Amazon::ROUNDING_SMART) {
                                $listPrice = AmazonTools::smartRounding($listPrice);
                            } else {
                                $listPrice = sprintf('%.02f', $listPrice);
                            }

                            $productsUpdate[$u]['ProductDescription']['MSRP'] = $listPrice;
                            $productsUpdate[$u]['ProductDescription']['MSRPCurrency'] = $params->toCurrency->iso_code;
                            $product_data['UVPListPrice'] = $listPrice;
                            if (!isset($product_data['Attributes']['UVPListPrice'])) {
                                $product_data['Attributes']['UVPListPrice'] = array();
                            }
                            $product_data['Attributes']['UVPListPrice']['currency'] = $params->toCurrency->iso_code;
                        }
                        ...
                        if ($generate_product_feed && $params->extendedDatas) {
                            ...
                            //TargetAudience exception
                            if(isset($product_data['TargetAudience'])){
                                $productsUpdate[$u]['ProductDescription']['TargetAudience'] = $product_data['TargetAudience'];
                                unset($product_data['TargetAudience']);
                            }
                            ...
                            if (method_exists('Product', 'externalName')) {
                                if (Product::externalName($params->id_lang, $id_product, $id_product_attribute, trim($name))) {
                            ...
                            $alternative_content = $this->getAlternativeContent($params, $options);
                            if (Tools::strlen($alternative_content['title'])) {
                                $productsUpdate[$u]['ProductDescription']['Title'] = $alternative_content['title'];
                            }
                            if (Tools::strlen($alternative_content['description'])) {
                                $productsUpdate[$u]['ProductDescription']['Description'] = $alternative_content['description'];
                            }
                        }
                        // Send Images
                        //
                        if ($params->sendImages && isset($product_data['ProductImage']) && is_array($product_data['ProductImage']) && count($product_data['ProductImage'])) {
                        ...
                        if ($generate_product_feed && $params->extendedDatas) {
                            ...
                            if ($brand) {
                                $productsUpdate[$u]['ProductDescription']['Brand'] = AmazonTools::encodeText(trim($brand), $params->safeEncoding);
                            }
                            ...
                            if ($product_data) {
                                ...
                                if(isset($combination['minimal_quantity']) && $combination['minimal_quantity'] > 0){
                                    $productsUpdate[$u]['ProductData']['MinimumOrderQuantity'] = $combination['minimal_quantity'];
                                }
                            }
                                ...
                                switch (Tools::strtoupper($params->weightUnit)) {
                                    ...
                                    $unit = $params->weightUnit;
                                        if ((float)$product_weight - (float)$params->shippingTare) {
                                            $productsUpdate[$u]['ProductDescription']['PackageWeight'] = (int)($product_weight - (float)$params->shippingTare);
                                        ...
                                        $productsUpdate[$u]['ProductDescription']['ShippingWeight'] = (int)$product_weight;
                            ...
                            foreach (array('Length', 'Width', 'Height', 'Weight') as $dimension) {
                                foreach (array('ItemDisplay', '') as $master_dimension) {
                                    ...
                                    if (isset($product_data[$target_dimension]) && isset($product_data['Parameters'][$target_dimension]) && isset($product_data['Attributes'][$target_dimension]) && is_array($product_data['Attributes'][$target_dimension])) {
                                        ...
                                        if ($dimension == 'Weight') {
                                            if (preg_match('/^G$/', Tools::strtoupper($dimension_unit)) || preg_match('/^GR$/', Tools::strtoupper($dimension_unit))) {
                                                $dimension_is_int = true;
                                            }
                                        }
                                        if ($dimension_unit && $dimension_value) {
                                            ...
                                            if ($dimension_is_int) {
                                                ...
                                                $productsUpdate[$u]['ProductDescription']['ItemDimensions'][$dimension]['value'] = (int)$dimension_value;
                                            }

                                            $product_variable_name = "product_has_$dimension_lowered";
                                            $$product_variable_name = true;
                                        }
                                    }
                                }
                            }
                            ...
                            if ($params->dimensionUnit && ((float)$package_length && (float)$package_height && (float)$package_width)) {
                                ...
                                if (isset($productsUpdate[$u]['ProductDescription']['ItemDimensions']) && !count($productsUpdate[$u]['ProductDescription']['ItemDimensions'])) {
                        ...
                        }else if ($generate_product_feed 
                        && isset($product_data['UVPListPrice'])
                        && isset($product_data['Attributes'])
                        && isset($product_data['Attributes']['UVPListPrice'])
                        ){
                            //UVPListPrice special case
                            if ($product_data) {
                                if (!isset($productsUpdate[$u]['ProductData'])) {
                                    $productsUpdate[$u]['ProductData'] = array();
                                }
                                if (!isset($productsUpdate[$u]['Attributes'])) {
                                    $productsUpdate[$u]['Attributes'] = array();
                                }
                                $productsUpdate[$u]['ProductData']['UVPListPrice'] = $product_data['UVPListPrice'];
                                $productsUpdate[$u]['ProductData']['Attributes']['UVPListPrice'] = $product_data['Attributes']['UVPListPrice'];
                            }
                        }
                        ...
                        if ($params->shippingAllowOverride) {
                            ...
                            if ($params->deleteShippingOverrides) {
                                ...
                                $shipping_text = '- '.$this->l('Delete Shipping Charges Overrides');
                                ...
                                if (empty($shipping_override_option)) {
                                    $this->errorOutput($this->l('Warning : Incontistency - You configured a shipping override but there are no associated mappings in the module configuration for this region'), self::WARNING);
                                    $shipping_text = null;
                                } else {
                            ...
                            $this->elc(
                                'Shipping Overrides: Active. ',
                                'ShippingPrice: ' . (isset($productsUpdate[$u]['ShippingPrice']) ? $productsUpdate[$u]['ShippingPrice'] : ''),
                                'ShippingOption: ' . (isset($productsUpdate[$u]['ShippingOption']) ? $productsUpdate[$u]['ShippingOption'] : ''),
                                'ShippingType: ' . (isset($productsUpdate[$u]['ShippingType']) ? $productsUpdate[$u]['ShippingType'] : '')
                            );
                            ...
                            if ($params->deleteShippingOverrides) {
                                ...
                                $productsUpdate[$u]['ShippingType'] = $params->smartShippingMethod ?: 'Additive';

                                $shipping_text = '- '.$this->l('Delete Shipping Override');
                            } else {
                                $shipping_cost = AmazonCarrier::shippingQuoteByWeight($product_weight, $params->id_carrier, $params->id_address, $params->useTax);
                                $this->elc(
                                    'Smart Shipping:',
                                    sprintf('weight: %s', $product_weight),
                                    sprintf('id_carrier: %s', $params->id_carrier),
                                    sprintf('id_address: %s', $params->id_address),
                                    sprintf('useTax: %s', $params->useTax)
                                );
                        ...
                        $sent_quantity = isset($productsUpdate[$u]['Quantity']) ? ($noqexport ? 'n/a' : $productsUpdate[$u]['Quantity']) : 'n/a';
                        $sent_price = $nopexport ? 'n/a' : AmazonTools::displayPrice($newPrice, $params->toCurrency);

                        $this->ed(
                            $actionType, $actionIdType, $actionIdCode,
                            $productsUpdate[$u]['SKU'],
                            $sent_quantity,
                            $params->fromCurrency->iso_code,
                            AmazonTools::displayPrice($stdPrice, $params->fromCurrency),
                            $sent_price,
                            $fba_text, $shipping_text
                        );
                        ...
                        if (($params->create || $params->update)
                            ...
                            // get parent product's alternative title and description if they exist
                            $alternative_content = $this->getAlternativeContent($params, $product_options);

                            if (Tools::strlen($alternative_content['description'])) {
                                $productsUpdate[$u]['ProductDescription']['Description'] = $alternative_content['description'];
                            } else if (method_exists('Product', 'externalDescription')) {
                                $description = Tools::substr(Product::externalDescription($params->id_lang, $id_product, $params->descriptionField), 0, Amazon::LENGTH_DESCRIPTION);
                                $productsUpdate[$u]['ProductDescription']['Description'] = mb_substr(AmazonTools::encodeText($description, $params->safeEncoding), 0, Amazon::LENGTH_DESCRIPTION);
                            } else {
                                $productsUpdate[$u]['ProductDescription']['Description'] = mb_substr(AmazonTools::encodeText($description, $params->safeEncoding), 0, Amazon::LENGTH_DESCRIPTION);
                            ...
                            if (Tools::strlen($alternative_content['title'])) {
                                $productsUpdate[$u]['ProductDescription']['Title'] = $alternative_content['title'];
                            } else if (method_exists('Product', 'externalName')
                                && Product::externalName(Product::externalName($params->id_lang, $id_product, $id_product_attribute, $master_name))
                            ) {
                            ...
                            foreach($productsUpdate[$u]['ProductData'] as $tagName=>$val){
                                if(AmazonXSD::isCustomField($p_universe, $tagName, $params->region)){
                                    unset($productsUpdate[$u]['ProductData'][$tagName]);
                                }
                            }
        ...
        if (count($productsUpdate) && $params->extendedDatas) {
            $results = array();
            foreach ($productsUpdate as $productData ) {
                $results[] = $this->prepareExtendedDatas($productData, $params->extendedDatasVariant);
            }
            $productsUpdate = $results;
        }
        ...
        if (!count($productsUpdate)) {
            ...
            $this->elc(
                'Memory used: ' . memory_get_usage(true) / (1024 * 1024) . ". ",
                'Memory peak: ' . memory_get_peak_usage(true) / (1024 * 1024) . ". ",
                'Product Data: ',
                array_map(function($debuggingProduct) {
                    if (isset($debuggingProduct['ProductData'])) {
                        $debuggingProduct['ProductData'] = "Large payload: " . count($debuggingProduct['ProductData']);
                    }
                    return $debuggingProduct;
                }, $productsUpdate)
            );
            if ($params->sendToAmazon || $params->xmlOnly) {
                $mwsSuccess = true;
                $this->elc('Relationship', $Relationship);
                if ($params->relationShipsOnly && count($Relationship)) {
                    $this->ed(sprintf('Updating relationships only - %s items', count($Relationship)));
                    $this->submitFeedRelationship($Relationship, $params);
                } else {
                    if ($params->delete) {
                    ...
                    // Operation Override
                    if ($params->delete) {
                        $this->setApiOperationMode(AmazonWebService::OPERATIONS_DELETE);
                    }

                    if ($has_product_feed) {
                        if ($this->submitFeedProductData($productsUpdate, $params)) {
                    ...
                    if ($has_quantity_feed) {
                        $mwsSuccess = $this->submitFeedInventory($productsUpdate, $params);
                    }
                    if ($has_price_feed) {
                        $this->submitFeedPrice($productsUpdate, $params);
                    }
                    $this->submitFeedOverride($productsUpdate, $params);
                    if ($params->sendImages) {
                        $this->submitFeedImage($productsUpdate, $params, $images_count);
                    }
                    if ($Relationship) {
                        $this->submitFeedRelationship($Relationship, $params);
                    }
                ...
                if ($mwsSuccess) {
                    ...
                    if (!$params->xmlOnly && is_array($synchProductsIds) && count($synchProductsIds)) {
                        AmazonProduct::marketplaceActionAcknowledge($action, $params->id_lang, $synchProductsIds, $params->currentDate);
                    }
        ...
        if ($params->sendToAmazon && !count($productsDelete)) {
            ...
            $this->elc('Product to delete: ', $productsDelete);
            if ($params->sendToAmazon || $params->xmlOnly) {
                if ($this->submitFeedDeletion($productsDelete, $params)) {
                    // Update Marketplace Product Action Table
                    if (is_array($synchProductsIds) && count($synchProductsIds)) {
                        AmazonProduct::marketplaceActionAcknowledge($action, $params->id_lang, $synchProductsIds, $params->currentDate);
                    }
                }
            } else {
        ...
        $batch = AmazonBatch::initInstance(
            uniqid(), $params->timestart, time(),
            $params->cronMode ? $this->l('Cron') : $this->l('Interactive'), $params->region,
            $mwsSuccess && $params->create ? count($productsUpdate) : 0,
            $mwsSuccess && !$params->create ? count($productsUpdate) : 0,
            $mwsSuccess ? count($productsDelete) : 0,
            false
        );
        $batches->add($batch);
        $batches->save();

        printf('%.02f"'.$cr, $batch->timestop - $params->timestart);
    }

    private function getProductCategorySet($id_product, $id_category_default, $id_category_default_has_profile)
    {
        $id_category_default = (int)$id_category_default;
        $categories = AmazonProduct::marketplaceGetCategories($id_product);
        ...
        if (!$id_category_default_has_profile) {
            $cindex = array_search($id_category_default, $categories);
            if ($cindex !== false) {
                unset($categories[$cindex]);
            }
        }
    ...
    private function prepareExtendedDatas($productData, $variant = AMAZON::EXTENDED_DATA_ALL)
    {

        switch ($variant) {
            case AMAZON::EXTENDED_DATA_NAME_DESCRIPTION :
                $result = $this->RemoveExtendedDataFeatures($productData);
                break;
            case AMAZON::EXTENDED_DATA_FEATURES :
                $result = $this->RemoveExtendedDataNameDescription($productData);
                break;
            default :
                $result = $productData;
        }
    ...
    private function RemoveExtendedDataFeatures($p)
    {
        if (count($p['ProductDescription'] )) {
            foreach ($p['ProductDescription'] as $key => $value) {
                if ($key != 'Title' && $key != 'Description' && $key != 'MerchantShippingGroupName') {
                    unset ($p['ProductDescription'][$key]);
                }
            }
        }
    ...
    private function RemoveExtendedDataNameDescription($p)
    {
        unset ($p['ProductDescription']['Title'], $p['ProductDescription']['Description']);
    ...
    public function validateField(&$productInstance, &$xsd_array, $p_universe, $p_product_type, $field, $value)
    {
        ...
        if ($target_definition instanceof stdClass) {
            ...
            if (property_exists($target_definition, 'type') && $target_definition->type == 'dateTime') {
                ...
                return (false);
            } elseif (property_exists($target_definition, 'type') && $target_definition->type == 'number') {
        ...
        if ($has_fixed) {
            ...
            if ($has_target_attribute) {
                ...
                if ($has_const_mapping) {
                    ...
                    if (!Tools::strlen($target_mapping_value)) {
                        ...
                        if (!Tools::strlen($target_mapping_value) && in_array($field, array('Size', 'SizeName'))) {
                            $target_mapping_value = $target_attribute;
                            $has_target_mapping_attribute = Tools::strlen($target_mapping_value);
                        }
                    }
                } else {
            ...
            if ($has_target_feature) {
                ...
                if ($has_const_mapping) {
                    ...
                    if (!Tools::strlen($target_mapping_value)) {
                        ...
                        if (!Tools::strlen($target_mapping_value) && in_array($field, array('Size', 'SizeName'))) {
                            $target_mapping_value = $target_feature_value;
                            $has_target_mapping_feature = Tools::strlen($target_mapping_value);
                        }
                    }
                } else {
            ...
            switch ($selected_field) {
                ...
                case AmazonSpecificField::WEIGHT:
                    ...
                    break;
                case AmazonSpecificField::RETAIL_PRICE:
                    $price = $combination['retail_price'];
                    if ($params->fromCurrency->iso_code != $params->toCurrency->iso_code) {
                        $price = Tools::convertPrice($price, $params->toCurrency);
                    }
                    $value = Tools::ps_round($price, 2);
                    break;
                case AmazonSpecificField::RETAIL_PRICE_WITH_TAX:
                    $price = $combination['retail_price_with_tax'];
                    if ($params->fromCurrency->iso_code != $params->toCurrency->iso_code) {
                        $price = Tools::convertPrice($price, $params->toCurrency);
                    }
                    $value = Tools::ps_round($price, 2);
    ...
    public function getPriceWithRule($profile_price_rule, $marketplace_price_rule, $price, $profile_supplier_price_rule = null)
    {
        ...
        $check = AmazonTools::priceRule($price, $profile_supplier_price_rule);
        $profile_supplier_price_rule_check = (is_array($profile_supplier_price_rule) && $check != 0 && ($check > $price || $check < $price));

        if ($profile_supplier_price_rule && $profile_supplier_price_rule_check) {
            $newPrice = AmazonTools::priceRule($price, $profile_supplier_price_rule);
            $current_price_rule = $profile_supplier_price_rule;
        } elseif ($profile_price_rule_check) {
    ...
    protected function getCustomFeaturesValues($id_lang, $id_feature)
    {
        // Fetch from cache if has
        $custom_features_values = $this->custom_features_values;
        if (isset($custom_features_values[$id_lang], $custom_features_values[$id_lang][$id_feature])) {
            return $custom_features_values[$id_lang][$id_feature];
        }

        // Otherwise, fetch from db
        $fetch_db = FeatureValue::getFeatureValuesWithLang($id_lang, $id_feature, true);
        // Store cache
        $this->custom_features_values[$id_lang][$id_feature] = $fetch_db;
     ...
     * @return mixed|string
     */
    protected function getProductCondition($params, $details)
    {
        if ($params->secondHand) {
            $productCondition =
                isset($details->condition) && !empty($details->condition) &&
                isset($params->conditionMap[$details->condition]) && !empty($params->conditionMap[$details->condition])
                    ? $params->conditionMap[$details->condition] : 'New';
        } else {
    ...
    /**
     * get alternative title, description of a product
     *
     * @param $params
     ...
     * @return array
     */
    private function getAlternativeContent($params, $product_options)
    {
        ...
        if ($params->alternative_content) {
            if (isset($product_options['alternative_title'])) {
                $alternative_content['title'] = mb_substr(AmazonTools::encodeText($product_options['alternative_title'], $params->safeEncoding), 0, Amazon::LENGTH_DESCRIPTION);
            }
            if (isset($product_options['alternative_description'])) {
                $alternative_content['description'] = mb_substr(AmazonTools::encodeText($product_options['alternative_description'], $params->safeEncoding), 0, Amazon::LENGTH_DESCRIPTION);
            }
        }
    ...
    protected function resolveBusinessPrice(
        $priceAfterRuleNConverter, $id_product,
        $id_product_attribute, $id_shop, $id_currency, $id_country,
        $id_business_group, $amzDiscountEnabled,
        $priceRulesEnabled, $profilePriceRules, $mkpPriceRules)
    {
        require_once dirname(__FILE__) . '/../includes/prestashop/amazon.specific_price.php';
        require_once dirname(__FILE__) . '/../includes/prestashop/amazon.single_specific_price.php';
        require_once dirname(__FILE__) . '/../includes/amazon/amazon.message.business_price.php';

        if (!$this->amazon_features['business']) {
            return AmazonBusinessPriceMessage::initEmptyInstance();
        }

        // Remove existing business price on Amazon if Discount/Specials option is off
        if (!$amzDiscountEnabled) {
            return AmazonBusinessPriceMessage::initDeleteInstance();
        }
        if (!$id_business_group) {
            return AmazonBusinessPriceMessage::initNormalInstance($priceAfterRuleNConverter, array(), null);
        }

        $amzExportProducts = $this;
        $specificPrices = AmazonSpecificPrice::getAllSpecificPrices($id_product, $id_product_attribute, $id_shop, $id_currency, $id_country, $id_business_group);
        $amzBusinessPriceMsg = AmazonBusinessPriceMessage::initNormalInstance(
            $priceAfterRuleNConverter,
            $specificPrices,
            function ($price) use ($amzExportProducts, $priceRulesEnabled, $profilePriceRules, $mkpPriceRules) {
                if ($priceRulesEnabled) {
                    call_user_func(array($amzExportProducts, 'getPriceWithRule'), $profilePriceRules, $mkpPriceRules, $price);
                }
                return $price;
            }
        );

        $this->elc('Business price', $specificPrices, $amzBusinessPriceMsg->priceSteps);
        return $amzBusinessPriceMsg;
    }

    protected function checkSkipInactiveOrDisabledProduct($product, $product_options, $product_actions)
    {
        //Flag to indicate if product is excluded
        $is_parent_disabled = (bool)$product_options['disable'];  

        if (isset($product_actions['sku']) && !empty($product_actions['sku'])) {
            $reference = $product_actions['sku'];
        ...
        // skip if the product is inactive, and the filter is not configured or sets to filter inactive products
        if (isset($product->active) && !$product->active
            && (!$status_filter // filter is not configured yet
                || (is_array($status_filter) && isset($status_filter['prestashop_inactive']) && $status_filter['prestashop_inactive']))
        ) {
            printf($this->l('Skipped, inactive product') . '. ID: %s Reference: %s' . nl2br(Amazon::LF),
                $product_actions['id_product'],
                $reference
            );
        ...
        if ($is_parent_disabled){
            printf($this->l('Skipped, disabled product'). '. ID: %s Reference: %s' .nl2br(Amazon::LF), 
                $product_actions['id_product'], 
                $reference
            );
    ...
    protected function submitFeedProductData($productsUpdate, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedProductData(
            $this->spConnector, $this->getApiOperationMode(),
            $productsUpdate, $params,
            $this->getBatchesKey($params), $this->logger
        );
        $feedContent = $fSending->getFeedContent();
        if (!$fSending->isSendFeed()) { //todo: check here
            $this->ed($this->l('Display XML only...'))->ed($feedContent, $this->displayFeedContent($feedContent));
            return false;
        }

        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_PRODUCT_DATA);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Product'));

        return $this->logErrorResponseMessage($fSending);
    }

    protected function submitFeedRelationship($productsUpdate, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedRelationship(
            $this->spConnector, $this->getApiOperationMode(),
            $productsUpdate, $params,
            $this->getBatchesKey($params), $this->logger
        );
        $feedContent = $fSending->getFeedContent();
        if (!$fSending->isSendFeed()) {
            $this->ed($this->l('Display XML only...'))->ed($feedContent, $this->displayFeedContent($feedContent));
            return false;
        }

        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_RELATIONSHIP);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Relationship'));

        return $this->logErrorResponseMessage($fSending);
    }

    protected function submitFeedInventory($productsUpdate, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedInventory(
            $this->spConnector, $this->getApiOperationMode(),
            $productsUpdate, $params,
            $this->getBatchesKey($params), $this->logger
        );
        ...
        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_INVENTORY);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Inventory'));

        return $this->logErrorResponseMessage($fSending);
    }

    protected function submitFeedPrice($productsUpdate, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedPrice(
            $this->spConnector, $productsUpdate, $params,
            $this->getBatchesKey($params), $this->logger
        );
        $feedContent = $fSending->getFeedContent();
        if (!$fSending->isSendFeed()) {
        ...
        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_PRICES);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Price'));

        return $this->logErrorResponseMessage($fSending);
    }
    
    protected function submitFeedOverride($productsUpdate, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedOverride(
            $this->spConnector, $this->getApiOperationMode(),
            $productsUpdate, $params,
            $this->getBatchesKey($params), $this->logger
        );
        ...
        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_OVERRIDES);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Overrides'));

        return $this->logErrorResponseMessage($fSending);
    }

    protected function submitFeedImage($productsUpdate, $params, $images_count)
    {
        $operationType = $this->getApiOperationMode() == AmazonWebService::OPERATIONS_DELETE
            ? AmazonSPFeedMessage::OPERATION_TYPE_DELETE : AmazonSPFeedMessage::OPERATION_TYPE_UPDATE;
        
        $fSending = AmazonFeedsSending::submitFeedImage(
            $this->spConnector, $operationType,
            $productsUpdate, $params,
            $images_count, $this->getBatchesKey($params), $this->logger
        );
        $feedContent = $fSending->getFeedContent();
        if (!$fSending->isSendFeed()) {
            $this->ed($this->l('Display XML only...'))->ed($feedContent, $this->displayFeedContent($feedContent));
        ...
        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_IMAGES);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Images'));

        return $this->logErrorResponseMessage($fSending);
    }

    protected function submitFeedDeletion($productsDelete, $params)
    {
        $fSending = AmazonFeedsSending::submitFeedDeletion($this->spConnector, $productsDelete, $params, $this->logger);
        $feedContent = $fSending->getFeedContent();
        if (!$fSending->isSendFeed()) {
            $this->ed($this->l('Display XML only...'))->ed($feedContent, $this->displayFeedContent($feedContent));
            return false;
        ...
        $feedLog = array(AmazonLogger::CHANNEL_PRODUCT_EXPORT, AmazonLogger::SUB_PE_DELETION);
        $this->ed($this->saveSentFeed($feedLog, $feedContent, $fSending->getSubmissionFeedId(), 'Deletion'));

        return $this->logErrorResponseMessage($fSending);
    }

    /**
     * @param AmazonFeedsSending $fSending
     * @return bool
     */
    private function logErrorResponseMessage($fSending)
    {
        if ($fSending->getSentResponse()->hasError()) {
            $errorLog = $fSending->getSentResponse()->getErrorMsg();
            if ($fSending->getSentResponse()->isThrottle()) {
                $errorLog .= $fSending->getSentResponse()->getThrottleMessage();
            }
            $this->ed($errorLog);
            return false;
        }
     ...
     * @return string
     */
    private function getBatchesKey($params)
    {
        if ($params->create && $params->cronMode) {
            return 'batch_products_cron';
        } elseif ($params->create && !$params->cronMode) {
            return 'batch_products';
        } elseif (!$params->create && $params->cronMode) {
            return 'batch_offers_cron';
        } else {
            return 'batch_offers';
        }
    }

    private $apiOperationMode;

    private function setApiOperationMode($mwsOperationMode)
    {
        $this->apiOperationMode = $mwsOperationMode;
    }

    private function getApiOperationMode()
    {
        return $this->apiOperationMode;
    }
}
...
class Params extends AmazonExportProducts
{
    ...
    public $extendedDatasVariant  = false;
    public $noPriceUpdate         = false;
    ...
    public $toCurrency;
    public $fromCurrency;
    public $sendImages = false;
    public $asinOverride          = false;
    public $extendDataIDLang      = null;

    public $mappingFeatureCustom  = false;

    //Include 

    // Product condition
    public $secondHand;
    public $conditionIgnore;

    //when to apply price rules
    public $apply_price_rules_after_conversion = false;
    ...
    protected $attributeNameInTitle;
    protected $stockOnly;
    ...
    protected $priceRulesEnabled     = false;
    ...
    public function __construct($id_lang)
    {
        ...
        // Data output language
        $inactive_languages = (bool)Configuration::get(AmazonConstant::CONFIG_INACTIVE_LANGUAGES);
        $this->extendDataIDLang = $this->id_lang;
        if ($inactive_languages) {
            $alternative_lang = AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_ALTERNATIVE_LANGS);
            $this->extendDataIDLang = $alternative_lang[$id_lang] ? $alternative_lang[$id_lang] : $this->id_lang;
        }

        //apply price rules, afetr price was converted to platform currency
        $this->apply_price_rules_after_conversion = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_PRICE_RULE_AFTER_CONVERSION);
        ...
        $params  = AmazonConfiguration::get('ATTRIBUTE_NAME_IN_TITLE');
        $this->attributeNameInTitle = (bool)(int)$params;
        ...
        //
        $this->specials = (bool)((int)AmazonConfiguration::get(AmazonConstant::PE_DISCOUNT));
        $this->specialsApplyRules = (bool)Configuration::get('AMAZON_SPECIALS_APPLY_RULES');
        $this->preorder = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_PRE_ORDER);
        ...
        $this->authorizeToDelete = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_PE_DELETE_PRODUCTS);

        $params = AmazonConfiguration::get('STOCK_ONLY');
        $this->stockOnly = (int)$params ? true : AmazonTools::getValue('stocks');

        $params = Configuration::get('AMAZON_PRICES_ONLY');
        $this->pricesOnly = (int)$params ? true : AmazonTools::getValue('prices');
        ...
        $this->profilePriceRules = AmazonConfiguration::get('price_rules');

        $this->mappingFeatureCustom = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_MAPPING_FEATURE_CUSTOM);
        ...
        $this->sendImages = (int)AmazonTools::getValue('images');
        $this->create = (int)AmazonTools::getValue('create');
        $this->delete = (int)AmazonTools::getValue('delete-xml');

        // Absolute URL to images
        // for images, use of ssl is not supported yet, source: https://images-na.ssl-images-amazon.com/images/G/02/rainier/help/Feeds_Error_Messages.pdf
        ...
        $this->shipping = AmazonConfiguration::get(AmazonConstant::SHIPPING_ALL_CONFIG);
        ...
        $this->conditionIgnore = AmazonConfiguration::get(AmazonConstant::CONFIG_PRODUCT_UPDATE_CONDITION_IGNORE);
        ...
        if (is_array($this->shipping) && isset($this->shipping['shipping_templates']) && $this->shipping['shipping_templates']) {
            ...
            if (is_array($shippingOptions) && isset($shippingOptions[$id_lang])) {
                ...
                if (isset($this->shipping['smart_shipping']['active']) && $this->shipping['smart_shipping']['active'] && $shippingOption) {
                    ...
                    $params = AmazonConfiguration::get('ADDRESS_MAP'); // Todo: check here
...
class CodeExemption
{
     ...
     * @return array
     */
    public static function modelNumber($universe, $productTag)
    {
        switch ($universe) {
            case 'ProductClothing':
                return array('Clothing', $productTag, 'ModelNumber');
            case 'Jewelry':
                return array('Jewelry', 'ModelNumber');
            default:
                return array($universe, $productTag, 'ModelNumber');
        }
    }
     ...
     * @return array
     */
    public static function modelName($universe, $productTag)
    {
        switch ($universe) {
            case 'ProductClothing':
                return array('Clothing', $productTag, 'ModelName');
            default:
                return array($universe, $productTag, 'ModelName');
        }
    }
     ...
     * @return array
     */
    public static function styleNumber($universe, $productTag)
    {
        switch ($universe) {
            case 'ProductClothing':
                return array('Clothing', $productTag, 'StyleNumber');
            default:
                return array($universe, $productTag, 'StyleNumber');
        }
    }

    /**
     * @param $p_sku_as_sup_ref_unconditionally
     * @param $supplier_reference
     * @param $combination_reference
     *
     * @return mixed
     */
    public static function mfrPartNumber($p_sku_as_sup_ref_unconditionally, $supplier_reference, $combination_reference)
    {
        if ($p_sku_as_sup_ref_unconditionally) {
            return $combination_reference;
