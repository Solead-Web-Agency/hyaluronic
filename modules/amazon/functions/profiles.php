...
class AmazonProfiles extends Amazon
{
    ...
    public function dispatch()
    {
        ...
        switch ($action) {
            case 'get_profile':
                ...
                break;
            case 'verify_custom_unitofmeasures':
                    $this->updateCustomUnitOfMeasures(false);
                    break;
            
            case 'update_custom_unitofmeasures':
                    $this->updateCustomUnitOfMeasures(true);
                    break;
            default:
    ...
    public function getProfileByKey()
    {
        ...
        foreach ($languages as $language) {
            ...
            $view_params['profile']['suppliers'] = $suppliers;
            ...
            /* START Custom price by supplier */
            $amzSupplierPriceRule = new AmazonSupplierPriceRule($profile_id, $id_lang);
            $view_params['profile']['supplier_price_rule'] = $amzSupplierPriceRule->getPriceRule();
            /* END Custom price by supplier */
    ...
    public function updateCustomUnitOfMeasures($persist=false){
        $profiles = AmazonConfiguration::get('profiles');
        ...
        if (isset($profiles['extra'])){
            $fieldName = 'UnitCount';
            $attributes_name = 'unitOfMeasure';
            $changed = false;
            
            foreach($profiles['extra'] as $profilekey=>$extrainfo){
                foreach($extrainfo as $profile_id_lang=>$profile_extra_info){
                    if (!(isset($actives[$profile_id_lang]) && (int)$actives[$profile_id_lang])) {
                        continue;
                    }
                    
                    if (isset($regions) && is_array($regions) && isset($regions[$profile_id_lang])) {
                        $region = Tools::strtoupper($regions[$profile_id_lang]);
                    } else {
                        continue;
                    }

                    $universe = $profile_extra_info['universe'];
                    $productType = $profile_extra_info['product_type'];
                    $config = AmazonXSD::getCustomSizeConfiguration($universe, $fieldName, $region);
                    ...
                    if (!isset($config['Attributes']) || !isset($config['Attributes'][$attributes_name])){
                        continue;
                    } 
                    
                    $values = [];
                    
                    if(isset($config['Attributes'][$attributes_name][$region])){
                        $values = AmazonTools::encode(
                            AmazonDataAdjustment::serialize(
                                $config['Attributes'][$attributes_name][$region]
                            )
                        );
                    }else if(isset($config['Attributes'][$attributes_name]['DEFAULT'])){
                        $values = AmazonTools::encode(
                            AmazonDataAdjustment::serialize(
                                $config['Attributes'][$attributes_name]['DEFAULT']
                            )
                        );
                    }
                    
                    if (Amazon::$debug_mode) {
                        $before_value = isset($profiles['extra'][$profilekey][$profile_id_lang]['values'][$fieldName])
                                        ? $profiles['extra'][$profilekey][$profile_id_lang]['values'][$fieldName]
                                        : AmazonTools::encode(AmazonDataAdjustment::serialize(array()));
                        $before_value = AmazonDataAdjustment::unSerialize(AmazonTools::decode($before_value));
                        $after_value = array($region=>AmazonDataAdjustment::unSerialize(AmazonTools::decode($values)));

                        CommonTools::p(sprintf('%s:%s - profilekey : %s', 
                            basename(__FILE__), 
                            __LINE__, 
                            print_r(
                                array(
                                    $profilekey => array('before'=>$before_value, 'after'=> $after_value)
                                )
                            , true)
                    ...
                    if(!isset($profiles['extra'][$profilekey][$profile_id_lang]['values'])){
                        $profiles['extra'][$profilekey][$profile_id_lang]['values'] = array();
                    }
                    if(!isset($profiles['extra'][$profilekey][$profile_id_lang]['values'][$fieldName])){
                        $profiles['extra'][$profilekey][$profile_id_lang]['values'][$fieldName] = array();
                    }
                    $profiles['extra'][$profilekey][$profile_id_lang]['values'][$fieldName] = $values;

                    if(!isset($profiles['extra'][$profilekey][$profile_id_lang]['attributes'])){
                        $profiles['extra'][$profilekey][$profile_id_lang]['attributes'] = array();
                    }
                    if(!isset($profiles['extra'][$profilekey][$profile_id_lang]['attributes'][$fieldName])){
                        $profiles['extra'][$profilekey][$profile_id_lang]['attributes'][$fieldName] = array();
                    }
                    $profiles['extra'][$profilekey][$profile_id_lang]['attributes'][$fieldName][$attributes_name] = "";
                    if(!$changed){
                        $changed = true;
                    }
            ...
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s:%s - new Profile information', basename(__FILE__), __LINE__ ));
            ...
            if($persist && $changed){
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s:%s - Proceed to save ', basename(__FILE__), __LINE__ ));
                }
                $profiles = AmazonConfiguration::updateValue('profiles', $profiles);
            }
        }
