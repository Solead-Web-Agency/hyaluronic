...
require_once(dirname(__FILE__).'/../classes/amazon.webservice.class.php');  // todo: Remove when migrated to SP-API
...
class AmazonProductsImport extends AmazonReportRealTime
{
    const GET_PRODUCTS = 'get_products';
    const PARSE_PRODUCTS = 'parse_products'; // Parse product & import data from report
    ...
    public static $expected = array(
                'en' => array('item-name', 'seller-sku', 'price', 'quantity', 'product-id', 'product-id-type', 'item-note', 'item-condition', 'status', 'status', 'status'),
                'fr' => array('nom-produit', 'sku-vendeur', 'prix', 'quantite', 'id-produit', 'type-id-produit', 'note-etat-article', 'etat-produit', 'status', 'statut', 'etat'),
                'it' => array('nome dell\'articolo', 'sku venditore', 'prezzo', 'quantita', 'identificativo del prodotto', 'tipo di identificativo del prodotto', 'note sull\'articolo', 'condizione dell\'articolo', 'stato'),
                ...
                'cn' => array('商品名称'/*product name*/,'卖家 SKU'/*Seller SKU*/,'价格'/*price*/,'数量'/*quantity*/,'商品编码'/*product_id*/,'商品编码类型'/*product id type*/,'商品备注'/*condition note*/,'商品状况'/*condition code*/,'卖家配送组'/*merchant shipping group*/,'asin1','状态'/*status*/),
                'de' => array('artikelbezeichnung', 'haendler-sku', 'preis', 'menge', 'produkt-id', 'produkt-id-typ', 'anmerkung zum artikel', 'artikelzustand', 'handlerversandgruppe', 'asin 1', 'status')
        );
    ...
    public static $amazon_attributes = array(
        ...
        'Material' => self::TYPE_OTHER, // SP-API
    ...
    protected $debugFilePrefix = 'products_import';
    
    protected $optionHide;

    /** @var AmazonWebService $ws */
    public $ws;
    protected $id_lang_default = null;
    ...
    {
        return self::FILE_NAME_PRODUCT_LISTINGS_DATA;
    }
    ...
    public function __construct()
    {
        ...
        // todo: Use mkp instead
        parent::__construct(Tools::getValue('instant_token'), Tools::getValue('marketplace_id'));

        ob_start();
        AmazonContext::restore($this->context);
        if($this->context && !isset($this->context->employee)){
            $id_employee = self::getAmzEmployeeId();
            $this->context->employee = new Employee($id_employee);
        }
        ob_get_clean();
        
        $this->optionHide = (bool)Tools::getValue('visibility-hidden');
    }
...
//            '%s%s.raw',
//            $this->storagePath,
//            'active_listings_data_APJ6JRA9NG5V4_test'
//        );
//
//        return true;
//    }
    ...
    private function matchingProductByAsin($asinList)
    {
        // todo: Cache the api
        // todo: Init $spMkp by marketplace ID
        $spMkp = $this->spMkp;
        ...
        sleep(1); // restore rate = 2
        $searchResult = $catalogApi->apiSearch(
            $asinList,
            AmazonSPAPICatalog::IDENTIFIERS_TYPE_ASIN,
            $spMkp->getSellerId(),
            array('attributes', 'dimensions', 'identifiers', 'images', 'productTypes', 'relationships', 'salesRanks', 'summaries')
        );
        $this->ed('Matching catalog by ASIN', $searchResult->getRawResponse());

        return $searchResult;
    }

    private function getCatalogItems($sku_list) {
        $apiResult = $this->matchingProductBySKU($sku_list);

        if ($apiResult->hasError()) {
            self::$errors[] = sprintf('Error when retrieving data for these sku: %s: %s', implode(', ', $sku_list), $apiResult->getErrorMsg());
        }

        /** @var AmazonSPDefItemSearchResults $itemSearchResults */
        $itemSearchResults = $apiResult->getStructuredPayload();

        if (!$itemSearchResults || !$itemSearchResults->hasItems()) {
            $this->ed('SKU not found', $sku_list);

            return null;
        }

        if (count($sku_list) > 1) {
            return $itemSearchResults->items;
        }

        return $apiResult->getStructuredPayload()->getFirstItem();
    }
     ...
     */
    private function matchingProductBySKU($skuList)
    {
        // todo: Cache the api
        // todo: Init $spMkp by marketplace ID
        $spMkp = $this->spMkp;
        ...
        $searchResult = $catalogApi->apiSearch(
            $skuList,
            AmazonSPAPICatalog::IDENTIFIERS_TYPE_SKU,
            $spMkp->getSellerId(),
            array('attributes', 'dimensions', 'identifiers', 'images', 'productTypes', 'relationships', 'salesRanks', 'summaries')
        );
        $this->ed('Matching catalog by SKU', $searchResult->getRawResponse());

        return $searchResult;
    }

    // todo: Can use multiple SKUs at once?
    // todo: Sleep to avoid throttle
    private function parseProductBaseItem($catalogItem, $sku)
    {
        if (!$catalogItem) {
            return array();
        }

        $marketplaceId = $this->spMkp->getMarketplaceId();
        $productIdLang = 0;
        $productLang = $catalogItem->getLang();
        // Iso lang in marketplace is not reliable?
//        $productLang = $this->spMkp->getIso();
        if ($productLang) {
            $productIdLang = Language::getIdByIso($productLang);
        }
        if (!$productLang || !$productIdLang) {
            $this->ed('Failed to parse language');
            return array();
        }

        $asin = $catalogItem->asin;
        self::$messages[] = sprintf('%s SKU: %s ASIN: %s', $this->l('Parsing product'), $sku, $asin);
        $browseNodeId = $catalogItem->getBrowseNodeId($marketplaceId);
        $parentAsins = $catalogItem->getParentAsins($marketplaceId);
        $parentAsin = null;
        if ($parentAsins) {
            $parentAsin = reset($parentAsins);
        }
        $dimensions = $catalogItem->getDimensions($marketplaceId);
        $itemDimensions = array();
        $packageDimensions = array();
        if ($dimensions && $dimensions->item) {
            $itemDimensions = array(
                'height' => array(
                    'value' => $dimensions->item->height->value,
                    'unit' => $dimensions->item->height->unit,
                ),
                'length' => array(
                    'value' => $dimensions->item->length->value,
                    'unit' => $dimensions->item->length->unit,
                ),
                'width' => array(
                    'value' => $dimensions->item->width->value,
                    'unit' => $dimensions->item->width->unit,
                ),
                'weight' => array(
                    'value' => $dimensions->item->weight->value,
                    'unit' => $dimensions->item->weight->unit,
                ),
            );
        }
        if ($dimensions && $dimensions->package) {
            $packageDimensions = array(
                'height' => array(
                    'value' => $dimensions->package->height->value,
                    'unit' => $dimensions->package->height->unit,
                ),
                'length' => array(
                    'value' => $dimensions->item->length->value,
                    'unit' => $dimensions->item->length->unit,
        ...
        $attributes = array();
        $productType = $catalogItem->getProductType($marketplaceId);
        if ($productType) {
            $attributes['ProductTypeName'] = $productType;
        }
        ...
        $amzProduct = array(
            ...
            'lang' => $productLang,
            'id_lang' => $productIdLang,
            'browse_node_id' => $browseNodeId,
            'title' => $catalogItem->getItemName($marketplaceId),
            'manufacturer' => $catalogItem->getManufacturer($marketplaceId),
            'brand' => $catalogItem->getBrand($marketplaceId),
            'part_number' => $catalogItem->getPartNumber($marketplaceId),
            'image' => $catalogItem->getLargeImage($marketplaceId),
            'color' => $catalogItem->getColor($marketplaceId),
            'size' => $catalogItem->getSize($marketplaceId),
            'package_qty' => $catalogItem->getPackageQuantity($marketplaceId),
        );

        $apiAttributes = $catalogItem->getAttributesByMkp($marketplaceId);
        if ($apiAttributes) {
            foreach ($apiAttributes as $apiAttributeName => $apiAttribute) {
                $apiAttributeName = str_replace('_', '', $apiAttributeName);
                foreach (self::$amazon_attributes as $predefinedKey => $type) {
                    $predefinedKey = strtolower($predefinedKey);
                    if (strstr($apiAttributeName, $predefinedKey) !== false) {
                        switch ($type) {
                            case self::TYPE_FEATURE:
                                $amzProduct['features'] = array($apiAttribute);
                                break;
                            case self::TYPE_TITLE:
                                $amzProduct['title'] = $apiAttribute;
                                break;
                            case self::TYPE_MANUFACTURER:
                                $amzProduct['manufacturer'] = $apiAttribute;
                                break;
                            case self::TYPE_BRAND:
                                $amzProduct['brand'] = $apiAttribute;
                                break;
                            case self::TYPE_CATEGORY:
                                $amzProduct['category'] = $apiAttribute;
                                break;
                            case self::TYPE_PART_NUMBER:
                                $amzProduct['part_number'] = $apiAttribute;
                                break;
                            case self::TYPE_IMAGE:
                                $amzProduct['image'] = $apiAttribute;
                                break;
                            case self::TYPE_COLOR:
                                $amzProduct['color'] = $apiAttribute;
                                $amzProduct['attributes'][$apiAttributeName] = $apiAttribute;   // Add color to attributes too
                                break;
                            case self::TYPE_SIZE:
                                $amzProduct['size'] = $apiAttribute;
                                $amzProduct['attributes'][$apiAttributeName] = $apiAttribute;   // Add size to attributes too
                                break;
                            case self::TYPE_ECOTAX:
                                $amzProduct['ecotax'] = $apiAttribute;
                                break;
                            case self::TYPE_PACKAGE_QTY:
                                $amzProduct['package_qty'] = $apiAttribute;
                                break;
                            default:
                                $amzProduct['attributes'][$apiAttributeName] = $apiAttribute;
                                break;
                        }
                    }
                }
        ...
        $amzProduct = $this->parseProductBaseItemVariant($amzProduct, $catalogItem, $marketplaceId);

        $this->parseProductBaseItemPrice($amzProduct);
    ...
    /**
     * @param $amzProduct
     * @param AmazonSPDefCatalogItem $catalogItem
     * @param $marketplaceId
     * @return array
     */
    private function parseProductBaseItemVariant($amzProduct, $catalogItem, $marketplaceId)
    {
        $variationRelationship = $catalogItem->getVariationRelationship($marketplaceId);
        if ($variationRelationship && $variationRelationship->variationTheme
            && $variationRelationship->variationTheme->attributes) {
            foreach ($variationRelationship->variationTheme->attributes as $attributeName) {
                // Move to PS attribute / variant
                if (isset($amzProduct['attributes'][$attributeName])) {
                    $amzProduct['variant'][$attributeName] = $amzProduct['attributes'][$attributeName];
                    // Unset it from PS feature
                    unset($amzProduct['attributes'][$attributeName]);
                }
            }
        }
    ...
    private function parseProductBaseItemPrice($amzProduct)
    {
        $parentAsin = $amzProduct['parent_asin'];
        $asin = $amzProduct['asin'];
        $sku = $amzProduct['reference'];
        if ($parentAsin && (float)self::$offers[$sku]['price']) {
            if (!isset(self::$variants[$parentAsin], self::$variants[$parentAsin][$asin])) {
                self::$variants[$parentAsin][$asin] = array();
            }
            self::$variants[$parentAsin][$asin]['price'] = self::$offers[$sku]['price'];
        }
    }

    private function parseProductParentItem($parentAsins)
    {
        if (!$parentAsins) {
            return array();
        }
        $marketplaceId = $this->spMkp->getMarketplaceId();
        $apiResult = $this->matchingProductByAsin($parentAsins);
        if ($apiResult->hasError()) {
            self::$errors[] = sprintf('Error: %s', $apiResult->getErrorMsg());
            return array();
        }
        ...
        /** @var AmazonSPDefItemSearchResults $itemSearchResults */
        $itemSearchResults = $apiResult->getStructuredPayload();
        if (!$itemSearchResults->hasItems()) {
            $this->ed('ASIN not found', $parentAsins);
            return $parentProperties;
        }

        foreach ($itemSearchResults->items as $parentItem) {
            $parentAsin = $parentItem->asin;
            $parentTitle = $parentItem->getItemName($marketplaceId);
            $parentImage = $parentItem->getLargeImage($marketplaceId);
            $variationRelationship = $parentItem->getVariationRelationship($marketplaceId);
            $parentProperties[$parentAsin] = array(
                'child_asins' => $variationRelationship->childAsins,
                'title' => $parentTitle,
                'image' => $parentImage,
            );
        }
    ...
    private function parseProductApplyParentProperties($amazon_products, $parentProperties)
    {
        foreach ($parentProperties as $properties) {
            $childAsins = $properties['child_asins'];
            $parentTitle = $properties['title'];
            $parentImage = $properties['image'];
            // If only 1 variant, use it own title / image
            if (count($childAsins) <= 1) {
                continue;
            }
            foreach ($childAsins as $childAsin) {
                $amzProduct = $this->findProductByAsinInTuple($amazon_products, $childAsin);
                if ($amzProduct) {
                    $amzSku = $amzProduct['reference'];
                    $amazon_products[$amzSku]['parent_title'] = $parentTitle;
                    if ($parentImage) {
                        $amazon_products[$amzSku]['parent_image'] = preg_replace(
                            '/\._[A-Z]{2}[0-9]{2}_/',
                            '._UL1500_',
                            $parentImage
                        );
    ...
    private function findProductByAsinInTuple($amzProducts, $asin)
    {
        foreach ($amzProducts as $amzProduct) {
            if ($amzProduct['asin'] == $asin) {
                return $amzProduct;
            }
        }
    ...
    private function parseProductSliceOfThisRun($amazon_products)
    {
        if (count($amazon_products)) {
            foreach ($amazon_products as $sku => $amazon_product) {
                self::$offers[$sku]['amazon'] = $amazon_product;
                $this->ed('Product to be imported', $amazon_product);
            }
            self::$messages[] = sprintf('%d %s', count($amazon_products), $this->l('product/s parsed...'));
        }
    }
    ...
    public function parseProducts()
    {
        ...
        if ($this->initialize()) {
            if (!$this->processReport(false)) {
                self::$errors[] = $debug = $this->l('An error happened during the parsing');

                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - error: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $debug));
                }
            } else {
                ...
                if ($offset && $offset >= $count_offers) {
                    self::$messages[] = $this->l('Import done successfully');
                    $stop = true;
                } elseif (is_array(self::$offers) && $count_offers) {
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - %d offers'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, count(self::$offers)));
                    }

                    $getMatchingProductLimit = 10;
                    ...
                    $sku_list = array();
                    $parent_asins = array();
                    $update_existing = (bool) Tools::getValue('update-existing');

                    if (is_array($items) && count($items)) {
                        // item/item parsing
                        $count = 0;
                        foreach ($items as $key => $item) {
                            $check_sku = trim(Tools::substr($item['sku'], 0, $this->skuLength));
                            
                            // skip an existing when only create
                            $productCheckArray = AmazonProduct::checkProduct($check_sku, $this->context->shop->id, null) ;
                            $exist_product = $productCheckArray['count'];
                            
                            if ($exist_product && (int) $exist_product > 0 && !$update_existing) {
                                continue;
                            }
                            ...
                            $sku_list[] = trim($item['sku']);
                        }
                        $count++;
                    }

                    /** SP API migration ************************************************************* $sku */
                    //Receive one request, many products by SKU
                    $catalogItems = $this->getCatalogItems($sku_list);
                        ...
                        if (!empty($catalogItems)) {
                            foreach ($catalogItems as $catalogItem) {
                                if (is_object($catalogItem)
                                    && property_exists($catalogItem, 'asin')
                                    && ($item['product-id'] === $catalogItem->asin)
                                ) {
                                    $amazon_products[$sku] = $this->parseProductBaseItem($catalogItem, $sku);
                                    unset($items[$key]);
                                }
                            }
                        }
                    ...
                    //Check if there are any Items left for which data has not been received, if there are, we make a query individually for each Item
                    if (!empty($items)) {
                        foreach ($items as $item) {
                            $sku = $item['sku'];
                            $amazon_products[$sku] = $this->parseProductBaseItem($this->getCatalogItems(array($sku)), $sku);
                        }
                    }

                    $parent_asins = array_column($amazon_products, 'parent_asin');
                    $parent_asins = array_unique(array_filter($parent_asins));

                    if (count($parent_asins) > 0) {
                        self::$messages[] = "Parent ASINs to be fetched: " . implode(', ', $parent_asins);
                    }

                    $secondStepResult = $this->parseProductParentItem($parent_asins);
                    $amazon_products = $this->parseProductApplyParentProperties($amazon_products, $secondStepResult);

                    $this->parseProductSliceOfThisRun($amazon_products);
                    ...
                    echo json_encode(array(
                        ...
                        'output' => ob_get_clean(),
                        'process' => !$stop && count(self::$offers) && $items_count,
                        'offset' => $offset && $items_count ? $offset : count(self::$offers),
                        'action' => self::PARSE_PRODUCTS,
                    ));
                    return;
                    /** SP API migration ************************************************************* $sku */


                    // todo: Remove the legacy parser in the future
                    if (is_array($sku_list) && count($sku_list)) {
                        $ufn = md5(AmazonDataAdjustment::serialize($sku_list));
                        $fn = 'a.'.$ufn.'.out';

                        if (file_exists($fn)) {
                            $result = simplexml_load_file($fn);
                        } else {
                            sleep(20); // prevents throttling
                            $result = $this->ws->getMatchingProductForId($sku_list, 'SellerSKU');
                        }
                        if (Amazon::$debug_mode) {
                            CommonTools::p(sprintf('%s - %s(#%d): %s - %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $result));
                        }
                        if (!$result instanceof SimpleXMLElement) {
                            ...
                            self::$errors[] = sprintf('Error: %s', print_r($result, true));
                            return (false);
                        }
                        ...
                        foreach ($xpath_base as $xpath_product) {
                            if (isset($xpath_product->Error->Message) && Tools::strlen($xpath_product->Error->Message)) {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $debug));
                                }
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
                            ...
                            if (!Tools::strlen($sku)) {
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - SKU not found'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
                                }
                                continue;
                            }
                            ...
                            if (!is_array($xpath_identifier)) {
                                self::$errors[] = $debug = $this->l('Unable to find identifier');
                            ...
                            if (is_array($xpath_categories)) {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($xpath_category, true)));
                                }
                            }
                            ...
                            $this->xpathSimpleQuery($xpath_product, $attributes_tag.'/ns2:Title');
                            $product_lang = null;
                            ...
                            if (is_array($lang_xpath) && count($lang_xpath)) {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - Failed to parse language'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
                                }
                                continue;
                            }
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - Parent ASIN: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $parent_asin));
                            }
                            ...
                            foreach (self::$amazon_attributes as $amazon_attribute => $type) {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - Xpath query: %s Result: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $query, print_r($value, true)));
                                }
                                ...
                                if (Amazon::$debug_mode) {
                                    //CommonTools::p(sprintf('%s - %s(#%d): %s - Attribute: %s Value: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $type, print_r($value, true)));
                                }
                                ...
                                switch ($type) {
                                    ...
                                    case self::TYPE_IMAGE:
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - Query: %s Value: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $query, print_r($value, true)));
                                        }
                                        if (is_array($value)) {
                                            $image = reset($value);
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - Image: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $image));
                                        }
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - Attributes: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($amazon_product, true)));
                            }
                        }
                        if (Amazon::$debug_mode) {
                            CommonTools::p(sprintf('%s - %s(#%d): %s - parent ASINs: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($parent_asins, true)));
                        }
//                        ddt('First run', json_encode($amazon_products), $amazon_products);
                        if (is_array($parent_asins) && count($parent_asins)) {
                            $result = $this->ws->getMatchingProductForId($parent_asins, 'ASIN');

                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - ASIN file: %s content: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $fn, print_r($result, true)));
                            }
                            ...
                            if ($result instanceof SimpleXMLElement) {
                                ...
                                if (is_array($xpath_base) && count($xpath_base)) {
                                    foreach ($xpath_base as $xpath_product_key => $xpath_product) {
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - variation_attributes: %s (%s)'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($variation_attributes, true), $xpath_product_key));
                                        }
                                        ...
                                        if (is_array($children_xpath) && count($children_xpath) > 1) {
                                            foreach ($children_xpath as $children_xpath_key => $child) {
                                                ...
                                                if (Amazon::$debug_mode) {
                                                    CommonTools::p(sprintf('%s - %s(#%d): %s - child: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($child, true)));
                                                }
                                                foreach ($variation_attributes as $variation_attribute_key => $variation_attribute) {
                                                    ...
                                                    if (Amazon::$debug_mode) {
                                                        //CommonTools::p(sprintf('%s - %s(#%d): %s - Attribute: %s (%s)'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($variation_attribute, true), $variation_attribute_key));
                                                        CommonTools::p(sprintf('%s - %s(#%d): %s - Parent ASIN: %s ASIN:%s Attribute: %s Value: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $parent_asin, $asin, $variation_attribute, $attribute_value));
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - Variants: %s (%s)'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($variants, true), $xpath_product_key));
                                    }
                                                ...
                                                foreach ($amazon_products as $sku => $amazon_product) {
                                                    if ($amazon_product['asin'] == $child_asin) {
                                                        ...
                                                        foreach ($attributes as $attribute_name => $attribute_value) {
                                                            if (Amazon::$debug_mode) {
                                                                CommonTools::p(sprintf('%s - %s(#%d): %s - Variation: %s Parent/Child: %s/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $attribute_value, $parent_asin, $child_asin));
                                                            }
                                    ...
                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - Variants: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($variants, true)));
                                    }
                                }
                            }
                        }
//                        ddt('Second run', json_encode($amazon_products), json_encode(self::$variants));
        ...
        if (Amazon::$debug_mode) {
            printf('%s - %s(#%d): %s - End', date('c'), basename(__FILE__), __LINE__, __FUNCTION__);
        }
        $offset += $items_count;
    ...
    public function createCategory($category_name)
    {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - id_amazon_category: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_amazon_category));
            CommonTools::p(sprintf('%s - %s(#%d): %s - Category: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $category_name));
        }
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - categories: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($categories, true)));
        }
        foreach ($categories as $category) {
            if (AmazonTools::toKey($category['name']) == AmazonTools::toKey($category_name)) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - Returning existing category: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($category, true)));
                }
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - parent category: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, (int)$category->id_parent));
        }
        $category->active = true;
        $category->add();

        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - Adding category: %s(%d)'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $category_name, $category->id));
        }
    ...
    public function importProducts($products)
    {
        ...
        if (!is_array($products)) {
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - not an array: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($products, true)));
            }
            return(false);
        }
        ...
        if ($attributes == null) {
            ...
            foreach ($languages as $language) {
                ...
                if (_PS_VERSION_ < '8') {
                    ...
                    $attributes = ProductAttribute::getAttributes($id_lang);
                }
        ...
        foreach ($products as $amazon_product) {
            if (!isset($amazon_product['amazon'])) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - no data: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($amazon_product, true)));
                }
                return(false);
            }
            ...
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - amazon_product: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($amazon_product, true)));
            }
            ...
            if (isset($amazon_data['parent_asin'])) {
                if (isset(self::$variants[$amazon_data['parent_asin']])
                    ...
                    && count(self::$variants[$amazon_data['parent_asin']]) > 0) { // create combinations only if there is more than one child
                    $is_variant = true;
                } else {
                    $is_variant = false;
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - variant product: %s/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $amazon_data['parent_asin'], $amazon_data['asin']));
                }
            } else {
                $is_variant = false;
            }
            ...
            if ($is_variant) {
                ...
                if (Validate::isLoadedObject($product)) {
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - Product is variant and loaded: %d/%s, amazon: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $amazon_data['title'], print_r($amazon_data, true)));
                    }
                    $amazon_data = $amazon_product['amazon'];
                    ...
                    if (isset($amazon_data['variant']) && count($amazon_data['variant'])) {
                        ...
                        foreach ($amazon_data['variant'] as $amazon_attribute => $amazon_attribute_value) {
                            ...
                            if (!isset($attributes_names[AmazonTools::toKey($amazon_attribute)])) {
                                ...
                                if (Validate::isLoadedObject($attribute_group)) {
                                    ...
                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - Created attribute group: %d/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute_group, $amazon_attribute));
                                    }
                                } else {
                                    self::$errors[] = $debug = sprintf('%s: "%s"', $this->l('Unable to create attribute group'), $amazon_attribute);

                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - '.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $debug));
                                    }
                                }
                            } else {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - Existing attribute group: %d/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute_group, $amazon_attribute));
                                }
                            }
                            ...
                            if (!isset($attributes_values_names[AmazonTools::toKey($amazon_attribute_value)]) && $id_attribute_group) {
                                ...
                                if (version_compare(_PS_VERSION_, '8', '>=')) {
                                    $attribute = new ProductAttribute(); // For PS8
                                } else {
                                    $attribute = new Attribute();
                                }
                                $attribute->id_attribute_group = $id_attribute_group;
                                $attribute->name = $attribute_value_names;
                                if (method_exists('ProductAttribute', 'getHigherPosition')) { // For PS8
                                    $attribute->position = ProductAttribute::getHigherPosition($id_attribute_group)+1;
                                }
                                ...
                                if ($attribute->validateFields(false, false)) {
                                    ...
                                    if (Validate::isLoadedObject($attribute)) {
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - Created attribute value: %d/%s/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute, $amazon_attribute, $amazon_attribute_value));
                                        }
                                    } else {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - Existing attribute value: %d/%s - '.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute, $amazon_attribute_value));
                                }
                            }
                        ...
                        if (is_array($product_combination) && count($product_combination)) {
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - Product combination: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($product_combination, true)));
                                CommonTools::p(sprintf('%s - %s(#%d): %s - Attribute combinations: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($attribute_combinations, true)));
                            }
                            $prices = array();
                            ...
                            if (isset(self::$variants[$parent_asin][$asin]['price'])) {
                                ...
                                if (Tools::getValue('update-price')) {
                                    ...
                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - update price and tax for: %d/%d, price: %.02f/%.02f tax: %.02f'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $id_product_attribute, $unit_price_tax_excl, $impact, $product_tax_rate));
                                    }
                                    $product->update();
                                }
                            }
                            if (!$attribute_combination_exists) {
                                ...
                                foreach ($product_combination as $id_attribute_group => $id_attribute) {
                                    ...
                                    if (Amazon::$debug_mode) {
                                        CommonTools::p(sprintf('%s - %s(#%d): %s - Create combination result: for %d/%d - %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute_group, $id_attribute, print_r($result, true)));
                                    }
                                }
                            } else {
                                ...
                                foreach ($attribute_combinations as $attribute_combination) {
                                    ...
                                    if ($id_product_attribute) {
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - Update attribute result: for %d/%d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute_group, $id_attribute));
                                        }
                                    } else {
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - Missing attribute "%d" for product id "%d"'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $id_attribute, $product->id));
                                        }
                                    }
                                }
                            }
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - update stock and price for: %d qty: %d, price: %.02f'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, (int)$amazon_product['qty'], $price));
                            }
                        }
                    }
                } else {
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - validation failed: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($amazon_product, true)));
                    }
                }
            } else {
                ...
                if (Validate::isLoadedObject($product)) {
                    ...
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - update stock and price for: %d qty: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, (int)$amazon_product['qty']));
                    }
                }
            }
        }
    ...
    public function addFeatures($product, $amazon_product)
    {
        ...
        if (is_array($product_features) && count($product_features)) {
            ...
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - existing product features: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($existing_product_features, true)));
            }
        }
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - existing features: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($features, true)));
            CommonTools::p(sprintf('%s - %s(#%d): %s - existing product features: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($existing_product_features, true)));
        }
        ...
        if (is_array($amazon_data) && isset($amazon_data['attributes']) && count($amazon_data['attributes'])) {
            foreach ($amazon_data['attributes'] as $attribute_name => $attribute_value) {
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - attribute: %s value: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $attribute_name, $attribute_value));
                }
                
                if(isset(FeatureValue::$definition) 
                   && isset(FeatureValue::$definition['fields'])
                   && isset(FeatureValue::$definition['fields']['value'])
                   && isset(FeatureValue::$definition['fields']['value']['size'])
                   && FeatureValue::$definition['fields']['value']['size']
                   && (int) FeatureValue::$definition['fields']['value']['size'] < Tools::strlen($attribute_value)
                ){
                    $attribute_value = Tools::substr($attribute_value, 0, FeatureValue::$definition['fields']['value']['size']); 
                }
                ...
                if (is_array($features) && count($features)) {
                    ...
                    foreach ($features as $id_lang => $features_lang) {
                        foreach ($features_lang as $id_feature => $feature) {
                            ...
                            if (AmazonTools::toKey($feature_name) == AmazonTools::toKey($attribute_name)) {
                                ...
                                if (Amazon::$debug_mode) {
                                    CommonTools::p(sprintf('%s - %s(#%d): %s - matching amazon attribute: %s feature: %s(%d) lang: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $attribute_name, $feature_name, $id_feature, $id_lang));
                                }
                            }
                        }
                    }
                            ...
                            if (isset($current_feature['values']) && count($current_feature['values'])) {
                                foreach ($current_feature['values'] as $id_feature_value => $feature_value) {
                                    if (AmazonTools::toKey($attribute_value) == AmazonTools::toKey($feature_value['value'])) {
                                        ...
                                        if (Amazon::$debug_mode) {
                                            CommonTools::p(sprintf('%s - %s(#%d): %s - matching amazon attribute value: %s feature value: %s(%d) lang: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $attribute_value, $feature_value['value'], $id_feature_value, $id_lang));
                                        }
                                    }
                                }
                            }
                    ...
                    if (!$existing_id_feature) {
                        ...
                        if (Amazon::$debug_mode) {
                            CommonTools::p(sprintf('%s - %s(#%d): %s - add feature: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r(get_object_vars($feature), true)));
                        }
                        $feature->add();
                        ...
                        if (Validate::isLoadedObject($feature)) {
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - feature added: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r(get_object_vars($feature), true)));
                            }
                        }
                    }
                    $feature_value = null;
                    ...
                    if (!$existing_id_feature_value && $existing_id_feature) {
                        ...
                        if (Amazon::$debug_mode) {
                            $feature_data = is_object($feature) ? get_object_vars($feature) : $feature; // prevent PHP Warning:  get_object_vars() expects parameter 1 to be object, array given
                            CommonTools::p(sprintf('%s - %s(#%d): %s - add feature: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($feature_data, true)));
                        }
                    }
                }
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - product_has_feature_value: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product_has_feature_value));
                }
                if (!$product_has_feature_value) {
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - add product feature value: %d/%d/%d\''.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $existing_id_feature, $existing_id_feature_value));
                    }
    ...
    public function importProduct(&$amazon_product)
    {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - importing product: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($amazon_product, true)));
        }

        if ($amazon_data['parent_asin'] && isset(self::$variants[$amazon_data['parent_asin']])
            && is_array(self::$variants[$amazon_data['parent_asin']])
            && count(self::$variants[$amazon_data['parent_asin']]) > 0) {
            $has_variant = true;
            $sku = trim(Tools::substr($amazon_data['parent_asin'], 0, $this->skuLength));
            $child_sku = trim(Tools::substr($amazon_product['sku'], 0, $this->skuLength));
        } else {
            $has_variant = false;
            $sku = trim(Tools::substr($amazon_product['sku'], 0, $this->skuLength));
            $child_sku = null;
        }

        if (!Validate::isReference($sku)) {
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - SKU validation failed: "%s"'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
            }
            return(false);
        }

        if (AmazonProduct::checkProduct($sku, $this->context->shop->id, $product_id_lang)['count']) {
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - "%s" already exists in database'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
            }
            // existing product
            ...
            if (!Validate::isLoadedObject($product)) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - "%s" unable to load product'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
                }
                return(false);
            }
            ...
            if (AmazonProduct::checkProduct($child_sku, $this->context->shop->id, $product_id_lang)['count']) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - "%s" already exists in database'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $child_sku));
                }
                // existing product
        ...
        if (isset($amazon_data['package_qty']) && (int)$amazon_data['package_qty']) {
            $product->unity = (string)$amazon_data['package_qty'];
        }
        ...
        $product->visibility = $this->_productVisibility();
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - id_category_default: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id_category_default));
        }
        $languages = self::$languages;

        $name_array = array();
        $link_array = array();
        $title = isset($amazon_data['parent_title']) && Tools::strlen($amazon_data['parent_title'])
            ? $amazon_data['parent_title'] : $amazon_data['title'];
        // 96903: html_entity_decode() has difficulty decoding non-breaking space (&nbsp;), it still remains
        $title = preg_replace('/\s/u', ' ', html_entity_decode($title));

        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - languages: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($languages, true)));
        }
        foreach ($languages as $language) {
            $id_lang = (int)$language['id_lang'];
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - name_array: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($name_array, true)));
        }
        $tags = str_replace('-', ',', Tools::link_rewrite($name_array[$this->id_lang_default]));
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - tags: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r($tags, true)));
        }
        if (is_array($name_array) && count($name_array)) {
            $product->name = $name_array;
        }
        if (is_array($link_array) && count($link_array)) {
        ...
        if (is_array($tags) && count($tags) && !Tools::strlen($product->meta_keywords)) {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - Prevalidate'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
        }
        if (!$product->validateFields(false, false)) {
            self::$errors[] = $debug = sprintf(
                '%s: "%s", error: %s, input: %s',
                $this->l('Unable to create product'),
                $amazon_data['title'],
                $product->validateFields(false, true),
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - Postvalidate'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
        }
        if (method_exists('Product', 'getIdTaxRulesGroupMostUsed') && !$product->id_tax_rules_group) {
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - Before <getIdTaxRulesGroupMostUsed> evaluation'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
            }
            $product->id_tax_rules_group = (int)Product::getIdTaxRulesGroupMostUsed();
        }

        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - After getIdTaxRulesGroupMostUsed'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
        }
        if (Validate::isLoadedObject($product)) {
            $update = true;
        } else {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - After Validate::isLoadedObject'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
        }
        ...
        try {
            if ($update) {
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - Before delete images %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
                }
                $this->deleteImages($product);

                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - Before product update %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
                }
                $product->update();

                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - product id: %d'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id));
                }
            } else {
                self::$messages[] = sprintf('%s: %s - %s', $this->l('Creating product'), $sku, $title);
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - Before add %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $sku));
                }
                $product->add();
            ...
            self::$errors[] = $debug = 'Throwable Exception: '.  $tex->getMessage(). "\n";
        ...
        if ($update && Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - updating existing product: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r(get_object_vars($product), true)));
        }
        ...
        if (method_exists('Tag', 'addTags')) {
            ...
            foreach ($languages as $language) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - updating tags: %d/%d %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $language['id_lang'], print_r($tags, true)));
                }
        ...
        if (Tools::getValue('update-price')) {
            ...
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - update tax for: %d tax: %.02f'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $product_tax_rate));
            }
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - final product: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, print_r(get_object_vars($product), true)));
        }
        return($product);
    }
    ...
    protected function addImages($product, &$amazon_product)
    {
        ...
        foreach ($languages as $language) {
            $id_lang = $language['id_lang'];
            $image_name = isset($amazon_data['parent_title']) && Tools::strlen($amazon_data['parent_title']) ? $amazon_data['parent_title'] : $amazon_data['title'];
            $image->legend[$id_lang] = sprintf('%s', mb_substr($image_name, 0, 128)) ;
        }
        ...
        if (($image->validateFields(false, true)) === true &&
            ...
            try {
                ...
                // Sometimes, the URL is "Not Found". E.g. https://m.media-amazon.com/images/I/71284CWEQVL.jpg
                if ($copy_succeed < 10) {
                    $image->delete();
                    $this->dd('Image not found: ', $image_url);
                    return;
                }
            } catch (Exception $e) {
                CommonTools::p(sprintf('Failed to copy src: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $path));
                $copy_succeed = false;
            }
            ...
            if (!$copy_succeed) {
                $image->delete();
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - failed to copy image: %d src: %s dst: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $tmpfile, $path));
                }
            } else {
                copy($tmpfile, $path);
            ...
            if (Amazon::$debug_mode) {
                CommonTools::p(sprintf('%s - %s(#%d): %s - image: %d src: %s dst: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $tmpfile, $path));
            }
            ...
            if (method_exists('ImageManager', 'resize')) {
                ...
                foreach ($imagesTypes as $image_type) {
                    ...
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - product: %d src: %s dst: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $product->id, $tmpfile, $dstfile));
                    }
    ...
    public function convertUnit($value, $source_unit)
    {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - value: %s, unit: %s target: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $value, $unit, $target_unit));
        }
        ...
        if (!$converted) {
            self::$errors[] = $debug = sprintf($this->l('Unable to convertToCurrency from "%" to "%s"', $source_unit, $target_unit));
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('%s - %s(#%d): %s - result: %.02f/%s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $converted, $target_unit));
        }
    ...
    protected function afterProcessReport($processResult)
    {
        if ($processResult) {
            ...
            self::$messages[] = $this->l('No offers to process...');
        }
    }

    protected function processReport($display = true)
    {
        $offers = array();

        if (Amazon::$debug_mode) {
            printf('%s - %s(#%d): processReport()'.nl2br(Amazon::LF), date('c'), basename(__FILE__), __LINE__, __FUNCTION__);
        }
        ...
        // File may enconded in Unicode, convert it before processing
        $fileContent = AmazonTools::fileGetContents($this->file_inventory);
        if ($fileContent === false) {
            $error = sprintf('%s (%s)', $this->l('Unable to read input file'), $this->file_inventory);
            self::$errors[] = $error;
            $this->dbt($error);
            return (false);
        }
        try {
            ...
            $this->dbt('WARNING: Cannot convert file content to UTF-8, process may have many problems!');
            $this->dbt(sprintf('Code: %s. Error: %s', $exception->getCode(), $exception->getMessage()));
        }

        if (is_null($fileContent) || empty($fileContent)) {
        ...
        $columns = explode("\t", AmazonTools::noAccents(Tools::strtolower($header)));
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf('Column keys array: %s' . nl2br(Amazon::LF), print_r($columns_keys, true)));
        }
        ...
        foreach ($lines as $line) {
            ...
            // if (Amazon::$debug_mode) {
            //     CommonTools::p(sprintf('Line exploded array: %s' . nl2br(Amazon::LF), print_r($result, true)));
            // }
            ...
            if (!count($values)) {
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('Required fields not found for Item parsed: %s'.nl2br(Amazon::LF), print_r($result, true)));
                }
                continue;
            }
    ...
    private function _productVisibility()
    {
        return $this->optionHide ? 'none' : 'both';
    }
}
