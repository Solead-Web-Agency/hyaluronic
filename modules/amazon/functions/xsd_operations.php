    ...
    public function extraFields($json_output = false)
    {
            ...
            foreach ($specificFields[$profile]['variation'] as $variationThemeItem) {
                if (is_array($variationThemeItem) && count($variationThemeItem)) {
                    foreach ($variationThemeItem['fields'] as $variationField) {
                        if(Tools::strtolower($variationField) == 'unitcount'){
                            $variationField = 'UnitCount';
                        }
        ...
        if ($universe == 'Beauty') {
            ...
            if (isset($p->ProductData->Luggage->VolumeCapacityName->attr->unitOfMeasure) &&
                ...
                $specificFields[$profile]['values']['VolumeCapacityName'] = AmazonTools::encode(AmazonDataAdjustment::serialize($p->ProductData->Luggage->VolumeCapacityName->attr->unitOfMeasure->allowedValues));
            }
        ...
        } elseif (isset($p->ProductData->Office)) {

            $specificFields[$profile]['field']['FcShelfLife'] = null;
            $specificFields[$profile]['recommended']['FcShelfLife'] = true;
            $specificFields[$profile]['attributes']['FcShelfLife'] = array();
            $specificFields[$profile]['attributes']['FcShelfLife']['unitOfMeasure'] = true;
        } 

        //add unitOfMeasure attribute for FcShelfLife, when it's included
        if(isset($p->DescriptionData->FcShelfLife)){
            $name = 'FcShelfLife';
            $tag = $p->DescriptionData->FcShelfLife;

            if (
                isset($tag->attr->unitOfMeasure) 
                && isset($tag->attr->unitOfMeasure->allowedValues)
                ...
                $specificFields[$profile]['values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($tag->attr->unitOfMeasure->allowedValues));
            }
        }
        ...
        if (isset($p->ProductData->$universe->ProductType->$productType) && $p->ProductData->$universe->ProductType->$productType instanceof stdClass) {
            ...
            foreach ($p->ProductData->$universe->ProductType->$productType as $name => $tag) {
                ...
                if (isset($tag->choice) && $tag->choice) {
                    ...
                    $specificFields[$profile]['choice_allowed_values'][$name] =  AmazonTools::encode(AmazonDataAdjustment::serialize($tag->allowedValues));
                }
            }
                        ...
                        if ($tag instanceof stdClass) {
                            if (AmazonXSD::isParentWithChildren($key)) {
                                ...
                                foreach ($children as $keyName => $tagValue) {
                                    if ($tagValue instanceof stdClass) {
                                        ...
                                        if (isset($tagValue->attr->unitOfMeasure) && isset($tagValue->attr->unitOfMeasure->allowedValues)
                                            ...
                                            $specificFields[$profile]['values'][$keyName] = AmazonTools::encode(AmazonDataAdjustment::serialize($tagValue->attr->unitOfMeasure->allowedValues));
                                        }
                                    }
                                }
                            } else {
                                ...
                                if (isset($tag->attr->unitOfMeasure) && isset($tag->attr->unitOfMeasure->allowedValues)
                                    ...
                                    $specificFields[$profile]['values'][$key] = AmazonTools::encode(AmazonDataAdjustment::serialize($tag->attr->unitOfMeasure->allowedValues));
                                }
                            }
                        }
                    }
        ...
        echo $callback.'('.json_encode(AmazonTools::fixEncoding($data)).')';
        die;
    }
        ...
        if (is_array($default_fields) && count($default_fields)) {
            foreach ($default_fields as $field) {
                ...
                if (isset($productDefinition->$field->attr->unitOfMeasure) && isset($productDefinition->$field->attr->unitOfMeasure->allowedValues)
                     ...
                     * MM, CM, etc...
                     */
                    $specificFields['values'][$field] = AmazonTools::encode(AmazonDataAdjustment::serialize($productDefinition->$field->attr->unitOfMeasure->allowedValues));
                }
            }
        }
        ...
        foreach ($productDefinition as $name => $tag) {
            ...
            if ($tag instanceof stdClass && isset($tag->type)) {
                if (isset($tag->allowedValues) && is_array($tag->allowedValues) && count($tag->allowedValues)) {
                    ...
                    $specificFields['allowed_values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($tag->allowedValues));
                    ...
                    $specificFields['allowed_values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($allowedValues));
                ...
                if (in_array($tag->type, array('text', 'number', 'boolean', 'dateTime', 'StringNotNull'))) {
                    ...
                    if (isset($tag->attr->unitOfMeasure) && isset($tag->attr->unitOfMeasure->allowedValues)
                        ...
                        $specificFields['values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($tag->attr->unitOfMeasure->allowedValues));
                    }else if(isset($tag->attr->unitOfMeasure) 
                            && isset($tag->attr->unitOfMeasure->mandatory)
                            && $tag->attr->unitOfMeasure->mandatory == "true"){
                        $specificFields['attributes'][$name] = array();
                        $specificFields['attributes'][$name]['unitOfMeasure'] = true;
                        //$specificFields['values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($tag->attr->unitOfMeasure->allowedValues));
                    }
        ...
        if (isset($config['Attributes']) && is_array($config['Attributes'])
            ...
            foreach($config['Attributes'] as $attribute=>$regionInfo){
                ...
                && is_array($regionInfo[$region])
                    ...
                    $elementTypes['values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($regionInfo[$region]));
                }else if(isset($regionInfo["DEFAULT"]) 
                && is_array($regionInfo["DEFAULT"])){
                    $elementTypes['attributes'][$name] = array();
                    $elementTypes['attributes'][$name][$attribute] = true;
                    $elementTypes['values'][$name] = AmazonTools::encode(AmazonDataAdjustment::serialize($regionInfo["DEFAULT"]));
                }
            }
        }
        ...
        foreach(AmazonXSD::$productXsdGenericFields as $fieldName=>$data){
            if(isset($p->$fieldName)){
                ...
                if (isset($p->$fieldName->attr->unitOfMeasure) &&
                    ...
                    $specificFields[$profile]['values'][$fieldName] = AmazonTools::encode(AmazonDataAdjustment::serialize($p->$fieldName->attr->unitOfMeasure->allowedValues));
                }
            }
        }
    }
