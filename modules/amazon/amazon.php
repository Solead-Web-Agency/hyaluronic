...
require_once(_PS_MODULE_DIR_.'/amazon/classes/amazon.price.price_rule.class.php');
require_once(_PS_MODULE_DIR_.'/amazon/classes/amazon.price.supplier_price_rule.class.php');
require_once(_PS_MODULE_DIR_.'/amazon/classes/amazon.data_adjustment.class.php');
if (version_compare(_PS_VERSION_, '8', '>=')) {
    require_once(dirname(__FILE__) . '/classes/WebserviceSpecificManagementMarketplaceOrdersPs8.php');
    require_once(dirname(__FILE__) . '/classes/WebserviceSpecificManagementMarketplaceOrderDetailsPs8.php');
} else {
    require_once(dirname(__FILE__) . '/classes/WebserviceSpecificManagementMarketplaceOrders.php');
    require_once(dirname(__FILE__) . '/classes/WebserviceSpecificManagementMarketplaceOrderDetails.php');
}
...
class Amazon extends Module
{
    ...
    const REMOVE = 'd';
    const UPDATE = 'u';
    const REPRICE = 'r'; // This feature was removed.
    ...
    private $_config     = array(
        ...
        // Combine `AMAZON_CARRIER` & `AMAZON_AMAZON_CARRIER` to `AMAZON_MKP_INCOMING_CARRIERS`
        'AMAZON_PRIME_CARRIER' => null,
        'AMAZON_AMAZON_PRIME_CARRIER' => null,
        'AMAZON_MKP_INCOMING_CARRIERS' => null,
        /*Orders Statuses*/
        ...
        'AMAZON_ALTERNATE_PRODUCT_IMAGE' => false,
        'AMAZON_ALTERNATE_PRODUCT_IMAGE_SRC' => null,
        'AMAZON_DESCRIPTION_FIELD' => self::FIELD_DESCRIPTION_LONG,
        'AMAZON_MAPPING_FEATURE_CUSTOM' => false,
        /*Shipping Overrides*/
        ...
        /* The number of days for calls auto-clear log files. */
        'AMAZON_CLEAR_LOGS_DAY' => 90,
        'AMAZON_VCS_CLEAN_UP' => 0,
        'AMAZON_DISABLE_LOGGING' => false,
        /* Marketplace Customer groups */
        'AMAZON_MKP_CUSTOMER_GROUP' => array(),
        'AMAZON_PRICE_RULES' => null,
        'AMAZON_EU_SEPARATE_SELLER_ACC' => false,
        /* Alternative language for markerplace if language is inactive */
        'AMAZON_ALTERNATIVE_LANGUAGES' => array(),
        /* Option to apply rule prices after price is converted to destiny currency */
        'AMAZON_APPLY_PRICE_RULES_AFTER_CONVERSION' => false,
        /* List Hooks inactive */
        'AMAZON_HOOKS' => array(),
    );
    ...
    //todo:Migrate to AmazonProductClass or AmazonModuleFeatures
    const EXTENDED_DATA_ALL = 1;
    const EXTENDED_DATA_NAME_DESCRIPTION = 2;
    const EXTENDED_DATA_FEATURES = 3;
     ...
     * @var string[]
     */
    public static $marketplace_columns = array('mp_order_id' => 'Marketplace Order ID', 'latest_ship_date' => 'Shipping deadline');
    ...
    public function __construct()
    {
        ...
        //Amazon Regular: bd88475a00b7e8a2c2c3c8b89680922d
		//Amazon Lite: 95bf55040245698af47e08725394866e
        $this->bootstrap = true;
        ...
        //Web service API
        require_once(_PS_MODULE_DIR_.'/amazon/includes/psapi/amazon.product.option.php');
        ...
        $this->displayName = $this->moduleFeatures->name ?: $this->l('Amazon');

        // Parse data use json_encode/json_decode
        $amazonDataAdjustment = new AmazonDataAdjustment('AMAZON_');
        $amazonDataAdjustment->parseDataToJson();
        $amazonDataAdjustment->parseDataToJson(true);

        $this->initContext();

        self::$debug_mode = Configuration::get('AMAZON_DEBUG_MODE') || Tools::getValue('debug') || PHP_SAPI == 'cli';
        $debug_amzn_configuration = Tools::getValue('debug_configuration');
        
        //add visibility to configuration SQL performed by Amazon module, enabled for technical analysis only
        AmazonConfiguration::setDebugConfiguration($debug_amzn_configuration);
        AmazonConfiguration::setMultiShopCheck($this->amazon_features);

        if (isset(AmazonProduct::$definition)) {
            $this->skuLength = (int)AmazonProduct::$definition['fields']['reference']['size'];
        }

        // variable to check the compatibility: https://common-services-force.monday.com/boards/1971464818/pulses/5863260667
    ...
    private function initContext()
    {
        ...
        $id_lang = (int)AmazonTools::getValue('id_lang');
        if ($id_lang) {
        ...
        $this->id_lang = (int)(isset($this->context->language->id) ? $this->context->language->id : Configuration::get('PS_LANG_DEFAULT'));

        if (!$this->ps14x) {
            $this->moduleCustomContextValue = AmazonContext::getKey($this->context->shop);
        }
        ...
        $this->adminConfigurationManager = new AmazonAdminConfigure(
            $this,
            $this->context,
    ...
    public $moduleCustomContextValue = '';
    ...
    public static function getAmazonFeatures()
    {
        ...
        if (is_array($allowed_features) && count($allowed_features)) {
            ...
            if (isset($allowed_features['uncheck_multishop']) && $allowed_features['uncheck_multishop']) {
                $amazon_features['uncheck_multishop'] = 1;
            }
        }
    ...
    public function install()
    {
        ...
        foreach ($this->_config as $key => $value) {
            ...
            if (is_array($value)) {
                $value = AmazonTools::encode(AmazonDataAdjustment::serialize($value));
            }
        ...
        switch ($action) {
            case Amazon::ADD:
                ...
                if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                    ...
                    // 2022-04-22 - Tran: Remove Amazon stat. Stat data won't be updated anymore by orders_report feature
                } else {
                    // For PS < 1.5
            ...
            case Amazon::REMOVE:
                ...
                // todo: Remove Amazon stat in the future
    ...
    private function createCustomer()
    {
        ...
        while ($max--) {
            ...
            if (!Customer::customerExists($email)) {
                ...
                $customer->lastname = 'Amazon Marketplace';
                ...
                $customer->newsletter_date_add = $now;
                $customer->add();
    ...
    private function _hookSetup($action)
    {
        ...
        // Hooks
        $hooks = (array)AmazonConfiguration::get(AmazonConstant::CONFIG_LIST_HOOKS);
        $displayProductAdditionalInfo = array_key_exists('displayProductAdditionalInfo', $hooks) ? (int)$hooks['displayProductAdditionalInfo'] : 1;
        ...
        if (version_compare(_PS_VERSION_, '1.7', '<')) {
            ...
            $expectedHooks[] = 'addWebserviceResources';
        }
    ...
    /**
     * Disable/Enable hooks depend on Amazon hooks setting (Parameters tab)
     * @return bool
     */
    private function _hooksSetting(){
        $pass = true;

        $hooks = (array)AmazonConfiguration::get(AmazonConstant::CONFIG_LIST_HOOKS);
        foreach ($hooks as $hook => $active) {
            if ((bool)$active && !$this->isRegisteredInHook($hook)) {
                if (!$this->registerHook($hook)) {
                    $this->_errors[] = $this->l('Unable to Register Hook').':'.$hook;
                    $pass = false;
                }
            } elseif(!(bool)$active && $this->isRegisteredInHook($hook)) {
                if (!$this->unregisterHook($hook)) {
                    $this->_errors[] = $this->l('Unable to Unregister Hook').':'.$hook;
                    $pass = false;
                }
            }
        }
    ...
    {
        return $this->hookManager->addWebServiceResources();
    }
    ...
    {
        $this->_initAnotherMarketPlaces();
        // May redirect if meet conditions
        $this->adminConfigurationManager->spApiAuthorization();
        ...
        if (!Tools::isSubmit('submit')) {
            if (version_compare($this->version, '5.2', '>=')) {
                $this->smarty->assign(array(
                    'info_class' => $this->getInfoClassName(),
                    'warning_class' => $this->getWarningClassName(),
                ));
                $this->_html .= $this->display(__FILE__, 'views/templates/admin/support/version5_upgrade.tpl');
            }

            if (!$this->migrationManager->getVersion()) {
                $this->smarty->assign(array(
                    'info_class' => $this->getInfoClassName(),
                    'versionCheckTexts' => array(
                        $this->l('Be effective, do not waste your time :'),
                        $this->l('To start, Supposing you only have basis knowledge on it, please use basis functions'),
                        $this->l('Do not try to hard tune the module. Almost all parameters are correctly configured by default.'),
                    ),
                ));
    ...
    private function _postValidation()
    {
        ...
        $licenseId = Tools::getValue('ps_order_id');
        ...
        $prime_carriers = Tools::getValue('prime_carrier');
        $amazon_prime_carriers = Tools::getValue('amazon_prime_carrier');
        ...
        if (!$licenseId) {
            $this->_postErrors[]= $this->l('You should enter you Order ID in Amazon tab');
        }
        ...
        foreach (AmazonTools::languages() as $language) {
            ...
            $marketPlaceId[$id_lang] = (in_array($region[$id_lang], array(
                    'ca',
                    'mx'
                )) && $marketPlaceIds[$id_lang]) ? $marketPlaceIds[$id_lang] : AmazonTools::lang2MarketplaceId($region[$id_lang]);
            ...
            // Postpone the validation of incoming carrier mapping, bring back if needed
            ...
            if (!empty($merchantId[$id_lang])) {
                ...
                if ($this->amazon_features['orders'] && $shippingOutgoing->isRequiredShippingMethod()) {
                    ...
                    if (!isset($carrierOutgoing['prestashop']) || !max($carrierOutgoing['prestashop'])) {
                        $this->_postErrors[]= $this->l('You must choose a Carrier').' ('.$language['name'].')';
                    }
                    if (!isset($carrierOutgoing['amazon']) || !max($carrierOutgoing['amazon'])) {
                    ...
                    foreach (AmazonTools::languages() as $language) {
                        ...
                        if (isset($profiles['extra'][$profile_key][$id_lang]) && is_array($profiles['extra'][$profile_key][$id_lang]) && count($profiles['extra'][$profile_key][$id_lang])) {
                            ...
                            if (isset($profiles['extra'][$profile_key][$id_lang]['variant']) && Tools::strlen($profiles['extra'][$profile_key][$id_lang]['variant'])) {
                                $vname = $profiles['extra'][$profile_key][$id_lang]['variant'];
                                $variationTheme = AmazonXSDTools::getCombinationFieldName($vname);
                                ...
                                foreach ($variationFields as $variationField) {
                                    $withError = true;
                                    //look for field in case insensitive way
                                    $found = preg_grep("/^$variationField$/i",array_keys($profiles['extra'][$profile_key][$id_lang]['field']));

                                    if(count($found)){
                                        $key = reset($found);
                                        $withError = (!isset($profiles['extra'][$profile_key][$id_lang]['field'][$key])
                                            ||
                                            !Tools::strlen($profiles['extra'][$profile_key][$id_lang]['field'][$key])
                                        );
                                    }

                                    if ($withError) {

                                        $this->_postErrors[]= sprintf($this->l('Profiles Tab: %s: Variant “%s“ is selected, then field "%s" is required.'), $profiles['name'][$id], $vname, $variationField).' - ';
                                        $this->_postErrors[]= $lang_name;
                                    }
                                }
    ...
    private function _postProcess()
    {
        ...
        // Hooks
        $hook_product_additional_info = (bool)Tools::getValue('hook_product_additional_info');
        $hooks = array();
        $hooks['displayProductAdditionalInfo'] = (int)$hook_product_additional_info;
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_LIST_HOOKS, $hooks);

        $this->_hookSetup(self::ADD);

        // Hooks Setting (Parameters tab)
        $hook_product_additional_info = (bool)Tools::getValue('hook_product_additional_info');
        $hooks = array();
        $hooks['displayProductAdditionalInfo'] = (int)$hook_product_additional_info;
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_LIST_HOOKS, $hooks);
        ...
        $product_country_vat_rates = Tools::getValue('product_country_vat_rate');
        $price_rules = Tools::getValue('price_rule');
        ...
        /* START: Incoming carrier */
        $incoming_carriers = Tools::getValue('incoming_carrier');
        /* END: Incoming carrier */
        ...
        $allow_IT_upload_invoice = (bool)Tools::getValue('allow_it_upload_invoice');
        ...
        $not_decrease_stock_for_shipped = Tools::getValue('not_decrease_stock_for_shipped');
        ...
        if (!isset($features['amazon_europe']) && isset($features['amazon_be'])) {
            unset($features['amazon_be']);
        }
        ...
        $specials = (bool)AmazonTools::getValue('specials');
        $specials_apply_rules = (bool)AmazonTools::getValue('specials_apply_rules');
        $regular_to_rpr = (bool)AmazonTools::getValue('regular_to_rpr');
        $preorder = (bool)AmazonTools::getValue('preorder');
        $import_by_id = (bool)AmazonTools::getValue('import_by_id');
        $taxes = (int)AmazonTools::getValue('taxes');
        $attribute_name_in_title = (bool)AmazonTools::getValue('attribute_name_in_title');
        $delete_products = (bool)AmazonTools::getValue('delete_products');
        $stock_only = (bool)AmazonTools::getValue('stock_only');
        $html_descriptions = (bool)AmazonTools::getValue('html_descriptions');
        $prices_only = (bool)AmazonTools::getValue('prices_only');
        $payment_region = (bool)AmazonTools::getValue('payment_region');
        $stock_zero = (bool)AmazonTools::getValue('stock_zero');
        $uncheck_product_status = (bool)AmazonTools::getValue('uncheck_product_status');
        $carrier_pickup_point = (bool)AmazonTools::getValue('carrier_pickup_point');
        $order_reference_format = AmazonTools::getValue('order_reference_format');
        $format_phone_ft_zero = AmazonTools::getValue('phone_ft_zero');
        $format_phone_remove_spacing = AmazonTools::getValue('phone_remove_spacing');
        ...
        $alternate_product_image = (bool)Tools::getValue('alternate_product_image');

        $mapping_feature_custom = (bool)Tools::getValue('mapping_feature_custom');
        ...
        $disable_special_features = (bool)Tools::getValue('disable_special_features');
        $product_country_vat = (bool)Tools::getValue('product_country_vat');
        $disable_ssl_check = (bool)Tools::getValue('disable_ssl_check');
        $alternative_languages = Tools::getValue('alternative_languages');
        ...
        $excluded_marketplace_columns = Tools::getValue('selected-marketplace_columns');

        $disable_logging = (bool)Tools::getValue('disable_logging');
        $clear_logs_day = (int)Tools::getValue('clear_logs_day');

        $clean_up_vcs = (int)Tools::getValue('clean_up_vcs');

        $price_rule_after_conversion = (bool)Tools::getValue('price_rule_after_conversion');

        $eu_separate_seller_acc = (bool)Tools::getValue('eu_separate_seller_acc');
        ...
        $product_country_vat_rate = array();
        $outofstock = array();
        $rounding = array();
        $ptc = array();
        ...
        foreach (AmazonTools::languages() as $language) {
            ...
            $active[$id_lang] = (int)$actives[$id_lang];
            ...
            $product_country_vat_rate[$id_lang] = isset($product_country_vat_rates[$id_lang]) ? $product_country_vat_rates[$id_lang] : null;
            ...
            // Marketplace incoming carrier
            $incoming_carrier[$id_lang] = isset($incoming_carriers[$id_lang]) ? array_values($incoming_carriers[$id_lang]) : array();
            $prime_carriers[$id_lang] = isset($prime_carriers[$id_lang]) ? $prime_carriers[$id_lang] : '';
            $amazon_prime_carriers[$id_lang] = isset($amazon_prime_carriers[$id_lang]) ? $amazon_prime_carriers[$id_lang] : '';
            ...
            $alternative_language[$id_lang] = isset($alternative_languages[$id_lang]) ? $alternative_languages[$id_lang] : '';
        }

        $activeBeforeSave = AmazonConfiguration::get(AmazonConstant::CONFIG_PS_LANG_ACTIVE);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PS_LANG_ACTIVE, $active);
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MKP_IT_VCS_ALLOW_UPLOAD_NON_BIZ_INVOICE, $allow_IT_upload_invoice);

        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PRODUCT_COUNTRY_VAT_RATE, $product_country_vat_rate);
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS, $incoming_carrier);

        if ($this->amazon_features['prime']) {
            AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PRIME_CARRIER, $prime_carriers);
            AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_PRIME_CARRIER, $amazon_prime_carriers);
        }
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_NOT_DECREASE_STOCK, (bool)$not_decrease_stock_for_shipped);
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PRE_ORDER, $preorder);
        AmazonConfiguration::updateValue('TAXES', $taxes);

        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_IMPORT_BY_ID, (bool)$import_by_id);
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_ALLOW_STOCK_ZERO, $stock_zero);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_UNCHECK_PRODUCT_STATUS, $uncheck_product_status);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_CARRIER_PICKUP_POINT, $carrier_pickup_point);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_REFERENCE_FORMAT, $order_reference_format);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_FORMAT_PHONE_FT_ZERO, $format_phone_ft_zero);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_OI_FORMAT_PHONE_REMOVE_SPACING, $format_phone_remove_spacing);
        ...
        Configuration::updateValue(AmazonConstant::CONFIG_INACTIVE_LANGUAGES, $inactive_languages);
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MKP_ALTERNATIVE_LANGS, $alternative_language);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_DISABLE_SPECIAL_FEATURES, $disable_special_features);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PRODUCT_COUNTRY_VAT, $product_country_vat);
        ...
        // Save alternate product image
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_ALTERNATE_PRODUCT_IMAGE, $alternate_product_image);
        $alternate_product_image_src = $this->_uploadImage('alternate_product_image_src');
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_ALTERNATE_PRODUCT_IMAGE_SRC, $alternate_product_image_src);

        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MAPPING_FEATURE_CUSTOM, $mapping_feature_custom);
        ...
        */
        ...
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_DISABLE_LOGGING, (bool)$disable_logging);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_CLEAR_LOGS_DAY, (int)$clear_logs_day);
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_VCS_CLEAN_UP, (int)$clean_up_vcs);

        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_PRICE_RULE_AFTER_CONVERSION, (bool) $price_rule_after_conversion);

        // Amazon Europe
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_AMZ_EU_SEPARATE_SELLER_ACC, (int)$eu_separate_seller_acc);

        // Marketplace column
        AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MARKETPLACE_COLUMNS, $excluded_marketplace_columns);

        require_once(_PS_MODULE_DIR_.'/amazon/includes/amazon.admin_configure.post_process.php');
        $adminConfigurationPostProcessManager = new AmazonAdminConfigPostProcess($this);
        $adminConfigurationPostProcessManager->postProcess();

        // Call clear log here
        $this->migrationManager->migrate();
        // Context management - save the full shop context. After do migration if any
        $employee = new Employee($id_employee);
        AmazonContext::save($this->context, $employee);

        if (!$this->_postErrors) {
            $this->_html .= html_entity_decode('&lt;div class="' . $this->getSuccessClassName() . '"&gt;'
    ...
    private function _saveProfiles($profiles)
    {
        ...
        $amz_price_rules = Tools::getValue('amz_price_rules'); // For supplier price rules
        ...
        $tags = array('universe', 'product_type', 'bullet_point_strategy', 'browsenode', 'item_type', 'price_rule', 'latency', 'shipping_group', 'code_exemption', 'code_exemption_options','bullet_point_labels', 'ptc', 'sku_as_supplier_reference', 'sku_as_sup_ref_unconditionnaly', 'parameters', 'attributes', 'category', 'supplier_price_rule');
        ...
        if (isset($profiles['name']) && is_array($profiles['name'])) {
            ...
            foreach ($profiles['name'] as $id => $profile) {
                ...
                foreach ($languages as $language) {
                    ...
                    if (!isset($profiles['universe'][$id]) || !array_key_exists($id_lang, $profiles['universe'][$id]) || empty($profiles['universe'][$id][$id_lang])) {
                        continue;
                    }
                    ...
                    try {
                        ...
                        if ($has_profile_fields) {
                            if( isset($p_extra['field']) ){

                                if(
                                    (
                                        isset($p_extra['field']['Color'])
                                        ||
                                        isset($p_extra['field']['ColorName'])
                                    )
                                    && !isset($p_extra['field']['ColorMap'])){
                                    $p_extra['field']['ColorMap'] = '';
                                }

                                if(
                                    (
                                        isset($p_extra['field']['Size'])
                                        ||
                                        isset($p_extra['field']['SizeName'])
                                    )
                                    && !isset($p_extra['field']['SizeMap'])){
                                    $p_extra['field']['SizeMap'] = '';
                                }
                            }
                            foreach ($p_extra['field'] as $field => $value) {
                                ...
                                $is_map_field = in_array($field, array('ColorMap', 'SizeMap'));
                                if (empty($value) && !is_numeric($value) && !$has_default && !$is_map_field) {
                                    continue;
                                }
                                ...
                                if(!$path){
                                    //special cases like Itemform variation, which related field is ItemForm
                                    $path = AmazonXSDTools::searchPath($productInstance, $field, $referenceElement, false);
                                }
                        ...
                        if ($has_variant_field && Tools::strlen($p_extra['variant'])) {
                            ...
                            foreach ($variationFieldsList as $tagName) {
                                ...
                                $perProductType = isset(AmazonXSD::$rewriteFieldsUniverseProductType[$category]);
                                $rewriteProductType = $perProductType && $referenceElement != null && in_array($referenceElement, AmazonXSD::$rewriteFieldsUniverseProductType[$category]);


                                if($perProductType
                                    && $rewriteProductType
                                    && isset(AmazonXSD::$rewriteFieldsUniverse[$category])
                                    && isset(AmazonXSD::$rewriteFieldsUniverse[$category][$tagName])){

                                        $field = AmazonXSD::$rewriteFieldsUniverse[$category][$tagName];

                                }else if (!$perProductType
                                    && isset(AmazonXSD::$rewriteFieldsUniverse[$category])
                                    && isset(AmazonXSD::$rewriteFieldsUniverse[$category][$tagName])) {
                        ...
                        // START Supplier price rule
                        $supplier_price_rule = is_array($amz_price_rules) && isset($amz_price_rules['supplier_price_rule']) ? $amz_price_rules['supplier_price_rule'][$id][$id_lang] : null;
                        $amzSupplierPriceRule = new AmazonSupplierPriceRule($id, $id_lang);
                        $amzSupplierPriceRule->updateSupplierPriceRule($supplier_price_rule);
                        // END Supplier price rule
    ...
    private function _displayForm()
    {
        ...
        $view_params = array(
            ...
            'psIs15' => $this->ps15x,
            'psIs14' => $this->ps14x,
            'class_success' => $this->getSuccessClassName(),
            'class_error' => $this->getErrorClassName(),
            'header' => array(
        ...
        $spApiResultHandler = $this->adminConfigurationManager->handleSpAPIStatus();
        if (count($spApiResultHandler)) {
            if ($spApiResultHandler['success']) {
                $view_params['header']['confirmation'][] = $spApiResultHandler['msg'];
            } else {
                $view_params['header']['error_content'][] = $spApiResultHandler['msg'];
            }
        }

        if (is_array($this->_postErrors) && count($this->_postErrors)) {
            foreach ($this->_postErrors as $err) {
                $view_params['header']['error_content'][] = $err;
            }
        }
        ...
        $this->_html .= $this->context->smarty->assign($view_params)
            ->fetch($this->path.self::$templates[self::TEMPLATE_HEADER]);
    ...
    {
        // Data mapping before load settings
        $this->migrationManager->migrateDuringLoadConfiguration();

        $this->config['instant_token'] =  Configuration::get(AmazonConstant::GB_CONFIG_INSTANT_TOKEN, null, 0, 0);
        $this->config['taxes'] = (bool)AmazonConfiguration::get('TAXES');
        ...
        $this->config['prime_carrier'] = AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_PRIME_CARRIER);
        $this->config['prime_carrier_prestashop'] = AmazonConfiguration::get(AmazonConstant::CONFIG_PRIME_CARRIER);
        $this->config['incoming_carriers'] = AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS);
        $this->config['allow_it_upload_invoice'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_IT_VCS_ALLOW_UPLOAD_NON_BIZ_INVOICE);
        ...
        $this->config['specials'] = (bool)AmazonConfiguration::get('SPECIALS');
        ...
        $this->config['import_by_id'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_IMPORT_BY_ID);
        ...
        $this->config['stocks_zero'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_ALLOW_STOCK_ZERO);
        $this->config['uncheck_product_status'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_UNCHECK_PRODUCT_STATUS);
        $this->config['carrier_pickup_point'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_CARRIER_PICKUP_POINT);
        $this->config['order_reference_format'] = AmazonConfiguration::get(AmazonConstant::CONFIG_OI_REFERENCE_FORMAT);
        $this->config['format_phone_ft_zero'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_FORMAT_PHONE_FT_ZERO);
        $this->config['format_phone_remove_spacing'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_OI_FORMAT_PHONE_REMOVE_SPACING);
        ...
        $this->config['eu_separate_seller_acc'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_EU_SEPARATE_SELLER_ACC);
        $this->config['alternative_languages'] = AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_ALTERNATIVE_LANGS);

        $this->config['mkp_groups'] = AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_CUSTOMER_GROUP);
        ...
        $this->config['not_decrease_stock_for_shipped'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_NOT_DECREASE_STOCK);
        ...
        if (!$this->config['fba_stock_behaviour']) {
            $this->config['fba_stock_behaviour'] = AmazonConstant::FBA_STOCK_BEHAVIOUR_SWITCH;
        }
        ...
        $this->config['alternate_product_image'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_ALTERNATE_PRODUCT_IMAGE);
        $this->config['alternate_product_image_src'] = AmazonConfiguration::get(AmazonConstant::CONFIG_ALTERNATE_PRODUCT_IMAGE_SRC);
        $this->config['mapping_feature_custom'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_MAPPING_FEATURE_CUSTOM);
        $this->config['disable_logging'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_DISABLE_LOGGING);
        $this->config['clear_logs_day'] = (int)AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_CLEAR_LOGS_DAY);
        $this->config['clean_up_vcs'] = (int)AmazonConfiguration::get(AmazonConstant::CONFIG_AMZ_VCS_CLEAN_UP);
        ...
        $this->config['disable_special_features'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_DISABLE_SPECIAL_FEATURES);
        $this->config['product_country_vat'] = (bool)AmazonConfiguration::get(AmazonConstant::CONFIG_PRODUCT_COUNTRY_VAT);
        ...
        //Option to apply rule prices after price is converted to destiny currency
        $this->config['price_rule_after_conversion'] = (bool) AmazonConfiguration::get(AmazonConstant::CONFIG_PRICE_RULE_AFTER_CONVERSION);


        self::loadFeatures(false, !$this->config['mapping_feature_custom']);
        self::loadAttributes();
        ...
        $price_rules = (array)AmazonConfiguration::get('PRICE_RULES');
        if (is_array($profiles) && count($profiles) && isset($profiles['name']) && is_array($profiles['name'])) {
            $this->profiles = $profiles;
            $this->profiles['price_rules'] = $price_rules;
        } else {
            $this->profiles = array();
        ...
        // Hooks
        $hooks = (array)AmazonConfiguration::get(AmazonConstant::CONFIG_LIST_HOOKS);
        $this->config['hook_product_additional_info'] = (is_array($hooks) && array_key_exists('displayProductAdditionalInfo', $hooks)) ? (int)$hooks['displayProductAdditionalInfo'] : 1;
    }
    ...
    private function _content()
    {
        $view_params = array(
            'module_path' => $this->path,
            'images_url' => $this->images,
            'amazon' => $this->adminConfigurationManager->tabAmazon(),  // Amazon Tab Content
            'class_success' => $this->getSuccessClassName(),
            'class_info' => $this->getInfoClassName(),
            'class_warning' => $this->getWarningClassName(),
            'class_error' => $this->getErrorClassName(),
        );
        ...
        $view_params['configure'] = array(
            'form_action' => AmazonTools::httpStripQueryParam($_SERVER['REQUEST_URI'], AmazonConstant::SP_API_RETURN_QUERY_NAME),
        ...
        // 2022-06-15 - Tran: Badge notification: Use get...ClassName() instead
    ...
    private function _informations(&$view_params)
    {
        ...
        //
        $display = true;
        foreach (array('actives', 'amazon_marketplace_ids') as $configuration_item) {
        ...
        if ($this->migrationManager->needMigrate()) {
            $module_infos['update'] = array(
                'display' => true,
                'level' => 'info '.$this->getWarningClassName(),
                'tutorial' => AmazonSupport::gethreflink(AmazonSupport::TUTORIAL_AFTER_INSTALLATION),
                'message' => sprintf(
                        $this->l('Module Update: Your version will be auto-updated from %s to %s after configuration changes'),
                        $this->migrationManager->getVersion(), $this->version
        ...
        $this->adminConfigurationManager->initInformation($display, $update_mode);
        $this->adminConfigurationManager->information->setTranslation(array(
            AmazonAdminConfigurationInformation::TRANS_PING => $this->l('Unable to ping Amazon. Please provide this URL to the support: %s'),
        ));
        $env_infos = array_merge($env_infos, $this->adminConfigurationManager->information->infoInitContent());
        ...
        if ($memory_limit = AmazonConfigurationCheck::getMemoryLimit() < $recommended_memory_limit) {   // Validation: Yes, it exist in `configuration_check.class.php`
        ...
        if (AmazonConfigurationCheck::hasOverrides()) { // Validation: Yes, it exist in `configuration_check.class.php`
        ...
        if (!AmazonConfigurationCheck::checkShopUrl()) {    // Validation: Yes, it exist in `configuration_check.class.php`
        ...
        foreach (array('birthday', 'company', 'siret', 'optin', 'newsletter') as $field) {
            if (!AmazonConfigurationCheck::mandatoryCustomerField($field)) {    // Validation: Yes, it exist in `configuration_check.class.php`
        ...
        if (!AmazonConfigurationCheck::checkAddress()) {    // Validation: Yes, it exist in `configuration_check.class.php`
        ...
        $hasRegions = (bool)$this->config['regions'];
        $languageConsistency = $this->adminConfigurationManager->checkLanguageConsistency($hasRegions);
        ...
        $view_params['informations'] = array(
            ...
            'ps_order_id' => AmazonConfiguration::get(AmazonConstant::CONFIG_PS_ORDER_ID),
            'env_infos' => $env_infos,
            'php_infos' => $php_infos,
    ...
    public function getNotificationClassNames()
    {
        return array(
            'success' => $this->getSuccessClassName(),
            'info' => $this->getInfoClassName(),
            'warning' => $this->getWarningClassName(),
            'error' => $this->getErrorClassName(),
        );
    }

    protected function getSuccessClassName()
    {
        return $this->psIsGt15x ? 'alert alert-success confirm' : 'confirm conf';
    }
    protected function getInfoClassName()
    {
        return $this->psIsGt15x ? 'alert alert-info' : 'info hint conf';
    }
    ...
    private function marketplaceTab(&$view_params, $id_lang, $id_country)
    {
        ...
        $alternativeLang = isset($this->config['alternative_languages'][$id_lang]) ? trim($this->config['alternative_languages'][$id_lang]) : '';
        ...
        // Carriers
        $incoming_carriers = (isset($this->config['incoming_carriers'][$id_lang]) && count($this->config['incoming_carriers'][$id_lang]) > 0) ? $this->config['incoming_carriers'][$id_lang] : array( 0 => array( 'carrier' => '', 'amazon_carrier' => '' ));
        ...
        $prime_carriers = isset($this->config['prime_carrier'][$id_lang]) ? $this->config['prime_carrier'][$id_lang] : array(null);
        $prime_carrier_prestashop = isset($this->config['prime_carrier_prestashop'][$id_lang]) ? $this->config['prime_carrier_prestashop'][$id_lang] : array(null);

        require_once(_PS_MODULE_DIR_.'/amazon/classes/amazon.preconfiguration.class.php');

        if (!AmazonConfiguration::get('ACTIVE')) {
            if (($preconfiguration = AmazonPreconfiguration::data($id_lang))) {
                ...
                $incoming_carriers = $preconfiguration['incoming_carriers'];
                $outgoing_carriers = $preconfiguration['outgoing_carriers'];
                $synch_field = $preconfiguration['synchronization_field'];
                $prime_carriers = $preconfiguration['prime_carriers'];
                $prime_carrier_prestashop = $preconfiguration['prestashop_prime_carrier'];
            }
        }
        ...
        //
        $psCarriers = array();
        $psCarrierHasModuleCarrier = false;
        foreach (self::$carriers as $psCarrier) {
            ...
            if ($psCarrier['is_module']) {
                $psCarrierHasModuleCarrier = true;
            }
        }
        ...
        if (!isset($view_params['settings']) || !is_array($view_params['settings'])) {
            $view_params['settings'] = array(
                ...
                'general' => array(
                    ...
                    'dev_mode'      => (bool)$this->moduleFeatures->dev_mode,
                ),
                'carriers' => array(
                    'fba_multichannel' => (bool)$this->config['fba_multichannel'],
                    'ps_carriers' => $psCarriers,
                    'config' => array(
                        'has_carrier_modules' => $psCarrierHasModuleCarrier,
                        'incoming'          => array(),
                        'outgoing'          => array(),
                        'fba_multichannel'  => array(),
                        'prime_carriers'    => array(),
                    ),
                ),
                'overrides' => array(
                    'standard' => array(),
        ...
        if ($active && $region) {
            try {
                $view_params['settings']['marketplace']['config'][$id_lang]['sp_api']
                    = $this->marketplaceTabBuildSPParams($region);
            } catch (Exception $exception) {
                $view_params['settings']['marketplace']['config'][$id_lang]['sp_api'] = array(
                    'enable' => false,
                );
            }
        }
        ...
        // When marketplace master is filled we use the same merchant ID for all the other marketplaces by detault.
        ...
        $view_params['settings']['locales']['config'][$id_lang]['currency_required'] = ($selected_currency != null) ? !Tools::strlen(trim($selected_currency)) : true;
        ...
        $view_params['settings']['marketplace']['config'][$id_lang]['lang_iso'] = $language->iso_code;
        ...
        $view_params['settings']['marketplace']['config'][$id_lang]['marketPlaceId'] = (in_array($region, array(
                'ca',
                'mx'
            )) && $marketplaceId) ? $marketplaceId : (isset($region) ? AmazonTools::lang2MarketplaceId($region) : '');
        ...
        // Product
        $product_country_vat_rates = AmazonConfiguration::get(AmazonConstant::CONFIG_PRODUCT_COUNTRY_VAT_RATE);
        $product_country_vat_rate_selected = null;
        if (is_array($product_country_vat_rates) && isset($product_country_vat_rates[$id_lang]) && $product_country_vat_rates[$id_lang]) {
            $product_country_vat_rate_selected = $product_country_vat_rates[$id_lang];
        }
        ...
        $view_params['settings']['general']['config'][$id_lang] = array(
            ...
            'product_country_vat_rate_selected' => $product_country_vat_rate_selected,
        );
        ...
        if (isset($this->config['price_rule'][$id_lang]) && isset($this->config['price_rule'][$id_lang]['type']) && $this->config['price_rule'][$id_lang]['type']) {
            ...
            if (isset($this->config['price_rule'][$id_lang]['rule']['from']) && is_array($this->config['price_rule'][$id_lang]['rule']['from']) && isset($this->config['price_rule'][$id_lang]['rule']['to']) && is_array($this->config['price_rule'][$id_lang]['rule']['to'])) {
                ...
                $price_rule_value = isset($this->config['price_rule'][$id_lang]['rule']['value']) ? $this->config['price_rule'][$id_lang]['rule']['value'] : false;
        ...
        $view_params['settings']['general']['config'][$id_lang]['mkp_groups'] =
            $this->adminConfigurationManager->marketplaces->customerGroups($marketplaceId, $id_lang);
        ...
        // Carrier list (prestashop - amazon)
        $view_params['settings']['carriers']['config']['amazon_carrier'] = $amazon_carrier_list;
        $view_params['settings']['carriers']['config']['prestashop_carrier'] = $carriers;

        $count_carriers = is_array($incoming_carriers) ? count($incoming_carriers) : 1;
        for ($index = 0, $sorted_index = 0; $sorted_index < $count_carriers; $index++) {
            if ($index && !$incoming_carriers[$index] && (!$incoming_carriers[$index]['amazon_carrier'] || !$incoming_carriers[$index]['carrier'])) {
                continue;
            }

            $view_params['settings']['carriers']['config']['incoming'][$id_lang][$sorted_index++] = array(
                'display_add' => $index > 0 ? 'style="display:none"' : '',
                'display_del' => $index === 0 ? 'style="display:none"' : '',
                'ps_carrier' => $incoming_carriers[$index]['carrier'],
                'amazon_carrier' => $incoming_carriers[$index]['amazon_carrier'],
                // Carrier Pickup points incoming orders
                'pickup_point' => $incoming_carriers[$index] && isset($incoming_carriers[$index]['pickup_point']),
            );
        }
        $view_params['settings']['carriers']['config']['carrier_modules_allowed'] = true;

        // Carriers Associations for prime orders
        //
        $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang] = array();
        $count_carriers = is_array($prime_carriers) ? count($prime_carriers) : 0;
        for ($index = 0; $index < $count_carriers; $index++) {
            if ($index && !$prime_carriers[$index] && $prime_carriers[$index] !== null) {
                continue;
            }

            $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index] = array();
            $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['display_add'] = ($index > 0 ? 'style="display:none"' : '');
            $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['display_del'] = ($index === 0 ? 'style="display:none"' : '');
            ...
            foreach ($amazon_carrier_list as $carrier) {
                ...
                $selected = isset($prime_carriers[$index]) && $key === $prime_carriers[$index];

                $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['amazon_carrier'][$key]['selected'] = $selected;
                $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['amazon_carrier'][$key]['name'] = $carrier;
            }

            foreach ($carriers as $carrier) {
                $id_carrier = (int)$carrier['id_carrier'];
                $selected = isset($prime_carrier_prestashop[$index]) && (int)$prime_carrier_prestashop[$index] === $id_carrier;

                $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['prestashop_carrier'][$id_carrier]['selected'] = $selected;
                $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['prestashop_carrier'][$id_carrier]['name'] = $carrier['name'];
                $view_params['settings']['carriers']['config']['prime_carriers'][$id_lang][$index]['prestashop_carrier'][$id_carrier]['is_module'] = $carrier['is_module'];
            }
        }
        ...
        // Allow IT upload invoice
        //
        if (isset($this->config['allow_it_upload_invoice'])) {
            $view_params['settings']['marketplace']['config'][$id_lang]['allowUploadInv'] = $this->config['allow_it_upload_invoice'];
        }

        // If the language for marketplace is inactive use this alternative language
        $view_params['settings']['alternative_languages'][$id_lang] = $alternativeLang;

        return ($view_params);
    }
    
    private function marketplaceTabBuildSPParams($region)
    {
        $spAuth = AmazonSPAuthPS::initFromCountryIso(Tools::strtolower($region));
        $mkpId = $spAuth->getMarketplaceId();
        $oauthUri = $spAuth->buildOAuthCentralUrl(
            AmazonTools::buildAbsolutePsAdminLink($this->context->link, 'AdminModules', $this->ps17x)
            . '&configure=' . $this->name . '&sp_mkp=' . $spAuth->getMarketplaceId()
        );
        if ($spAuth->belongToUnifiedEU()) {
            $spRegion = new AmazonSPConnectorPSRegion($spAuth->getRegion());
            $isAuth = $spRegion->isAuthenticated();
            $sellerId = $spRegion->getSellerId();
            $refreshToken = $spRegion->getRefreshToken();
        } else {
            $isAuth = $spAuth->isAuthenticated();
            $sellerId = $spAuth->getSellerId();
            $refreshToken = $spAuth->getRefreshToken();
        }
        
        return array(
            'enable'            => AmazonConstant::SP_API_ENABLE,
            'is_authorized'     => $isAuth,
            'seller_id'         => $sellerId,
            'marketplace_id'    => $mkpId,
            'oauth_uri'         => $oauthUri,
    ...
    private function _parameters(&$view_params)
    {
        ...
        $view_params['parameters']['settings']['stock_zero'] = (bool)$this->config['stocks_zero'];
        $view_params['parameters']['settings']['uncheck_product_status'] = (bool)$this->config['uncheck_product_status'];
        $view_params['parameters']['settings']['carrier_pickup_point'] = (bool)$this->config['carrier_pickup_point'];
        $view_params['parameters']['settings']['format_phone_ft_zero'] = (bool)$this->config['format_phone_ft_zero'];
        $view_params['parameters']['settings']['format_phone_remove_spacing'] = (bool)$this->config['format_phone_remove_spacing'];
        $view_params['parameters']['settings']['order_reference_format'] = $this->config['order_reference_format'];

        $view_params['parameters']['settings']['auto_create'] = (bool)$this->config['auto_create'];
        $view_params['parameters']['settings']['hook_product_additional_info'] = (bool)$this->config['hook_product_additional_info'];
        ...
        $view_params['parameters']['settings']['languages'] = Language::getLanguages(false);
        ...
        $view_params['parameters']['settings']['disable_special_features'] = (bool)$this->config['disable_special_features'];
        $view_params['parameters']['settings']['product_country_vat'] = (bool)$this->config['product_country_vat'];
        ...
        // Alternate product image (main)
        $view_params['parameters']['settings']['alternate_product_image'] = (bool)$this->config['alternate_product_image'];
        $view_params['parameters']['settings']['alternate_product_image_src'] = $this->config['alternate_product_image_src'];

        $view_params['parameters']['settings']['mapping_feature_custom'] = (bool)$this->config['mapping_feature_custom'];

        $view_params['parameters']['settings']['employee'] = array();

        // Clear log files setting
        $view_params['parameters']['settings']['disable_logging'] = (bool)$this->config['disable_logging'];
        $clear_logs_day_selected = (int)$this->config['clear_logs_day'];
        $view_params['parameters']['settings']['clear_logs_day_selected'] = $clear_logs_day_selected;
        $view_params['parameters']['settings']['clear_logs_day'][5] = ($clear_logs_day_selected === 5); // Min option clear log is 5 days
        foreach (range(10, 90, 10) as $day) {
            $view_params['parameters']['settings']['clear_logs_day'][$day] = ($clear_logs_day_selected === $day) || ($clear_logs_day_selected === 0 && $day === 90);
        }

        // Clean up VCS files setting
        $clean_up_vcs_selected = (int)$this->config['clean_up_vcs'];
        $view_params['parameters']['settings']['clean_up_vcs_selected'] = $clean_up_vcs_selected;
        foreach (range(10, 90, 10) as $day) {
            $view_params['parameters']['settings']['clean_up_vcs'][$day] = ($clean_up_vcs_selected === $day);
        }
        ...
        // EU: Use different seller account for marketplace setting
        $view_params['parameters']['settings']['eu_separate_seller_acc'] = (bool)$this->config['eu_separate_seller_acc'];

        // New option for marketplace columns
        $marketplace_excluded_columns = AmazonConfiguration::get(AmazonConstant::CONFIG_MARKETPLACE_COLUMNS);
        if (!$marketplace_excluded_columns) {
            $view_params['parameters']['marketplace_columns']['available'] = self::$marketplace_columns;
            $view_params['parameters']['marketplace_columns']['filtered'] = array();
        } else {
            $available_marketplace_columns = array();
            $filtered_marketplace_columns = array();
            foreach (self::$marketplace_columns as $col_key => $col_name) {
                if (in_array($col_key, $marketplace_excluded_columns)) {
                    $filtered_marketplace_columns[$col_key] = $col_name;
                } else {
                    $available_marketplace_columns[$col_key] = $col_name;
                }
            }
            $view_params['parameters']['marketplace_columns']['available'] = $available_marketplace_columns;
            $view_params['parameters']['marketplace_columns']['filtered'] = $filtered_marketplace_columns;
        }
    ...
    private function _profiles(&$view_params)
    {
        ...
        $amzSupplierPriceRule = new AmazonSupplierPriceRule();
        $empty_supplier_price_rule = $amzSupplierPriceRule->getPriceRule();
        ...
        foreach ($languages as $language) {
            ...
            $view_params['profiles']['empty_profile'][$id_lang] = array(
                ...
                'supplier_price_rule'       => $empty_supplier_price_rule,
                'latency'                   => null
            );
    ...
    private function _filters(&$view_params)
    {
        ...
        $view_params['filters']['statuses'] = array(
            'prestashop_inactive' => is_array($status_filter) && isset($status_filter['prestashop_inactive'])
                ...
                : 1,
            'zero_price' => is_array($status_filter) && isset($status_filter['zero_price'])
                ? $status_filter['zero_price']
                : 1
        );
            ...
            foreach ($action_queues as $key => $action_queue) {
                ...
                switch ($action_queue['action']) {
                    ...
                    case self::UPDATE:
                        ...
                        break;
                    default:
                        $action_queues[$key]['action_name'] = $this->l('undefined');
                }
            }
    ...
    private function _fba(&$view_params)
    {
        $view_params['fba'] = array(
            ...
            'stock_behaviour_synch' => AmazonConstant::FBA_STOCK_BEHAVIOUR_SYNC,
            'stock_behaviour_switch' => AmazonConstant::FBA_STOCK_BEHAVIOUR_SWITCH,
        ...
        if ($this->config['fba_stock_behaviour'] == AmazonConstant::FBA_STOCK_BEHAVIOUR_SYNC) {
            $view_params['fba']['stock_init'] = array();
            $view_params['fba']['stock_init']['enabled'] = true;

            $marketplace_countries = AmazonSpecificField::countrySelector(); // Always Break down EU
    ...
    private function _cron(&$view_params)
    {
        $view_params['cron'] = array(
            'path_tpl' => $this->path . '/views/templates/admin/configure/cron/',
            'selected_tab' => $this->selectedTab() === 'cron',
            'images_url' => $this->images,
            'url' => $this->url,
            'validation' => $this->_validate(true),
            'tutorial' => AmazonSupport::gethreflink(AmazonSupport::TUTORIAL_CRON),
            'tutorial_cron_params' => AmazonSupport::gethreflink(AmazonSupport::TUTORIAL_CRON_PARAMS),
            'display' => false,
            'prestashop' => array(
        ...
        $overrodeCronParams = $this->adminConfigurationManager->cron->getCronParams();
        $view_params['cron']['cron_params'] = $overrodeCronParams;
        ...
        // Cron for all marketplace
        foreach ($languages as $language) {
            $id_lang = $language['id_lang'];
            if (!(isset($actives[$id_lang]) && (int)$actives[$id_lang]) || !is_array($marketPlaceIds) || !isset($marketPlaceIds[$id_lang])) {
                continue;
            }
            ...
            $orderReportsCron = new AmazonAdminBuildCron($id_lang, $lang, $flag);

            if ($this->config['features']['orders_reports']) {
                $params = '/functions/orders_reports.php?cron_token='.$this->config['tokens'][$id_lang].$context_param.'&mkp='.$marketPlaceIds[$id_lang];
                $view_params['cron']['orders']['report'][$id_lang] = $orderReportsCron->buildCronURL(array(
                    'url' => $base_url.$params,
                    'title' => $this->l('Orders Reports'),
                    'frequency' => 1,
                ));
            }
        }
        ...
        if (is_array($marketPlaceIds) && count($marketPlaceIds)) {
            foreach ($languages as $langkey => $language) {
                ...
                if ($this->customerThreadIsActive()) {
                    $imapCron = new AmazonAdminBuildCron($id_lang, $lang, $flag);
                    $params = '/functions/imap.php?cron_token='.$this->config['tokens'][$id_lang].$context_param.'&cron=1&lang='.$regions[$id_lang];
                    $view_params['cron']['messaging']['grab'][$id_lang] = $imapCron->buildCronURL(array(
                        'url'       => $base_url.$params,
                        'title'     => $this->l('Messaging'),
                        'frequency' => -1,
                    ));
                }
        ...
        $additionalParams = $this->adminConfigurationManager->cron
            ->setCronToken($this->getConfig()['tokens'])
            ->setAdditionalParams($overrodeCronParams)
            ->tabCronAdditionalParams($marketPlaceIds, $actives);
        $view_params = array_merge_recursive($view_params, array('cron' => $additionalParams));
    }
    ...
    protected static function loadAttributes($inactives = false)
    {
        ...
        foreach ($languages as $language) {
            ...
            if (version_compare(_PS_VERSION_, '8', '>=')) {
                $attributes = ProductAttribute::getAttributes($id_lang, true);
            } else {
    ...
    public function hookActionOrderHistoryAddAfter($params)
    {
        ...
        $this->hookManager->manageFBAIncomingOrder($params);
    }
    ...
    // TODO: this hook wad declare in module, but not defined
    //this declaration prevents 500 internal error
    //alternative: remove hook actionAttributeGroupDelete, if not needed
    public function hookActionAttributeGroupDelete($params)
    {
        return (true);
    }

    //TODO: this mhook wad declare in module, but not defined
    //this declaration prevents 500 internal error
    //alternative: remove hook actionAttributeGroupDelete, if not needed
    ...
    public function hookUpdateCarrier($params)
    {
        ...
        foreach ($shops as $shop) {
            ...
            $carriers = AmazonConfiguration::get(AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS, null, $shop['id_shop_group'], $shop['id_shop']);
                ...
                foreach ($carriers as $id_lang => $ps_carriers) {
                    if ($ps_carriers && is_array($ps_carriers)) {
                        foreach ($ps_carriers as $index => $carrier) {
                            if ($carriers[$id_lang][$index]['carrier'] == $params['id_carrier']) {
                                $carriers[$id_lang][$index]['carrier'] = $params['carrier']->id;
                            }
                        }
                    }
                }
            ...
            AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS, $carriers, false, $shop['id_shop_group'], $shop['id_shop']);

            // Amazon Prime Carrier Mapping
            $prime_carriers = AmazonConfiguration::get('PRIME_CARRIER', null, $shop['id_shop_group'], $shop['id_shop']);

            if ($prime_carriers && is_array($prime_carriers)) {
                foreach ($prime_carriers as $id_lang => $ps_carriers) {
                    if ($ps_carriers && is_array($ps_carriers)) {
                        foreach ($ps_carriers as $index => $carrier) {
                            if ($prime_carriers[$id_lang][$index] == $params['id_carrier']) {
                                $prime_carriers[$id_lang][$index] = $params['carrier']->id;
                            }
                        }
                    }
                }
            ...
            AmazonConfiguration::updateValue('PRIME_CARRIER', $prime_carriers, false, $shop['id_shop_group'], $shop['id_shop']);
    ...
    public static function dbt($args)
    {
        ...
        $callerStackStr = implode(' -> ', array_reverse($callerStack)) . ': ';
    ...
    protected function buildSpPrimeLabelUrl()
    {
        return $this->context->link->getBaseLink() . $this->name . '/labels/';
    }

    // todo: Pass AmzOrder instead
    protected function buildSpPrimeParams($id_order)
    {
        require_once dirname(__FILE__) . '/classes/AmzOrder.php';

        $amzOrder = new AmzOrder($id_order);
        $enable = $this->isPrimeEnable() && $amzOrder->amzInfo()->is_prime;

        $result = array(
            'images_url' => $this->images,
            'enable' => $enable,
            'id_order' => $id_order,
            'marketplace_id' => $amzOrder->amzInfo()->marketplace_id,
            'initialized' => false,
            'data' => array(),
        );
        ...
        if ($enable) {
            ...
            $isInitialized = $shippingServices->isInitialized();
            $result['initialized'] = $isInitialized;
            $result['data'] = $isInitialized ? $shippingServices->paramsForTemplate($this->buildSpPrimeLabelUrl()) : array();
        }
     ...
     * @return void
     */
    private function _initAnotherMarketPlaces()
    {
        $features = self::getAmazonFeatures();
        if (isset($features['amazon_europe']) && isset($features['amazon_be']) && (bool)$features['amazon_europe'] && (bool)$features['amazon_be']) {
            $lang_id = Language::getIdByIso(AmazonConstant::AMAZON_BE_ISO);
            if (!$lang_id) {
                $belgium = new Language();
                $belgium->name = 'Belgium';
                $belgium->active = 0;
                $belgium->iso_code = AmazonConstant::AMAZON_BE_ISO;
                $belgium->language_code = 'fr-fr';
                $belgium->locale = 'fr-FR';
                $belgium->date_format_lite = 'd/m/Y';
                $belgium->date_format_full = 'd/m/Y H:i:s';
                $belgium->is_rlt = 0;
                if ($belgium->validateFields(false)) {
                    $belgium->add();
        ...
        if (isset($features['amazon_europe']) && isset($features['amazon_za']) && (bool)$features['amazon_europe'] && (bool)$features['amazon_za']) {
            $lang_id = Language::getIdByIso(AmazonConstant::AMAZON_ZA_ISO);
            if (!$lang_id) {
                $belgium = new Language();
                $belgium->name = 'South Africa';
                $belgium->active = 0;
                $belgium->iso_code = AmazonConstant::AMAZON_ZA_ISO;
                $belgium->language_code = 'en-za';
                $belgium->locale = 'en-ZA';
                $belgium->date_format_lite = 'd/m/Y';
                $belgium->date_format_full = 'd/m/Y H:i:s';
                $belgium->is_rlt = 0;
                if ($belgium->validateFields(false)) {
    ...
    private function _uploadImage($fileName)
    {
        if (isset($_FILES[$fileName]) 
            ...
            $target_dir = dirname(__FILE__) . '/views/img/configure/';
            ...
            return _PS_BASE_URL_.$this->url . 'views/img/configure/' . $filename . "." . $ext;
        }
        return '';
    }
}
