...
if (!class_exists('CommonTools')) {
    abstract class CommonTools extends Tools
    {
         ...
         * @return bool
         */
        public static function eanUpcCheckOld($code)
        {
        ...
        //validaes several kinds of EAN codes
        //Based on answer on this article: https://stackoverflow.com/questions/29076255/how-do-i-validate-a-barcode-number-using-php
        public static function eanUpcCheck($barcode)
        {
            ...
            //check valid lengths:
            $l = Tools::strlen($barcode);
            if (!in_array($l, array(8, 12, 13, 14, 17, 18))) {
                return false;
            }
            //get check digit
            $check = Tools::substr($barcode, -1);
            $barcode = Tools::substr($barcode, 0, -1);
            $sum_even = $sum_odd = 0;
            $even = true;
            while (Tools::strlen($barcode) > 0) {
                $digit = Tools::substr($barcode, -1);
                if ($even) {
                    $sum_even += 3 * $digit;
                } else {
                    $sum_odd += $digit;
                ...
                if ($stream_context == null && preg_match('/^https?:\/\//', $url)) {
                    if (preg_match('/^https:\/\//', $url) && !$disable_ssl_check) {
                        $contextOptions = array(
                            'ssl' => array(
                                'verify_peer'   => true,
                                'cafile'        => Tools::strlen($certificate)
                                    ? $certificate
                                    : CommonCertificates::getCertificate()
                            )
                        );
