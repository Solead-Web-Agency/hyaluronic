...
if (!class_exists('CommonCertificates')) {
    abstract class CommonCertificates
    {
        ...
        /** @var int $last_update Last update time of site.crt (CS server certificates) */
        protected static $last_update;
        ...
        public static function getCertificate()
        {
            ...
            if (file_exists($cert_file)) {
                ...
                $cert_content = CommonTools::fileGetContents(
                    self::URL,
                    false,
                    null,
                    30,
                    self::getDefaultCertificatePath()
                );
                if (self::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s::%s()/#%d', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__));
        ...
        private static function cleanup()
        {
            ...
            foreach ($files as $file) {
                 ...
                 *
                 * In order to exclude cacert.{num}.pem files from Git, we need a way to invalidate exist files.
                 * Set $last_update time in module child class to delete them.
                 */
                if (($file_time < $now - self::EXPIRES)
                    || (static::$last_update && $file_time <= static::$last_update)
                ) {
                    unlink($file);
                }
            }
