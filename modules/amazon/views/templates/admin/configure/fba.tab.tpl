...
<div id="menudiv-fba" class="tabItem {if $fba.selected_tab}selected{/if} panel form-horizontal">
    ...
    <div class="form-group available_if_fba_multichannel_enable" {if !$fba.multichannel}style="display: none;"{/if}>
        <div class="row">
            <label class="control-label col-lg-offset-3 col-lg-2 glossary_target" rel="fba_multichannel_force_currency">
                <span>{l s='Force currency' mod='amazon'}</span>
            </label>
            <div class="margin-form col-lg-2">
                <select name="fba_multichannel_force_currency" title="Force currency">
                    <option>{l s='Leave empty to use default setting' mod='amazon'}</option>
                    {foreach from=$fba.multichannel_force_currencies item=currency}
                        {if isset($currency.id) && !empty($currency.id)}
                        <option value="{$currency.id|intval}" {if ($currency.id == $fba.multichannel_force_currency)}selected{/if}>
                            {$currency.name|escape:'htmlall':'UTF-8'}</option>
                        {/if}
                    {/foreach}
                </select>
            </div>
    ...
    <div id="fba-stock-init" {if !$fba.stock_init.enabled}style="display:none"{/if}>
                    ...
                    <table class="country-selector">
                        <tr>
                            {foreach from=$fba.marketplaces.countries item=marketplace}
                                <td>
                            <span class="amazon-tab-selector{if $marketplace.default} active{/if}" rel="{$marketplace.lang_code|escape:'quotes':'UTF-8'}">
        ...
        {else}
            {foreach from=$fba.marketplaces.countries item=marketplace}
                <span style="display:none;" class="amazon-tab-selector active" rel="{$marketplace.lang_code|escape:'quotes':'UTF-8'}">
                <input type="hidden" rel="id_lang" value="{$marketplace.id_lang|escape:'quotes':'UTF-8'}"/>
                </span>
            {/foreach}
        {/if}
        ...
        {foreach from=$fba.marketplaces.countries item=marketplace}
            <div class="amazon-tab amazon-tab-{$marketplace.id_lang|escape:'quotes':'UTF-8'}"
                 rel="{$marketplace.lang_code|escape:'quotes':'UTF-8'}"
                 data-id-lang={$marketplace.id_lang|intval} {if !$marketplace.default}style="display:none"{/if}>
