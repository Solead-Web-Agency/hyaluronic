        ...
        <div id="default-price-rule-{$id_lang|intval}" class="default-price-rule" style="display: inline-block;">
            {foreach from=$profile.price_rule.rule.from key=index item=value}
                <div class="price-rule">
                    ...
                    {/if}*}
                    <input type="number" name="profiles[price_rule][_key_][{$id_lang|intval}][rule][percent][]" rel="percent"
                           min="-99" max="99"
                           step=".01"
                           style="width:60px;{if ($profile.price_rule.type != 'percent')}display:none;{/if}"
                           class="form-input"
                           value="{$profile.price_rule.rule.percent[$index]|escape:'htmlall':'UTF-8'}"/>

                    <input type="number" name="profiles[price_rule][_key_][{$id_lang|intval}][rule][value][]" rel="value"
                           min="-99" max="99"
                           step=".01"
                           style="width:60px;{if ($profile.price_rule.type != 'value')}display:none;{/if}"
                           class="form-input"
                           value="{$profile.price_rule.rule.value[$index]|escape:'htmlall':'UTF-8'}"/>

                    <span style="margin-right: 4px;{if ($profile.price_rule.type != 'percent')}display:none;{/if}" rel="percent">%</span>
                    <span style="margin-right: 4px;{if ($profile.price_rule.type != 'value')}display:none;{/if}" rel="value">{$profile.price_rule.currency_sign|escape:'htmlall':'UTF-8'}</span>
