...
{if isset($data.fields.recommended) && is_array($data.fields.recommended) && count($data.fields.recommended)}
    {foreach from=$data.fields.recommended item=recommended}
        {include file="{$data.path|escape:'quotes':'UTF-8'}/configure/helpers/specific_field.hidden.inc.tpl" data=$recommended}
    {/foreach}
{/if}

{if isset($data.fields.featured) && is_array($data.fields.featured) && count($data.fields.featured)}
    {foreach from=$data.fields.featured item=featured}
        {include file="{$data.path|escape:'quotes':'UTF-8'}/configure/helpers/specific_field.hidden.inc.tpl" data=$featured}
    {/foreach}
{/if}
