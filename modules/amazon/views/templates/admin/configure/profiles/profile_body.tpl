...
<div class="form-group">
    <h4 class="marketplace-heading"><img src="{$marketplace['image']|escape:'quotes':'UTF-8'}" alt="{$marketplace['name_long']|escape:'htmlall':'UTF-8'}"/>{$marketplace['name_long']|escape:'htmlall':'UTF-8'}
    </h4>
</div>
...
<div class="form-group universe-section">
    ...
    <div class="margin-form col-lg-9">
        ...
        <option value=""></option>
        {if isset($universes[$id_lang])}
            {foreach from=$universes[$id_lang] key=universe_key item=universe}
        ...
        <img src="{$profiles.images_url|escape:'quotes':'UTF-8'}refresh_static.png" class="type_clear_xsd"
             rel="{$profiles.images_url|escape:'quotes':'UTF-8'}refresh.png"/>

        <span class="clear-xsd-status" style="display:none;"></span>
        ...
        <span class="xsd-reset" style="display:none;">
            <img src="{$profiles.images_url|escape:'quotes':'UTF-8'}small-loader.gif"/>&nbsp;&nbsp;{l s='The universe is being reloaded' mod='amazon'}
        </span>
    ...
    <div class="margin-form col-lg-6">
        <div class="mandatory_fields">
            ...
            {elseif isset($profile.specifics)}
                ...
                <input type="hidden" rel="profile_key" value="{$profile_key|escape:'htmlall':'UTF-8'}"/>
                {$profile.specifics|escape:'htmlall':'UTF-8'}<!-- VALIDATION: Generated HTML with Quotes, unable to escape it... -->
            {/if}
        </div>
...
<div class="form-group margin-form  bullet-points">
    ...
    <div class="col-lg-9">
        <select name="profiles[bullet_point_strategy][_key_][{$id_lang|intval}]">
            <option value=""></option>
            <option value="{$profiles.bullet_point_strategy_a|escape:'htmlall':'UTF-8'}" {if isset($profile.bullet_point_strategy) && $profile.bullet_point_strategy  == $profiles.bullet_point_strategy_a}selected{/if}>{l s='Attributes' mod='amazon'}</option>
            <option value="{$profiles.bullet_point_strategy_af|escape:'htmlall':'UTF-8'}" {if isset($profile.bullet_point_strategy) && $profile.bullet_point_strategy  == $profiles.bullet_point_strategy_af}selected{/if}>{l s='Attributes, Features' mod='amazon'}</option>
            ...
            <option value="{$profiles.bullet_point_strategy_daf|escape:'htmlall':'UTF-8'}" {if isset($profile.bullet_point_strategy) && $profile.bullet_point_strategy  == $profiles.bullet_point_strategy_daf}selected{/if}>{l s='Short Description, Attributes, Features' mod='amazon'}</option>
...
{if (isset($profile.ptc) && is_array($profile.ptc))}
    <div class="form-group margin-form profiles-ptc-list">
        ...
        <div class="margin-form col-lg-9">
            <select name="profiles[ptc][_key_][{$id_lang|intval}]">
                <option></option>
                {foreach from=$profile.ptc item=ptc}
...
<!-- START Price rRule -->
{include file="{$profiles.module_path|escape:'htmlall':'UTF-8'}/views/templates/admin/configure/profiles/setting_fields/price_rule.tpl"}

<!-- Supplier price rule -->
{include file="{$profiles.module_path|escape:'htmlall':'UTF-8'}/views/templates/admin/configure/profiles/setting_fields/supplier_price_rule.tpl"}
...
<!-- Default Shipping Templates --->
{if (isset($profile.shipping_templates.enabled) && $profile.shipping_templates.enabled) && isset($profile.shipping_templates.groups[$id_lang]) && is_array($profile.shipping_templates.groups[$id_lang]) && count($profile.shipping_templates.groups[$id_lang])}
<div class="form-group profiles-shipping-templates">
    ...
    <div class="margin-form col-lg-9">
        ...
        <option></option>
        {foreach from=$profile.shipping_templates.groups[$id_lang] key=group_key item=group_name}
    ...
    <div class="form-group margin-form">
        ...
        <div class="col-lg-9">
            ...
            <input name="profiles[code_exemption_options][_key_][{$id_lang|intval}][private_label]" type="checkbox"
                   value="1"
                   {if isset($profile.code_exemption) && $profile.code_exemption_options && $profile.code_exemption_options.private_label}checked{/if} />&nbsp;&nbsp;<span
