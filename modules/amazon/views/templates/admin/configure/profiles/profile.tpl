...
<input type="hidden" rel="profiles[to_delete][{$profile_header.profile_id|escape:'htmlall':'UTF-8'}]" value="1"/>
...
<!-- div profile -->
<div {if $master}id="master-profile"{else}id="profile-{$profile_header.profile_id|intval}"{/if} 
    class="{$profile_class|escape:'htmlall':'UTF-8'} form-group"
    ...
    {if $master}
        ...
        <br/>
        <div class="form-group profile_name">
    ...
    {else}
        <input type="hidden" rel="profiles[name][_key_]" class="profile-name" value="{$profile_header.name|escape:'quotes':'UTF-8'}"/>
    {/if}
    ...
    {elseif isset($marketplace)}
        <div class="margin-form col-lg-9 col-lg-offset-3 main-country-selector">
            <table class="country-selector">
                <tr>
                    <td>
                        <span class="amazon-tab-selector"
                              rel="{$marketplace.iso_code|escape:'quotes':'UTF-8'}">
                            <img src="{$marketplace.image|escape:'quotes':'UTF-8'}"
                                 title="{$marketplace.name_long|escape:'quotes':'UTF-8'}"/><span
                                    class="name">{$marketplace.name_short|escape:'quotes':'UTF-8'}</span>
    ...
    {if $master}
        ...
        {/foreach}
    {else}
        <div class="amazon-profile-item" rel="{$profile_header.profile_id|intval}" style="display:none">
            <div style="text-align: center">
                <img src="{$profiles.images_url|escape:'quotes':'UTF-8'}loading.gif"/>&nbsp;&nbsp;{l s='Loading profile' mod='amazon'}
            </div>
        </div>
    {/if}
</div>
