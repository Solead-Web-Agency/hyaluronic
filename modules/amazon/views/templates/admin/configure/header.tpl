...
{if $psIs17}
    <link href="{$header.module_url|escape:'htmlall':'UTF-8'}views/css/amazon16.css?version={$header.version|escape:'htmlall':'UTF-8'}"
          rel="stylesheet" type="text/css"/>
    <link href="{$header.module_url|escape:'htmlall':'UTF-8'}views/css/amazon17.css?version={$header.version|escape:'htmlall':'UTF-8'}"
          rel="stylesheet" type="text/css"/>
{elseif $psIsGt15}
    <link href="{$header.module_url|escape:'htmlall':'UTF-8'}views/css/amazon16.css?version={$header.version|escape:'htmlall':'UTF-8'}"
          rel="stylesheet" type="text/css"/>
...
{if isset($header.confirmation) && $header.confirmation|count}
    {foreach from=$header.confirmation item=confirmation}
        <div class="{$class_success|escape:'htmlall':'UTF-8'}">
            {$confirmation|escape:'htmlall':'UTF-8'}
        </div>
    {/foreach}
{/if}
{if isset($header.error_content) && $header.error_content|count}
    <div class="{$class_error|escape:'htmlall':'UTF-8'}">
        {foreach from=$header.error_content item=error}
...
<!-- header -->
<div style="padding:10px 0 {if $psIsGt15}40px{else}10px{/if} 0">
