...
<div id="menudiv-amazon" class="tabItem {if $amazon.selected_tab}selected{/if} panel form-horizontal">
    ...
    <div class="form-group">
        <label class="col-lg-3">{if $psIsGt15}
            <h2>{else}<h3>{/if}{$amazon.name|escape:'htmlall':'UTF-8'}
                    v{$amazon.version|escape:'htmlall':'UTF-8'}{if $psIsGt15}
            </h2>
            {else}</h3>{/if}</label>

        <div class="margin-form col-lg-9" style="color: {$amazon.ps_order_id_color_identity|escape:'htmlall':'UTF-8'}">
            <label class="control-label col-lg-12" style="text-align: left; margin-bottom: 10px; color: {$amazon.ps_order_id_color_identity|escape:'htmlall':'UTF-8'}">
                {l s='Your PrestaShop Addons Order ID' mod='amazon'}</label>
            <div class="margin-form col-lg-12">
                <input type="text" name="ps_order_id" title="PrestaShop Order ID"
                       value="{$amazon.ps_order_id|escape:'htmlall':'UTF-8'}"
                       style="width:300px; border-color: {$amazon.ps_order_id_color_identity|escape:'htmlall':'UTF-8'}"
                />
                <p>
                    {l s='Please enter here your PrestaShop Addons Order ID, you can find it here:' mod='amazon'}
                    <a href="https://addons.prestashop.com/en/order-history" target="_blank">
                        https://addons.prestashop.com/en/order-history</a>
                </p>
                <hr />
            </div>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="margin-form col-lg-9">
            <p style="font-weight: bold;">
                {l s='This module is subject to a commercial license from Feed.biz.' mod='amazon'}<br>
    ...
    {$amazon.tab_footer|escape:'quotes':'UTF-8'}
</div>
