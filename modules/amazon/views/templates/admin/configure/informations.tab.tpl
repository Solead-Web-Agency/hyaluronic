...
<div id="menudiv-informations" class="tabItem {if $informations.selected_tab}selected{/if} panel form-horizontal">
    ...
    {if ($informations.display)}
        <div class="form-group">
            <label class="control-label col-lg-3 participation-label" style="cursor:pointer">{l s='Amazon Marketplace Status' mod='amazon'}</label>{if !$psIsGt15}
        <br/>{/if}
            <div align="left" class="margin-form col-lg-9">
                <div id="participation-loader"></div>
                <div id="participation-error" class="warn {if $psIsGt15}alert alert-warning{/if}"
                ...
                <table class="table amz-participation" cellpadding="0" cellspacing="0"
                    ...
                    <tr class="active">
                        <th class="left">{l s='Merchant' mod='amazon'}</th> {* The seller id *}
                        <th class="left">{l s='Marketplace' mod='amazon'}</th> {* Marketplace name (from connector) *}
                        <th class="left">{l s='CC' mod='amazon'}</th> {* country Code *}
                        <th class="left">{l s='Currency' mod='amazon'}</th> {* The RED one is the currency is not imported in Localization. *}
                        <th class="center">{l s='Local' mod='amazon'}</th> {* if the language for this is active on local the status is green and opposite. *}
                        <th class="center">{l s='Remote' mod='amazon'}</th> {* if the participant is not in Suspended Listings (this property from the amazon response follow the connector create from marketplace) the status is green and opposite. *}
                    </tr>
                    </thead>
                    <tbody>
                ...
                <pre id="participation-debug" style="display:none;"></pre>
            </div>
        </div>
    {/if}
    ...
    <div id="amz-env-infos" class="form-group" style="display:none;">
        <label class="control-label col-lg-3">{l s='Environment' mod='amazon'}</label>{if !$psIsGt15}<br/>{/if}
    ...
    {if !$informations.ps_order_id}
        <div class="form-group">
            <label class="control-label col-lg-3">{l s='Order ID' mod='amazon'}</label>{if !$psIsGt15}<br/>{/if}
            <div class="margin-form amz-info col-lg-9 col-lg-offset-3">
                <div class="alert alert-warning warn">
                    <p>
                        <span>{l s='You should enter you Order ID in Amazon tab' mod='amazon'}</span>
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="form-group">
        {if !$amazon.is_lite}
        <label class="control-label col-lg-3">{l s='PHP Settings' mod='amazon'}</label>{if !$psIsGt15}<br/>{/if}


        <div align="left" class="margin-form amz-info col-lg-9">
          
                    <div class="info {if $psIsGt15}alert alert-info{/if}">
                          <p>
        ...
        <div align="left" class="margin-form amz-info col-lg-9 col-lg-offset-3">
            {if ! $informations.php_info_ok}
                ...
                {/foreach}
            {else}
                <p class="amz-info-level-ok {if $psIsGt15}alert alert-success{/if}">
    ...
    <div class="form-group">
        <label class="control-label col-lg-3">{l s='Prestashop' mod='amazon'}</label>{if !$psIsGt15}<br/>{/if}
        <div align="left" class="margin-form amz-info col-lg-9">
    ...
    <div class="form-group">
        ...
        <div class="margin-form col-lg-9" id="support-information-download">
            <a class="support-url"
               href="{$informations.support_information_url|escape:'htmlall':'UTF-8'}&action=support_zip_file"
               data-file-name="{$informations.support_information_file_name|escape:'htmlall':'UTF-8'}">
    ...
    <div class="form-group">
        ...
        <div class="margin-form col-lg-9">
            <a target="_blank" href="{$informations.support_information_url|escape:'htmlall':'UTF-8'}&action=support_zip_file">
                {l s='Mirror link' mod='amazon'}
            </a>
        </div>
    </div>

    <div class="form-group">
        <label class="control-label col-lg-3">&nbsp;</label>{if !$psIsGt15}<br/>{/if}
        <div align="left" class="margin-form amz-info col-lg-9">
            <input type="button" class="button btn" id="support-information-prestashop"
                   value="{l s='Prestashop Info' mod='amazon'}"
                   rel="{$informations.support_information_url|escape:'quotes':'UTF-8'}&action=prestashop-info"/>&nbsp;&nbsp;
            <input type="button" class="button btn" id="support-information-php" value="{l s='PHP Info' mod='amazon'}"
                   rel="{$informations.support_information_url|escape:'quotes':'UTF-8'}&action=php-info"/>&nbsp;&nbsp;
            <span rel="amazon-expert-mode" class="amazon-expert-mode">
                ...
                <input type="button" class="button btn" id="support-mode_dev"
                       ...
                       rel="{$informations.support_information_url|escape:'quotes':'UTF-8'}&action=mode-dev"/>&nbsp;&nbsp;
            </span>
