...
<div class="{$alert_class.error|escape:'quotes':'UTF-8'}" id="amazon-remove-failed-orders-error" style="display: none; margin-top: 20px;"></div>
...
<table class="table table-hover" id="cron_failed_orders" style="width: 100%; margin-bottom:30px; display:none;">
    ...
    {foreach from=$cron_failed_orders item=failed_order}
        <tr class="row_hover">
            ...
            <td>
                {if ($failed_order->attempt == 1)}
                    {$failed_order->reason|escape:'htmlall':'UTF-8'}
                {else}
                    {foreach from=$failed_order->reason item=reason}
                        {$reason|escape:'htmlall':'UTF-8'} <br/>
                    {/foreach}
                {/if}
            </td>
        </tr>
