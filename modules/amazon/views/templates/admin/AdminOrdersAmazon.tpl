...
<ul id="menuTab" class="nav">
    <li id="menu-import" class="menuTabButton selected">
        <a href="#"><span>&nbsp;<img src="{$images|escape:'quotes':'UTF-8'}import-big-1.png"
                                     alt="{l s='Import' mod='amazon'}"/>&nbsp;&nbsp;{l s='Import' mod='amazon'}</span></a>
    </li>
    <li id="menu-report" class="menuTabButton">
        <a href="#"><span>&nbsp;<img src="{$images|escape:'quotes':'UTF-8'}report-big.png"
                                     alt="{l s='Report' mod='amazon'}"/>&nbsp;&nbsp;{l s='Report' mod='amazon'}</span></a>
    </li>
</ul>

<div id="tabList" class="panel">
    {if $active_import_orders}
        <div id="menudiv-import" class="tabItem selected" rel="import">
                ...
                <fieldset class="panel">
                    {if $ps16x}
                        <div class="panel-heading">
                            <img src="{$images|escape:'quotes':'UTF-8'}cog.gif" alt=""
                                 class="middle"/> {l s='Parameters' mod='amazon'}
                        </div>
                    {else}
                        <legend>
                            <img src="{$images|escape:'quotes':'UTF-8'}cog.gif" alt=""
                    ...
                    {if $ps16x}<h2 style="color:silver">{l s='Import Orders' mod='amazon'}</h2>{else}<h3
                            style="color:silver">{l s='Import Orders' mod='amazon'}</h3>{/if}

                    <table class="amz-options">
                        <tr>
                            <td>
                                    ...
                                    <tr>
                                        {if isset($fba) && $fba}
                                            <td>{l s='Channel' mod='amazon'}&nbsp;&nbsp;
                                                <select type="text" name="channel" title="{l s='Channel' mod='amazon'}">
                                        ...
                                        {/if}
                                        <td>{l s='From' mod='amazon'}
                                            <input type="text" id="datepickerFrom" name="datepickerFrom" title="From"
                                                   value="{$start_date|escape:'quotes':'UTF-8'}" style="width:100px"/>
                                        </td>
                                        <td>{l s='To' mod='amazon'}
                                            <input type="text" id="datepickerTo" name="datepickerTo" title="To"
                                                   value="{$current_date|escape:'quotes':'UTF-8'}" style="width:100px"/>
                                        </td>
                                        <td>{l s='Status' mod='amazon'}
                                            <select type="text" name="statuses" id="statuses"
                                                    title="{l s='Status' mod='amazon'}">
                                                <option value="All">{l s='Retrieve all orders status' mod='amazon'}</option>
                                                <option value="Pending">{l s='Pending - This order is pending on the MarketPlace' mod='amazon'}</option>
                                                <option value="Unshipped"
                                                        selected="selected">{l s='Unshipped - This order is awaiting shipment' mod='amazon'}</option>
                        ...
                        <tr>
                            <td>
                                <div class="checkbox" style="display: none">
                                    <label><input type="checkbox" name="demo_mode" value="1"/> Demo mode (fake
                                        order)</label>
                                </div>
                            </td>
                        </tr>
            ...
            {include file=$tpl_path|escape:'quotes':'UTF-8'|cat:'views/templates/admin/AdminOrdersFailedCron.tpl' cron_failed_orders=$cron_failed_orders}

            <span><strong>{l s='Orders' mod='amazon'}</strong></span>
            <form action="#" id="amazonOrders" name="amazonOrders" method="POST">
                <table class="table order table-hover" cellpadding="0" cellspacing="0"
                    ...
                    <tr class="row_hover" id="order-model" style="display:none;">
                        ...
                        <td class="center" rel="flag">
                            <img src="{$images|escape:'quotes':'UTF-8'}/geo_flags/fr.gif" alt="fr"/></td>
                        ...
                        <td class="center" rel="business">&nbsp;</td>
                        <td class="center" rel="quantity">&nbsp;</td>
                        <td class="right" rel="total">&nbsp;</td>
                        {if $experimental}
                            <td class="right" rel="invoice">&nbsp;</td>
                        {/if}
                    </tr>
                    </tbody>
                </table>
            ...
            <div style="margin-top:80px;"><br/></div>
            <br/>
            ...
            <div class="{$alert_class.warning|escape:'quotes':'UTF-8'}" id="amazon-import-warning"
                 style="margin-top:20px;"></div>
            <div class="{$alert_class.error|escape:'quotes':'UTF-8'}" id="amazon-import-error"
                 style="margin-top:20px;"></div>
    ...
    {else}
        <div class="alert alert-warning">
            <p>Please enable the import order feature on the Amazon configuration page's tab Features.</p>
        </div>
    {/if}
