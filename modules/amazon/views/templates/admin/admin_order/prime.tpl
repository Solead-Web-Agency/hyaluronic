...
{if $prime.enable}
    ...
    <div id="amazon-order-prime-data" data-id-order="{$prime.id_order|intval}" data-marketplace-id="{$prime.marketplace_id|escape:'htmlall':'UTF-8'}">
        <!-- Error -->
        {if ($prime.data && $prime.data.has_error)}
            <div id="amazon-shipping-service-errors">
                <div class="alert alert-danger">
                    <p>{l s='Getting shipping services has the following errors:' mod='amazon'}</p>
                    <ul>
                        {foreach from=$prime.data.errors item=prime_error}
        ...
        <!-- Prime logic -->
        {if !$prime.initialized || $prime.data.step === 0}
        ...
        {elseif $prime.data.step === 1}
            ...
            <select id="amazon-order-prime-carriers">
                <option value=""></option>
                {foreach from=$prime.data.carriers key=service_id item=carrier}
        ...
        {elseif $prime.data.step === 2}
            <div class="amazon-order-prime-selection">{l s='The label was created.' mod='amazon'}</div>
            <div class="amazon-order-prime-selection">
                {l s='Shipment ID:' mod='amazon'} <span>{$prime.data.shipment_id|escape:'htmlall':'UTF-8'}</span>
            </div>
            <a class="button btn btn-primary" href="{$prime.data.label_url|escape:'htmlall':'UTF-8'}"
               title="{l s='Open the shipment label' mod='amazon'}"
               target="_blank"> <i class="icon-file-text"></i> {l s='Print label' mod='amazon'}</a>
        {else}
            <p>{l s='Something went wrong! Error while loading Prime order.' mod='amazon'}</p>
            <p>{l s='Unrecognize Prime status!' mod='amazon'}</p>
        {/if}
    </div>
    
    <div id="amazon-order-prime-get-exist-label" class="hidden">
        <input type="text" title="Testing only"
               value="{$prime.data && $prime.data.shipment_id|escape:'htmlall':'UTF-8'}" />
