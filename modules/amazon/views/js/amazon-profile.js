...
$(document).ready(function () {
    ...
    $('.profile-edit-img', $('#menudiv-profiles', context)).click(function () {
        ...
        // first fectch
        if(!$('.amazon-profile-item[rel=' + profile_id + ']').hasClass('active')) {
            $('.amazon-profile-item[rel=' + profile_id + ']').show();
    ...
    function profileEdit(profile_id) {
        ...
        $.ajax({
            type: 'POST',
            url: $('#amazon_profiles_url').val(),
            data: 'action=get_profile&profile_id=' + profile_id + '&rand=' + new Date().valueOf() + '&callback=?',
            success: function (result) {
                $('.amazon-profile-item[rel=' + profile_id + ']').html('');
            
                $('.amazon-profile-item[rel=' + profile_id + ']').slideDown('slow');
                $('.amazon-profile-item[rel=' + profile_id + ']').html(result);
                $('.amazon-profile-item[rel=' + profile_id + ']').addClass('active');
                
                var cloned = $('#profile-' + profile_id);
                
                cloned.find('input[rel^="profiles[name]"]').attr('name', 'profiles[name][_key_]');
                
                if (window.console)
                    console.log('Profile Name : ', cloned.find('input[rel^="profiles[name]"]'));
                
                onChangeSortVariant(cloned);
                onChangeMultipleSelect(cloned);
                checkMaxInputVarsAgain();
                
                return profileAdd(cloned);
            },
    ...
    function profileAdd(cloned) {
        ...
        cloned.find('.profile-universe', context).change(function () {
            if ($('.chosen-single span', context).hasClass('chosen-default')) {
                $('.chosen-single span', context).removeClass('chosen-default');
            }
        ...
        cloned.find('.supplier-price-rule-type', context).change(function () {
            var type = $(this).val();
            if (type !== 'percent' && type !== 'value')
                return (false);
            $(this).parent().find('select[rel="percent"], select[rel="value"]').hide();
        ...
        cloned.find('.supplier-price-rule-add', context).click(function () {
            var source_i = $(this).parent();
            var dest_i = $(this).parent().parent();
            ...
            cloned2.find('.supplier-price-rule-add, .supplier-price-rule-remove', context).toggle();
            cloned2.find('.supplier-price-rule-remove', context).
            click(function () {
                $(this, context).parent().remove();
            });
            if ($('#menudiv-profiles .optional-fields') && $('#menudiv-profiles .optional-fields').length)
        ...
        cloned.find('.supplier-price-rule-remove', context).click(function () {
            $(this, context).parent().remove();
        });
        ...
        var last_profile_id = Number($('#master-profile').attr('rel'));
        var new_profile_id = last_profile_id + 1;
        $('#master-profile').attr('rel', new_profile_id);
	...
	if (window.console)
            console.log('Duplicate Profile id : ', profile_id);
	
	// when original profile is not load yet
        if(!$('.amazon-profile-item[rel=' + profile_id + ']').hasClass('active')) {
	    $('.amazon-profile-item[rel=' + profile_id + ']').show();
            profileEdit(profile_id);
        }
        ...
        var new_name = $('input[rel^="profiles[name]"]', profile).val()+$('.profile-name', cloned_header).attr('rel');
    ...
    $('#menudiv-profiles', context).delegate('.type_reload', 'click', function () {
        ...
        selectUniverse($(source_select).closest('.profile.profile-stored'));
    });
    // Clear xsd file
    $('#menudiv-profiles', context).on('click', '.type_clear_xsd', function () {
        var source_select = $('select.profile-universe', $(this).parents().get(2));
        var source_div = $(source_select).closest('.profile.profile-stored');

        var profile = $('input[name^="profiles[name]"]', source_div).val();
        var universe = $('select.profile-universe', source_div).val();
        if (window.console) {
            console.log('x', source_div);
            console.log('profile', profile);
            console.log('selected universe', universe);
        }

        var loader = $('.xsd-reset', $(source_div));

        $.ajax({
            type: 'POST',
            url: $('#xsd_operations_url').val(),
            data: 'action=refreshXsd&universe=' + universe + '&callback=?',
            beforeSend: function () {
                if (!universe) {
                    console.log('The universe value is empty!');
                    return false;
                }
                loader.show();
            },
            complete: function () {
                // Clear profile selected
                $('.mandatory_fields', source_div).html('');
                $('.product_type', source_div).find('option').remove().end()
                    .append('<option value=""></option>').val('');
                $('.profile-universe', source_div).val('');
                $('.chosen-single span', source_div).addClass('chosen-default').text('Select an Option');
            ...
            success: function (result) {
                ...
                let response = $('.clear-xsd-status', $(source_div));
    ...
    $('.profile-del-img', $('#menudiv-profiles', context)).click(function () {
        ...
        $('input[rel="profiles[to_delete][' + profile_id + ']"]').attr('name', 'profiles[to_delete][' + profile_id + ']');
    });
    ...
    */
    ...
    $('#menudiv-profiles', context).delegate('.load-optionals', 'click', function () {
        ...
        if ($(source_div).hasClass('amazon-profile-item')) {
            source_div = $(this).parents().get(5);
        } 
    ...
    $('#menudiv-profiles', context).delegate('.add-optional', 'click', function () {
        //source div is obtained the same way as for ".load-optionals"" on click
        var source_div = $(this).parents().get(5);

        if ($(source_div).hasClass('amazon-profile-item')) {
            source_div = $(this).parents().get(6);
        } 
    ...
    function addOptionalField(source_div, attribute_name) {
        ...
        var escapedProfile = $.escapeSelector(profile);
        var loader = $('.' + escapedProfile).find('.xsd-load');
