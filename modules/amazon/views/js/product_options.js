...
$(document).ready(function () {
    ...
    function ProductOptionInit() {
        ...
        // Transparency code
        //
        $('.amz-propagate-transparencycode-cat').click(function () {
            if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

            propagate($(this), 'transparencycode', 'propagate-transparencycode-cat');
        });

        $('.amz-propagate-transparencycode-shop').click(function () {
            if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

            propagate($(this), 'transparencycode', 'propagate-transparencycode-shop');
        });

        $('.amz-propagate-transparencycode-manufacturer').click(function () {
            if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

            propagate($(this), 'transparencycode', 'propagate-transparencycode-manufacturer');
        });

        $('.amz-propagate-transparencycode-supplier').click(function () {
            if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);
        ...
        $('input[id^="amz-ean-asin-"]').click(function () {
            ...
            $.ajax({
                type: 'POST',
                url: $('#amazon-product-options-json-url').val(),
                data: $('.amazon-details input').serialize() + '&' + $('input[name="ean13"], input[name="upc"], input[name="marketPlaceId"],  input[name^="advertisingAPI"]').serialize() + '&action=asin-probe&rand=' + new Date() + '&id_lang=' + lang + '&callback=?',
                dataType: 'json',
                beforeSend: function (data) {
        ...
        // Brand
        //
        $('.amz-propagate-brand-cat').click(function () {
            if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

            propagate($(this), 'brand', 'propagate-brand-cat');
        });

        $('.amz-propagate-brand-shop').click(function () {
            if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

            propagate($(this), 'brand', 'propagate-brand-shop');
        });

        $('.amz-propagate-brand-manufacturer').click(function () {
            if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

            propagate($(this), 'brand', 'propagate-brand-manufacturer');
        });

        $('.amz-propagate-brand-supplier').click(function () {
            if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);
        ...
        // Manufacturer
        //
        $('.amz-propagate-manufacturer-cat').click(function () {
            if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

            propagate($(this), 'manufacturer', 'propagate-manufacturer-cat');
        });

        $('.amz-propagate-manufacturer-shop').click(function () {
            if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

            propagate($(this), 'manufacturer', 'propagate-manufacturer-shop');
        });

        $('.amz-propagate-manufacturer-manufacturer').click(function () {
            if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

            propagate($(this), 'manufacturer', 'propagate-manufacturer-manufacturer');
        });

        $('.amz-propagate-manufacturer-supplier').click(function () {
            if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);
        ...
        // Brand
        //
        $('.amz-propagate-product_tax_override-cat').click(function () {
            if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

            propagate($(this), 'product_tax_override', 'propagate-product_tax_override-cat');
        });

        $('.amz-propagate-product_tax_override-shop').click(function () {
            if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

            propagate($(this), 'product_tax_override', 'propagate-product_tax_override-shop');
        });

        $('.amz-propagate-product_tax_override-manufacturer').click(function () {
            if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

            propagate($(this), 'product_tax_override', 'propagate-product_tax_override-manufacturer');
        });

        $('.amz-propagate-product_tax_override-supplier').click(function () {
            if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);
