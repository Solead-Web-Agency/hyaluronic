...
$(document).ready(function () {
    ...
    $(function () {
        ...
        const $loader = $('#participation-loader'),
            $error = $('#participation-error'),
            $debug = $('#participation-debug');
        $.ajax({
            type: 'POST',
            url: $('#check_url').val(),
            dataType: 'json',
            data: {action: 'participations'},
            beforeSend: function() {
                $loader.show();
                $error.hide();
            },
            ...
            success: function (data) {
                ...
                if (!data.error) {
                    for (const merchant in data.participations) {
                        for (const marketplace in data.participations[merchant]) {
                            const marketplaceInfo = data.participations[merchant][marketplace];
                            const line = $('#amz-participation-model').clone().appendTo('table.table.amz-participation tbody').show();
                    ...
                    $error.html($('#infos_ajax_error').val()).show();
                }
    ...
    //
    $(function () {
        logtime('Amazon ping', false);
        const $guidelineWrapper = $('#amz-env-infos'),
            $guideline = $('#error-service_check_url');

        function showGuideline() {
            $guidelineWrapper.show();
            $guideline.show();
        }

        $.ajax({
            type: 'GET',
            url: $('#service_check_url').val(),
            dataType: 'json',
            success: function (data) {
                if (!data.pass) {
                    showGuideline();
    ...
    $('#maintenance-delete-logs').click(function () {
        // Valid Values loader
        $(function () {
    ...
    $('.addnewcarrier', $('div[id^="carrier-group"]', context)).click(function () {
        ...
        var indexX = $('#incoming-' + id_lang).children('div.carrier-group').length - 1;
        ...
        // Incoming Carrier index
        var indexIC = $('#new-carriers-' + id_lang).children('div.carrier-group').length + indexX;
        cloned.attr('id', '#carrier-group-' + id_lang + '-' + newIndex).find('.amazon_carrier').attr('name', 'incoming_carrier[' + id_lang + '][' + indexIC + '][amazon_carrier]');
        cloned.attr('id', '#carrier-group-' + id_lang + '-' + newIndex).find('.carrier').attr('name', 'incoming_carrier[' + id_lang + '][' + indexIC + '][carrier]');
        cloned.attr('id', '#carrier-group-' + id_lang + '-' + newIndex).find('.pickup_point').attr('name', 'incoming_carrier[' + id_lang + '][' + indexIC + '][pickup_point]');
    ...
    // for prime order carriers
    $('.addnewprimecarrier', $('div[id^="prime-carrier-group"]', context)).click(function () {
        var lang = $('input[name=selected_tab]').val();
        var id_lang = $('#id-lang-' + lang).val();
        var indexX = $(this).attr('rel');

        cloned = $('#prime-carrier-group-' + id_lang + '-' + indexX).clone().appendTo('#new-prime-carriers-' + id_lang);
        newIndex = parseInt(indexX) + 1;

        cloned.attr('id', '#prime-carrier-group-' + id_lang + '-' + newIndex).find('select').val(0);

        cloned.find('.addnewprimecarrier').remove();
        cloned.find('.removeprimecarrier').show().attr('rel', newIndex).
        click(function () {
            $(this).parent().remove();
        });
    ...
    $('.removeprimecarrier', context).click(function () {
        $(this).parent().remove();
    });
    ...
    $('input[name=amz-checkme]').click(function () {
        var state = Boolean($(this).prop('checked'));
        $('input[id^=category]').prop('checked', state);
    });

    /*$('input[name=amz-checkme]', $('#menudiv-categories')).click(function ()
    {
    ...
    $('.sp_check_connection').click(function () {
        const $parent = $(this).parents('.sp_api_authorization').first(),
            $loader = $parent.find('.check-loader'),
            $responseSuccess = $parent.find('.sp-check-response-success'),
            $responseError = $parent.find('.sp-check-response-error'),
            marketplaceId = $(this).data('marketplaceId'),
            langId = $(this).data('lang-id');
        const $sellerId = $('.custom-authorize-' + langId + ' .seller_id').val(),
            $refreshToken = $('.custom-authorize-' + langId + ' .refresh_token').val();
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: $('#check_url').val(),
            ...
            success: function (data) {
                if (data.success) {
                    $responseSuccess.text(data.msg).slideDown();      
                } else {
                    $responseError.text(data.reason).slideDown();
                }
            },
            error: function () {
                $responseError.text('Unexpected error').slideDown();
            }
        });
    });

    /* ON/OFF enter authorize */
    $('.authorize_switch').on('change', function () {
        let langId = $(this).attr('data-lang_id');
        if ($(this).val() == 0) {
            $('.custom-authorize-' + langId).css('display', 'none');
        } else {
            $('.custom-authorize-' + langId).css('display', 'block');
        }
    });
    ...
    $('.import-config-type', context).click(function () {
        $('.import-config-type').toggle();
        $('.order-import-advanced-settings').slideToggle();
    });

    $('.hook-config-type', context).click(function () {
        $('.hook-config-type').toggle();
        $('.hook-settings').slideToggle();
    });

    $('.config-amz-europe', context).click(function () {
        $('.config-amz-europe').toggle();
        $('.amz-europe-advanced-settings').slideToggle();
    });
    ...
    //
    // ON/OFF alternate product image upload
    //
    $('input[name="alternate_product_image"]').on('change', function () {
        console.log($(this).val());
        if ($(this).val() == 0) {
            $('.alternate_product_image_upload').css('display', 'none');
        } else {
            $('.alternate_product_image_upload').css('display', 'block');
        }
    });
    ...
    $('#marketplace-column-move-right', $('#menudiv-parameters')).click(function () {
        var result = !$('#selected-marketplace_columns option:selected').remove().appendTo('#available-marketplace_columns');
        $('#selected-marketplace_columns option').prop('selected', true);
        return result;
    });
    $('#marketplace-column-move-left', $('#menudiv-parameters')).click(function () {
        return !$('#available-marketplace_columns option:selected').remove().appendTo('#selected-marketplace_columns');
    });

    $('input[name="submit"], button[name="submit"]').click(function () {
        //$('#available-suppliers option').attr('selected', true); //prevent submission
        $('#selected-suppliers option').attr('selected', true);
        //$('#available-manufacturers option').attr('selected', true);//prevent submission
    ...
    $('.price-rule-type', $('#content .tabItem')).change(function () {
        ...
        $(this).parent().find('span[rel="percent"], span[rel="value"]').hide();
        $(this).parent().find('span[rel="' + type + '"]').show();
        $(this).parent().find('input[rel="percent"], input[rel="value"]').hide();
        $(this).parent().find('input[rel="' + type + '"]').show();
    });

    /*
     * General Supplier Price Rules
     */
    $('.supplier-price-rule-add', $('#content .tabItem')).click(function () {
        var source_i = '';
        source_i = $(this).parent();

        var dest_i = $(this).parent().parent();
        var is_first_rule = $(dest_i).find('.supplier-price-rule').length;
        ...
        cloned.find('.supplier-price-rule-add, .supplier-price-rule-remove').toggle();
        cloned.find('.supplier-price-rule-remove').
        click(function () {
            $(this).parent().remove();
        });
    });
    $('.supplier-price-rule-remove', $('#content .tabItem')).click(function () {
        $(this).parent().remove();
    });
    $('.supplier-price-rule-type', $('#content .tabItem')).change(function () {
        var type = $(this).val();
        console.log($(this));
        if (type !== 'percent' && type !== 'value')
            return (false);
    ...
    */
    ...
    $('#cron-params .cron-checkbox').click(function (e) {
        var $selectedParam = $(this),
            cron_name = $selectedParam.parents('.cron-checkbox-group').data('cronName'),
            param_name = $selectedParam.data('paramName'),
            params = '',
            index_checked_param = 0,
            replaced_urls = [],
            $psCronData = $('#prestashop-cronjobs-params'),
            atLeastOne = $selectedParam.hasClass('cron_checkbox_at_least_one');
        $selectedParam.parents('.cron-checkbox-group').find('[data-param-name="' + param_name + '"]').each(function () {
            if ($(this).is(':checked')) {
        ...
        if (!params && atLeastOne) {
            $selectedParam.prop('checked', 'true');
