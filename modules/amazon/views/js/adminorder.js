...
$(document).ready(function () {
    ...
    // 2020-07-28: Tran moved Prime to separate file

    $(document).on('keydown keyup', '#custom_text_for_label', function(e) {
	var textString = $(this).val();
	var textLength = $(this).attr('alt');
	if(textString.length > textLength) {
	    $(this).parent().find('#custom_text_for_label_warning').show();
	    $(this).val(textString.substring(0, textLength));
	} else {
	    $(this).parent().find('#custom_text_for_label_warning').hide();
	}
    });
    ...
    $(document).on('click', '#save_shipping_service', function () {
        var $saveBtn = $(this),
            endpoint = $saveBtn.data('href'),
            idOrder = $saveBtn.data('idOrder'),
            shippingService = $('#order_info_shipping_service').val();

        if (!endpoint || !idOrder || $saveBtn.hasClass('saving')) {
            return;
        }
        ...
        $.ajax({
            ...
            url: endpoint,
            dataType: 'json',
            data: { id_order: idOrder, shipping_service: shippingService },
            beforeSend: function () {
                $saveBtn.addClass('saving').find('i').addClass('icon-refresh icon-spin').removeClass('icon-truck');
            },
            complete: function () {
                $saveBtn.removeClass('saving').find('i').addClass('icon-truck').removeClass('icon-refresh icon-spin');
            },
            success: function (data) {
                if (!data.success) {
    ...
    // Glossary
    $('.glossary_target').each(CommonServices.tippingGlossary);
});
