...
$(document).ready(function () {
    ...
    // Transparency code
    //
    $('.amazon-tab', amazon_context).delegate('.amz-propagate-transparencycode-cat', 'click', function () {
        if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

        propagate($(this), 'transparencycode', 'propagate-transparencycode-cat');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-transparencycode-shop', 'click', function () {
        if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

        propagate($(this), 'transparencycode', 'propagate-transparencycode-shop');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-transparencycode-manufacturer', 'click', function () {
        if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

        propagate($(this), 'transparencycode', 'propagate-transparencycode-manufacturer');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-transparencycode-supplier', 'click', function () {
        if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);

        propagate($(this), 'transparencycode', 'propagate-transparencycode-supplier');
    });
    ...
    // Brand
    //
    $('.amazon-tab', amazon_context).delegate('.amz-propagate-brand-cat', 'click', function () {
        if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

        propagate($(this), 'brand', 'propagate-brand-cat');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-brand-shop', 'click', function () {
        if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

        propagate($(this), 'brand', 'propagate-brand-shop');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-brand-manufacturer', 'click', function () {
        if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

        propagate($(this), 'brand', 'propagate-brand-manufacturer');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-brand-supplier', 'click', function () {
        if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);

        propagate($(this), 'brand', 'propagate-brand-supplier');
    });

    // Mnufacturer
    //
    $('.amazon-tab', amazon_context).delegate('.amz-propagate-manufacturer-cat', 'click', function () {
        if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

        propagate($(this), 'manufacturer', 'propagate-manufacturer-cat');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-manufacturer-shop', 'click', function () {
        if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

        propagate($(this), 'manufacturer', 'propagate-manufacturer-shop');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-manufacturer-manufacturer', 'click', function () {
        if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

        propagate($(this), 'manufacturer', 'propagate-manufacturer-manufacturer');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-manufacturer-supplier', 'click', function () {
        if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);

        propagate($(this), 'manufacturer', 'propagate-manufacturer-supplier');
    });

    // product_tax_override
    //
    $('.amazon-tab', amazon_context).delegate('.amz-propagate-product_tax_override-cat', 'click', function () {
        if (!confirm($('#amz-text-propagate-cat').val()))  return (false);

        propagate($(this), 'product_tax_override', 'propagate-product_tax_override-cat');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-product_tax_override-shop', 'click', function () {
        if (!confirm($('#amz-text-propagate-shop').val()))  return (false);

        propagate($(this), 'product_tax_override', 'propagate-product_tax_override-shop');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-product_tax_override-manufacturer', 'click', function () {
        if (!confirm($('#amz-text-propagate-manufacturer').val()))  return (false);

        propagate($(this), 'product_tax_override', 'propagate-product_tax_override-manufacturer');
    });

    $('.amazon-tab', amazon_context).delegate('.amz-propagate-product_tax_override-supplier', 'click', function () {
        if (!confirm($('#amz-text-propagate-supplier').val()))  return (false);

        propagate($(this), 'product_tax_override', 'propagate-product_tax_override-supplier');
    });
    ...
    $('.amazon-tab', amazon_context).delegate('.amz-probe-asin', 'click', function (ev) {
        ...
        var $this = $(this),
            ...
            $loader = $container.find('img.asin-loader'),
            $selectedItem = $('input[name=complex_id_product]:checked'),
            $itemContainer = $($selectedItem.parents().get(1)),
            field_ean = $itemContainer.find('td[rel="ean13"]').text().trim(),
            field_upc = $itemContainer.find('td[rel="upc"]').text().trim();
        ...
        $.ajax({
            type: 'POST',
            url: $('#amazon-product-options-json-url').val() + '&' + $('input[name^=amazon_token]').serialize(),
            dataType: 'json',
            data: {
                action: 'asin-probe',
                id_product: $('#amazon-id-product').val(),
                id_product_attribute: String($selectedItem.data('idProductAttribute')),
                id_lang: id_lang,
                ean13: field_ean,
                upc: field_upc,
            },
            ...
            success: function (data) {
                if (data.success) {
                    $container.find('input').val(data.asin).addClass('asin-probed');
                    showSuccessMessage($('#amazon-product-options-message-success').val());
                } else if (data.reason) {
    ...
    // 2022-05-20 - Tran removed unused .amz-probe-multi-asin click handler
                    ...
                    $.ajax({
                        success: function (data) {
                            ...
                            if (data.error) {
                                ...
                                showErrorMessage(data.output ? data.output : $('#amazon-product-options-message-error').val());
                            }
                            else
                                showSuccessMessage($('#amazon-product-options-message-success').val());
