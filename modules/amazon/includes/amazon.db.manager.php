...
class AmazonDBManager
{
    ...
    /**
     * Old carriers, Amazon carriers, and pickup points are being mapped to new data - incoming carriers.
     * @return void
     */
    public function migrateCarrierMappingIncomingCarrier()
    {
        if (!AmazonConfiguration::getIdByName('AMAZON_' . AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS)) {
            $result = array();
            $carriers = AmazonConfiguration::get('CARRIER');
            $amazon_carriers = AmazonConfiguration::get('AMAZON_CARRIER');
            $pickup_points = AmazonConfiguration::get('MKP_CARRIER_PICKUP_POINTS');

            if (is_array($carriers) && is_array($amazon_carriers) && count($carriers) == count($amazon_carriers)) {
                foreach ($carriers as $id_lang => $carrier) {
                    $mappingOfLang = array();
                    if ($carrier && is_array($carrier)) {
                        foreach ($carrier as $index => $psCarrier) {
                            if (!$psCarrier || !isset($amazon_carriers[$id_lang]) || !isset($amazon_carriers[$id_lang][$index])) {
                                continue;
                            }
                            $mappingOfLang[$index]['carrier'] = $psCarrier;
                            $mappingOfLang[$index]['amazon_carrier'] = $amazon_carriers[$id_lang][$index];
                            if (isset($pickup_points[$id_lang]) && isset($pickup_points[$id_lang][$index])) {
                                $mappingOfLang[$index]['pickup_point'] = $pickup_points[$id_lang][$index];
                            }
                        }
                    }

                    $result[$id_lang] = $mappingOfLang;
                }
            }

            AmazonConfiguration::updateValue(AmazonConstant::CONFIG_MKP_INCOMING_CARRIERS, $result);
        }
    }
    ...
    public function addMarketPlaceTables()
    {
        return $this->addTableMkpOrders()
            ...
            && $this->removeTableAmzProductLang(); // TODO: Remove function removeTableAmzProductLang
    }
    ...
    private function addTableMkpOrders()
    {
        ...
        // Missing column latest_ship_date
        if (!AmazonTools::fieldExistsR($mkpOrderTbl, 'latest_ship_date')) {
            $result = $result && $this->executeStatement("ALTER TABLE `$mkpOrderTbl` ADD COLUMN `latest_ship_date` datetime DEFAULT NULL");
        }

        // Missing column marketplace_id
        if (!AmazonTools::fieldExistsR($mkpOrderTbl, 'marketplace_id')) {
            $result = $result && $this->executeStatement("ALTER TABLE `$mkpOrderTbl` ADD COLUMN `marketplace_id` VARCHAR(16) NULL DEFAULT NULL");
        }
    ...
    protected function addTableMkpProductOption()
    {
        ...
        if (!AmazonTools::tableExists($productOptionTbl)) {
            $sql = 'CREATE TABLE IF NOT EXISTS `' . $productOptionTbl . '` (
                ...
                `id_shop` INT(11) UNSIGNED NULL DEFAULT 1,
                `transparencycode` varchar(64) DEFAULT NULL,
                `product_tax_override` varchar(64) default null,
                `id_mpo_webservice` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
                UNIQUE KEY `id_product_2021_02_08` (`id_product`, `id_product_attribute`, `id_lang`, `id_shop`),
                KEY `ASIN` (`asin1`),
                PRIMARY KEY (`id_mpo_webservice`)
            ) ENGINE=InnoDB ' . $this->tableOptions;
        ...
        $updateFields = array(
            ...
            'id_shop' => 'INT(11) UNSIGNED NULL DEFAULT 1',
            'transparencycode' => 'varchar(64) DEFAULT NULL AFTER `latency`',
            'brand' => 'varchar(64) DEFAULT NULL AFTER `alternative_description`',
            'manufacturer' => 'varchar(64) DEFAULT NULL AFTER `brand`',
            'product_tax_override' => 'varchar(64) DEFAULT NULL ',
            'id_mpo_webservice' => 'INT(11) UNSIGNED NOT NULL AUTO_INCREMENT AFTER `product_tax_override`, ADD PRIMARY KEY(`id_mpo_webservice`)',
        );
    ...
    protected function removeTableAmzProductLang()
    {
        $tableName = _DB_PREFIX_ . 'amazon_product_lang';
