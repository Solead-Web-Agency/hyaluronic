...
class AdvancedVatManagerCron17ModuleFrontController extends ModuleFrontController
{
    public $auth = false;
    public $ssl = true;
    public $ajax;
    ...
    public function display()
    {
        ...
        if (Tools::substr(Tools::hash('advancedvatmanager'), 0, 12) != Tools::getValue('token')) {
        ...
        if (Tools::getValue('id_shop') == Context::getContext()->shop->id) {
            ...
            if ($addresses) {
                foreach ($addresses as $address) {
                    ...
                    if (!ValidationEngine::$skip_validation_process) {
                        ...
                        if ($cv->addCustomersVAT(ValidationEngine::getVat(), $id_customer, $id_address, ValidationEngine::getVATValidation(),ValidationEngine::$company_valid, Configuration::get('ADVANCEDVATMANAGER_CRON_AUTOINSERT_COMPANY')?ValidationEngine::getRegisteredCompanyName():null, ValidationEngine::getStatus(), ValidationEngine::getSystemFail())) {
                             $this->ajaxRender(sprintf('The address with ID#%s and VAT number %s has been saved into the Customer VAT list',$id_address, ValidationEngine::getVat()).'<br />');
                        }
                        else {
                            $this->ajaxRender(sprintf('The address with ID#%s and VAT number %s could not be saved into the Customer VAT list',$id_address, ValidationEngine::getVat()).'<br />');
                        }
                        ...
                        if (!$ve->getVATValidation()) {
                            if (Configuration::get('ADVANCEDVATMANAGER_CRON_DELETE_EMPTY') && !ValidationEngine::getVat()) {
                                ...
                                if (Db::getInstance()->update('address', array('deleted' => '1'), 'id_address ='.(int)$id_address)) {
                                    $this->ajaxRender(sprintf('The address with ID#%s has been deleted',$id_address).'<br />');
                                }
                                else {
                                    $this->ajaxRender(sprintf('The address with ID#%s could not be deleted',$id_address).'<br />');    
                                }  
                            } 
                            ...
                            if (Configuration::get('ADVANCEDVATMANAGER_CRON_SENDEMAIL_EMPTY') && !ValidationEngine::getVat()) {
                                if ($module->sendEmail(2, CustomersVAT::getCustomerAddress($id_customer, $id_address))) {
                                    $this->ajaxRender(sprintf('Email to customer ID#%s has been sent successfully.',$id_customer).'<br />');    
                                }
                                else {
                                    $this->ajaxRender(sprintf('Email to customer ID#%s could not be sent due an error.',$id_address).'<br />');
                                }    
                            } 
                ...
                return $this->ajaxRender('No address to check.'); 
            }
        }
        else {
