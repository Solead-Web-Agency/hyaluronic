...
{if $gupsellproducts}
    ...
    <div class="gupsellpro-box{if $version17} gps17 {/if} " data-gupsell-id="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
        <section class="page-product-box gupsellpro gupsellpro17 {if isset($ajaxcal) && $ajaxcal}gupsell_products_ajax{/if} gupsellbox-allproducts gupsellbox-checkproducts" id="gupsellpro-{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}" data-gupsell-id="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
            ...
            <div class="upsellOptionsfrequentlyprice">
                                ...
                                <div class="gupsell-Price-text">
                                    ...
                                    {if $upsellObj->apply_discount == 1 && $amountdiscount > 0}
                                        ...
                                        <span class="gupselldiscount">
                                            {l s='SAVE ' mod='g_upsellpro'}
                                            <div class="gupselldiscount-amount-action-label">
                                                {convertPrice price=$gupselltotalpricediscount}
                                                <small>
                                                </small>
                                            </div>
                                        </span>
                ...
                {if $html_template != 'popup' && $html_template != 'floating'}
                    <div class="gupsellfooter-popup-action gupsell-button-addtocart checkout-button" data-id-upsell="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
    ...
    {if $html_template == 'popup' || $html_template == 'floating'}
            <div class="gupsellfooter-popup-action">
                ...
                {/if}
                <button type="button" class="button btn btn-default gupsellbtn-default {if $upsellObj->remove_product == 0}gupsell-addcart-checkout{else}gupsell-updatecart-checkout{/if}" disabled {if $html_template == 'floating'} style="width:100%" {/if}>
                    {if $upsellObj->remove_product == 0}
                        {$getConfigFieldsValues['GSELL_SETTING_BUTTON_CHECKOUT'][$id_lang]|escape:'html':'UTF-8'}
                    {else}
                        {$getConfigFieldsValues['GSELL_SETTING_BUTTON_UPGRADECART'][$id_lang]|escape:'html':'UTF-8'}
