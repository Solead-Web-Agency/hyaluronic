...
{if $gupsellproducts}
    ...
    <div class="gupsellpro-box{if $version17} gps17 {/if}" data-gupsell-id="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
        <section class="page-product-box gupsellpro gupsellpro17 {if isset($ajaxcal) && $ajaxcal}gupsell_products_ajax{/if} gupsellbox-allproducts gupsellbox-checkproducts" id="gupsellpro-{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}" data-gupsell-id="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
            ...
            {foreach from=$gupsellproducts key=numberkey item='product' name=product}
                <div class="item {cycle values="odd,even"} col-xs-12 {if $numberkey == 0} active {/if} {if $mostpopular == $product.id_product}item_popular{/if}" data-type="volume" data-product-id="{$product.id_product|escape:'html':'UTF-8'}"  for="checkbox-for-{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}-{$product.id_product|escape:'html':'UTF-8'}">
                            ...
                            <div class="item-contentaction-content-left">
                                <div class="gupsellpro_des">
                                    <div itemprop="name" class="product-name">
                                        <a href="#" title="{$product.name|htmlspecialchars}"><span>{if $upsellObj->qty > 0 }{$upsellObj->qty|escape:'html':'UTF-8'}{else}1{/if}</span> {l s='Quantities' mod='g_upsellpro'}{*$product.name|escape:'html':'UTF-8'*}</a>
                                    </div>
                                </div>
