    ...
    <div class="{if $html_template != 'popup'}gupsellpro-box {if $version17} gps17 {/if}{/if} {if $html_template == 'floating'}floating-product-box-show-product gnone{/if} {if isset($ajaxcal) && $ajaxcal}gupsell_products_ajax{/if} " data-gupsell-id="{$upsellObj->id_g_upsellrule|escape:'html':'UTF-8'}">
            ...
            {if $html_template != 'popup' && $html_template != 'floating'}
                {if $count_product == 1} 
                    <div class="gupsellfooter-popup-action gupsellonly_action">
                        <button type="button" class="btn gupsellbtn gupsellbtn-default {if $upsellObj->remove_product == 0}gupsell-addcart-checkout{else}gupsell-updatecart-checkout{/if}">
                            {if $upsellObj->remove_product == 0}
        ...
        {if $html_template == 'floating'}
            <div class="upsel-footer-floating">
                <div class="upsel-footer-floating-title-checkout {if $count_product > 1}gnone{/if}">
                    {if $count_product == 1} 
                        <button type="button" class="btn gupsellbtn gupsellbtn-default {if $upsellObj->remove_product == 0}gupsell-addcart-checkout{else}gupsell-updatecart-checkout{/if}">
                            {if $upsellObj->remove_product == 0}
                                {$getConfigFieldsValues['GSELL_SETTING_BUTTON_CHECKOUT'][$id_lang]|escape:'html':'UTF-8'}
    ...
    {if $html_template == 'popup'}
                <div class="gupsellfooter-popup gupselltext-center">
                    ...
                    <div class="button-footer-popup">
                        ...
                        {else}
                            <a type="button" class="btn gupsellbtn gupsellbtn-default {if $count_product > 1}gnone{/if}" href="{$cart_show|escape:'html':'UTF-8'}">
                                {$getConfigFieldsValues['GSELL_SETTING_BUTTON_CHECKOUT'][$id_lang]|escape:'html':'UTF-8'} <i class="pull-right">→</i>
                            </a>
                        {/if}
                    </div>
                </div>
