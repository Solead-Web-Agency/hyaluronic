...
{if $prefix_right}
    <div class="input-group-{if $is_symfony_page}prepend{else}addon{/if}">
        <span class="input-group-text">{$prefix_right|escape:'htmlall':'UTF-8'}</span>
    </div>
{/if}
...
{if $helper}
    <p
        {if $helper_attr}
            {if !array_key_exists('class', $helper_attr)}
                class="help-block"
            {/if}

            {foreach $helper_attr as $key => $value}
                {if $value !== true}
                    {if $key == 'class'}
                        class="help-block {$value|escape:'htmlall':'UTF-8'}"
                    {else}
        ...
        {/if}
    >
        {$helper|escape:'htmlall':'UTF-8'}
    </p>
{/if}

{* MARGIN-FORM - CLOSED TAG *}
{if $margin_form_attr}
    </div>
{/if}

{* FORM-GROUP - CLOSED TAG *}
{if $form_group}
    </div>
{/if}