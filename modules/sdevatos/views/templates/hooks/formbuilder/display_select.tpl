...
<select
    {if $attr}
        {if !array_key_exists('class', $attr) && $is_symfony_page}
            class="custom-select"
        {/if}
        ...
        {foreach $attr as $key => $value}
            {if $value !== true && $value !== false}
                {if $key == 'class'}
                    class="{if $is_symfony_page}custom-select{/if} {$value|escape:'htmlall':'UTF-8'}"
                {else}
...
>
    ...
    {if $default_option}
        {if is_array($default_option)}
            {foreach $default_option as $key => $option name=default_options}
                <option value="{$key|escape:'htmlall':'UTF-8'}"
                    {if $default_option_attr}
                        {foreach $default_option_attr as $key => $value}
                            {if $value !== true}
                                {$key|escape:'htmlall':'UTF-8'}="{$value|escape:'htmlall':'UTF-8'}"
                            {else}
                                {$key|escape:'htmlall':'UTF-8'}="{$key|escape:'htmlall':'UTF-8'}"
