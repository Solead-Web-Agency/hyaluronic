...
class SmsLinkPaymentMethod extends StandardPaymentMethod
{
    ...
    public function getResourceDetail($resource_id = '')
    {
        ...
        if (!is_string($resource_id) || !$resource_id) {
            $this->logger->addLog('SmsLinkPaymentMethod::getResourceDetail() - Invalid argument given, $resource_id must be a non empty string.');
    ...
    protected function updateOrderStateFromPendingToPaid($order_id = 0, $is_live = true)
    {
        ...
        if (!is_int($order_id) || !$order_id) {
            $this->logger->addLog('SmsLinkPaymentMethod::updateOrderState() - Invalid argument given, $order_id must be a non null integer.', 'error');

            return false;
        }

        if (!is_bool($is_live)) {
            $this->logger->addLog('SmsLinkPaymentMethod::updateOrderState() - Invalid argument given, $is_live must be a valid boolean.', 'error');
        ...
        if (!$this->validate_adapter->validate('isLoadedObject', $order)) {
            $this->logger->addLog('SmsLinkPaymentMethod::updateOrderState() - Invalid argument given, $order getted must be a valid object.', 'error');

            return false;
        }

        $state_addons = $is_live ? '' : '_test';
        $pending_os = $this->configuration->getValue('order_state_sms_link' . $state_addons);
        ...
        if (!$update_order_history) {
            $this->logger->addLog('SmsLinkPaymentMethod::updateOrderState() - Failed to update order state', 'error');
