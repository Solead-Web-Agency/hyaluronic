...
jQuery(document).ready(function() {
	$("input:radio[name=filter_prices], input:radio[name=filter_stock], input:radio[name=filter_weight]").click(function() {
		toggleFields($(this).attr('name'));
	});

	$("input:radio[name=show_text], input:radio[name=show_text_prodpage], input:radio[name=countdown], input:radio[name=countdown_prodpage]").click(function() {
		toggleFields($(this).attr('name'));
	});

	toggleFields('filter_prices');
	toggleFields('filter_stock');
	toggleFields('filter_weight');
	toggleFields('show_text');
	toggleFields('show_text_prodpage');
	toggleFields('countdown');
	toggleFields('countdown_prodpage');


	if (jQuery('#groupinc_configuration_form #mode').val() == 2 || jQuery('#groupinc_configuration_form #mode').val() == 3) {
		jQuery('#groupinc_configuration_form #type').val(0);
		jQuery('#groupinc_configuration_form #type').closest(":has(label)").slideUp();
		jQuery('#price_calculation').closest(":has(label)").slideUp();
		jQuery('#percentage').closest(":has(label)").slideUp();
		jQuery('#percentage').val('');
		jQuery('#fix').closest(":has(label)").slideDown();
	} else {
		jQuery('#groupinc_configuration_form #type').closest(":has(label)").slideDown();
		jQuery('#price_calculation').closest(":has(label)").slideDown();
		jQuery('#percentage').closest(":has(label)").slideDown();
		jQuery('#fix').closest(":has(label)").slideUp();
	}
	...
	jQuery('#groupinc_configuration_form #mode').change(function() {
		if ($(this).val() == 2 || $(this).val() == 3) {
			...
			jQuery('#groupinc_configuration_form #type').closest(":has(label)").slideDown();
			jQuery('#price_calculation').closest(":has(label)").slideDown();
			jQuery('#price_application').closest(":has(label)").slideDown();
			jQuery('#percentage').closest(":has(label)").slideDown();
			jQuery('#fix').closest(":has(label)").slideUp();
		}
		jQuery('#groupinc_configuration_form #type').change();
	});
...
function toggleFields(fieldName)
{
    if ($('#'+fieldName+'_on').is(':checked')) {
        $('.form-group').each(function() {
        	if (fieldName == 'filter_prices') {
        		if ($(this).find('#fp_product_price_on').length > 0 || $(this).find('#fp_attribute_price_on').length > 0) {
        			$(this).slideDown('slow');
        		}
        	}
