...
require_once dirname(__FILE__) . '/classes/RppsValidator.php';
...
class TheCheckout extends Module
{
    ...
    private function _initPsKernel() {
        ...
        if(!$kernel){
            ...
            $kernel = new \AppKernel(_PS_MODE_DEV_ ? 'dev' : 'prod', _PS_MODE_DEV_); // <-- despite validator thinks this is virtual class, it is not
            $kernel->boot();
        }
    }

    public function initPsKernel() {
        $initForModules = array('myparcelnl', 'buckaroo3');
        $doInit = false;
...
//                    ),
    ...
    /**
     * Hook pour valider les adresses avec RPPS
     */
    public function hookActionValidateCustomerAddressForm($params)
    {
        ...
        // Valider l'adresse pour RPPS
        $validation = RppsValidator::validateAddressForOrder($address);
        
        if (!$validation['valid']) {
            // Ajouter l'erreur au champ "other"
            if ($otherField = $form->getField('other')) {
                $otherField->addError($validation['error']);
            }
            return false;
        }
    ...
    /**
     * Hook pour filtrer les méthodes de paiement
     */
    public function hookPaymentOptions($params)
    {
        ...
        foreach ($params['payment_options'] as $key => $paymentOption) {
            $moduleName = $paymentOption->getModuleName();
            
            if (!RppsValidator::isPaymentMethodAllowed($moduleName, $cart)) {
                unset($params['payment_options'][$key]);
            }
        }
    }

    /**
     * Hook pour valider l'étape complète (validation finale)
     */
    public function hookActionValidateStepComplete($params)
    {
        if (!isset($params['step']) || $params['step'] !== 'addresses') {
            return true;
        }
        ...
        if ($cart->id_address_delivery) {
            ...
            $validation = RppsValidator::validateAddressForOrder($deliveryAddress, $cart);
            
            if (!$validation['valid']) {
                $params['errors'][] = $validation['error'];
                return false;
            }
        }
