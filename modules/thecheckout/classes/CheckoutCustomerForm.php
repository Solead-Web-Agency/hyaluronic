...
class CheckoutCustomerForm extends AbstractForm
{
    ...
    public function __construct(
        Smarty $smarty,
        Context $context,
        $translator, // PS 1.7.8+: PrestaShopBundle\Translation\TranslatorInterface, older PS versions: Symfony\Component\Translation\TranslatorInterface
...
//    }
    ...
    public function validate($silentRegistration = false)
    {
        ...
        if (!empty($birthdayField)) {
            ...
            if (!empty($birthdayValue)) {
                $dateBuilt = DateTime::createFromFormat($this->localContext->language->date_format_lite,
                    $birthdayValue);
                if (!empty($dateBuilt)) {
        ...
        if (method_exists('Validate', 'isAcceptablePasswordLength') &&
            ...
            if (Validate::isAcceptablePasswordScore($passwordField->getValue()) === false) {
                ...
                if ($this->localContext->shop->theme->get('global_settings.new_password_policy_feature') !== true) {
                    $zxcvbn = new Zxcvbn();
    ...
    {
        if ($this->localContext->customer->isLogged()) {
    ...
    private function validateByModules()
    {
        ...
        foreach ($formFieldsAssociated as $moduleName => $formFields) {
            if ($moduleId = Module::getModuleIdByName($moduleName)) {
                ...
                if ($moduleName === 'dm_gdpr') {
                    // dm_gdpr module relies on $_GET['password'] and $_GET['dm_gdpr_active'], so we need to set them to global context
                    parse_str(Tools::getValue('account'), $_GET);
                }
