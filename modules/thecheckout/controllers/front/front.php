...
class TheCheckoutModuleFrontController extends ModuleFrontController
{
    ...
    private $isAmazonPayCheckout = false;
    ...
    public function init()
    {
        ...
        // In PS 8.x, classes/controller/FrontController.php was updated to call CartRule::autoRemoveFromCart($this->context);
        // this causes call in classes/Product.php: $address = Address::initialize($id_address, true); - which caches old address
        Cache::clean('Address::initialize_*');
...
//                }
                ...
                if ($entity === 'Country') {
                    ...
                    if ($this->module->config->force_customer_to_choose_country && empty($addressData)) {
                        $guessCountry = false; // Guess country based on selected language, set to true to enforce guessing when force_customer_to_choose_country is ON
...
//                       }
    ...
    private function setupFormFieldsInvoice()
    {
        ...
        if (
            ...
            // Verify that address belongs to current customer
            if ($addressData->id_customer > 0 && $addressData->id_customer != $this->context->customer->id) {
                // Address doesn't belong to current customer, clear cart's invoice address
                $this->context->cart->id_address_invoice = 0;
                $this->context->cart->save();
                $addressData = array();
            }
            // Also exclude addresses with "auto" values
            if ($addressData && isset($addressData->alias) && $addressData->alias === 'auto') {
                // This is likely a problematic auto-generated address, don't use it
                $this->context->cart->id_address_invoice = 0;
                $this->context->cart->save();
                $addressData = array();
            }
    ...
    private function setupFormFieldsDelivery()
    {
        ...
        if (
            ...
            // Verify that address belongs to current customer
            if ($addressData->id_customer > 0 && $addressData->id_customer != $this->context->customer->id) {
                // Address doesn't belong to current customer, clear cart's delivery address
                $this->context->cart->id_address_delivery = 0;
                $this->context->cart->save();
                $addressData = array();
            }
...
//            (null == trim($formFieldsInvoiceMapped['vat_number']['value']));
        ...
        if (Tools::isSubmit('submitReorder') && $id_order = (int)Tools::getValue('id_order')) {
            ...
            if (!$duplication || !Validate::isLoadedObject($duplication['cart'])) {
                ...
                if ($this->module->config->use_old_address_on_reorder) {
                    ...
                    // Verify addresses belong to current customer before reusing them
                    if (!$oldDeliveryAddress->deleted && 
                        ($oldDeliveryAddress->id_customer == $this->context->customer->id || $oldDeliveryAddress->id_customer == 0)) {
                        $this->context->cart->id_address_delivery = $oldCart->id_address_delivery;
                    }
                    if (!$oldInvoiceAddress->deleted && 
                        ($oldInvoiceAddress->id_customer == $this->context->customer->id || $oldInvoiceAddress->id_customer == 0)) {
    ...
    public function initContent()
    {
        ...
        // Clean up problematic addresses before processing
        $this->cleanupProblematicAddresses();
...
//            $this->context->cookie->opc_form_radios = json_encode($opc_form_radios);
            ...
            if ($this->context->customer->isLogged()) {
                ...
                if ($this->context->cart->id != $this->context->cookie->addreses_reset_at_cart_id) {
                    ...
                    if (count($lastOrderAddresses)) {
                        if (!$this->module->config->use_old_address_on_reorder) {
                            // Verify addresses belong to current customer before assigning
                            $invoiceAddr = new Address($lastOrderAddresses['id_address_invoice']);
                            $deliveryAddr = new Address($lastOrderAddresses['id_address_delivery']);
                            
                            if ($invoiceAddr->id && 
                                ($invoiceAddr->id_customer == $this->context->customer->id || $invoiceAddr->id_customer == 0)) {
                            ...
                            if ($deliveryAddr->id && 
                                ($deliveryAddr->id_customer == $this->context->customer->id || $deliveryAddr->id_customer == 0)) {
            ...
            if ($this->module->config->force_email_overlay && !$this->context->customer->isLogged() && !$this->context->customer->id && !$this->module->config->move_login_to_account) {
            ...
            $page["body_classes"]["checkout-option-guest"]     = $this->module->config->move_login_to_account && Configuration::get('PS_GUEST_CHECKOUT_ENABLED');
            $page["body_classes"]["checkout-option-customer"]  = $this->module->config->move_login_to_account && !Configuration::get('PS_GUEST_CHECKOUT_ENABLED');
            ...
            $this->context->smarty->assign(array(
                ...
                'prestashop_version_8_plus'          => version_compare(_PS_VERSION_, '8.0.0', '>='),
...
//            $option['price'] = " -- [price] '" . $option['price'] . "' == [freeShippingLabel] '" . $freeShippingLabel ."' (". ($option['price'] === $freeShippingLabel) .")";
    ...
    public function getPaymentOptions()
    {
        ...
        $opc_form_radios        = json_decode($this->context->cookie->opc_form_radios, true);
        if (isset($opc_form_radios['paymentOptionModuleName']) && !empty($opc_form_radios['paymentOptionModuleName'])) {
            $this->selected_payment_option = $opc_form_radios['paymentOptionModuleName'];
        } else {
...
//        die(json_encode($result));
        $options = defined('JSON_INVALID_UTF8_SUBSTITUTE') ? JSON_INVALID_UTF8_SUBSTITUTE : 0; // this was introduced in PHP 7.2, so treat old versions with fallback
        die(json_encode($result, $options));
    }
    ...
    private function ajaxRefreshCartSummary()
    {
    ...
    private function ajaxSelectPaymentOption()
    {
        ...
        if (Tools::getValue('selectedPaymentModuleName') !== null) {
            $opc_form_radios        = json_decode($this->context->cookie->opc_form_radios, true);
            $opc_form_radios['paymentOptionModuleName'] = Tools::getValue('selectedPaymentModuleName');
            $this->context->cookie->opc_form_radios = json_encode($opc_form_radios);
        }
    ...
    private function getShippingOptionsBlock()
    {
        ...
        // 20.01.2025 - this has been removed, now we'll decide only based by customer's choice - if chosen, use that, if not, use null (=Prestashop's default)
//        if (version_compare(_PS_VERSION_, '1.7.3') >= 0) {
//            $actualDeliveryOptions = json_decode($this->context->cart->delivery_option, true);
//        } else {
//            $actualDeliveryOptions = Tools::unSerialize($this->context->cart->delivery_option);
//        }
        ...
        if (isset($opc_form_radios['delivery_option'])) {
            $customerSelectedDeliveryOption = reset($opc_form_radios['delivery_option']);
            $this->context->smarty->assign('customerSelectedDeliveryOption',
                $customerSelectedDeliveryOption);
        }

        @$this->getCheckoutSession()->setDeliveryOption(
            // if customer did not make deliberate choice, let's reset to null so that default shipping for current cart/address gets chosen
            ($customerSelectedDeliveryOption)?array($this->context->cart->id_address_delivery => $customerSelectedDeliveryOption):null
        );
...
//            $this->context->smarty->assign(array('customer' => $this->getTemplateVarCustomer(2)));
        ...
        $this->context->smarty->assign(
            array(
                ...
                'force_email_wait_for_enter'                 =>
                    $this->module->config->force_email_overlay && $this->module->config->register_guest_on_blur && !$this->context->customer->isLogged() && !$this->context->customer->id && Configuration::get('PS_GUEST_CHECKOUT_ENABLED'),
                'wait_for_account'                           =>
    ...
    private function cleanupProblematicAddresses()
    {
        $hasChanges = false;
        
        // Check invoice address
        if ($this->context->cart->id_address_invoice > 0) {
            $invoiceAddr = new Address($this->context->cart->id_address_invoice);
            
            // Remove addresses with "auto" alias or that don't belong to the customer
            if ($invoiceAddr->alias === 'auto' || 
                ($invoiceAddr->id_customer > 0 && $invoiceAddr->id_customer != $this->context->customer->id)) {
                $this->context->cart->id_address_invoice = 0;
                $hasChanges = true;
            }
        }
        ...
        if ($this->context->cart->id_address_delivery > 0) {
            $deliveryAddr = new Address($this->context->cart->id_address_delivery);
            
            // Remove addresses with "auto" alias or that don't belong to the customer
            if ($deliveryAddr->alias === 'auto' || 
                ($deliveryAddr->id_customer > 0 && $deliveryAddr->id_customer != $this->context->customer->id)) {
                $this->context->cart->id_address_delivery = 0;
                $hasChanges = true;
            }
        }
    ...
    private function getTransientAddressByCartId($cart_id)
    {
        ...
        // Add customer ID verification to prevent loading other customers' addresses
        if ($this->context->customer->id > 0) {
            $query->where('(id_customer = ' . (int)$this->context->customer->id . ' OR id_customer = 0 OR id_customer IS NULL)');
        } else {
            $query->where('(id_customer = 0 OR id_customer IS NULL)');
        }
...
//            });
        ...
        if (isset($formData['id_state']) && (!isset($states_flattened) || !is_array($states_flattened) || !in_array($formData['id_state'], $states_flattened))) {
            unset($_POST['id_state']);
            unset($formData['id_state']);
        }
...
//                        );
    ...
    private function ajaxCheckEmail()
    {
        ...
        if ($cannotOrderAsGuest && $id_customer) {
            ...
            $accountResult['notices']['isCustomerAndGuestDisabled'] = 1;

        } elseif (
            (/*$this->module->config->force_email_overlay || */$this->module->config->register_guest_on_blur && "forced_email_continue" !== Tools::getValue('triggerEl'))
            && Configuration::get('PS_GUEST_CHECKOUT_ENABLED')) {
        ...
        if (Configuration::get('PS_GUEST_CHECKOUT_ENABLED') && $id_customer && !$cannotOrderAsGuest) {
            $accountResult['notices']['email'] = $this->module->getTranslation('You already have an account with us. Sign in or continue as guest.');
            $accountResult['notices']['isEmailUsed'] = 1;
        }

        $accountResult['notices']['guestCheckoutEnabled'] = (Configuration::get('PS_GUEST_CHECKOUT_ENABLED') == '1');
    ...
    private function ajaxModifyAddressSelection()
    {
        ...
        if ((-1 == $addressId)
            ...
            if ($this->context->customer->isLogged()
                ...
                // Additional security check for non-logged or guest customers
                $address = new Address($addressId);
                if ($address->id && 
                    ($address->id_customer == 0 || 
                     $address->id_customer == $this->context->customer->id ||
                     ($this->context->customer->isGuest() && !$address->id_customer))) {
                    $shallGenerateNewAddressBlock = $this->assignNewAddressIdToCart($addressType, $addressId);
                }
            }
        }
...
//            }
    ...
    private function _formatPrice($value) {
        if (version_compare(_PS_VERSION_, '8.0.0', '>=')) {
            ...
            return $locale->formatPrice((float) $value, $this->context->currency->iso_code);
        } else {
            return Tools::displayPrice(Tools::ps_round($value, _PS_PRICE_COMPUTE_PRECISION_)); // <-- for older PS versions only
        }
    }
    ...
    private function addPaymentFee($cart, $paymentFee)
    {
        ...
        if (isset($cartCopy['subtotals']['tax'])) {
            ...
            $cartCopy['subtotals']['tax']['value']  = $this->_formatPrice($totalTaxes);
        }
        ...
        $cartCopy['totals']['total_excluding_tax']['value']  = $this->_formatPrice($totalTaxExcl);
        ...
        $cartCopy['totals']['total_including_tax']['value']  = $this->_formatPrice($totalTaxIncl);
        ...
        $cartCopy['totals']['total']['value']  = $this->_formatPrice($total);

        $fee_value = $this->_formatPrice($paymentFee);
    ...
    private function wrapInNonZeroCustomerIdForVATDeduction($embed)
    {
        ...
        $customerNotSet       = (int)$this->context->cookie->id_customer == 0;
        // VATNUMBER_MANAGEMENT comes from EU VAT module, which is no longer used, so we do hardcode it to false here
        $vatManagementEnabled = false; // Configuration::get('VATNUMBER_MANAGEMENT');
        ...
        if ($customerNotSet && $vatManagementEnabled) {
            ...
            if (count($allCustomers)) {
                $this->context->cookie->id_customer = $allCustomers[0]['id_customer'];
        ...
        if ($customerNotSet && $vatManagementEnabled) {
            ...
            $cart = new Cart($this->context->cookie->id_cart);
            if (isset($cart->id) && $cart->id && $id_address_delivery && $id_address_invoice) {
                $cart->id_customer = 0;
    ...
    private function getCartSummaryBlock($paymentFee = 0)
    {
        ...
        // Probably this could go for PS 1.7.6+, as Cart::areProductsAvailable() was introduced; but it seems it's been buggy in PS 1.7.x versions, so let's keep it to 8+
        if ($this->isShopVersion8Plus()) {
            $cartController = new CartController();

            $originalAction = $_POST['action'] ?? null;
            // Temporarily set $_POST['action'] to 'show', that's necessary for init() call below
            $_POST['action'] = 'show';
            $isAvailable = $cartController->init();

            // Restore the original value of $_GET['action']
            ...
            if (true !== $isAvailable && isset($cartController->errors) && is_array($cartController->errors) && !empty($cartController->errors)) {
                $cartQuantityError = $cartController->errors[0];
            }
        } else {
        ...
        $hidepriceFilePath = _PS_MODULE_DIR_ . 'hideprice/hideprice.php';

        if (file_exists($hidepriceFilePath)) {
            include_once($hidepriceFilePath);

            if (class_exists('HidePriceConfiguration')) {
                $hideprice_module = new HidePriceConfiguration();
                $hideprice_context = $this->context;
        ...
        $this->context->smarty->assign(array(
            ...
            'carrierSelectedObject'           => new Carrier($this->context->cart->id_carrier, $this->context->language->id)
        ));
