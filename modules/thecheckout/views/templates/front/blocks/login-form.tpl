...
{if !$z_tc_config->move_login_to_account}<div class="inner-wrapper">{/if}
  <div id="checkout-personal-information-step" class="express-checkout checkout-step -current -reachable js-current-step -clickable"><div class="content"></div></div>
  <div class="offer-login {if $z_tc_config->move_login_to_account}expanded{/if}">
  ...
  <div class="overlay-email">
      <button class="btn btn-secondary use-another-email" data-link-action="x-use-another-email-overlay">{l s='Use another email address.' mod='thecheckout'}</button>
      <div class="login-opened">
        <div class="new-customer-email">{l s='Please enter your email address to continue.' mod='thecheckout'}</div>
        <div class="email-input">
          <input type="text" name="forced-email" placeholder="{l s='Your email address' d='Shop.Forms.Labels'}" aria-label="{l s='Your email address' d='Shop.Forms.Labels'}">
          <button class="btn btn-primary" id="forced_email_continue" data-link-action="x-forced-email-continue" type="button" class="form-control-submit">
        ...
        <div class="overlay-general-error" style="display: none" id="overlay-general-error"></div>
        <div class="notice-already-registered" style="display: none;">
          <div class="msg-sign-in-or-guest">{l s='You already have an account with us. Sign in or continue as guest.' mod='thecheckout'}</div>
          <div class="buttons">
            <div class="btn btn-primary" data-link-action="x-offer-login-overlay">{l s='Sign in' mod='thecheckout'}</div>
            <div class="btn btn-primary" data-link-action="x-offer-guest-overlay">{l s='Continue as guest' mod='thecheckout'}</div>
          </div>
        </div>
        <div class="notice-has-to-login" style="display: none;">
          <div class="msg-has-to-login">{l s='The email is already used, please sign in' mod='thecheckout'}</div>
          <div class="buttons">
            <div class="btn btn-primary" data-link-action="x-offer-login-overlay">{l s='Sign in' mod='thecheckout'}</div>
          </div>
        </div>
        <div class="notice-guest-or-customer" style="display: none;">
          <div class="msg-sign-up-or-guest">{l s='No account yet? Sign up to save your details for future purchases or continue as a guest.' mod='thecheckout'}</div>
          <div class="buttons">
            <div class="btn btn-primary" data-link-action="x-offer-register-overlay">{l s='Sign up' mod='thecheckout'}</div>
            <div class="btn btn-primary" data-link-action="x-offer-guest-overlay">{l s='Continue as guest' mod='thecheckout'}</div>
          </div>
        </div>
        <div class="notice-has-to-signup" style="display: none;">
          <div class="msg-has-to-signup">{l s='No account yet? Sign up to save your details for future purchases.' mod='thecheckout'}</div>
          <div class="buttons">
            <div class="btn btn-primary" data-link-action="x-offer-register-overlay">{l s='Sign up' mod='thecheckout'}</div>
          </div>
        </div>
