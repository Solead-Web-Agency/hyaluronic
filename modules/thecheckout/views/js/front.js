...
const copyShippingToOrderReview = () => {
    ...
    if (selectedOption) {
        ...
        const cleanExtraContent = extraContent ? extraContent.cloneNode(true) : null;

        if (cleanExtraContent) {
            // Remove all hidden inputs
            cleanExtraContent.querySelectorAll('input[type="hidden"]').forEach(el => el.remove());

            // Remove all modal elements
            cleanExtraContent.querySelectorAll('.modal').forEach(el => el.remove());
        }
        ...
        const reviewContent = `
            ...
            <div class="carrier-extra-content">${cleanExtraContent ? cleanExtraContent.innerHTML : ''}</div>
        `;
...
$(document).ready(function () {
    ...
    $('body').on('click', '[data-link-action=x-forced-email-continue]', function () {
        ...
        if (!tc_helper_validateEmail(enteredEmail)) {
            ...
            checkEmail('.overlay-email', $(this), function (jsonData) {
                $('.notice-guest-or-customer').hide();
                $('.notice-already-registered').hide();
                $('.notice-has-to-login').hide();
                $('.notice-has-to-signup').hide();

                if ('undefined' !== typeof jsonData.notices && 'undefined' !== typeof jsonData.notices['isEmailUsed'] && jsonData.notices['isEmailUsed'] === 1) {
                    $('.notice-already-registered').show();
                    return false;
                }

                if ('undefined' !== typeof jsonData.notices && 'undefined' !== typeof jsonData.notices['isCustomerAndGuestDisabled'] && jsonData.notices['isCustomerAndGuestDisabled'] === 1) {
                    $('.notice-has-to-login').show();
                    return false;
                }

                if (jsonData.hasErrors) {
                    if ('undefined' !== typeof jsonData.errors && 'undefined' !== typeof jsonData.errors['email']) {
                        $('.overlay-general-error').text(jsonData.errors['email']).show();
                    }
                } else {
                    $('.overlay-general-error').text('').hide();
                    if ('undefined' !== typeof jsonData.notices && 'undefined' !== typeof jsonData.notices['guestCheckoutEnabled']) {
                        if (jsonData.notices['guestCheckoutEnabled']) {
                            $('.notice-guest-or-customer').show();
                        } else {
                            $('.notice-has-to-signup').show();
                        }
                    }
                }
            });
    ...
    $('body').on('click', '[data-link-action=x-offer-login-overlay]', function (event) {
        openLoginForm();
        $('.checkout-block #login-form [name=email]').val($('.overlay-email [name=forced-email]').val());
        $('.checkout-block #login-form [name=password]').focus();
        $('.overlay-email .notice-already-registered').hide();
        $('.overlay-email .notice-has-to-login').hide();
        $('.overlay-email').addClass('login-opened');
        return false;
    });

    $('body').on('click', '[data-link-action=x-use-another-email-overlay]', function (event) {
        $('.checkout-block #login-form [name=email]').val($('.overlay-email [name=forced-email]').val());
        $('.checkout-block #login-form [name=password]').focus();
        $('.overlay-email').removeClass('login-opened');
        return false;
    });

    $('body').on('click', '[data-link-action=x-offer-guest-overlay]', function (event) {
        var enteredEmail = $('[name=forced-email]').val();
        checkEmail('form.account-fields', $(this), function (jsonData) {
            if (!jsonData.hasErrors) {
                $('body').removeClass('force-email-overlay');
                // if register_guest_on_blur is enabled, this will silently register email address
                $('#thecheckout-account [name=email]').val(enteredEmail).trigger('change');
                // prestashop.on('thecheckout_checkEmail', function(data) { console.log('email checked!', data.reason); })
                prestashop.emit('thecheckout_checkEmail', {
                    reason: 'x-offer-guest-overlay',
                });
            }
        });
    });

    $('body').on('click', '[data-link-action=x-offer-register-overlay]', function (event) {
        var enteredEmail = $('[name=forced-email]').val();
        var url = prestashop?.urls?.pages?.register;
        if (url) {
            if (url.indexOf('?') > -1 && url.indexOf('?') < url.length - 1) {
                url += '&email=' + enteredEmail;
            }
            location.href = url;
        }
    });

    $('body').on('click', '[data-link-action^=x-checkout-option]', function (event) {
        $(this).closest('.checkout-options-buttons').find('button').removeClass('active');
        $(this).addClass('active');
        $('body').removeClass('checkout-option-guest checkout-option-login checkout-option-customer').addClass($(this).data('link-action').replace('x-', ''));
    });
    ...
    $(document).ajaxError(function myErrorHandler(event, xhr, ajaxOptions, thrownError) {
        ...
        // Change with custom JS: let tc_disableAjaxErrorOverlay = true;
        var disableAjaxErrorOverlay = (typeof tc_disableAjaxErrorOverlay !== 'undefined') ? tc_disableAjaxErrorOverlay : false;

        if (ajaxOptions.data && !disableAjaxErrorOverlay) {
            // Convert data to a query string (if not already)
            const dataString = typeof ajaxOptions.data === 'string' ? ajaxOptions.data : $.param(ajaxOptions.data);

            // Check if ajax_request parameter is present
            if (dataString.includes('ajax_request')) {
        ...
        console.info('Ajax Options:', ajaxOptions);
    });
...
function closeLoginForm() {
    $('.offer-login').removeClass('expanded');
    $('.checkout-block #login-form').fadeOut();
...
function updateQuantityFromInput(el) {
    ...
    const $el = $(el);  // even if el is null/undefined, $el will be an empty jQuery object
    const $container = $el.closest('.qty-container');
    if ($container.length) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            if ($container.length) {
                removeQtyLoadingTimeout = setTimeout(function () {
...
function checkEmail(accountFormSelector, triggerEl, callback) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            // when 'continue' button on email overlay was used, just check email and return result to callback method
            if ("forced_email_continue" !== triggerEl.attr('id')) {
                if (jsonData.hasErrors) {
                    ...
                    if (triggerEl.attr('id') === 'tc_save_account') { scrollToError(); }
                } else {
...
function _getExtraAccountParams() {
    ...
    // dpdbaltics module
    $('.carrier-extra-content.dpdbaltics:visible [name]').each(function () {
        var inputName = $(this).attr('name');
        var inputValue = encodeURIComponent($(this).val());

        if (inputName === 'dpd-phone' ||
            inputName === 'dpd-phone-area' ||
            inputName === 'dpd-city' ||
            inputName === 'dpd-street') {

            extraAccountParams += '&' + inputName + '=' + inputValue;
        }
    });

    // correosoficial module (yes, the module name is correctly written with single 'f')
    $('.carrier-extra-content.correosoficial:visible [name]').each(function () {
        var inputName = $(this).attr('name');
        var inputValue = encodeURIComponent($(this).val());

        if (inputName.startsWith('SearchCityPaqByCPInput_') ||
            inputName.startsWith('CityPaqSelect_') ||
            inputName.startsWith('SearchOfficeByCPInput_') ||
            inputName.startsWith('OfficeSelect_')) {

            extraAccountParams += '&' + inputName + '=' + inputValue;
        }
    });

    // Add g-recaptcha-response if it exists
    if ($('[name=g-recaptcha-response]').length) {
        extraAccountParams += '&g-recaptcha-response=' + encodeURIComponent($('[name=g-recaptcha-response]').val());
    }

    // blpaczka
    if ($('#blpaczka_carrier_list_hook_container:visible [name=blpaczka_pudo_api_type]').length === 1) {
        extraAccountParams += '&blpaczka_pudo_api_type=' + encodeURIComponent($('#blpaczka_carrier_list_hook_container:visible [name=blpaczka_pudo_api_type]').val());
    }

    if ($('#blpaczka_carrier_list_hook_container:visible [name=blpaczka_pudo_code]').length === 1) {
        extraAccountParams += '&blpaczka_pudo_code=' + encodeURIComponent($('#blpaczka_carrier_list_hook_container:visible [name=blpaczka_pudo_code]').val());
    }
...
function signIn() {
    ...
    let requestData = "&ajax_request=1&action=signIn&" +
        $('.checkout-block #login-form').serialize() +
        dynamicToken();

    // Add g-recaptcha-response if it exists
    // Important consideration: this will work only for initial login attempt, not for subsequent ones - as the ets_recaptcha_free does not implement token refresh
    if ($('[name=g-recaptcha-response]').length) {
        requestData += '&g-recaptcha-response=' + encodeURIComponent($('[name=g-recaptcha-response]').val());
    }

    resetRecaptcha(); // theoretically would work only for V2 recaptcha, for V3 we'd need to request new token, if captcha module implements it
...
function updateShippingPrices(shippingBlockHtml) {
    if ('undefined' !== typeof shippingBlockHtml && null !== shippingBlockHtml) {
        var $parsedShippingBlock = $(shippingBlockHtml.trim());

        $parsedShippingBlock.find('.delivery-option').each(function() {
            var $parsedDeliveryOption = $(this);
            var carrierRefMatch = $parsedDeliveryOption.attr('class')?.match(/carrier-ref-\d+/);
            var carrierRef = carrierRefMatch ? carrierRefMatch[0] : null;
...
// Meant to be called from other modules, if they modify something in cart summary that requires refresh
function tc_refreshCartSummary() {
    // url - implicitly using current
    $.ajax({
        ...
        cache: false,
        dataType: "json",
        data: "ajax_request=1&action=refreshCartSummary" + "&token=" + static_token,
        success: function (jsonData) {
...
function selectPaymentOption(optionId, paymentFee) {
    ...
    // url - implicitly using current
    let selectedPaymentModuleName = payment.getSelectedOptionModuleName();
    $.ajax({
        customPropAffectedBlocks: '#thecheckout-cart-summary',
        ...
        cache: false,
        dataType: "json",
        data: "selectedPaymentModuleName=" + selectedPaymentModuleName + "&optionId=" + optionId + "&payment_fee=" + paymentFee + "&ajax_request=1&action=selectPaymentOption" + "&token=" + static_token,
        success: function (jsonData) {
...
function updatePaymentBlock(paymentModulesList, html, checksum, triggerElementName) {
    if ('undefined' !== typeof html && null !== html && (paymentBlockChecksum != checksum || 'thecheckout-confirm' == triggerElementName)) {
        ...
        // get selected options prior to refresh;
        // 1. firstly try from data-module-name, if unavailable,
        // 2. get from newly loaded markup, if not set,
        // 3. get by ID (order) - this may not be reliable, it's a last resort
        // Try to get selected option by module name or ID, with fallbacks
        var selectedOption = '#none'; // we need method level variable, not only block scoped (thus var)
        let moduleName = payment.getSelectedOptionModuleName() || payment.getSelectedOptionModuleName(html);

        if (moduleName) {
            selectedOption = `[data-module-name=${moduleName}]`;
        } else {
            const optionId = payment.getSelectedOption();
        ...
        paymentBlockElement.find('input[type=text], select, input[name="issuer"], input[name="transferGateway"], form input[type=radio]:checked, form input[type=checkbox]:checked, textarea, input[type=date], input[type=tel]').each(function () {
            if ("undefined" !== typeof $(this).attr('id') && !$(this).is(':radio') && $(this).attr('name') !== 'pmethod') {
                var idSelector = '[id=' + CSS.escape($(this).attr('id')) + ']';
                payment_fields_values[idSelector] = $(this).val();
    ...
    // Other payment options support - if after reload there's no selected payment option, show all
    if ($('#checkout-payment-step.payment-selected').length && $('.payment-options [name=payment-option]:checked').length === 0) {
        $('#expand_other_payment_options').click();
        // when there's only 1 payment option, extra button will not show-up, so make sure to remove .payment-selected to show all payment options
        $('#checkout-payment-step.payment-selected').removeClass('payment-selected');
    }
...
function updateCartSummaryBlockAndRestoreInputs(cartSummaryBlockElement, html, activeQtyButtonCls, qtyControl) {
    ...
    // when cart block is restored, we do not need loading timeout on qty anymore (new markup will replace it)
    if (removeQtyLoadingTimeout) {
        clearTimeout(removeQtyLoadingTimeout);
    }
...
$( document ).on( "ajaxSend", function(event, jqxhr, settings) {
    if (typeof settings.data === 'string') {
        ...
        if (action) {
            ...
            // For certain actions - updateQuantity, selectDeliveryOption, selectPaymentOption, account-modification - add to body classes disable-user-inputs
            if (['updateQuantity', 'selectDeliveryOption', 'selectPaymentOption', 'account-modification'].includes(action)) {
                $('body').addClass('disable-user-input');
                clearTimeout(disableUserInputTimeout);
                // Set a new timeout to remove the class after 2 seconds, should the ajaxSuccess not be called properly
                disableUserInputTimeout = setTimeout(() => {
                    $('body').removeClass('disable-user-input');
                }, 2000);
            }
        }
    }
...
$( document ).on( "ajaxSuccess", function(event, jqxhr, settings) {
    if (typeof settings.data === 'string') {
        ...
        if (action) {
            ...
            // if there's no ongoingXhrs with keys - updateQuantity, selectDeliveryOption, selectPaymentOption, account-modification - remove disable-user-inputs class
            const actionsToCheck = ['updateQuantity', 'selectDeliveryOption', 'selectPaymentOption', 'account-modification'];
            const hasOngoingXhrs = actionsToCheck.some(action => tc_ongoingXhrs[action]);
