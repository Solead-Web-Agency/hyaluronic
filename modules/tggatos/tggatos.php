...
class TggAtos extends PaymentModule
{
	...
	public function canProcess($mode = NULL, Cart $cart = null, $skipHealthChecks = FALSE)
	{
		...
		switch ($mode)
		{
			case self::MODE_SINGLE:
				if (!$this->getConfVar(self::CNF_SINGLE)) return false;
				break;
			case self::MODE_2TPAYMENT:
				if (!$this->getConfVar(self::CNF_2TPAYMENT)) return false;
				break;
			case self::MODE_3TPAYMENT:
				if (!$this->getConfVar(self::CNF_3TPAYMENT)) return false;
				break;
			case NULL:
				break;
			default:
		...
		if (!$skipHealthChecks) {
			if (!$this->getConfVar(self::CNF_BANK) || !array_key_exists($this->getConfVar(self::CNF_BANK), $this->_banks))
				return false;
			if ($this->getConfVar(self::CNF_PRODUCTION) && !$this->getConfVar(self::CNF_MERCHANT_ID))
				return false;
			if (!$this->getConfVar(self::CNF_NO_TID_GENERATION))
			{
				if ($this->getConfVar(self::CNF_MIN_TID) > $this->getConfVar(self::CNF_MAX_TID))
					return false;
				...
				if ($last_tid >= $this->getConfVar(self::CNF_MAX_TID))
					return false;
			}
		}
	...
	public function hookPaymentOptions()
    {
        ...
        for ($paymentMode = 1; $paymentMode <= 3; ++$paymentMode) {
            ...
            $_paymentOption
                ...
                //->setLogo($this->getPathUri().'/views/img/bank_logo/'.$this->getConfVar(self::CNF_BANK).'.gif') //TODO: should we? result is really ugly with default theme...
            ;
	...
	public function getPaymentRedirectionForm($amount, Currency $currency, $mode, $mergeParams = array(), $transaction_id = NULL)
	{
		...
		$data = array();
		$params = array(
			'language' => $this->getConfVar(self::CNF_ISO_LANG) ? $this->getConfVar(self::CNF_ISO_LANG) : $this->context->language->iso_code,
			'merchant_id' => $this->getConfVar(self::CNF_PRODUCTION) ? $this->getConfVar(self::CNF_MERCHANT_ID) : $this->_demoCertificates[$this->getConfVar(self::CNF_BANK)],
			'currency_code' => $currency->iso_code_num,
			'caddie' => $this->encodeCaddieField($currency->iso_code),
			'amount' => (int)round($atosAmount),
			'pathfile' => $this->getConfVar(self::CNF_PARAM_PATH).self::PATHFILE,
		...
		} elseif (!$this->getConfVar(self::CNF_NO_TID_GENERATION)) {
		...
		{
			case self::MODE_SINGLE:
				$params['payment_means'] = $this->getConfVar(self::CNF_PAYMENT_MEANS);
				$params['capture_mode'] = $this->getConfVar(self::CNF_CAPTURE_MODE);
				$params['capture_day'] = $this->getConfVar(self::CNF_CAPTURE_DAY);
				break;
			case self::MODE_2TPAYMENT:
				$params['payment_means'] = $this->getConfVar(self::CNF_2TPAYMENT_MEANS);
				$params['capture_mode'] = 'PAYMENT_N';
				$params['capture_day'] = $this->getConfVar(self::CNF_2TPAYMENT_DELAY);
				$initialAmount = $this->defaultCurrencyConvert($this->getConfVar(self::CNF_2TPAYMENT_FP_FXD), $currency, self::CONVERT_FROM_DEFAULT) + $this->getConfVar(self::CNF_2TPAYMENT_FP_PCT) / 100 * $amount;
				if ($currency->decimals)
					$initialAmount *= 100;
				$initialAmount = str_pad((string)(int)Tools::ps_round($initialAmount), 3, '0', STR_PAD_LEFT);
				array_push($data, 'NB_PAYMENT=2', 'PERIOD='.$this->getConfVar(self::CNF_2TPAYMENT_SPACING), 'INITIAL_AMOUNT='.$initialAmount);
				break;
			case self::MODE_3TPAYMENT:
				$params['payment_means'] = $this->getConfVar(self::CNF_3TPAYMENT_MEANS);
				$params['capture_mode'] = 'PAYMENT_N';
				$params['capture_day'] = $this->getConfVar(self::CNF_3TPAYMENT_DELAY);
				$initialAmount = $this->defaultCurrencyConvert($this->getConfVar(self::CNF_3TPAYMENT_FP_FXD), $currency, self::CONVERT_FROM_DEFAULT) + $this->getConfVar(self::CNF_3TPAYMENT_FP_PCT) / 100 * $amount;
				if ($currency->decimals)
					$initialAmount *= 100;
				$initialAmount = str_pad((string)(int)Tools::ps_round($initialAmount), 3, '0', STR_PAD_LEFT);
				array_push($data, 'NB_PAYMENT=3', 'PERIOD='.$this->getConfVar(self::CNF_3TPAYMENT_SPACING), 'INITIAL_AMOUNT='.$initialAmount);
				break;
		}
		if ($this->getConfVar(self::CNF_FORCE_RETURN)) {
			array_push($data, 'NO_RESPONSE_PAGE');
		}
		if ($this->getConfVar(self::CNF_ONECLICK_ENABLE)) {
			array_push($data, 'WALLET_ID=CUSTOMER_ID');
		}
		$bBypass3DS = false;
		$iNo3DSMaxCartAmount = $this->getConfVar(self::CNF_3DS_BYPASS_UNDER_AMOUNT);
		...
		$iNo3DSWhenValidatedOrderExistsThatOld = $this->getConfVar(self::CNF_3DS_BYPASS_IF_VALIDATED_ORDER_OVER_AGE);
		...
		unset($bBypass3DS);
		$controls = str_replace("\n", '', str_replace("\r", '', $this->getConfVar(self::CNF_DATA_CONTROLS)));
		if (!empty($controls))
		{
		...
		foreach ($this->_confVarsByName as $name => $varconf)
			if (!empty($varconf['autofeed']) && !empty($varconf['atos']))
				$params[$varconf['atos']] = $this->getConfVar($name);
		...
		$customData = trim($this->getConfVar(self::CNF_CUSTOM_DATA), ' ;');
		if (!empty($customData))
		{
	...
	public function processResponse(TggAtosModuleResponseObject $response)
	{
		...
		if ($this->context->cart->orderExists())
		{
			...
			if ($response->capture_mode == 'PAYMENT_N')
			{
				...
				$timezone = new DateTimeZone($this->getConfVar(self::CNF_TID_TZ));
            ...
            $orderState = $this->getConfVar(self::CNF_OS_PAYMENT_FAILED);
            switch ($response->response_code)
			{
				case '00':
					...
					{
						case self::MODE_SINGLE:
							$orderState = $this->getConfVar(self::CNF_OS_PAYMENT_SUCCESS);
							break;
						case self::MODE_2TPAYMENT:
							$orderState = $this->getConfVar(self::CNF_2TPAYMENT_OS);
							break;
						case self::MODE_3TPAYMENT:
							$orderState = $this->getConfVar(self::CNF_3TPAYMENT_OS);
							break;
					}
					switch ($response->complementary_code)
					{
						...
						case '00':
							break;
						default:
							$orderState = $this->getConfVar(self::CNF_OS_NONZERO_COMPCODE);
							break;
					}
					break;
				case '17':
					$orderState = $this->getConfVar(self::CNF_OS_PAYMENT_CANCELLED);
					break;
			}
			if (!$orderState)
				return NULL;
			...
			if ($this->context->currency->decimals)
				$amount /= 100;
			$extraVars = array();
			$orderLog = $this->getConfVar(self::CNF_ORDER_MESSAGE) ? array() : null;
			...
			$extraVars['transaction_id'] = $response->{$this->getConfVar(self::CNF_OP_FIELD_TID)};
			$this->validateOrder(
	...
	public function getMinAmount($mode)
	{
		...
		{
			case self::MODE_SINGLE:
				return $this->getConfVar(self::CNF_MINAMOUNT);
			case self::MODE_2TPAYMENT:
				return $this->getConfVar(self::CNF_2TPAYMENT_MINAMOUNT);
			case self::MODE_3TPAYMENT:
				return $this->getConfVar(self::CNF_3TPAYMENT_MINAMOUNT);
			default:
	...
	public function getBankReturnUri($context)
	{
		...
		switch ($context)
		{
			case self::RETURN_CONTEXT_USER:
				$protocol = $this->getConfVar(self::CNF_RETURN_PROTOCOL_USER);
				$domain = $this->getConfVar(self::CNF_RETURN_DOMAIN_USER);
	...
	{
		return $this->getConfVar(self::CNF_DEBUG_MODE)
				|| (
					($gid = $this->getConfVar(self::CNF_DEBUG_GID))
	...
	{
		$DB = Db::getInstance(true);
		$timezoneStr = $this->getConfVar(self::CNF_TID_TZ);
		...
		if ($tid < $this->getConfVar(self::CNF_MIN_TID))
		{
			$this->reserveTransactionId($DB, $now, $this->getConfVar(self::CNF_MIN_TID) - 1, false, true);
			$tid = $this->reserveTransactionId($DB, $now);
		}
		if ($tid > $this->getConfVar(self::CNF_MAX_TID))
			return false;
		return $tid;
	}
	...
	{
		$max_wait_mts = microtime(true) + (int)$this->getConfVar(self::CNF_CONCURRENCY_MAX_WAIT);
		$success = false;
	...
	public function getContent()
	{
		...
		$errorLog = $this->getConfVar(self::CNF_LOG_PATH) . self::FILE_ERROR_LOG;
...
';
        ...
        if ($declaration['input'] == self::IN_NONE) {
            ...
            switch ($declaration['input']) {
                case self::IN_NONE:
                    break;
                case self::IN_TEXT:
                    $html .= '<input type="text" name="tggatos_' . Tools::htmlentitiesUTF8($name) . '" id="tggatos_conffield_' . Tools::htmlentitiesUTF8($name) . '" value="' . Tools::htmlentitiesUTF8($this->getConfVar($name)) . '" style="' . $styles . '" />';
                    break;
                case self::IN_TEXTAREA:
                    $html .= '<textarea cols="80" rows="10" name="tggatos_' . Tools::htmlentitiesUTF8($name) . '" id="tggatos_conffield_' . Tools::htmlentitiesUTF8($name) . '" style="' . $styles . '">' . Tools::htmlentitiesUTF8($this->getConfVar($name)) . '</textarea>';
                    break;
                case self::IN_SELECT:
                    ...
                    foreach ($_values as $value => $display) {
                        $html .= '<option value="' . Tools::htmlentitiesUTF8($value) . '" ' . ($this->getConfVar($name) == $value ? 'selected="selected"' : '') . '>' . Tools::htmlentitiesUTF8($display) . '</option>';
                    }
                    $html .= '</select>';
                    unset($_values);
                    break;
                case self::IN_CHECKBOX:
                    $html .= '
                        ...
                        <input type="checkbox" name="tggatos_' . Tools::htmlentitiesUTF8($name) . '" id="tggatos_conffield_' . Tools::htmlentitiesUTF8($name) . '" value="1"' . ($this->getConfVar($name) ? ' checked="checked"' : '') . ' style="' . $styles . '" />';
                    break;
                default:
	...
	{
		return array(
			'DEBUG' => $this->getConfVar(self::CNF_DEBUG_MODE) ? 'YES' : 'NO',
			'D_LOGO' => $this->getConfVar(self::CNF_CARD_IMG_PATH),
			'F_CERTIFICATE' => $this->getConfVar(self::CNF_PARAM_PATH) . self::CERTIF,
			'F_PARAM' => $this->getConfVar(self::CNF_PARAM_PATH) . self::PARMCOM,
			'F_DEFAULT' => $this->getConfVar(self::CNF_PARAM_PATH) . self::PARMCOM . '.' . $this->getConfVar(self::CNF_BANK)
		);
	}

	public function updateAtosParamFiles()
	{
		if (!$this->getConfVar(self::CNF_BANK))
			return;
		if ($this->getConfVar(self::CNF_PRODUCTION) && !$this->getConfVar(self::CNF_MERCHANT_ID))
			return;
		$this->initConfVars();
		...
		foreach ($this->_confVarsByName as $name => $declaration)
			if (!empty($declaration['parmcom']) && $this->getConfVar($name))
				$parmcom_content[] = $declaration['parmcom'] . '!' . $this->getConfVar($name) . '!';
		$parmcom = $this->getConfVar(self::CNF_PARAM_PATH) . self::PARMCOM . '.' . ( $this->getConfVar(self::CNF_PRODUCTION) ? $this->getConfVar(self::CNF_MERCHANT_ID) : $this->_demoCertificates[$this->getConfVar(self::CNF_BANK)] );
		try {
	...
	{
		if ($this->getConfVar(self::CNF_RESPONSE_LOG_TXT))
		{
			try 
			{
				$file = $this->getConfVar(self::CNF_LOG_PATH).date('Y-m-d').'.log';
				$handle = fopen($file, 'a');
		...
		if ($this->getConfVar(self::CNF_RESPONSE_LOG_CSV))
		{
			try 
			{
				$file = $this->getConfVar(self::CNF_LOG_PATH).date('Y-m-d').'.csv';
	...
	public function getMerchantIdList($prependEmptyLine = FALSE) 
	{
		...
		$path = $this->getConfVar(self::CNF_PARAM_PATH);
	...
	public function autoCheck()
	{
		...
		if (version_compare($this->version, $this->getConfVar(self::CNF_VERSION), '>'))
			$this->postUpdate();
		$errorLogFile = $this->getConfVar(self::CNF_LOG_PATH) . self::FILE_ERROR_LOG;
		...
		if (!$this->getConfVar(self::CNF_PRODUCTION))
		{
			$errorsIndex['BASIC']++;
		...
		if (!$this->getConfVar(self::CNF_NO_TID_GENERATION) && !$this->getConfVar(self::CNF_TID_TZ))
		{
			$errorsIndex['ADVANCED']++;
			$this->_errors[] = $this->l('Your transaction_id time zone is not set.');
		}
		if ($this->getConfVar(self::CNF_DEBUG_MODE))
		{
			$errorsIndex['ADVANCED']++;
			$this->_errors[] = $this->l('Debug mode is active.');
		}
		if ($this->getConfVar('CHECK_VERSION'))
		{
		...
		if (!$this->getConfVar(self::CNF_BINARIES_IN_PATH))
		{
			if (file_exists($this->getConfVar(self::CNF_BIN_PATH)) && is_dir($this->getConfVar(self::CNF_BIN_PATH)))
			{
				$cwd = getcwd();
				if (!chdir($this->getConfVar(self::CNF_BIN_PATH)))
				{
					$errorsIndex['ADVANCED']++;
					$this->_errors[] = sprintf($this->l('Unable to set binaries path ( %s ) as current working directory. ATOS binary executable call will probably fail.'), $this->getConfVar(self::CNF_BIN_PATH));
				}
				chdir($cwd);
			}
			else
			{
				$errorsIndex['ADVANCED']++;
				$this->_errors[] = sprintf($this->l('Binaries path \'%s\' does not exist, or is not a directory, or rights on it are to low to see it from PHP user. ATOS binary executable call will probably fail.'), $this->getConfVar(self::CNF_BIN_PATH));
			}
		}
		...
		if (!( $this->getConfVar(self::CNF_SINGLE) || $this->getConfVar(self::CNF_2TPAYMENT) || $this->getConfVar(self::CNF_3TPAYMENT) ))
		{
		...
		if ($this->getConfVar(self::CNF_LOG_PATH) == $this->_confVarsByName[self::CNF_LOG_PATH]['default'])
		{
		...
		if ($this->getConfVar(self::CNF_PARAM_PATH) == $this->_confVarsByName[self::CNF_PARAM_PATH]['default'])
		{
	...
	public function rawCall($bin_name, array $args = array(), $redirect_stderr_to_stdout = false)
	{
		...
		return new TggAtosModuleSystemCall(escapeshellcmd(( $this->getConfVar(self::CNF_BINARIES_IN_PATH) ? '' : $this->getConfVar(self::CNF_BIN_PATH) ) . $bin_name . $this->getConfVar(self::CNF_BIN_SUFFIX)) . $args);
	}
	...
	{
		$current_version = $this->getConfVar(self::CNF_VERSION);
		if (!$current_version)
			$current_version = '0';
		$toUpdate = array();
	...
	public function error($line, $message, $severity = 4, $object = NULL, $dieIfDevMode = true, $dieAnyway = false, $file = __FILE__)
	{
		...
		$logpath = $this->getConfVar(self::CNF_LOG_PATH);
