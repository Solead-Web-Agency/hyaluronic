...
class EDQuery
{
    ...
    {
        $q = array();
        @$additional_recent_data = (array)$map['additional']['migrate_recent_data'];
        $additional_recent_data_reset = reset($additional_recent_data);

        if ($map['entity']['taxes'][0] == 1) {
            if (!$additional_recent_data_reset) {
                $q['taxes'] = 'SELECT count(1) as `c`  FROM  `' . pSQL($this->tp) . 'tax_rules_group` ';
            }
        }
        if ($map['entity']['manufactures'][0] == 1) {
            if (!$additional_recent_data_reset) {
                $q['manufacturers'] = 'SELECT count(1) as `c` FROM  `' . pSQL($this->tp) . 'manufacturer`';
            }
        }
        if ($map['entity']['categories'][0] == 1) {
        ...
        if ($map['entity']['carriers'][0] == 1) {
            if (!$additional_recent_data_reset) {
                $q['carriers'] = 'SELECT count(1) as `c` FROM  `' . pSQL($this->tp) . 'carrier`';
            }
        }
        ...
        if ($map['entity']['orders'][0] == 1 && $map['entity']['customers'][0] == 1) {
    ...
    public function category()
    {
        ...
        if (version_compare($this->version, '1.5', '<')) {
            ...
            $q['category_lang'] = 'SELECT l.* FROM ' . pSQL($this->tp) . 'category_lang  l
                                   ...
                                   WHERE l.id_shop = 1 AND l.id_lang IN ( ' . pSQL($this->languages) . ' ) ORDER BY l.id_category ASC';
    ...
    public function carrier()
    {
        ...
        $main = 'SELECT  id_carrier FROM ' . pSQL($this->tp) . 'carrier c 
                         ORDER BY id_carrier ASC LIMIT ' . (int)$this->offset . ',' . (int)$this->row_count;

        $q['carrier'] = 'SELECT c.* '.$versionfield.' FROM ' . pSQL($this->tp) . 'carrier c '. $versionQuery.'
                        ORDER BY c.id_carrier ASC LIMIT ' . (int)$this->offset . ',' . (int)$this->row_count;
        ...
        $q['carrier_lang'] = 'SELECT DISTINCT d.* FROM ' . pSQL($this->tp) . 'carrier_lang   d
                             INNER JOIN (' . $main . ')  c ON c.id_carrier = d.id_carrier WHERE d.id_shop = 1 AND d.id_lang IN ( ' . pSQL($this->languages) . ' ) ORDER BY d.id_carrier ASC';
    ...
    public function product()
    {
        ...
        $versionQueryFP = 'LEFT JOIN
                                  ...
                                  GROUP BY pps.id_feature) s ON f.id_feature = s.id_feature ';
        ...
        if ($this->recent_data) {
            ...
            $q['product_lang'] = 'SELECT DISTINCT  a.* FROM ' . pSQL($this->tp) . 'product_lang  a
                                INNER JOIN (' . $main_recent_data . ')  pp ON pp.id_product = a.id_product WHERE a.id_shop = 1 AND a.id_lang IN ( ' . pSQL($this->languages) . ' )  ORDER BY a.id_product ASC';
            ...
            $q['feature_product'] = 'SELECT DISTINCT  f.*' . $versionfield . '  FROM ' . pSQL($this->tp) . 'feature_product  f '. $versionQueryFP .'
                                        INNER JOIN (' . $main_recent_data . ')  pp ON pp.id_product = f.id_product   ORDER BY f.id_product ASC';
            ...
            $q['feature'] = 'SELECT DISTINCT f.* ' . $versionfield . ' FROM ' . pSQL($this->tp) . 'feature  f ' . $versionQueryFP . '
            ...
            $q['product_lang'] = 'SELECT DISTINCT  a.* FROM ' . pSQL($this->tp) . 'product_lang  a
                                INNER JOIN (' . $main . ')  pp ON pp.id_product = a.id_product WHERE a.id_shop = 1 AND a.id_lang IN ( ' . pSQL($this->languages) . ' )  ORDER BY a.id_product ASC';
            ...
            $q['feature_product'] = 'SELECT DISTINCT  f.* ' . $versionfield . '  FROM ' . pSQL($this->tp) . 'feature_product  f '. $versionQueryFP .'
                                        INNER JOIN (' . $main . ')  pp ON pp.id_product = f.id_product   ORDER BY f.id_product ASC';
    ...
    public function order()
    {
        ...
        if ($this->recent_data) {
            ...
            $q['message_readed'] = 'SELECT DISTINCT mr.* FROM ' . pSQL($this->tp) . 'message_readed mr
                           ...
                           INNER JOIN (' . $main_recent_data . ') o ON o.id_order = m.id_order ORDER BY mr.id_message ASC';
            ...
            $q['order_slip_detail'] = 'SELECT DISTINCT md.* FROM ' . pSQL($this->tp) . 'order_slip_detail md
                           ...
                           INNER JOIN (' . $main_recent_data . ') o ON o.id_order = m.id_order ORDER BY md.id_order_slip ASC';
            ...
            //     $q['order_slip_detail_tax'] = 'SELECT * FROM ' . pSQL($this->tp) . 'order_slip_detail';
            // }
            if (version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.7.0.0', '>') && version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.7.7.0', '<')) {
                $q['order_slip_detail_tax'] = 'SELECT DISTINCT mdt.* FROM ' . pSQL($this->tp) . 'order_slip_detail_tax mdt
                           INNER JOIN ' . pSQL($this->tp) . 'order_slip  m ON m.id_order_slip = mdt.id_order_slip_detail
                           INNER JOIN (' . $main_recent_data . ') o ON o.id_order = m.id_order ORDER BY mdt.id_order_slip ASC';
            }
            ...
            $q['order_return_detail'] = 'SELECT DISTINCT md.* FROM ' . pSQL($this->tp) . 'order_return_detail md
                           ...
                           INNER JOIN (' . $main_recent_data . ') o ON o.id_order = m.id_order ORDER BY md.id_order_return ASC';
            ...
            $q['country'] = 'SELECT DISTINCT c.* ' . $versionfield . ' FROM ' . pSQL($this->tp) . 'country c
                           ...
                           INNER JOIN (' . $main_recent_data . ') o ON o.id_address_delivery = m.id_address ORDER BY c.id_country ASC';
            ...
            $q['state'] = 'SELECT DISTINCT c.*  FROM ' . pSQL($this->tp) . 'state c
                            ...
                            INNER JOIN (' . $main_recent_data . ') o ON o.id_address_delivery = m.id_address ORDER BY c.id_state ASC';

            $q['zone'] = 'SELECT DISTINCT z.*  FROM ' . pSQL($this->tp) . 'zone z 
                            INNER JOIN ' . pSQL($this->tp) . 'state c ON z.id_zone = c.id_zone 
                            INNER JOIN ' . pSQL($this->tp) . 'address m ON m.id_state = c.id_state
                            INNER JOIN (' . $main_recent_data . ') o ON o.id_address_delivery = m.id_address ORDER BY z.id_zone ASC';
            ...
            $q['message_readed'] = 'SELECT DISTINCT mr.* FROM ' . pSQL($this->tp) . 'message_readed mr 
                            INNER JOIN ' . pSQL($this->tp) . 'message m ON   m.id_message = mr.id_message
                            INNER JOIN (' . $main . ') o ON o.id_order = m.id_order ORDER BY mr.id_message ASC';
            ...
            $q['order_slip_detail'] = 'SELECT DISTINCT md.* FROM ' . pSQL($this->tp) . 'order_slip_detail md
                            INNER JOIN ' . pSQL($this->tp) . 'order_slip  m ON m.id_order_slip = md.id_order_slip
                            INNER JOIN (' . $main . ') o ON o.id_order = m.id_order ORDER BY md.id_order_slip ASC';
            ...
            if (version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.7.0.0', '>') && version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.7.7.0', '<')) {
                $q['order_slip_detail_tax'] = 'SELECT DISTINCT mdt.* FROM ' . pSQL($this->tp) . 'order_slip_detail_tax mdt
                           INNER JOIN ' . pSQL($this->tp) . 'order_slip  m ON m.id_order_slip = mdt.id_order_slip_detail
                           INNER JOIN (' . $main . ') o ON o.id_order = m.id_order ORDER BY mdt.id_order_slip ASC';
            }
            ...
            $q['order_return_detail'] = 'SELECT DISTINCT md.* FROM ' . pSQL($this->tp) . 'order_return_detail md
                            INNER JOIN ' . pSQL($this->tp) . 'order_return m ON m.id_order_return = md.id_order_return
                            INNER JOIN (' . $main . ') o ON o.id_order = m.id_order ORDER BY md.id_order_return ASC';
            ...
            $q['country'] = 'SELECT DISTINCT c.* ' . $versionfield . ' FROM ' . pSQL($this->tp) . 'country c
                            INNER JOIN ' . pSQL($this->tp) . 'address  m ON m.id_country = c.id_country '. $versionQuery . '
                            INNER JOIN (' . $main . ') o ON o.id_address_delivery = m.id_address ORDER BY c.id_country ASC';

            $q['country_lang'] = 'SELECT DISTINCT c.*  FROM ' . pSQL($this->tp) . 'country_lang c
                            INNER JOIN ' . pSQL($this->tp) . 'address  m ON m.id_country = c.id_country
                            INNER JOIN (' . $main . ') o ON o.id_address_delivery = m.id_address WHERE  c.id_lang IN ( ' . pSQL($this->languages) . ' ) ORDER BY c.id_country ASC';

            $q['state'] = 'SELECT DISTINCT c.*  FROM ' . pSQL($this->tp) . 'state c
                            INNER JOIN ' . pSQL($this->tp) . 'address  m ON m.id_state = c.id_state
                            INNER JOIN (' . $main . ') o ON o.id_address_delivery = m.id_address ORDER BY c.id_state ASC';

            $q['zone'] = 'SELECT DISTINCT z.*  FROM ' . pSQL($this->tp) . 'zone z 
                            INNER JOIN ' . pSQL($this->tp) . 'state c ON z.id_zone = c.id_zone 
                            INNER JOIN ' . pSQL($this->tp) . 'address m ON m.id_state = c.id_state
                            INNER JOIN (' . $main . ') o ON o.id_address_delivery = m.id_address ORDER BY z.id_zone ASC';
        ...
        $groupedqueriesconfiguration['order_slip_detail_tax'] = 'id_order_slip';
    ...
    public function cms()
    {
        ...
        $q['cms_role_lang'] = 'SELECT DISTINCT rl.* FROM ' . pSQL($this->tp) . 'cms_role_lang rl
                                ...
                                INNER JOIN  (' . $main . ') c ON c.id_cms = cl.id_cms WHERE  id_lang IN ( ' . pSQL($this->languages) . ' ) ORDER BY rl.id_cms_role';
