...

{
    ...
    const TYPE_ACCESSORIES = 'acc';

    const TYPE_STATE = 'st';

    const TYPE_CATEGORY = 'c';

    const TYPE_MESSAGE_THREADS = 'msg';
    ...
    const TYPE_CATALOG_PRICE_RULES = 'cat_price_rules';
    //TYPE_CATALOG PRICE RULES
...

    {
        ...
        if(Tools::strtoupper($entityType) == "CART RULES" or Tools::strtoupper($entityType) == "CATALOG PRICE RULES"){
            $type = "cart";
        }else{
    ...
    public static function getWarningLogsCount($entityType)

    {
        ...
        $sql = "SELECT COUNT(*) as `c` FROM " . _DB_PREFIX_ . "migrationpro_warning_logs WHERE entity_type = '" . pSQL($type) . "'";



        return Db::getInstance()->executeS($sql)[0]['c'];
