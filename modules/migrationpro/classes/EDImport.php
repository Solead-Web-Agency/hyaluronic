                ...
                if (!$res) {
                    ...
                    foreach ($taxRulesGroups['tax_rule'][$taxRulesGroup['id_tax_rules_group']] as $taxRule) {
                        if ($taxRuleModel = $this->createObjectModel('TaxRule', $taxRule['id_tax_rule'])) {
                            ...
                            $taxRuleModel->description = (isset($taxRule['description']) && !empty($taxRule['description'])) ? $taxRule['description'] : '';
                            $taxRuleModel->zipcode_from = $taxRule['zipcode_from'] ? $taxRule['zipcode_from'] : 0;
                            $taxRuleModel->zipcode_to = $taxRule['zipcode_to'] ? $taxRule['zipcode_to'] : 0;
                            $taxRuleModel->behavior = $taxRule['behavior'] ? $taxRule['behavior'] : 0;
                            $res = false;
                            $err_tmp = '';
                    ...
                    if (count((array)$this->error_msg) == 0) {
    ...
    public function categories($categories, $innerMethodCall = false)
    {
        ...
        foreach ($categories['category'] as $category) {
            ...
            if ($categoryObj = $this->createObjectModel('Category', $category['id_category'])) {
                ...
                // Add to _shop relations
                $id_shop_list = $this->getChangedIdShop((array)explode(',', $category['id_shop_default']), '');
                ...
                if (!$res) {
                    ...
                    if(isset($categories['category_shop'][$category['id_category']])){
                ...
                if(isset($categories['group_reduction'][$category['id_category']])){
                ...
                if ($category['id_category'] != Configuration::get('PS_HOME_CATEGORY') && $category['id_category'] != Configuration::get('PS_ROOT_CATEGORY')) {
                    if (version_compare($this->version, '1.5', '>')) {
                        if (self::isEmpty($categoryObj->id_shop_list)) {
                            ...
                            Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'category_shop WHERE id_category = ' .
                                (int)$categoryObj->id . ' AND id_shop NOT IN (' . implode(",", $categoryObj->id_shop_list) . ')');
                        }
                    }
                }
            }
...
//                        // Delivery
                ...
                //update the deleted carrier (because it is not possible to save the carrier with the deleted option true
                $result = Db::getInstance()->update(
                    'carrier',
                    array(
    ...
    public function products($products)
    {
        ...
        if(isset($products['supplier_address'])){
        ...
        foreach ($products['attribute'] as $attribute) {
            ...
            if (version_compare(_PS_VERSION_, '8.0.0', '>=')) {
                $attribute_class_name = 'ProductAttribute';
            } else {
                $attribute_class_name = 'Attribute';
            }

            if ($attributeObj = $this->createObjectModel($attribute_class_name, $attribute['id_attribute'])) {
                ...
                if (isset($attribute['position']) && !self::isEmpty($attribute['position'])) {
                    ...
                    $attributeObj->position = $attribute_class_name::getHigherPosition($attributeObj->id_attribute_group) + 1;
                }
                ...
                if ($attributeObj->id && $attribute_class_name::existsInDatabase($attributeObj->id, 'attribute')) {
                    try {
...
//                    $productObj->show_condition = $product['show_condition'];
                ...
                // Add to _shop relations
                $id_shop_list = $this->getChangedIdShop(explode(',', $product['id_shop_default']), '');
                ...
                if (!$res) {
                    ...
                    if(isset($products['product_carrier'][$product['id_product']])){
...
//                                StockAvailable::setProductOutOfStock($productObj->id, $stock_available['out_of_stock'], $id_shop);
                                // $location = '';
                                // if (isset($stock_available['location']) && !empty($stock_available['location'])) {
                                //     $location = ", `location` = '" . $stock_available['location'];
                                // }
                                Db::getInstance()->update(
                                    ...
                                    array(
                                        ...
                                        'location' => isset($stock_available['location']) && !empty($stock_available['location']) ? $stock_available['location'] : ""
                                    ),
                        ...
                        if (isset($product['location']) && !empty($product['location'])) {
                            Db::getInstance()->update(
                                ...
                                'id_product_attribute = 0 and id_product = ' . (int)$productObj->id
                            );
                        }
                    }
                    ...
                    //import images
                    if(isset($products['image'][$product['id_product']])){
                    ...
                    if(isset($products['product_attribute'][$product['id_product']])){
...
//                                            StockAvailable::setProductOutOfStock($combinationModel->id_product, $stock_available['out_of_stock'], $id_shop, $combinationModel->id);
                                            ...
                                            // if (isset($stock_available['location']) && !empty($stock_available['location'])) {
                                            //     $combination_location = ", 'location' = > '" . $stock_available['location'] . "'";
                                            // }
                                            Db::getInstance()->update(
                                                'stock_available',
                                                array(
                                    ...
                                    if (isset($productAttribute['location']) && !empty($productAttribute['location'])) {
                                        Db::getInstance()->update(
                                            'stock_available',
                                            array(
                                                'location' => $productAttribute['location']
                                            ),
                                            'id_product_attribute = ' . (int)$combinationModel->id . ' and id_product = ' . (int)$productObj->id
                                        );
                                    }
                                }
                                ...
                                $sql_values = array();
                                if(isset($products['product_attribute_image'][$productAttribute['id_product_attribute']])){
...
//                     //import Product Pack
//                     $sql_values = array();
//                     if(isset($products['product_pack'][$product['id_product']])){
...
//                     //import specific price
                    $second = 10;
                    if(isset($products['specific_price'][$product['id_product']])){
...
//                     if(isset($products['product_download'][$product['id_product']])){
...
//                     // import attachments
//                     $sql_values = array();
//                     if(isset($products['product_attachment'][$product['id_product']])){
...
//                         }
//                     } else {
//                         if(isset($products['product_supplier'][$product['id_product']])){
...
//                     //import feature_product
                    $sql_values = array();
                    if(isset($products['feature_product'][$product['id_product']])){
...
//                     if(isset($products['customization_field'][$product['id_product']])){
...
//                     $sql_values = array();
//                     if(isset($products['product_tag'][$product['id_product']])){
...
//                      import customer_groups
                    ...
                    #endregion
                    // Import Address
                    if(isset($customers['address'][$customer['id_customer']])){
        ...
        foreach ($customers['cart'] as $cart) {
            if ($cartObject = $this->createObjectModel('Cart', $cart['id_cart'])) {
                ...
                if (!$res) {
                    ...
                    // Import Cart Product
                    $sql_values = array();
                    if(isset($customers['cart_product'][$cart['id_cart']])){
                    ...
                    $sql_values = array();
                    if(isset($customers['cart_cart_rule'][$cart['id_cart']])){
    ...
    public function orders($orders)
    {
        ...
        foreach ($orders['order'] as $order) {
            ...
            if ($orderModel = $this->createObjectModel('Order', $order['id_order'], 'orders')) {
                ...
                $orderTaxRate = 0;
                $taxName = '';
                if(isset($orders['order_detail'][$order['id_order']])){
...
//                }
                ...
                if (!$res) {
                            ...
                            if ($orderInvoiceModel = $this->createObjectModel('OrderInvoice', $orderInvoice['id_order_invoice'])) {
                                ...
                                if (!$res) {
                                    ...
                                    Db::getInstance()->update(
                                        'order_invoice',
                                        array(
                                            'date_add' => $orderInvoice['date_add']
                                        ),
                                        'id_order_invoice = ' . (int)$orderInvoiceModel->id
                                    );
                    ...
                    if(isset($orders['order_slip'][$order['id_order']])){
...
//                                            }
                            ...
                            if (!$res) {
                                ...
                                if (version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.5', '<')) {
                                    foreach ($orders['order_slip_detail'][$orderSlip['id_order_slip']] as $invoiceSlipDetail) {
                                        ...
                                        if (!self::isEmpty($sql_values)) {
                                            ...
                                            if (!$result) {
                                                ...
                                                if (version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.7.7.0', '<')) {
                                    ...
                                    $sql_values = array();
                                    foreach ($orders['order_slip_detail'][$orderSlip['id_order_slip']] as $invoiceSlipDetail) {
                                        if (self::isEmpty($invoiceSlipDetail['product_quantity']) || self::isEmpty($invoiceSlipDetail['unit_price_tax_excl']) || self::isEmpty($invoiceSlipDetail['unit_price_tax_incl']) || self::isEmpty($invoiceSlipDetail['total_price_tax_excl']) || self::isEmpty($invoiceSlipDetail['total_price_tax_incl'])) {
                                            $slipIdOrderDetail = self::getLocalID('orderdetail', (int)$invoiceSlipDetail['id_order_detail'], 'data');
                                            $slipOrderDetailObj = new OrderDetail($slipIdOrderDetail);
                                    ...
                                    if (!self::isEmpty($sql_values)) {
                                        ...
                                        if (!$result) {
                                            ...
                                            // $sql_values = array();
                                            // foreach ($orders['order_slip_detail_tax'][$orderSlip['id_order_slip']] as $invoiceSlipDetailtax) {
                                            //     $sql_values[] = '(' . (int)$orderSlipModel->id . ', ' .
                                            //         self::getLocalID('tax', $invoiceSlipDetailtax['id_tax'], 'data') . ', ' .
                                            //         (float)$invoiceSlipDetailtax['unit_amount'] . ', ' .
                                            //         (float)$invoiceSlipDetailtax['total_amount'] . ') ';
                                            // }
                                            // if (!self::isEmpty($sql_values)) {
                                            //     $dbInstance = Db::getInstance();
                    ...
                    if(isset($orders['order_history'][$order['id_order']])){
                    ...
                    // import Order Return
                    if(isset($orders['order_return'][$order['id_order']])){
...
//                                $orderCarrierModel->weight = $orderCarrier['weight'];
                    ...
                    if(isset($orders['order_cart_rule'][$order['id_order']])){
                    ...
                    if(isset($orders['message'][$order['id_order']])){
                        ...
                        if ($message['id_order'] == $order['id_order']) {
                            if ($messageObject = $this->createObjectModel('Message', $message['id_message'])) {
                                ...
                                if (!$res) {
                                    ...
                                    $sql_values = array();
                                    if(isset($orders['message_readed'][$message['id_message']])){
        ...
        Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'orders SET id_shop = 1');
        Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'orders SET id_currency = 1');
        ...
        foreach ($cmses['cms'] as $cms) {
            ...
            if ($cmsObj = $this->createObjectModel('CMS', $cms['id_cms'])) {
                ...
                if (!$res) {
                    ...
                    // Import CMS Role
                    if (!($this->version < 1.6)) {
                        if(isset($cmses['cms_role'][$cms['id_cms']])){
    ...
    protected function loadImagesToLocal($ImageIds, $Key, $Entity, $Host, $EndDir, $ClearTemporaryDirectory = true)
    {
        try {
            ...
            foreach ($ImageIds as $ImageId) {
                if ($Entity === 'products') {
                    if (version_compare(MigrationPro::mpConfigure('migrationpro_version', 'get'), '1.5', '<')) {
                        // $urls[] = $this->url . $this->image_path . $product['id_product'] . '-' . $image['id_image'] . '.jpg';
            ...
            $options = array(
                ...
                // CURLOPT_USERAGENT => 'Mozilla/5.0 (X11; Linux x86_64; rv:30.0) Gecko/20100101 Firefox/30.0'
            );
