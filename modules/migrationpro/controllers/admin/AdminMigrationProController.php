...
class AdminMigrationProController extends AdminController
{
    ...
    public function ajaxProcessGenerateZip()
    {
        if (!MigrationPro::mpConfigure('migrationpro_token', 'get')) {
            $check_token = (array)explode('\\', _PS_ADMIN_DIR_);
            $get_token = end($check_token);

            $token = md5(Configuration::get('PS_SHOP_DOMAIN') . $get_token);
    ...
    public function ajaxProcessValidateStep()
    {
        ...
        if (count((array)$this->warnings) || count((array)$this->errors)) {
        ...
        die(self::jsonEncode($this->response));
    }
...
//                        $error_msg = $context->smarty->fetch($module_path . 'views/templates/admin/connection_error.tpl');  // FARID will cahnge new file
    ...
    private function createMapping($maps)
    {
        ...
        $mapses = (array)$maps;
        foreach ($mapses as $mapkey => $mapval) {   //   $mapkey =   mapping, adittional, entity
            $mapvals = (array)$mapval;
            foreach ($mapvals as $mapval_key => $mapval_val) {    //$mapval_key = multi_shops, currencies , order_states, tax, product, recent data ...
                $mapval_vals = (array)$mapval_val;
                foreach ($mapval_vals as $key => $value) {
    ...
    {
        $source_shop_url_check = (array)explode('/', str_replace('/index.php', '', $_SERVER['SCRIPT_NAME']));
        $check_source_shop_url = rtrim(preg_replace('/https?:\/\/(www\.)?/', '', $source_shop_url), '/') . '/' . end($source_shop_url_check);
...
//                $res &= Db::getInstance()->execute('TRUNCATE TABLE `' . _DB_PREFIX_ . 'address`');
    ...
    {
        if(version_compare(_PS_VERSION_, '8.0.0', '>=')) {
            return GuzzleHttp\Utils::jsonEncode($value);
        }else{
