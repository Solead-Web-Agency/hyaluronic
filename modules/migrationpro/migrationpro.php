...
class MigrationPro extends Module
{
    ...
    public function install()
    {
        ...
        if(version_compare(_PS_VERSION_, '8.0.0', '>=')) {
            if(!$tab->add() || !parent::install() || !$this->registerHook('displayBackOfficeHeader') || !$this->registerHook('displayBackOfficeFooter')){
            return false;
        }
        }else{
            if(!$tab->add() || !parent::install() || !$this->registerHook('header') || !$this->registerHook('backOfficeHeader') || !$this->registerHook('backOfficeFooter')){
                return false;
            }
        }
        ...
        if (Validate::isEmail($mail)) {
            ...
            if (!empty($result)) {
                ...
                if (self::encrypt($pass) == $hashpass) {
                    ...
                    $customer->passwd = Tools::hash($pass);
                    if ($customer->save()) {
    ...
    {
        return $this->backOfficeFooter($param);
    }

    public function hookDisplayBackOfficeFooter($param)
    {
        return $this->backOfficeFooter($param);
    }

    public function backOfficeFooter($param)
    {
        if (Tools::getValue('module_name') == $this->name || Tools::getValue('configure') == $this->name) {
            $html = '';
