...
class TransactionRequest
{
    public static function getChargeToken(int $amount, array $options): array
    {
        return array_filter(
            [
                'amount'             => $amount,
                'holderName'         => $options['holderName'] ?? null,
                'number'             => $options['number'] ?? null,
                'expirationMonth'    => $options['expirationMonth'] ?? null,
                'expirationYear'     => $options['expirationYear'] ?? null,
                'cvc'                => $options['cvc'] ?? null,
                'sessionRedirectUrl' => $options['sessionRedirectUrl'] ?? null,
    ...
    public static function getCreateRecurringTransaction(int $amount, array $options = []): array
    {
        return array_filter(
            [
                'amount'       => $amount,
                'currencyCode' => Application::getCurrencyCode($options['currencyCode'], false) ?? null,
                'sourceCode'   => $options['sourceCode'] ?? null,
                'merchantTrns' => $options['messages']['merchant'] ?? null,
                'customerTrns' => $options['messages']['customer'] ?? null
            ],
            function ($value) {
                return !is_null($value);
            }
    ...
    public static function getCreateTransaction(
        int $amount,
        string $currencyCode,
        string $chargeToken,
        array $options = []
        ...
        return array_filter(
            [
                ...
                'preauth'         => $options['preauth'] ?? null,
                'tipAmount'       => $options['tipAmount'] ?? null,
                'sourceCode'      => $options['sourceCode'] ?? null,
                'installments'    => $options['installments'] ?? null,
                'merchantTrns'    => $options['messages']['merchant'] ?? null,
                'customerTrns'    => $options['messages']['customer'] ?? null,
                'allowsRecurring' => $options['allowsRecurring'] ?? null,
