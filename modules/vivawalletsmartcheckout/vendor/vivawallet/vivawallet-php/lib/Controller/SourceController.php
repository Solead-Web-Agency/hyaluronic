...
namespace Vivawallet\VivawalletPhp\Controller;

use Vivawallet\VivawalletPhp\Api\SourceClient;
use Vivawallet\VivawalletPhp\Application;
use Vivawallet\VivawalletPhp\Core\Source\SourceItem;
use Vivawallet\VivawalletPhp\Core\Source\SourceList;
...
class SourceController
{
    public static function getSourceList(
        ...
        if ($sourcesResponse->isSuccessful()) {
            $sourceList         = $sourcesResponse->getBody() instanceof SourceList
                ? $sourcesResponse->getBody()
                : new SourceList();
            $filteredSourceList = $sourceList->filterByDomain($domain)->filterByState(1)->filterByUrls($urls);
            if (!count($filteredSourceList)) {
                $createdSourceItem    = $information['createdSourceItem'] = new SourceItem(
                    (object)array_filter(
                        [
                            'code'       => $sourceList->getFreeSourceCode("$cmsAbbreviation-"),
                            'name'       => sprintf(Application::SOURCE_NAME_FORMAT, $cmsAbbreviation, $domain),
                            'domain'     => $domain,
                            'successUrl' => $urls['success'] ?? null,
                            'failureUrl' => $urls['failure'] ?? null,
