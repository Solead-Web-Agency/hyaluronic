...
use Vivawalletsmartcheckout\Service\CaptureService;

final class CaptureController extends PrestaShopAdminController
{
    ...
    #[Route(
        path: '/vivawalletsmartcheckout/capture/{orderId}/ajax',
        name: 'vivawalletsmartcheckout_capture_ajax',
            ...
            if (!(int) $requestData['isCaptureAction']) {
                Logger::log(
                    [
                        ...
                        'arguments' => $arguments,
                        'response' => 'Wrong action. The action must be capture.',
                    ],
                    'vivaPayments'
                );
            }

            $transactionResponse = $this->captureService->captureAuthorizedTransaction($orderShopId, ...$arguments);
