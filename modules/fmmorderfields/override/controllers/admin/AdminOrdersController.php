...
class AdminOrdersControllerCore extends AdminController
{
	...
	$sql = 'SELECT id_order, COUNT(*) AS times FROM  '._DB_PREFIX_.'order_detail GROUP BY id_order';
	//echo $sql;
		$finalcheck = Db::getInstance()->executeS($sql);
		foreach ($finalcheck as $item){
			$sql = 'UPDATE `'._DB_PREFIX_.'orders`
			SET `total_products_quantity` = "'.$item['times'].'"
		...
		$this->fields_list = array(
			...
			'total_products_quantity' => array(
			'title' => $this->l('Products'),
			'havingFilter' => false,
			'prefix' => '<span class="eachProductAmount">',
			'suffix' => '</span>',
		),
		);
		...
		$this->fields_list = array_merge($this->fields_list, array(
			'total_paid_tax_incl' => array(
				...
				'badge_success' => true,
				'prefix' => '<span class="thisProductTotal">',
			'suffix' => '</span>'
			),
			'conversion_rate' => array(
			'title' => $this->l('Conversion Rate'),
			'prefix' => '<span class="convRate">',
			'suffix' => '</span>'
		),
			'payment' => array(
	...
	if($_POST['submitFilterorder'] && $_POST['submitFilterorder'] == 1) {
		
		Configuration::updateValue('ORDER_SET_FILTER', 1);
		
			//echo Configuration::get('ORDER_SET_FILTER'); 
		 
		} elseif(Tools::isSubmit('submitResetorder')) {
					Configuration::updateValue('ORDER_SET_FILTER', 0);
					//echo Configuration::get('ORDER_SET_FILTER'); 
			}
		parent::postProcess();
	}
...
class AdminOrdersControllerCore extends AdminController
{
	...
	// Custom FMM Code for Filter check
	//echo '<pre>'; print_r($_POST);
	if($_POST['submitFilter'] && $_POST['submitFilter'] == 'Filter') {
		
		Configuration::updateValue('ORDER_SET_FILTER', 1);
		
			//echo Configuration::get('ORDER_SET_FILTER'); 
		 
		} elseif($_POST['submitResetorder'] && $_POST['submitResetorder'] == 'Reset') {
					Configuration::updateValue('ORDER_SET_FILTER', 0);
					//echo Configuration::get('ORDER_SET_FILTER'); 
			}
