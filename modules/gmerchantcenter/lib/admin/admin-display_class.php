...
class BT_AdminDisplay implements BT_IAdmin
{
    ...
    private function assign()
    {
        ...
        $aAssign = array(
            ...
            'sGSaOverview' => BT_GmcModuleTools::getTemplatePath(_GMC_PATH_TPL_NAME . _GMC_TPL_ADMIN_PATH . _GMC_TPL_GSA_OVERVIEW),
    ...
    private function displayTabs(array $aPost = null)
    {
        ...
        BT_GmcWarning::create()->run(
            'file-exists',
            _GMC_PATH_ROOT . _GMC_XML_PHP_NAME,
            _PS_ROOT_DIR_ . '/' . _GMC_XML_PHP_NAME
        );
        ...
        $aAssign['sShoppingAction'] = BT_GmcModuleTools::getTemplatePath(_GMC_PATH_TPL_NAME . _GMC_TPL_ADMIN_PATH . _GMC_TPL_GSA);
    ...
    private function displayGsa(array $aPost = null)
    {
        ...
        $oShop = new ShopUrl(GMerchantCenter::$iShopId);

        $iGsaShopId =  GsaClient::getShopId(GMerchantCenter::$conf['GMC_API_KEY']);

        $aAssign = array(
            'sApiKey' => GMerchantCenter::$conf['GMC_API_KEY'],
            'aGroups' => Group::getGroups(GMerchantCenter::$iCurrentLang, GMerchantCenter::$iShopId),
            'bShopLink' => GMerchantCenter::$conf['GMC_SHOP_LINK_API'],
            'iDefaultCustomerGroup' => GMerchantCenter::$conf['GMC_GSA_CUSTOMER_GROUP'],
            'sMerchantId' => GMerchantCenter::$conf['GMC_MERCHANT_ID'], 
            'aCarriers' => Carrier::getCarriers(GMerchantCenter::$iCurrentLang, false, false, false, null, 'ALL_CARRIERS'),
            'iCarrierId' => GMerchantCenter::$conf['GMC_GSA_DEFAULT_CARRIER'],
            'sShopUrl' => rtrim((string)$oShop->getURL(), '/'),
            'iGsaShopId' => (!empty($iGsaShopId) && is_object($iGsaShopId))? $iGsaShopId->result : '',
            'sApiUrl' => _GMC_API_URL,
            'sApiUrlRegister' => _GMC_API_URL . '/register',
            'aGsaCarriers' => $GLOBALS['GMC_GSA_CARRIERS_DATA'],
            'aGsaCarriersMapped' => is_string(GMerchantCenter::$conf['GMC_GSA_CARRIERS_MAP']) ? unserialize(GMerchantCenter::$conf['GMC_GSA_CARRIERS_MAP']) : '',
        );
        ...
        return array(
            'tpl' => _GMC_TPL_ADMIN_PATH . _GMC_TPL_GSA,
            'assign' => $aAssign,
        );
    }
    ...
    private function displayFeed(array $aPost = null)
    {
        ...
        $aAssign = array(
            ...
            'bUrlError' => GMerchantCenter::$conf['GMC_URL_PROD_ERROR'],
            ...
            if (!empty(GMerchantCenter::$aAvailableLangCurrencyCountry)) {
                foreach (GMerchantCenter::$aAvailableLangCurrencyCountry as $sKey => $aData) {
                    ...
                    if (is_file(_GMC_SHOP_PATH_ROOT . $sFileName)) {
                        ...
                        $sLink = $aAssign['sGmcLink'] . _GMC_MODULE_URL . 'cron.php?id_shop=' . GMerchantCenter::$iShopId . '&id_lang=' . (int) $aData['langId'] . '&country=' . $aData['countryIso'] . '&currency_iso=' . $aData['currencyIso']
                    ...
                    if (!empty($aAssign['bCheckOutputFile'])) {
                        $sLink = $aAssign['sGmcLink'] . __PS_BASE_URI__ . _GMC_XML_PHP_NAME . '?id_shop=' . GMerchantCenter::$iShopId . '&id_lang=' . (int) $aData['langId'] . '&country=' . $aData['countryIso'] . '&currency_iso=' . $aData['currencyIso'] . '&token=' . GMerchantCenter::$conf['GMC_FEED_TOKEN'];
                        ...
                        foreach ($aGTag as $iKey => &$aProdValue) {
                            ...
                            if (empty($aTmpProduct[$aProdValue['productId']])) {
                                ...
                                if (!empty($iAttributeId)) {
                                    ...
                                    $aCombinationAttr = BT_GmcModuleDao::getProductComboAttributes(
                                        $iAttributeId,
                                        $iLangId,
                                        GMerchantCenter::$iShopId
                                    );
