        ...
        <fieldset>
            ...
            <p><select name="industryName" id="industryName">
                ...
                {foreach $listIndustries as $industry}
                    ...
                    <option value="{$industry['name']|escape:'htmlall':'UTF-8'}"{if isset($smarty.post.industryName) && $industry['name'] == $smarty.post.industryName} selected="selected"{/if}>{$industry['label']|escape:'htmlall':'UTF-8'}</option>
                    {/if}
                {/foreach}
            </select></p>
