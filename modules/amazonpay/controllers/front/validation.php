...
class AmazonpayValidationModuleFrontController extends ModuleFrontController
{
    ...
    {
        AmazonPayLogger::getInstance()->addLog('Validation called', 3);
        ...
        if (!Context::getContext()->cart->isVirtualCart()) {
            try {
                if (Context::getContext()->cart->id_address_delivery > 0) {
                    $deliveryAddressLocal = new AmazonPayAddress((int) Context::getContext()->cart->id_address_delivery);
                    $difference_detected = false;
                    $sessionInfo = $amazonPayCheckoutSession->getInformation();
                    $shippingAddressAmazon = $sessionInfo['shippingAddress'];

                    $hasShippingAddress = false;

                    if (is_array($shippingAddressAmazon) && isset($shippingAddressAmazon['countryCode']) && $shippingAddressAmazon['countryCode'] != '') {
                        $hasShippingAddress = true;
                        if (!AmazonPayAddressHelper::compareAddresses($deliveryAddressLocal->postcode, $shippingAddressAmazon['postalCode'])) {
                            $difference_detected = true;
                        }
                        if (!AmazonPayAddressHelper::compareAddresses($deliveryAddressLocal->city, $shippingAddressAmazon['city'])) {
                            $difference_detected = true;
                        }
                        if ($deliveryAddressLocal->id_country != Country::getByIso($shippingAddressAmazon['countryCode'])) {
                            $difference_detected = true;
                        }
                        if (in_array(Tools::strtolower($shippingAddressAmazon['countryCode']), ['de', 'at', 'uk'])) {
                            if (!AmazonPayAddressHelper::compareAddresses($deliveryAddressLocal->address1, $shippingAddressAmazon['addressLine1'])) {
                                $difference_detected = true;
                            }
                        }
                    }
                    ...
                    if ($hasShippingAddress) {
                        if ($difference_detected) {
                            ...
                            $deliveryAddressLocal->processFromArray($sessionInfo['shippingAddress']);
                            try {
                                $deliveryAddressLocal->save();
                            } catch (Exception $e) {
                                AmazonPayLogger::getInstance()->addLog('Could not update local delivery address', 1, $e);
                            }
                        }
                    }
                }
            } catch (Exception $e) {
                AmazonPayLogger::getInstance()->addLog('Could not fetch ChargePermission or perform address check', 1, $e);
            }
        }
        ...
        if (($amazonPayCheckoutSession->isCompleted() || $amazonPayCheckoutSession->isOpenAndInAmazonPayBugWorkarround()) && $complete) {
            ...
            try {
                Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'order_payment`
                                                   SET `transaction_id` = \'' . pSQL($amazonPayOrder->amazon_charge_id) . '\'
