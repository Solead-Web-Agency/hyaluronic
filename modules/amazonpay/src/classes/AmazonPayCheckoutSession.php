...
class AmazonPayCheckoutSession
{
    ...
    public function getChargePermissionInfo($chargePermissionId)
    {
        try {
            $result = $this->client->getChargePermission($chargePermissionId);
            if ($result['status'] === 200) {
                ...
                $this->logStatusError('getChargePermission', $result);

                return false;
            }
        } catch (Exception $e) {
            $this->logCallException('getChargePermission', $e, $result ?? []);
