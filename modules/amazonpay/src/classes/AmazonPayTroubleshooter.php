...
class AmazonPayTroubleshooter
{
    ...
    protected $test_details = [
        ...
        '####MODULE_INTEGRITY####' => [],
        '####COMPULSORY_FIELDS####' => [],
        '####SERVER_ACCESSIBILITY####' => [],
        '####DELIVERY_METHOD_INCOMPATIBILITY####' => [],
        '####EXPRESS_CHECKOUT_INCOMPATIBILITY####' => [],
    ];
    ...
    public static function generateResults($amazonpay)
    {
        ...
        echo json_encode(['troubleshooter' => $troubleshooter_html]);
        exit;
    }
    ...
    {
        $all_available = true;
        $urls = ['https://api.amazon.com/user/profile', 'https://eu.account.amazon.com/ap/oa', 'https://mws.amazonservices.com'];
        foreach ($urls as $url) {
            try {
    ...
    {
        $return = true;
        $delivery_modules = ['cubyn', 'relaiscolis', 'relaiscolisplus', 'colissimo'];
    ...
    {
        $return = true;
        $delivery_modules = ['onepagecheckout', 'onepagecheckoutps', 'supercheckout'];
        foreach ($delivery_modules as $m) {
            if ($this->checkForModule($m)) {
