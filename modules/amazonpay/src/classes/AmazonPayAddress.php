...
class AmazonPayAddress extends Address
{
    ...
    public function processFromArray($arr)
    {
        ...
        if (in_array(Tools::strtolower($arr['countryCode']), ['de', 'at', 'uk'])) {
            if ($s_company_name != '') {
    ...
    public static function saveAddressAmazonReference(Address $address, $orderref, $id_customer, $amazon_address = false)
    {
        ...
        if (self::findByAmazonOrderReferenceIdOrNew($orderref, $amazon_address, $id_customer, true)) {
            Db::getInstance(_PS_USE_SQL_SLAVE_)->update('amazonpay_address_reference', [
                'amazon_order_reference_id' => pSQL($orderref),
                'amazon_hash' => pSQL($amazon_hash),
            ], 'id_address = \'' . (int) $address->id . '\'');
        } else {
            Db::getInstance(_PS_USE_SQL_SLAVE_)->insert('amazonpay_address_reference', [
                'id_address' => (int) $address->id,
                'amazon_order_reference_id' => pSQL($orderref),
                'amazon_hash' => pSQL($amazon_hash),
            ]);
    ...
    public static function fetchInvalidInput($address, $additional_data = false)
    {
        ...
        if (property_exists($className, 'definition') && isset($className::$definition['fields'])) {
            $fieldsDefinition = $className::$definition['fields'];
            $fieldsToValidate = array_keys($fieldsDefinition);
        } else {
            $fieldsToValidate = [];
        }

        foreach ($fieldsToValidate as $field_to_validate) {
            if (!property_exists($address, $field_to_validate)) {
                continue;
            }

            $check_value = $address->{$field_to_validate};

            $validation = $address->validateField(
                $field_to_validate,
                $check_value,
                null,
                [],
        ...
        if ((int) $address->id_country > 0) {
            ...
            if ($country->need_identification_number) {
                $validation = $address->validateField('dni', $address->dni, null, [], true);
                if ($validation !== true) {
                    $fields_to_set[] = 'dni';
                    self::$validation_errors[] = $validation;
                }
