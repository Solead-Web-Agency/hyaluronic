...
require_once dirname(__FILE__) . '/src/classes/Helper/AmazonPayAddressHelper.php';
...
class Amazonpay extends PaymentModule
{
        ...
        Media::addJsDef(
            [
                'amazonpay' => [
                    ...
                    'legacy_address_form_action' => $this->context->link->getModuleLink($this->name, 'setaddresslegacy', ['amazonCheckoutSessionId' => !empty(Context::getContext()->cookie->amazon_pay_checkout_session_id) ? Context::getContext()->cookie->amazon_pay_checkout_session_id : '']),
                ],
            ]
        );
    }
    ...
    public function getMappedCarriers()
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($mapped_carriers)) {
            $mapped_carriers = [];
        }
    ...
    public function getCarrierOptions()
    {
        ...
        foreach ($carriers as $carrier) {
            $ret[] = ['id' => 'carrier_' . $carrier['id_carrier'] . '_on',
    ...
    /**
     * @param $carrier_id
     * @param int|bool $parent_carrier_id
     *
     * @return bool
     */
