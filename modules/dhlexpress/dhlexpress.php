...
class Dhlexpress extends CarrierModule
{
    ...
    public function install()
    {
        ...
        Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'dhl_custom_product` (
    ...
    public function postProcessAddress()
    {
        ...
        foreach (array_keys($fields) as $field) {
            $validate = $dhlAddress->validateField($field, Tools::getValue($field), null, [], true);
            if (true !== $validate) {
                $errors[] = $validate;
            }
        }
    ...
    public function postProcessPackage()
    {
        ...
        $errors = [];
        foreach (array_keys($fields) as $field) {
            $validate = $dhlPackage->validateField($field, Tools::getValue($field), null, [], true);
            if (true !== $validate) {
                $errors[] = $validate;
            }
        }
    ...
    public function postProcessCommercialInvoice()
    {
        ...
        Configuration::updateValue('DHL_DEFAULT_ORIGIN_COUNTRY', Tools::getValue('DHL_DEFAULT_ORIGIN_COUNTRY'));
        Configuration::updateValue('DHL_INVOICE_SHIPPING_COST', Tools::getValue('DHL_INVOICE_SHIPPING_COST'));
        if (!empty($_FILES['file']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
            ...
            $new = imagecreatetruecolor($crop_width, (int) $crop_height);
            switch ($extension) {
                case 'jpg':
                case 'jpeg':
            ...
            imagecopyresampled($new, $current_image, 0, 0, $x1, $y1, $crop_width, (int) $crop_height, $w, $h);
    ...
    public function getFrontOfficeSettingsFormFields()
    {
        $return = [
            ...
            'DHL_USE_WEIGHT_PACKAGE',
            'DHL_USE_WEIGHT_PRODUCT',
            'DHL_WEIGHT_PRICES',
            'DHL_WEIGHTING_TYPE',
    ...
    public function getBackOfficeSettingsFormFields($key = false)
    {
        ...
        $return['InvoiceDetails'] = array(
            'DHL_DEFAULT_HS_CODE',
            'DHL_DEFAULT_ORIGIN_COUNTRY',
            'DHL_INVOICE_SHIPPING_COST',
            'DHL_ENABLE_PLT',
            'DHL_SIGNATURE_NAME',
            'DHL_SIGNATURE_TITLE',
        );
    ...
    {
        if (Configuration::get('DHL_USE_WEIGHT_PACKAGE')) {
            $pieces = $this->getPiecesByOnePackage($products);
        ...
        foreach ($products as $product) {
            ...
            if ($withQuantities) {
                while ($i < $qty) {
                    $product['weight'] = Tools::ps_round($product['weight'], 3) == 0 ? 0.001 : Tools::ps_round(
                        $product['weight'],
                        3
                    );
                    // check Heavy Weight
                    $nbPackage = $product['weight'] / 70;
                    if ($nbPackage > 1) {
                        for ($j = 0; $j < (int) $nbPackage; $j++) {
                            $productsParam[] = array(
                                'PieceID' => $j + 1,
                        ...
                        if ($product['weight'] > ((int) $nbPackage * 70)) {
                            $lastPackageWeight = $product['weight'] - ((int) $nbPackage * 70);
                            $productsParam[] = array(
                                'PieceID' => (int) $nbPackage + 1,
                                'Height' => Tools::ps_round($product['height'], 3),
                                'Depth' => Tools::ps_round($product['depth'], 3),
                                'Width' => Tools::ps_round($product['width'], 3),
                                'Weight' => $lastPackageWeight,
                            );
                        }
                    } else {
                        $productsParam[] = array(
     ...
     * @return array
     */
    public function getPiecesByOnePackage($products)
    {
        $productsParam = [];
        $productsDepth = [];
        $productsWidth = [];
        $totalHeightProducts = 0;
        $totalWeightProducts = 0;
        $totalVolumetricWeight = 0;
        foreach ($products as $product) {
            $productsDepth[] = $product['depth'];
            $productsWidth[] = $product['width'];
            $totalHeightProducts += $product['height'];
            $totalWeightProducts += $product['weight'] * $product['quantity'];
            $totalVolumetricWeight += ($product['depth'] * $product['height'] * $product['width']) / 5000;
        }
        $totalWeightProducts = Tools::ps_round($totalWeightProducts, 3) == 0 ? 0.001 : Tools::ps_round($totalWeightProducts, 3);
        $totalVolumetricWeight = Tools::ps_round($totalVolumetricWeight, 3) == 0 ? 0.001 : Tools::ps_round($totalVolumetricWeight, 3);
        $highestWeight = $totalWeightProducts > $totalVolumetricWeight ? $totalWeightProducts : $totalVolumetricWeight;
        // check Heavy Weight
        $nbPackage = $highestWeight / 70;
        if ($nbPackage > 1) {
            for ($i = 0; $i < (int) $nbPackage; $i++) {
                $productsParam[] = [
                    'PieceID' => $i + 1,
                    'Height' => 0,
                    'Depth' => 0,
            ...
            if ($highestWeight > ((int) $nbPackage * 70)) {
                $lastPackageWeight = $highestWeight - ((int) $nbPackage * 70);
                $productsParam[] = [
                    'PieceID' => (int) $nbPackage + 1,
                    'Height' => 0,
                    'Depth' => 0,
    ...
    public function getPackageShippingCost($cart, $shippingCost, $products)
    {
        ...
        if (Configuration::get('DHL_ENABLE_LOG')) {
            ...
            $hash = Tools::hash(_PS_MODULE_DIR_ . $this->name . '/logs/');
        ...
        $logger->info('Packages.', array('pieces' => $productsParam));
        $extracharges = DhlExtracharge::getExtrachargesList($this->context->language->id);
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        $productCustomDetails = DhlCustomProduct::getByIdProduct((int) $idProduct);
        $this->context->smarty->assign(
            array(
                'countries' => $countries,
    ...
    {
        if (Tools::getValue('dhl_update_product')) {
            require_once(dirname(__FILE__) . '/classes/DhlCustomProduct.php');
            $idProduct = $params['id_product'];
            $hsCode = Tools::getValue('dhl_hs_code');
            $shortDescription = Tools::getValue('dhl_short_desc');
            $countryOrigin = Tools::getValue('dhl_country_origin');
            $customProduct = DhlCustomProduct::getByIdProduct((int) $idProduct);
