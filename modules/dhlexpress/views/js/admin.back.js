...
function inputValueControl(elm) {
  var num = $('input[name= '+elm+']').val();
  if (num.indexOf(".")!== -1) {
    var n = parseFloat(num).toFixed(2);
    $('input[name= '+elm+']').val(n);
  }
  var shipppingValue = document.getElementById("dhl_label_shippig_cost_value").value;
  var declaredValue = document.getElementById("dhl-label-declared-value").value;
  var total =  parseFloat(shipppingValue) + parseFloat(declaredValue);
  var result =  total.toFixed(2);
  $('#dhl-label-total-value').attr('value', result);
}
