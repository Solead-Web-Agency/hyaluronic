...
class AdminDhlLabelController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        require_once(dirname(__FILE__) . '/../../classes/DhlCustomProduct.php');
        ...
        if (Configuration::get('DHL_ENABLE_LOG')) {
            ...
            $hash = Tools::hash(_PS_MODULE_DIR_ . $this->module->name . '/logs/');
    ...
    public function displayLabelForm($dhlOrder)
    {
        ...
        //$senderAddresses = DhlAddress::getAddressList();
        $shipping_amount_tax_incl = 0;
        $shipping_amount_tax_excl = 0;
        $cartRules = $order->getCartRules();
        if ($cartRules) {
            foreach ($order->getCartRules() as $cart_rule) {
                if ($cart_rule['free_shipping']) {
                    $shipping_amount_tax_incl = $order->total_shipping_tax_incl;
                    $shipping_amount_tax_excl = $order->total_shipping_tax_excl;
                }
            }
            $order_reduction_amount_tax_incl = ($order->total_discounts_tax_incl - $shipping_amount_tax_incl);
            $order_reduction_amount_tax_excl = ($order->total_discounts_tax_excl - $shipping_amount_tax_excl);
            $declared_value_with_taxes = $cart->getOrderTotal(true, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING) - $order_reduction_amount_tax_incl;
            $declared_value_without_taxes = $cart->getOrderTotal(false, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING) - $order_reduction_amount_tax_excl;
        }
        ...
        $this->context->smarty->assign(
            array(
                'declared_value_with_taxes' => $cartRules ? $declared_value_with_taxes : $cart->getOrderTotal(true, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING),
                'declared_value_without_taxes' => $cartRules ? $declared_value_without_taxes : $cart->getOrderTotal(false, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING),
                'currency_iso' => $orderCurency->iso_code,
                'customer_address' => $customerAddrDelivery,
                'customer_country_iso' => $customer_country_iso,
                'id_order' => $idOrder,
                'shipping_cost_without_taxes' => $shipping_amount_tax_excl == 0 ? number_format($order->total_shipping_tax_excl, 2) : number_format(0, 2),
                'show_shipping' => Configuration::get('DHL_INVOICE_SHIPPING_COST'),
                'total_order' => $order->total_paid_tax_excl,//$cart->getOrderTotal(false),
                'dhl_sending_doc' => $doc,
    ...
    /**
     * @param DhlShipmentValidationPltRequest $shipmentRequest
     * @return void
     */
    ...
    public function setShipmentSenderConsignee(DhlShipmentValidationRequest &$shipmentRequest)
    {
        if ($this->data['form']['id_return_label']) {
            ...
            $registrationNumbers['tva'] = array(
                'Number' => $this->data['sender']->vat_number,
                'NumberTypeCode' => 'VAT',
                'NumberIssuerCountryCode' => Country::getIsoById($idCountryOwner),
            );
            if ($this->data['sender']->eori) {
                $registrationNumbers['eori'] = array(
                    'Number' => $this->data['sender']->eori,
                    'NumberTypeCode' => 'EOR',
                    'NumberIssuerCountryCode' => Country::getIsoById($idCountryOwner),
                );
            }
            if ($this->data['sender']->vat_gb) {
                $registrationNumbers['gb'] = array(
                    'Number' => $this->data['sender']->vat_gb,
                    'NumberTypeCode' => 'SDT',
                    'NumberIssuerCountryCode' => 'GB',
                );
            }
            $shipmentRequest->setRegistrationNumbersShipper($registrationNumbers);
        }
    }
    ...
    public function buildFormData()
    {
        ...
        $shippingCost = Tools::getValue('dhl_label_shippig_cost_value');
        $this->data = array(
            'form' => array(
     ...
     * @throws Exception
     */
    public function buildProductsData($bulk_label)
    {
    ...
    public function generateLabel()
    {
        ...
        if ($this->data['options']['doc']) {
            ...
            if (DhlTools::isDeclaredValueRequired($isoFrom, $isoTo, $pcTo) &&
                ...
                $signaturePath = _PS_MODULE_DIR_ . $this->module->name . '/views/img/' . $signatureName . '.jpg';
                $shipmentRequest->setEdatas(
                    array(
                        ...
                        'ChargeValue' => isset($this->data['form']['shipping_cost']) ? $this->data['form']['shipping_cost'] : '',
                        'SignatureImage' => file_exists($signaturePath) ? base64_encode(Tools::file_get_contents($signaturePath)) : '',
        ...
        if ($response && $response instanceof DhlShipmentValidationResponse) {
            ...
            if (empty($errors)) {
                ...
                if ($this->data['form']['id_dhl_order'] && !$this->data['form']['free_label']) {
                    ...
                    $dhlLabel->total_pieces = (int) count($this->data['packages']);
            ...
            if (empty($errors)) {
                ...
                if (!$services || empty($services) || !$services['currency']) {
                    ...
                    if (!$freeLabel) {
                        ...
                        $shippingCostPaid = $shipping_amount_tax_excl == 0 ? number_format($order->total_shipping_tax_incl, 2) : number_format(0, 2);
    ...
    /**
     * @param $country_code_customer
     * @param $country_code_sender
     * @param $zip_code_sender
     * @return int
     */
    ...
    public function assingParamsInvoice($order)
    {
        ...
        $shipping_amount_tax_excl = 0;
        $order_reduction_amount_tax_excl = 0;
        $cartRules = $order->getCartRules();
        $totalQuantity = 0;
        if ($cartRules) {
            foreach ($order->getCartRules() as $cart_rule) {
            ...
            $reductionProduct = $order_reduction_amount_tax_excl / (int) $totalQuantity;
        }
        foreach ($orderDetails as $key => $orderDetail) {
            $productCustomDetails = DhlCustomProduct::getByIdProduct((int) $orderDetail['product_id']);
            if ($productCustomDetails->id_country_origin) {
                $orderDetails[$key]['default_country'] = $productCustomDetails->id_country_origin;
            } else {
                $orderDetails[$key]['default_country'] = Configuration::get('DHL_DEFAULT_ORIGIN_COUNTRY');
            }
            if ($productCustomDetails->hs_code) {
                $orderDetails[$key]['hs_code'] = $productCustomDetails->hs_code;
            } else {
                $orderDetails[$key]['hs_code'] = Configuration::get('DHL_DEFAULT_HS_CODE');
            }
            if ($productCustomDetails->short_desc) {
                $orderDetails[$key]['product_name'] = $productCustomDetails->short_desc;
            }
            if ($cartRules) {
                $productPrice = $orderDetails[$key]['unit_price_tax_excl'] - $reductionProduct;
                $orderDetails[$key]['unit_price_tax_excl'] = number_format($productPrice, 3);
            }
        }
        $this->context->smarty->assign(
            array(
