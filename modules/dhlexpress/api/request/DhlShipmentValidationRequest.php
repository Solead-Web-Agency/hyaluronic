...
class DhlShipmentValidationRequest extends AbstractDhlRequest
{
     ...
     */
    public function setRegistrationNumbersShipper($registrationNumbers)
    {
        $this->xml->Shipper->Contact->insertAfter(
            new SimpleXMLExtended('<RegistrationNumbers></RegistrationNumbers>')
        );
        foreach ($registrationNumbers as $registrationNumber) {
            $registrationNumberElem = $this->xml->Shipper->RegistrationNumbers->addChild('RegistrationNumber');
            $registrationNumberElem->addChild('Number', $registrationNumber['Number']);
            $registrationNumberElem->addChild('NumberTypeCode', $registrationNumber['NumberTypeCode']);
            $registrationNumberElem->addChild('NumberIssuerCountryCode', $registrationNumber['NumberIssuerCountryCode']);
        }
    }
    ...
    public function setEdatas($edatas)
    {
        ...
        if ($edatas['ChargeValue']) {
            $this->xml->ExportDeclaration->addChild('OtherCharges');
            $this->xml->ExportDeclaration->OtherCharges->addChild('OtherCharge');
            $this->xml->ExportDeclaration->OtherCharges->OtherCharge->addChild('OtherChargeCaption', 'Freight Charges');
            $this->xml->ExportDeclaration->OtherCharges->OtherCharge->addChild('OtherChargeValue', $edatas['ChargeValue']);
            $this->xml->ExportDeclaration->OtherCharges->OtherCharge->addChild('OtherChargeType', 'FRCST');
        }
        $this->xml->ExportDeclaration->addChild('SignatureImage', $edatas['SignatureImage']);
