...
function ChangeOrderStatus(id, state) {
    $.post(previewOrderUrl, {
        id_employee: qov_employee_id,
        secure_key: module_secure_key,
        idorder: id,
        ostatus: state
    }, function (data) {
...
$(document).ready(function () {
    ...
    if ($_GET.controller == "AdminOrders") {
        ...
        $('.order td').not($('.order td .btn-group-action').parent('td')).not($('.order td myparcel-tracktrace').parent('td')).not($('.order td:last-child, .order td:first-child').prev('td')).not('.order td:last-child, .order td:first-child').toggle(function (e) {
                $('.QuickOrderDetails').hide();
                $('.QuickOrderDetails').remove();
                if ($('.DetailsOrder' + getidorder($(this).parent().html())).length == 0) {
                    $([document.documentElement, document.body]).animate({
                        scrollTop: $(this).parent('tr').offset().top - 136
                    }, 1000);
                    var that = $(this);
                    e.preventDefault();
                    $.post(previewOrderUrl, {
                        id_employee: qov_employee_id,
                        secure_key: module_secure_key,
                        id_order: getidorder($(this).parent().html())
                    }, function (data) {
                        $(data).insertAfter(that.closest('tr'));
                    });
                } else {
                    $('.DetailsOrder' + getidorder($(this).parent().html())).remove();
                }
            },
            function () {
                $('.DetailsOrder' + getidorder($(this).parent().html())).remove();
...
$(document).ready(function () {
    ...
    $("body").keydown(function (e) {
        if (e.keyCode == 37) { // left
            $('.QuickOrderDetails').prev('tr').prev('tr').find('td:eq(1)').click();

        } else if (e.keyCode == 39) { // right
            $('.QuickOrderDetails').next('tr').find('td:eq(1)').click();
        }
    });
});
