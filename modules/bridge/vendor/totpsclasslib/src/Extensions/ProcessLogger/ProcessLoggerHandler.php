...
use BridgeClasslib\Extensions\ProcessMonitor\ProcessMonitorHandler;
use Configuration;
use Db;
use Hook;
use Tools;
...
class ProcessLoggerHandler
{
    ...
    {
        return false === (bool)Configuration::get('BRIDGE_EXTLOGS_ERASING_DISABLED');
    }
    ...
    {
        $numberOfDays = Configuration::get('BRIDGE_EXTLOGS_ERASING_DAYSMAX');
     ...
     * 'actionSkipBridgeProcessLoggerSave' returns false (weak comparison)
     *
     * @return bool
     *
     * @throws \PrestaShopException
    ...
    protected static function getSkippingHooksResult()
    {
        ...
        if (Hook::getIdByName('actionSkipBridgeProcessLoggerSave')) {
            $hookModuleProcessLoggerReturnArray = Hook::exec(
                'actionSkipBridgeProcessLoggerSave',
                [
                    'logs' => self::$logs,
                ],
                null,
