...
{else}
  ...
  {* Custom 2026-06-08 : téléphone obligatoire UNIQUEMENT côté front (label + attribut), sans toucher la
     validation serveur (ps_required_field reste vide) -> aucun risque de 500. *}
  ...
  <div class="form-group row align-items-center {if !empty($field.errors)}has-error{/if}">
    <label class="col-md-2 col-form-label{if $field.required || $hfmPhoneForced} required{/if}" for="field-{$field.name}">
    ...
    <div class="col-md-2 form-control-comment">
      {block name='form_field_comment'}
        {if (!$field.required && !$hfmPhoneForced && !in_array($field.type, ['radio-buttons', 'checkbox']))}
