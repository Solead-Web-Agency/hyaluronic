    ...
    foreach ($xml->Produits->article as $article) {
        ...
        if ($ean_xml != '') {
            ...
            if ($result && $result['ean13'] == $ean_xml) {
                ...
                echo "Occurrence $counter:<br>ID du produit = $id_product<br>Référence = $reference<br>";
                echo "Nom du produit Presta = $product_name_presta<br>Nom du produit XML = $libelle_xml<br>";
                echo "Product ID XML = $product_id_xml<br>EAN en cours = $ean_presta<br>";
                echo "Après modification :<br>Prix HT Presta = $prix_ht_xml<br>Prix TTC Presta = $prix_ttc_presta<br>Prix d'achat HT Presta = $pamp_xml<br>Stock Presta = $stock_xml<br>";
                echo "TVA Presta = $taux_tva_xml<br>Groupe de règles fiscales Presta = " . $product->id_tax_rules_group . "<br>";
                echo "Prix TTC XML = " . number_format($prix_ttc_xml, 2) . "<br>Stock XML = $stock_xml<br><br>";
            }
        }
    }
} catch (Exception $e) {
