...
class SourceClient
{
    ...
    public function createSource(SourceItem $sourceItem): Response
    {
        return $this->httpClient->request(
            ...
            . Application::ENDPOINTS['sources'],
            [
                'json'    => [
                    'domain'      => $sourceItem->getDomain(),
                    'sourceCode'  => $sourceItem->getCode(),
                    'name'        => $sourceItem->getName(),
                    'state'       => $sourceItem->getState(),
                    'pathSuccess' => $sourceItem->getSuccessUrl(),
                    'pathFail'    => $sourceItem->getFailureUrl(),
                    'paramCancelOrder'  => $sourceItem->getParamCancelOrder(),
                ],
                'headers' => [
    ...
    {
        $sourcesResponse = $this->httpClient->request(
            'get',
            Application::BASE_URLS[$this->authentication->getEnvironment()]['api']
            . Application::ENDPOINTS['sources'],
            [
