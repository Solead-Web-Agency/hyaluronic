...
namespace Vivawallet\VivawalletPhp\Core\Source;
...
class SourceList extends \ArrayObject
{
    ...
    public function filterByDomain(string $domain): SourceList
    {
        return new self(
            array_filter(
                (array)$this,
                function (SourceItem $source) use ($domain) {
    ...
    public function filterByState(int $state): SourceList
    {
        return new self(
            array_filter(
                (array)$this,
                function (SourceItem $source) use ($state) {
    ...
    public function filterByUrls(array $urls): SourceList
    {
        return new self(
            array_filter(
                (array)$this,
                function (SourceItem $source) use ($urls) {
                    return (!isset($urls['success']) || $source->getSuccessUrl() === $urls['success'])
                        && (!isset($urls['failure']) || $source->getFailureUrl() === $urls['failure']);
                }
            )
        );
    }
    ...
    public function getFreeSourceCode(string $prefix, int $step = 1): string
    {
        $maxExistingSourceIds = array_map(
            function (SourceItem $source) use ($prefix) {
                return (int)ltrim(substr($source->getCode(), strlen($prefix)), '0'); // get only the number.
            },
            array_filter(
                (array)$this,
                function (SourceItem $source) use ($prefix) {
